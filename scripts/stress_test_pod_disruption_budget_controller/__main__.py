...
DEFAULT_POD_DISRUPTION_BUDGET_ATTRS = dict(
...
def _select_object_ids_batch(yp_client, object_type, id_prefix):
    ...
    return [
        response[0] for response in batch_select(yp_client, object_type, filter_query, ["/meta/id"])
    ]
...
def _create_objects(yp_client, object_type, default_object_attrs, object_count, id_prefix, dry_run):
    ...
    object_ids_to_create = [
        pod_set_id for pod_set_id in all_object_ids if pod_set_id not in created_object_ids
    ]

    logging.info(
        "Creating {} {}s with id prefix '{}'".format(
            len(object_ids_to_create), object_type, id_prefix
        )
    )
...
def _get_pod_set_ids_for_update_indices(yp_client, podset_count, id_prefix):
    ...
    created_pod_sets = batch_select(
        yp_client,
        "pod_set",
        filter_by_prefix_query,
        ["/meta/id", "/spec/pod_disruption_budget_id"],
    )
...
def _update_pod_sets(batch_yp_client, dry_run, podset_count, id_prefix):
    pod_set_ids_for_update_indices = _get_pod_set_ids_for_update_indices(
        batch_yp_client, podset_count, id_prefix
    )
    ...
    pod_disruption_budget_id_prefix = [
        pod_disruption_budget_id_prefix + str(index + 1) for index in pod_set_ids_for_update_indices
    ]

    logging.info(
        "Attaching {} pod sets to pod disruption budgets".format(
            len(pod_set_ids_for_update_indices)
        )
    )
    for pod_set_id, pod_disruption_budget_id in zip(pod_set_ids, pod_disruption_budget_id_prefix):
        logging.debug(
            "Attaching pod set (id: '{}') to pod disruption budget (id: '{}')".format(
                pod_set_id, pod_disruption_budget_id
            )
        )
        if not dry_run:
            batch_yp_client.update_object(
                "pod_set",
                pod_set_id,
                set_updates=[
...
def create_objects(yp_client, dry_run, podset_count, id_prefix):
    ...
    logging.info(
        "Running in 'create' mode. Pod set and pod disruption budget count: {}, id prefix: '{}'".format(
            podset_count, id_prefix
        )
    )

    with create_batch_yp_client(yp_client, batch_size=100, retries_count=5) as batch_yp_client:
        _create_objects(
            batch_yp_client, "pod_set", DEFAULT_POD_SET_ATTRS, podset_count, id_prefix, dry_run,
        )

        _create_objects(
            batch_yp_client,
            "pod_disruption_budget",
            DEFAULT_POD_DISRUPTION_BUDGET_ATTRS,
            podset_count,
            id_prefix,
            dry_run,
        )
...
def remove_objects(yp_client, dry_run, id_prefix):
    logging.info(
        "Running in 'remove' mode. Removing pod sets and pod disruption budgets with id prefix '{}'".format(
            id_prefix
        )
    )
...
def main():
    ...
    global_parser.add_argument(
        "--address", help="Address to run script on",
    )
    global_parser.add_argument(
        "--id-prefix", default=DEFAULT_ID_PREFIX, help="Prefix for object ids",
    )
    global_parser.add_argument(
        "--dry-run",
        action="store_true",
    ...
    global_parser.add_argument(
        "-v", "--verbose", action="store_true", help="Increase verbose level (from info to debug)",
    )

    parser = argparse.ArgumentParser(
        description="Stress test for YP pod disruption budget controller"
    )
    parser.register("action", "parsers", AliasedSubParsersAction)
    subparsers = parser.add_subparsers()
    subparsers.required = True

    create_objects_parser = subparsers.add_parser(
        "create", help="'create' - mode for creating objects.", parents=[global_parser]
    )
    create_objects_parser.set_defaults(func=create_objects)

    create_objects_parser.add_argument(
        "--podset-count", type=int, required=True, help="Pod set count",
    )

    remove_objects_parser = subparsers.add_parser(
        "remove", help="'remove' - mode for removing objects.", parents=[global_parser]
    )
    ...
    if address is None:
        raise argparse.ArgumentError(
            None, "argument --address or environment variable YP_ADDRESS must be specified",
        )
    ...
    if parsed_args.dry_run:
        logging.info(
            "Running in mode --dry-run. Actions will not be made,"

...
def main(argv):
    def parse_args():
        ...
        parser.add_argument("--arcadia-root", required=True, help="Path to arcadia root, need for YT local")
    ...
    insert_environ_path(os.path.realpath(arcadia_interop.prepare_yt_environment(args.sandbox_path + "/yt_build", arcadia_root=args.arcadia_root)))
    ...
    yp_instance = YpInstance(args.sandbox_path + "/yp", enable_ssl=True, db_version=INITIAL_DB_VERSION)
    yp_instance.prepare()

...
def main():
    ...
    pod_to_node = dict(
        [
            (row["meta.id"], row["spec.node_id"])
            for row in client.select_rows("* from [//yp/db/pods]")
        ]
    )
        ...
        for field in ["status.scheduled_allocations", "status.actual_allocations"]:
            ...
            for allocation in row[field]:
                ...
                if pod not in pod_to_node:
                    print "Resource {} at {} has invalid allocation belonging to nonexistent pod {}".format(
                        resource, node, pod
                    )
                    has_invalid_allocation = True
                elif pod_to_node[pod] != node:
                    print "Resource {} at {} has invalid allocation belonging to pod {} assigned to {}".format(
                        resource, node, pod, pod_to_node[pod]
                    )
                    has_invalid_allocation = True
                else:

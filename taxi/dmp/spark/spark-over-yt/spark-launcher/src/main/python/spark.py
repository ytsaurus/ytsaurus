...
from yt.wrapper.cypress_commands import create, exists
...
spark_version = args.version or "2.4.3-bin-custom-spark"
spark_launcher_version = "0.0.1-SNAPSHOT"
...
run_launcher = "/opt/jdk11/bin/java -cp spark-launcher-assembly-0.0.1-SNAPSHOT.jar"
...
file_paths = ["{0}/{1}.tgz".format(spark_base_path, spark_name),
              "{0}/spark-launcher-assembly-{1}.jar".format(spark_base_path, spark_launcher_version)]
...
master_command = "{0} && {1} {2} --id {3} --operation-id $YT_OPERATION_ID"\
    .format(unpack_tar, run_launcher, master_main_class, id)

worker_command = "{0} && {1} {2} --id {3} --cores {4} --memory {5}" \
    .format(unpack_tar, run_launcher, worker_main_class, id, worker_cores, worker_memory)

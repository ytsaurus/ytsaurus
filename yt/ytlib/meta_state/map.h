...
class TDefaultMetaStateValueOperations
{
public:
...
template <
    class TKey,
    class TValue,
    class TMetaStateValueOperations = TDefaultMetaStateValueOperations<TValue>,
...
{
public:
    typedef TMetaStateMap<TKey, TValue, TMetaStateValueOperations, THash> TThis;
    ...
    explicit TMetaStateMap(
        TMetaStateValueOperations operations = TMetaStateValueOperations())
    ...
    TValue* FindForUpdate(const TKey& key)
    {
        ...
        switch (State) {
            ...
            case EState::SavingSnapshot: {
                ...
                TValue* clonedValue = Operations.Clone(mapIt->Second()).Release();
...
private:
    ...
    TMetaStateValueOperations Operations;
    ...
    TVoid DoSave(TOutputStream* output)
    {
        ...
        FOREACH(const auto& item, items) {
            ...
            Operations.Save(item.Second(), output);
        }
    ...
    TVoid DoLoad(TInputStream* input)
    {
        ...
        for (i32 index = 0; index < size; ++index) {
            TKey key;
            ::Load(input, key);
            TValue* value = Operations.Load(input).Release();

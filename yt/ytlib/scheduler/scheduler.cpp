...
private:
        ...
        : public TConfigurable
    {
        ...
        {
            Register("type", Type);
            Register("transaction_id", Type);
            Register("spec", Type);
            Register("start_time", Type);
        }
    };
    ...
    TOperationPtr ParseOperationYson(const TOperationId& id, const TYson& yson)
    {
        ...
        auto type = attributes->Get<EOperationType>("type");
        auto transactionId = TTransactionId::FromString(attributes->Get<Stroka>("transaction_id"));
        auto operation = New<TOperation>(
            id,
        )
    }

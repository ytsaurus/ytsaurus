...
TEST_F(TExpressionTest, FunctionNullArgument)
{
    ...
    auto buffer = New<TRowBuffer>();
    callback(variables.GetLiteralvalues(), variables.GetOpaqueData(), &result, row, buffer.Get());
...
TEST_P(TTernaryLogicTest, Evaluate)
{
    ...
    compiledExpr1(variables1.GetLiteralvalues(), variables1.GetOpaqueData(), &result, row, buffer.Get());
    ...
    compiledExpr2(variables2.GetLiteralvalues(), variables2.GetOpaqueData(), &result, row, buffer.Get());
    EXPECT_TRUE(CompareRowValues(result, expected) == 0);
}

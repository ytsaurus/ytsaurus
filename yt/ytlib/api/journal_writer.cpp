...
public:
    ...
    {
        return BIND(&TJournalWriter::DoOpen, MakeStrong(this))
            .Guarded()
    ...
    {
        return BIND(&TJournalWriter::DoWrite, MakeStrong(this))
            .Guarded()
            .AsyncVia(NChunkClient::TDispatcher::Get()->GetWriterInvoker())
            .Run(data);
    }
    ...
    {
        return BIND(&TJournalWriter::DoClose, MakeStrong(this))
            .Guarded()
            .AsyncVia(NChunkClient::TDispatcher::Get()->GetWriterInvoker())
            .Run();
    }
...
private:
    ...
    typedef TOldMultiChunkSequentialWriter<TJournalChunkWriterProvider> TWriter;
    ...
    void DoOpen()
    {
        ...
        {
            auto req = TJournalYPathProxy::PrepareForUpdate(path);
        ...
        LOG_INFO("Journal info received (Account: %s, ChunkListId: %s)",
            ~writerOptions->Account,
            ~ToString(chunkListId));

        auto provider = New<TJournalChunkWriterProvider>(
            Config_,
            writerOptions);

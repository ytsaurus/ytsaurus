...
public:
    ...
    virtual bool Read(std::vector<TVersionedRow>* rows) override
    {
        for (const auto& block : UncompressedBlocks_) {
            LOG_DEBUG("XXX Releasing block {%v, %v}", (void*)block.Begin(), block.Size());
        }
...
protected:
    ...
    TSharedRef CaptureUncompressedBlock(int blockIndex)
    {
        ...
        if (LastUncompressedBlockIndex_ != blockIndex) {
            LOG_DEBUG("XXX Dropping last captured block %v {%v, %v}", LastUncompressedBlockIndex_, (void*)LastUncompressedBlock_.Begin(), LastUncompressedBlock_.Size());
            LastUncompressedBlock_ = GetUncompressedBlock(blockIndex);
            LastUncompressedBlockIndex_ = blockIndex;
        }

        LOG_DEBUG("XXX Captured block %v {%v, %v}", LastUncompressedBlockIndex_, (void*)LastUncompressedBlock_.Begin(), LastUncompressedBlock_.Size());
...
private:
    ...
    TSharedRef GetUncompressedBlock(int blockIndex)
    {
        ...
        if (uncompressedBlock) {
            ...
            LOG_DEBUG("XXX GetUncompressedBlock %v -> found cached uncompressed block {%v, %v}", blockId, (void*)uncompressedBlock.Begin(), uncompressedBlock.Size());
        ...
        if (compressedBlock) {
            ...
            LOG_DEBUG("XXX GetUncompressedBlock %v -> found cached compressed block {%v, %v}", blockId, (void*)block.Begin(), block.Size());
...
private:
    ...
    void CreateBlockReader()
    {
        ...
        LOG_DEBUG("XXX Creating block reader %v {%v, %v}", BlockIndex_, (void*)uncompressedBlock.Begin(), uncompressedBlock.Size());

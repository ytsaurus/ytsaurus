...
public:
    TCacheBasedVersionedChunkReaderBase(
        TCacheBasedChunkStatePtr state,
        const TColumnFilter& columnFilter,
        TTimestamp timestamp,
...
private:
    ...
    TSharedRef GetUncompressedBlockFromCache(int blockIndex)
    {
        ...
        auto uncompressedBlock = ChunkState_->PreloadedBlockCache->Find(blockId, EBlockType::UncompressedData);
        if (uncompressedBlock) {
            return uncompressedBlock;
        }

        auto compressedBlock = ChunkState_->PreloadedBlockCache->Find(blockId, EBlockType::CompressedData);
        if (compressedBlock) {
            ...
            if (codecId != NCompression::ECodec::None) {
                ChunkState_->PreloadedBlockCache->Put(blockId, EBlockType::UncompressedData, uncompressedBlock, Null);
            }
            return uncompressedBlock;
        }
    ...
    switch (state->ChunkMeta->GetChunkFormat()) {
        case ETableChunkFormat::VersionedSimple:
            return New<TCacheBasedSimpleVersionedLookupChunkReader>(
                ...
                timestamp);
        case ETableChunkFormat::VersionedColumnar: {
            auto underlyingReader = CreateCacheReader(state->ChunkMeta->GetChunkId(), state->PreloadedBlockCache);

            return New<TColumnarVersionedLookupChunkReader>(
                New<TChunkReaderConfig>(),
                state->ChunkMeta,
                std::move(underlyingReader),
                state->PreloadedBlockCache,
                keys,
                columnFilter,

    ...
    public static <T> @Nullable Object convertValueTo(@Nullable T value, ColumnValueType type) {
        return switch (type) {
            case STRING, ANY, COMPOSITE -> {
                ...
                } else if (value instanceof MessageLite messageLite) {
                    yield messageLite.toByteArray();
                } else {
                    yield CodecRegistry.getInstance().getYsonCodec().encode(value);
                }
            }

...
class NodeAttributesChange(NodeDiffBase):
    ...
    def is_empty(self) -> bool:
        return not self.actual_node.exists or (
            self.desired_node.node_type == self.actual_node.node_type
        ...
        for attribute_path, desired_value, actual_value in self.desired_node.attributes.changed_attributes(
            ...
            if desired_value is None:
                ...
                log.warning(
                    ...
                    self.desired_node.cluster_name,
                    self.desired_node.path,
                    attribute_path,
                    desired_value,
                )
            else:
    ...
    def make(cls, desired_node: YtNode, actual_node: YtNode) -> NodeAttributesChange:
        ...
        ), f"Nodes at '{desired_node.cluster_name}:{desired_node.path}' and '{actual_node.cluster_name}:{actual_node.path}' are in different clusters"
        assert (
            desired_node.path == actual_node.path
        ), f"Nodes at '{desired_node.cluster_name}:{desired_node.path}' and '{actual_node.cluster_name}:{actual_node.path}' have different paths"

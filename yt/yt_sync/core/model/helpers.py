...
def get_lca(path1: str, path2: str) -> str:
    tokens1 = path1.split("/")
    tokens2 = path2.split("/")
    last_common = 0
    for i, (t1, t2) in enumerate(zip(tokens1, tokens2)):
        if t1 == t2:
            ...
            break
    return "/".join(tokens1[: last_common + 1])


def get_list_lca(paths: list[str]) -> str:
    assert paths, "Cannot find lca path for empty list of paths"

    lca_path: str | None = None
    for node_path in sorted(paths):
        if lca_path is None:
            lca_path = node_path
        else:
            lca_path = get_lca(lca_path, node_path)
    return lca_path

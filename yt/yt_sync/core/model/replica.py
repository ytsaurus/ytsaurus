...
class YtReplica:
    ...
    enabled: bool
    mode: Mode
    enable_replicated_table_tracker: bool = False
    ...
    def update_from_attributes(self, attributes: Types.Attributes):
        ...
        assert not (state is not None and enabled is not None), (
            "State should be set by 'state' or 'enabled' attribute, not both for replica "
            + f"{self.cluster_name}:{self.replica_path}"
        )
        if state is not None:
            assert (
                state in self.State.all()
            ), f"Unknown state='{state}' for replica {self.cluster_name}:{self.replica_path}"
        ...
        if mode:
            assert mode in self.Mode.all(), f"Unknown mode='{mode}' for replica {self.cluster_name}:{self.replica_path}"
        ...
        if content_type:
            ...
            ), f"Unknown content_type '{content_type}' for replica {self.cluster_name}:{self.replica_path}"

...
class YtTable:
    ...
    # Filled on demand in scenarion with ReadReplicationProgressAction on unmounted state.
    ...
    def is_data_table(self) -> bool:
        return not (self.is_ordered or self.is_consumer)
    ...
    def make(
        ...
        result = cls(
            ...
            exists=exists,
            schema=YtSchema.make(schema, strict_schema, ensure_schema),
        ...
        if exists:
            ...
            # if result.is_replicated:
            #     result.attributes.attributes.setdefault("replicated_table_options", {})
            # if result.is_replicated and not is_rtt_enabled:
            #     rtt_options = deepcopy(result.attributes.get_value("replicated_table_options", {}))
            #     result.is_rtt_enabled = bool(get_rtt_enabled_from_attrs(rtt_options))

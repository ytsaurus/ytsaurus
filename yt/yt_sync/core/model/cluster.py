...
class YtCluster:
    ...
    def add_consumer(self, consumer: YtNativeConsumer):
        # TODO: tests!
    ...
    def add_node(self, node: YtNode) -> YtNode:
        ...
        assert node.path not in self.nodes, f"Duplicated node at '{node.path}'"
    ...
    def _check_path_clearance(self, item: YtTable | YtNode):
        is_node = isinstance(item, YtNode)
        for existing_node in self.nodes.values():
            if existing_node.path.startswith(item.path + "/"):
                assert (
                    is_node and item.node_type == YtNode.Type.FOLDER
                ), f"Node at '{item.path}' is not a folder and is in the way of '{existing_node.path}'"
            elif item.path.startswith(existing_node.path + "/"):
                assert (
                    existing_node.node_type == YtNode.Type.FOLDER
                ), f"Node at '{existing_node.path}' is not a folder and is in the way of '{item.path}'"

        if is_node and item.node_type == YtNode.Type.LINK:
            for existing_node in self.nodes.values():
                if existing_node.node_type != YtNode.Type.LINK:
                    continue
                assert (
                    existing_node.link_target_path != item.path
                ), f"Link at '{existing_node.path}' is linked to other link at '{item.path}'"
                assert (
                    item.link_target_path != existing_node.path
                ), f"Link at '{item.path}' is linked to other link at '{existing_node.path}'"

        for existing_table in self.tables.values():
            if existing_table.path.startswith(item.path + "/"):
                assert (
                    is_node and item.node_type == YtNode.Type.FOLDER
                ), f"Node at '{item.path}' is not a folder and is in the way of '{existing_table.path}'"
            else:
                assert not item.path.startswith(
                    existing_table.path + "/"
                ), f"Node at '{existing_table.path}' is not a folder and is in the way of '{item.path}'"
        ...
        for item in list(self.tables.values()) + [
            ...
            if item.folder in self.nodes:
                ...
                assert node.node_type == YtNode.Type.FOLDER, (
                    f"Node {self.name}:{node.path} type mismatch: actual='{str(node.node_type)}', "
                    + f"expected='{str(YtNode.Type.FOLDER)}'"
                )
                continue
    ...
    def _add_folders_upto_lca(self):
        lca_path: str = get_list_lca([node.path for node in self.nodes.values()])
        assert lca_path != "/", "LCA path cannot be root, split your requests"
        ...
        for node in self.nodes_sorted:
            ...
            while path != lca_path and path not in self.nodes:
                self.add_node(
                    YtNode.make(
                        cluster_name=self.name,
                        path=path,

...
class Settings:
    ...
    'remote_copy' operation is exception thoug: if no explicit spec for given or 'default' cluster is given then
    DEFAULT_REMOTE_COPY_SPEC is used.

    Spec example:
    operations_spec = {
        "default": { "default": { ... }, "remote_copy": { ... }}  # default specs
        "hahn: { "default": { .. }, "map": { ... } }  # hann operations spec
    ...
    """
    Ensure YT native queue consumers registrations at the end of clean and ensure scenarios.
    Function 'add_consumer' does nothing if this flag is not set.
    """
    ...
    def make(cls, cluster_settings: dict[str, Any]) -> Settings:
        """Cluster settings is in GrUT format, see grut/tools/admin/settings/clusters.py for example."""
        is_chaos = cluster_settings.get("use_chaos", False)
        rtt_enabled = cluster_settings.get("enable_replicated_table_tracker", False)

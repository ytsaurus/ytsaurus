...
class NodeSpec:
    ...
    def parse(cls, raw: dict[str, Any], with_ensure: bool = True) -> Node:
        assert isinstance(raw, Mapping), "dict-like object expected as raw data"
        result = cls(spec=from_dict(data_class=Node, data=raw, config=Config(cast=[Node.Type])))
        if with_ensure:
            result.ensure()
        return result
...
class TypeV1Spec(TypeSpecBase):
    ...
    def is_required(self) -> bool:
        return self.column.required is True
...
class TypeV3Spec(TypeSpecBase):
    ...
    SIMPLE_TYPES: ClassVar[frozenset[TypeV3.Type]] = NUMERIC | BYTES | BOOL | DATETIME
...
class SchemaSpec:
    ...
    def ensure(cls, schema: list[Column] | None):
        ...
        for i, column in enumerate(schema):
            ...
            TypeV1Spec(column).ensure()
            TypeV3Spec(column).ensure()

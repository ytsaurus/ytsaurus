...
def generate_db(nodes: list[YtNode]) -> YtDatabase:
    db = YtDatabase()
    builder = DesiredStateBuilder(YtClientFactory(dry_run=False, token=None), db, False)
...
class TestTreesGeneration:
    ...
    def settings() -> Settings:
        return Settings(
            ...
            ensure_folders=True,
            ensure_native_queue_consumers=True,
    ...
    def test_trees_generation(
        ...
        desired_db = generate_db(desired_nodes)
        actual_db = generate_db(actual_nodes)

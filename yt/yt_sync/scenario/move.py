...
class MoveScenario(ScenarioBase):
    ...
    def _setup_destination_database(self):
        ...
        if self.settings.ensure_folders:
            self.node_manager.ensure_nodes(
            ...
            for cluster in self.actual.clusters.values():
                ...
                for node in cluster.nodes_sorted:
                    ...
                    if node.node_type == YtNode.Type.LINK:
                        self.node_manager.remove_node(node, batch_client)
                        continue
                    moved_nodes = True
    ...
    def pre_action(self):
        ...
        self._setup_destination_database()

    def post_action(self):
        super().post_action()
        if self.settings.ensure_folders:
            self.node_manager.ensure_links(self.destination_desired, self.destination_actual)
    ...
    def generate_actions(self) -> list[ActionBatch]:
        ...
        for main_cluster, is_destination in (
            ...
            for table in main_cluster.tables_sorted:
                ...
                action_collector.add(main_cluster_name, table.key, SwitchReplicaStateAction(table, False))

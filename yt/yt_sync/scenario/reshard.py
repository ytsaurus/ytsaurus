...
class ReshardScenario(ScenarioBase):
    ...
    def pre_action(self):
        assert check_and_log_db_diff(
            self.settings, self.desired, self.db_diff, set([TableDiffType.TABLET_COUNT_CHANGE])
        ), "Bad diff"
    ...
    def generate_actions(self) -> list[ActionBatch]:
        ...
        affected_table_keys: set[str] = set(
        ...
        if self.settings.is_chaos:
            result.extend(rtt_controller.switch_rtt_off(sync_replicas, affected_table_keys))
        ...
        for actual_cluster, _ in switcher.clusters():
            ...
            for diff_set in self.db_diff.table_diff_for(cluster_name=actual_cluster.name, diff_types=self.diff_types):
                if not self._has_diff_for(actual_cluster, diff_set):
                    continue
                if should_switch_replicas(self.settings, True, False):
        ...
        if self.settings.is_chaos:
            result.extend(rtt_controller.switch_rtt_on(switcher.get_sync_clusters(), affected_table_keys))
    ...
    def _make_actions_for_replication_log(
        ...
        chaos_toolbox.create_replicated_log(desired_log_replica, desired_tmp_log, actual_tmp_log)
        ...
        chaos_toolbox.create_replicated_log(desired_log_replica, desired_log, actual_log)

...
from yt.yt_sync.core.diff.base import NodeDiffType
...
class _SchemaChangeActionGenerator(_ActionGeneratorBase):
    ...
    def _fill_chaos_sorted_table(
        ...
        actual_tmp_log = actual_cluster.get_or_add_temporary_table_for(actual_log, False)
        desired_tmp_log = desired_cluster.get_or_add_temporary_table_for(desired_log, False)
        ...
        if self.is_data_modification_required:
            actual_tmp_table = actual_cluster.get_or_add_temporary_table_for(actual_table, False)
    ...
    def _fill_chaos_sorted_full_downtime(
        ...
        action_collector.add(
            ...
            self.table_key,
            CloneChaosReplicaAction(
                desired_replicated_table, actual_replicated_table, actual_tmp_table, actual_table, True
            ),
        )
        ...
        chaos_toolbox.create_replica_table(desired_log, actual_log)

        # enable log
        action_collector.add(
            self.actual_db.main.name,
            self.table_key,
            SwitchReplicaStateAction(actual_replicated_table, True, actual_log),
        )
    ...
    def _fill_chaos_sorted_light_change(
        ...
        # create temorary replication log with new schema
        chaos_toolbox.create_and_mount_replicated_log(
            desired_log_replica,
            desired_tmp_log,
            actual_tmp_log,
    ...
    def _fill_replicated(self, action_collector: DbTableActionCollector):
        ...
        if self.is_data_modification_required:
            # use current pivot keys for transformed table if table key not changed
            desired_for_reshard = desired_table if self.is_key_changed else deepcopy(actual_table)
            ...
            if self.actual_cluster.is_mountable:
                self.add_action_to(action_collector, ReshardTableAction(desired_for_reshard, actual_table))
        else:
            self.add_action_to(action_collector, AlterTableSchemaAction(desired_table, actual_table))
...
class EnsureBase(ScenarioBase):
    ...
    def pre_action(self):
        assert check_and_log_db_diff(self.settings, self.desired, self.db_diff, self.diff_types), "Bad diff"
    ...
    def generate_actions(self) -> list[ActionBatch]:
        ...
        full_dowtime_tables: set[str] = self._collect_full_downtime_tables()
        switcher = ReplicaSwitcher(
            self.settings,
            self.actual,
            skip_wait=full_dowtime_tables,
        )
    ...
    def _create_action_generators(
        ...
        for diff_set in self.db_diff.table_diff_for(actual_cluster.name, self.diff_types):
            ...
            full_downtime_required: bool = diff_set.table_key in full_dowtime_tables
            generators.insert(
                0,
...
class EnsureAttributes(EnsureBase):
    ...
    def pre_action(self):
        super().pre_action()
        self.node_manager.ensure_nodes_from_diff(self.db_diff, self.actual, mode=NodeManager.EnsureMode.ATTRIBUTES_ONLY)
        self.node_manager.ensure_links_from_diff(self.db_diff, self.actual, attributes_only=True)
...
class EnsureFull(EnsureBase):
    ...
    def pre_action(self):
        ...
        if self.settings.ensure_folders:
            self.node_manager.ensure_nodes(self.desired, self.actual)

    def post_action(self):
        super().post_action()
        if self.settings.ensure_folders:
            self.node_manager.ensure_links(self.desired, self.actual)
        ...
        if self.settings.ensure_native_queue_consumers:
            ensure_db_consumers(self.desired, self.actual, self.yt_client_factory)

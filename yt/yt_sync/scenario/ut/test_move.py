...
def generate_yt_config(
    ...
    def _make_config(table_path: str, exists: tuple[bool, bool, bool]):
        return {
            "primary": {
                "get": {
                    f"{get_folder(table_path)}&/@type": MockResult(result=YtNode.Type.FOLDER),
                    **(
                        {
...
class TestMove:
    ...
    def desired_db(
        ...
        builder = DesiredStateBuilder(yt_client_factory, db, settings.is_chaos)
    ...
    def test_destination_desired(
        ...
        builder = DesiredStateBuilder(yt_client_factory, empty_db, settings.is_chaos)

...
class SyncReplicasScenario(ScenarioBase):
    ...
    def __init__(
        ...
        self.node_manager = NodeManager(self.yt_client_factory, settings)
    ...
    def pre_action(self):
        ...
        if self._has_missing_replicas():
            if not self.source_cluster_name:
                always_async_cluster = next(iter(self.settings.always_async), None)
                if always_async_cluster:
                    self.source_cluster_name = always_async_cluster
                else:
                    async_cluster = next(iter(self.actual.async_replicas_relaxed), None)
                    if async_cluster is not None:
                        self.source_cluster_name = async_cluster.name
            assert self.source_cluster_name, "Source cluster name is not defined"
            assert (
                self.source_cluster_name in self.actual.clusters
            ), f"Unknown source cluster {self._source_cluster_name}"
            self._check_diff_for_source_cluster()
        ...
        if self.settings.ensure_folders:
            self.node_manager.ensure_nodes_from_diff(self.db_diff, self.actual)

    def post_action(self):
        super().post_action()
        if self.settings.ensure_folders:
            self.node_manager.ensure_links_from_diff(self.db_diff, self.actual)
                ...
                if table_diff.has_deleted_replicas():
                    for replica in table_diff.deleted_replicas():
                        ...
                        action_collector.add(
                            main_cluster_name,
                            table_key,
                            SwitchReplicaStateAction(table_diff.actual_main_table, False, replica_table),
                        )
                        action_collector.add(
    ...
    def _check_diff_for_source_cluster(self):
        for diff_set in self.db_diff.table_diff_for(cluster_name=None, diff_types=self.diff_types):
            for table_diff in diff_set.diffs:
                assert isinstance(table_diff, ReplicasChange)
                assert not table_diff.has_missing_replicas(
    ...
    def _add_replica_for_ordered_replicated(
        ...
        action_collector.add(
            main_cluster_name,
            table_key,
            SwitchReplicaStateAction(table_diff.actual_main_table, True, missing_actual_table),
        )

    ...
    def do_modify_request(self, batch_client: YtClientProxy) -> Any | None:
        ...
        if self._desired_table.has_pivot_keys:
            if self._desired_table.schema.is_ordered:
                LOG.debug(
                    "Pivot keys not supported for resharding ordered table %s:%s, tablet_count used instead.",
        ...
        return batch_client.reshard_table(self._desired_table.path, **resharding_properties)

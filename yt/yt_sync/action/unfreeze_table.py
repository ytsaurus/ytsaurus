...
class UnfreezeTableAction(AwaitingResultAction):
    def __init__(self, table: YtTable):
        super().__init__()
        self._table: YtTable = table

    def do_modify_request(self, batch_client: YtClientProxy) -> Any:
        if not self._table.tablet_state.is_frozen:
            LOG.info(
                "Skip unfreeze table %s, can unfreeze table only in state '%s' but got '%s'",
                self._table.rich_path,
                YtTabletState.FROZEN,
                self._table.tablet_state.state,
            )
            return None
        LOG.warning("Unfreeze table %s:%s", self._table.cluster_name, self._table.path)
        return batch_client.unfreeze_table(self._table.path)

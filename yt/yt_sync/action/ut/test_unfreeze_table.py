...
from yt.yt_sync.action.unfreeze_table import UnfreezeTableAction
...
def yt_table(cluster_name: str, table_path: str, default_schema: Types.Schema) -> YtTable:
    ...
    table.tablet_state.set(YtTabletState.FROZEN)
    return table


@pytest.fixture
def unfreeze_action(yt_table: YtTable) -> UnfreezeTableAction:
    return UnfreezeTableAction(yt_table)


def test_freeze_success(unfreeze_action: UnfreezeTableAction, yt_table: YtTable):
    check_path = f"{yt_table.path}/@tablet_state"
    yt_client_factory = MockYtClientFactory(
        {
            yt_table.cluster_name: {
                "unfreeze_table": {yt_table.path: MockResult(result=True)},
    ...
    assert unfreeze_action.schedule_next(yt_client) is True
    unfreeze_action.process()

    call_tracker = yt_client_factory.get_call_tracker(yt_table.cluster_name)
    assert call_tracker
    mount_calls = call_tracker.calls["unfreeze_table"]
    ...
    assert unfreeze_action.schedule_next(yt_client) is False
    assert yt_table.tablet_state.is_mounted


def test_freeze_not_ready(unfreeze_action: UnfreezeTableAction, yt_table: YtTable):
    check_path = f"{yt_table.path}/@tablet_state"
    yt_client_factory = MockYtClientFactory(
        {
            yt_table.cluster_name: {
...
@pytest.mark.parametrize("state", [YtTabletState.MOUNTED, YtTabletState.UNMOUNTED])
def test_not_frozen(unfreeze_action: UnfreezeTableAction, yt_table: YtTable, state: str):
    ...
    call_tracker = yt_client_factory.get_call_tracker(yt_table.cluster_name)
    assert call_tracker
    assert "unfreeze_table" not in call_tracker.calls

...
def actual_replicated(
    ...
    table = YtTable.make(
        table_path, primary_cluster, YtTable.Type.REPLICATED_TABLE, table_path, True, {"schema": default_schema}
    )
...
def test_yt_error(desired: YtReplica, actual_replicated: YtTable):
    desired.enable_replicated_table_tracker = True
...
def test_success(desired: YtReplica, actual_replicated: YtTable):
    ...
    assert actual.enable_replicated_table_tracker is True
    ...
    assert call.kwargs == {"enable_replicated_table_tracker": True, "enabled": False}

...
class SetReplicationProgressAction(ActionBase):
    ...
    def schedule_next(self, batch_client: YtClientProxy) -> bool:
        ...
        ), f"Can't read replication_progress from non-existing table {self._dst_table.rich_path}"
        ...
        self._response = batch_client.alter_table(
            self._src_table.path, replication_progress=self._src_table.chaos_replication_progress
        )
        return False

...
class MountTableAction(AwaitingResultAction):
    def __init__(self, table: YtTable):
        super().__init__()
        self._table: YtTable = table

    def do_modify_request(self, batch_client: YtClientProxy) -> Any:
        if self._table.tablet_state.is_mounted:
            LOG.info("Skip mount table %s, already mounted", self._table.rich_path)
            return None
        LOG.warning("Mount table %s", self._table.rich_path)
        return batch_client.mount_table(self._table.path)
    ...
    def is_required_state(self, result: Any) -> bool:
        ...
        return YtTabletState.MOUNTED == str(result)

    def postprocess(self):
        self._table.tablet_state.set(YtTabletState.MOUNTED)

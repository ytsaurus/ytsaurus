...
def _build_process_resources(d):
    ...
    d.add(Rowset()
        ...
        .all(yt_host)
        .row()
            .cell("Scheduler Memory", SchedulerMemory("yt.memory.generic.bytes_in_use_by_app").container_legend_format(), unit="bytes")
            .cell("Controller Agent Memory", CAMemory("yt.memory.generic.bytes_in_use_by_app").container_legend_format(), unit="bytes")
        .row()

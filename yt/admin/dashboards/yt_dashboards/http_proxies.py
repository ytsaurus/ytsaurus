...
def add_proxy_parameters(
    dashboard: Dashboard,
    default_cluster: str = "*",
    default_proxy_role: str = "data",
    default_host: str = "Aggr",
):
    # cluster
...
def build_http_proxies_rowsets(backend: str, has_porto: bool):
    rowsets = []
    ...
    rowsets.append(Rowset()
        ...
        .row(height=12)
            .cell(
                ...
                MonitoringExpr(HttpProxy("yt.http_proxy.bytes_in.rate").value("command", "-").all("user").aggr("network"))
            )
            .cell(
                "Top Users By Bytes Out Rate",
                MonitoringExpr(HttpProxy("yt.http_proxy.bytes_out.rate").value("command", "-").all("user").aggr("network"))
            )
        )
...
def build_http_proxies(backend: str, has_porto: bool = True):
    dashboard = Dashboard()

    for rowset in build_http_proxies_rowsets(backend, has_porto):
        dashboard.add(rowset)

    dashboard.set_title("HTTP proxies [AUTOGENERATED]")
    dashboard.set_description("")
    add_proxy_parameters(dashboard)

...
Право `manage` на пул позволяет изменять с помощью команды `update_operation_parameters` только следующие параметры:

- `weight`;
- `weight` и `resource_limits` в `scheduling_options_per_pool_tree`;
- `scheduling_tag_filter`;
- `controller_agent_tag`.

Для изменения остальных параметров требуется право `manage` на саму операцию; для некоторых параметров могут потребоваться дополнительные права. В частности, право `manage` на пул не позволяет изменить параметр `pool` и переместить операцию в другой пул.

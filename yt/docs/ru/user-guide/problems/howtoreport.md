...
{% include notitle [Об использовании LLM при составлении обращений](../../_includes/user-guide/problems/howtoreport-llm-usage.md) %}

## Случаи обращений {#cases-of-requests}

Система {{product-name}} состоит из различных компонент и сервисов, поэтому существует несколько способов обратиться за помощью или задать вопрос.

Для каждой ситуации есть лучший способ сообщить о проблеме или попросить помощи с системой. Выберите его перед запросом — так вы сократите время до того, как нужный специалист получит ваш запрос.

Вопросы могут относиться к следующим веткам:

* [Вопросы по работе с веб-интерфейсом](#web-interface-issues)
* [Вопросы по текущей работе системы](#system-operation-issues)
* [Вопросы по разработке](#development-questions)
* [Прочие вопросы](#other-questions)
...
Если проблема связана с медленной работой джобов операции, предпримите предварительно максимальные усилия по самостоятельной диагностике проблемы с помощью [Job Shell](jobshell-and-slowjobs.md) и [статистик джобов](jobstatistics.md).
...
## Как написать результативное сообщение об ошибках {#how-to-write-effective-bug-reports}
...
### Формулирование проблемы {#problem-formulation}
...
### Вспомогательная информация {#auxiliary-information}
...
Если информацию обрезает используемое вами API, напишите на рассылку {% if lang == ru %}[community_ru@](mailto:community_ru@ytsaurus.tech){% else %}[community@](mailto:community@ytsaurus.tech){% endif%}.
...
Для продуктовых процессов настройте подходящую ротацию и компрессию debug-логов и **не выключайте** их. Подробные логи нужны для того, чтобы установить причину проблемы.

{% endnote %}

Система {{product-name}} пишет подробные логи на стороне сервера, но если проблема, например, заключается в сетевом взаимодействии между клиентом и сервером, то серверные логи бесполезны.
...
**Если проблема воспроизводится, постарайтесь найти минимальный пример для воспроизведения.** Часто минимальный пример воспроизведения проблемы позволяет найти ошибку в пользовательском коде. Если ваши вычисления зависят от дополнительных данных (таблицы, локальные файлы), подготовьте надежные ссылки на них. Дополнительные данные могут потребоваться для самостоятельного воспроизведения проблемы командой сервиса {{product-name}}, если нельзя будет найти причину проблемы путем чтения логов.

### Структура и порядок {#structure-and-order}

**Постарайтесь разделить проблемы.** Не пишите в одном сообщении об ошибке о разных проблемах. Напишите два сообщения.

### Самостоятельный поиск {#self-help}

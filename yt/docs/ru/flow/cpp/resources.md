...
`TYTDirectoryLastFileSource` позволяет временно выбрать конкретный файл вместо последнего:
...
### Обновляемый файл в {{product-name}}

`TYTFileSource` следит за идентификатором и ревизией файловой ноды:
...
Перезапись файла на том же пути создаёт новую версию ресурса. Воркеры скачивают именно ту ревизию,
которую выбрал контроллер, поэтому изменение файла между обнаружением и скачиванием не приводит к
публикации данных под неправильной версией.

### Последний файл в каталоге {{product-name}}

`TYTDirectoryLastFileSource` выбирает файл с лексикографически наибольшим именем среди
непосредственных детей каталога:
...
Для последовательных выпусков используйте сортируемые имена, например `000001`, `000002`.
Пустой каталог не заменяет уже найденную версию. Параметр `pinned_file_name` позволяет выбрать
одного непосредственного ребёнка каталога.

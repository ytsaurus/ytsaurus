...
`ProtoState.Set()` по-прежнему заменяет сообщение целиком и не принимает пустое, `Clear()` удаляет стейт. Пустое Protobuf-сообщение сериализуется в ноль байт, а ноль байт — это отсутствие стейта: дефолт `&T{}` не записывается, а сообщение, у которого сняли все поля, стейт удаляет. `RawStateAccessor` изменяемого значения не отдаёт: `Get()` возвращает копию байтов, и записываются они только через `Set()`. Стейты [External State](external-state.md) на месте тоже не отслеживаются.
...
| `Set(value *T)` | `error` | Заменить значение стейта целиком; пустое сообщение записать нельзя |
| `Clear()` | — | Удалить стейт для текущего ключа |
| `ReadOnly()` | `ReadOnlyProtoState[T, PT]` | Представление только на чтение |

{% note info %}

В отличие от Python, где `get_or_default()` без аргументов отдаёт пустой экземпляр Proto-класса, в Go значение по умолчанию задаётся явно — передайте `&T{}`, если хотите начать с пустого сообщения. Пустое сообщение сериализуется в ноль байт и в стейт не записывается.

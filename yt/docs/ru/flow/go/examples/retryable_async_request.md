...
Расширение примера [Async Request](async_request.md): обработчик запросов повторяет каждую неудачную попытку через [таймеры](../../../flow/concepts/glossary.md#timer). Запрос целиком сохраняется во внутреннем [стейте](../../../flow/concepts/glossary.md#state) и повторяется с фиксированной задержкой, пока внешний сервис на него не ответит.
...
- [Async Request (Go)](async_request.md)

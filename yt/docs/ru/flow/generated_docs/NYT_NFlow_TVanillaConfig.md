...
|| `upload_temp_path` | **Тип**: `TString`
**Значение по умолчанию**: `TString("")`
Директория, в которой заливаемый файл создаётся как временный перед перемещением в кэш. Если не задана, используется родительская директория `cache_path`. Временный файл удаляется после попадания в кэш или при ошибке заливки; файл, оставшийся после падения лаунчера, мастер удаляет автоматически через несколько часов. ||

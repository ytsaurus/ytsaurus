...
**Значение по умолчанию**: `{}`
Именованные взвешенные классы квоты. Ключи не должны содержать `/` и `:`. Активные классы делят полосу пропорционально весам, а свободная доля перераспределяется. ||

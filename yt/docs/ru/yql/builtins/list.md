...
`ListFold` допускает [линейные типы](../types/linear.md) `Linear<T>` в типе состояния `U`. Если состояние содержит линейные значения, `updateLambda` должна на каждой итерации потребить каждое из них ровно один раз и вернуть состояние того же типа.

В Query Tracker для использования линейного состояния в `ListFold` выберите версию языка 2026.02 или новее.
...
Линейное состояние позволяет изменять мутабельный словарь без копирования на каждой итерации:
...
* Диапазоны `Decimal` поддерживаются начиная с версии языка [2026.02](../changelog/2026.02.md). В Query Tracker выберите версию языка 2026.02 или новее. Начало, конец и явно заданный шаг должны иметь в точности одинаковый тип `Decimal(N, M)`.
...
* Если параметр типа `Float`, `Double` или `Decimal` равен `NaN` или бесконечности, список будет пустым.
...
Строит кортеж из списка и явно указанной ширины кортежа. Все элементы кортежа будут иметь тот же тип, что и тип элемента списка. Если длина списка не соответствует указанной ширине кортежа, будет возвращена ошибка. Для опционального списка на выходе получается опциональный кортеж. Для NULL аргумента - NULL.
...
Выбрать топ значений из списка. `ListTopSort*` — дополнительно отсортировать список возвращенных значений. По умолчанию выбираются наименьшие значения (функции без суффикса — алиас к функциям `*Asc`; `*Desc` — выбор наибольших значений).

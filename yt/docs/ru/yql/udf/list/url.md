...
Подробнее про Punycode см. [комментарии в используемой библиотеке]({{source-root}}/library/cpp/unicode/punycode/punycode.h).

## IsAllowedByRobotsTxt
``` yql
Url::IsAllowedByRobotsTxt(
  String{Flag:AutoMap}, -- URL
  String?, -- бинарное представление robots.txt в формате {{source-root}}/library/robots_txt
  Uint32 -- идентификатор робота из {{source-root}}/library/robots_txt/robotstxtcfg.h?rev=3514373#L14
) -> Bool
```
{% endif %}

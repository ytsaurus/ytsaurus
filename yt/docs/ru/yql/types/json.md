...
* Тип в JSON — `bool`.
* Пример значения {{product-name}} — `true`.
...
* Тип в JSON — `number`.
* Пример значения {{product-name}} — `123456`, `-123456`.
...
* Тип в JSON — `number`.
* Пример значения {{product-name}} — `123456`.
...
* Тип в JSON — `number`.
* Пример значения {{product-name}} — `0.12345679`.
...
* Тип в JSON — `number`.
* Пример значения {{product-name}} — `0.12345678901234568`.
...
* Тип в JSON — `string`.
* Пример значения {{product-name}} — `-320.789`.
...
* Тип в JSON — `string`.
* Пример значения {{product-name}} — последовательность из 4 байт:
...
* Тип в JSON — `string`.
* Пример значения {{product-name}} — код на С++:
...
* Тип в JSON — `string`.
* Пример значения {{product-name}} — `18367`.
...
* Тип в JSON — `string`.
* Пример значения {{product-name}} — `1586966302`.
...
* Тип в JSON — `string`.
* Пример значения {{product-name}} — `1586966302504185`.
...
Дата. Int32, количество дней относительно unix epoch (может быть отрицательным).

* Тип в JSON — `string`.
* Пример значения {{product-name}} — `-8722`.
* Пример значения JSON — `"1946-02-14"`.

## Datetime64 {#datetime64}

Дата и время. Int64, количество секунд относительно unix epoch.

* Тип в JSON — `string`.
* Пример значения {{product-name}} — `-753511371`.
* Пример значения JSON — `"1946-02-14T19:17:09Z"`.

## Timestamp64 {#timestamp64}

Дата и время. Int64, количество микросекунд относительно unix epoch.

* Тип в JSON — `string`.
* Пример значения {{product-name}} — `-753511370765432`.
* Пример значения JSON — `"1946-02-14T19:17:09.234568Z"`.

## Interval64 {#interval64}

Временной интервал. Int64, точность до микросекунд.
...
Временные типы с меткой временной зоны. Значение позиции во времени хранится в UTC.
...
* Пример значения {{product-name}} — `null`.

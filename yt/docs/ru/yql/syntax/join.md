...
Стратегию StarJoin можно выбрать принудительно через [SQL хинт](lexer.md#sql-hints):
...
```
При указании хинта:
- в качестве главной таблицы форсируется _левая сторона_ JOIN, для которого указан хинт
- вставляется дополнительная сортировка по ключам для всех входов, для которых это необходимо
- в случаях, когда стратегия StarJoin неприменима - выводится предупреждение с причиной

...
Передача параметров поддерживается в SDK и CLI.

{% note warning %}

Передача параметров через веб-интерфейс {{product-name}} в настоящее время не поддерживается.
...
Примеры передачи параметров для DECLARE можно найти в разделе [{#T}](../types/json.md).

...
Режим ANSI SQL включается с помощью специального комментария `--!ansi-lexer`, который должен стоять в начале программы.
...
Для записи в теле программы произвольного идентификатора он заключается в обратные кавычки (бэктики):
``` sql

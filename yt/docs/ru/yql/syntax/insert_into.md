...
* `MONOTONIC_KEYS` &mdash; добавление монотонно возрастающих по ключу сортировки данных в сортированную таблицу. Не совместим с `TRUNCATE`. Не требует операции sorted merge, как при обычном добавлении в сортированную таблицу. Пользователь гарантирует, что добавляемые данные по ключам не нарушают общий порядок сортировки. Если на исходной таблице стоял флаг уникальности по ключам, то он сохраняется — в этом случае добавляемые данные также не должны нарушать уникальность.
* `COLUMN_GROUPS` &mdash; задать спецификацию поколоночных групп для таблицы. Спецификация задаётся в виде yson-словаря, где ключём является имя группы, а в значении задаётся список имён колонок. Можно также указать значение `#` (yson entity), которое трактуется как все остальные колонки, не перечисленные в остальных группах. Пример: `{group1=[name;age];group2=[ip;last_access];default=#}`. Если спецификация групп не задана, вставка в пользовательскую таблицу выполняется без поколоночных групп.
* `SECURITY_TAGS=yson` &mdash; задать [теги чувствительных данных]({{yt-docs-root}}/user-guide/storage/security-tags) для итоговой таблицы в виде yson-списка (пример: `SECURITY_TAGS=[tag1;tag2]`). Если также указан `TRUNCATE`, этот модификатор целиком определяет `security_tags` создаваемой таблицы; иначе — теги, которые будут добавлены к уже имеющимся.

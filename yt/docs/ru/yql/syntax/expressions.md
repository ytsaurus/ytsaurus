...
Табличный контекст также возникает после [UNION ALL](select/union.md#union-all), [JOIN](join.md#join), [PROCESS](process.md#process), [REDUCE](reduce.md#reduce);
...
При использовании этого синтаксиса для обращения к контейнерам в столбцах таблиц обязательно нужно указывать полное имя столбца, включая имя или алиас таблицы через точку (смотрите первый пример ниже).

...
#### Пример {#tmpfolder-example}

```yql
PRAGMA yt.TmpFolder = '//tmp/yql/ivanov/folder';
```

#### Результат {#tmpfolder-result}
...
Директории для работы запроса создаются в рамках указанной транзакции. Это может приводить к конфликтам при попытках записывать даннные в ранее не существующую директорию с помощью двух запросов с разными ExternalTx.
...
Используйте прагму только совместно с прагмой [yt.QueryCacheTtl](#querycachettl): без указания интервала TTL она не имеет эффекта.
...
## yt.MaxOutputTables {#maxoutputtables}
...
{% if audience == "internal"%}
## yt.RuntimeClusterSelection {#runtimeclusterselection}
...
## yt.RuntimeCluster {#runtimecluster}
...
Режим выбора кластера задаётся прагмой [`yt.RuntimeClusterSelection`](#runtimeclusterselection).
...
## yt.DefaultRuntimeCluster {#defaultruntimecluster}
...
#### Пример {#yt.OmitInaccessibleRows-example}
...
#### Результат {#yt.OmitInaccessibleRows-result}
...
#### Ограничения {#yt.OmitInaccessibleRows-restrictions}

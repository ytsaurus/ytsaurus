...
Все динамические прагмы и некоторые статические позволяют задать настройки per-cluster через синтаксис `PRAGMA <cluster_name>.<pragma_name>`. Это пригодится, чтобы действие прагмы распространялось только на определённый кластер. Например: `PRAGMA foo.TmpFolder` позволяет задать путь к временной директории только для YT-кластера `foo`.

{% note info %}

В настройке per-cluster используется название прагмы без префикса `yt`. Например: `PRAGMA foo.TmpFolder`, а не `PRAGMA foo.yt.TmpFolder`.

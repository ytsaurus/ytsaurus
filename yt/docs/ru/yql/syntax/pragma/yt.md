...
| --- | --- | --- |
| Строка: disable / join / all | disable; Начиная с [2026.02](../../changelog/2026.02.md) - join | Статическая |

Добавляет предварительную конвертацию динамических таблиц в статические. В режиме `join` конвертируются только таблицы, являющиеся входами джойнов. Это позволяет разрешить использование стратегии map join поверх таких таблиц. В режиме `all` все динамические таблицы запроса преобразуются в статические. В режиме `disable` никакой конвертации не происходит.
...
Управляет моментом запуска [стоимостного оптимизатора](global.md#costbasedoptimizer) для запросов с несколькими `JOIN`. Прагма позволяет не ждать материализации всех входов и сохранить параллельность вычислений, если полный пересчёт порядка соединений не обязателен.

| Тип значения | По умолчанию | Статическая /<br/>динамическая |
| --- | --- | --- |
| Неотрицательное число | 0 | Статическая |

Значение определяет, сколько входов `JOIN` должно быть готово и материализовано на диск перед переупорядочиванием:

* `0` — дождаться всех входов и выполнить полное переупорядочивание. Этот режим даёт оптимизатору максимум информации, но снижает параллельность и может мешать склейке операций.
* `1` — не ждать входы и оптимизировать только уже готовые поддеревья. Режим сохраняет параллельность, но может изменить меньшую часть плана.
* `N > 1` — экспериментальный режим: запустить оптимизацию, когда готово не менее `N` входов.

#### Пример {#cost-based-optimizer-partial-example}
...
#### Результат {#cost-based-optimizer-partial-result}

Стоимостный оптимизатор будет переупорядочивать готовые поддеревья `JOIN`, не ожидая материализации остальных входов. Расширенная статистика будет запрашиваться только для таблиц, суммарное число чанков которых на кластере не превышает 10 000.

#### Особенности и ограничения {#cost-based-optimizer-partial-limitations}

* Прагма действует только при включённом [`CostBasedOptimizer`](global.md#costbasedoptimizer).
* Значения больше `1` включают экспериментальный режим: семантика порога и построение плана могут измениться в следующих версиях YQL. Для постоянных запросов рекомендуется использовать `0` или `1`.
* Частичный режим сохраняет больше параллельности, но может построить менее выгодный порядок `JOIN`, чем полное переупорядочивание после готовности всех входов.

## yt.ExtendedStatsMaxChunkCount {#yt.extendedstatsmaxchunkcount}

Ограничивает сбор расширенной поколоночной статистики для стоимостного оптимизатора. Используйте прагму, чтобы получение статистики для таблиц с большим числом чанков не задерживало оптимизацию запроса.

| Тип значения | По умолчанию | Статическая /<br/>динамическая |
| --- | --- | --- |
| Положительное число | — | Статическая |

Значение задаёт максимальное суммарное число чанков входных таблиц на одном кластере:

* Если число чанков не превышает порог, YQL запрашивает расширенную статистику.
* Если порог превышен, статистика не запрашивается и оптимизатор использует менее точные оценки.
* `0` снимает ограничение по числу чанков.
* Если прагма не задана, расширенная статистика не запрашивается.

#### Пример {#extended-stats-max-chunk-count-example}
...
#### Результат {#extended-stats-max-chunk-count-result}

Для входных таблиц с суммарным числом чанков не более 10 000 на кластер оптимизатор получит расширенную поколоночную статистику. Для более крупных наборов таблиц оптимизация продолжится без неё.

#### Особенности и ограничения {#extended-stats-max-chunk-count-limitations}

* Прагма влияет только на сбор статистики для стоимостного оптимизатора и не ограничивает чтение самих таблиц.
* Значение `0` может привести к долгому получению статистики для очень больших таблиц.
* Рекомендуемое начальное значение — `10000`; его стоит менять с учётом числа чанков во входных таблицах и допустимого времени оптимизации.
...
Передаёт совместимую часть условия `WHERE` в {{product-name}} через [`input_query`]({{yt-docs-root}}/user-guide/data-processing/operations/operations-options#common_options). {{product-name}} использует статистику `min`/`max`, чтобы не читать чанки и блоки, для которых условие заведомо ложно. Прагма полезна для выборочного чтения больших статических таблиц со строгой схемой.

| Тип значения | По умолчанию | Статическая /<br/>динамическая |
| --- | --- | --- |
| Флаг | false | Статическая |

#### Пример {#use-ql-filter-example}
...
#### Результат {#use-ql-filter-result}

Совместимые сравнения из `WHERE` будут переданы в `input_query`. Если статистика таблицы позволяет доказать, что в чанке или блоке нет значений `key` из указанного диапазона, {{product-name}} пропустит его без чтения с диска. Остальная часть условия продолжит вычисляться средствами YQL.

#### Особенности и ограничения {#use-ql-filter-limitations}

* Поддерживаются числовые типы, `Bool`, `String`, `Utf8` и их `Optional`-варианты.
* Поддерживаются сравнения `<`, `<=`, `>`, `>=`, `==`, `!=` между колонкой и константным выражением, а также `AND`, `OR`, `NOT`, `EXISTS`, `COALESCE`.
* Таблица должна иметь строгую схему.
* Динамические таблицы не поддерживаются.
* Прагма не применяется к таблицам с пользовательской схемой или колонками, заданными через `WITH SCHEMA` или `WITH COLUMNS`.
...
Задаёт сетевой проект для джобов регулярных операций, создаваемых после установки прагмы. Используйте динамическую прагму, если сетевой проект нужно изменить только для части запроса, не затрагивая evaluation-стадию и уже сформированную часть плана.
...
#### Пример {#network-project-example}
...
#### Результат {#network-project-result}

Джобы регулярных операций в части запроса после прагмы будут запущены с сетевым проектом `my-network-project`.

#### Особенности и ограничения {#network-project-limitations}

* Прагма динамическая: она влияет только на последующую часть запроса.
* Установка `yt.NetworkProject` частично отключает DQ. DQ может использоваться для evaluation-стадии или части запроса до установки прагмы.
* Если прагма не задана, используется значение `yt.StaticNetworkProject`.
...
Команда `concatenate` — запись как есть — выполняется быстрее, чем `merge`, которой требуется время на объединение чанков и последующую запись. Используйте `concatenate` для записи таблиц с относительно небольшим количеством чанков.
...
По умолчанию система вычисляет хеш из метаданных таблиц по нескольким параметрам, включая номер ревизии — {{product-name}} `revision`. Номер ревизии — неотрицательное целое число, которое растёт при каждом изменении таблицы. Подробнее про атрибут `revision` читайте в разделе [Дерево метаинформации]({{yt-docs-root}}/user-guide/storage/cypress#time_attributes).

С помощью прагмы вы можете управлять сбросом кеша: включать или исключать номер ревизии при вычислении хеша из метаданных таблиц.
...
Чтобы исключить номер ревизии из вычисления хеша, установите флаг в значение `true`. Тогда при изменении содержимого входных таблиц система не сбросит Query Cache.
...
Прагма управляет кешированием результатов MapReduce-операций в {{product-name}}.

Кеш хранит результаты предыдущих операций. Если система находит в кеше результат такой же операции, то не запускает её повторно, а берёт готовое значение. Так кеш ускоряет операции, которые повторяют вычисления из других запросов.
...
В режиме `normal` при запуске MapReduce‑операции система ищет результат в кеше по пути `//<tmp_folder>/query_cache/<hash>`. Если кеша нет, {{product-name}} проверяет активные операции других запросов. Возможно, среди них уже есть аналогичная операция, которая получит искомый результат. Если такая операция ещё не записала этот результат в кеш, новый запрос дожидается её завершения. После этого система берёт готовый результат из кеша.

Учитывайте, что иногда чтение из кеша занимает больше времени, чем запуск операции с нуля. Так бывает, когда у операции, которая вычисляет результат и записывает его в кеш, меньше ресурсов, чем у той, что была запущена позже и ожидает, чтобы прочитать результат из кеша. В этом случае ожидающий запрос выполнится медленнее, чем при запуске с нуля.
...
Задаёт сетевой проект для джобов всех MapReduce-операций запроса, включая evaluation-стадию. Используйте статическую прагму, когда весь запрос должен выполняться в одном сетевом проекте.

| Возможные значения | Значение по умолчанию | Вид |
| --- | --- | --- |
| Строка | - | Статическая, [per-cluster](*per-cluster) |

#### Пример {#static-network-project-example}
...
#### Результат {#static-network-project-result}

Джобы всех MapReduce-операций запроса, включая evaluation-стадию, будут запущены с сетевым проектом `my-network-project`.

#### Особенности и ограничения {#static-network-project-limitations}

* Прагма статическая и задаётся отдельно для каждого кластера.
* Установка `yt.StaticNetworkProject` полностью отключает DQ при исполнении запроса.
* Подробнее о сетевых проектах см. в [документации {{product-name}}]({{yt-docs-root}}/user-guide/data-processing/operations/mtn).

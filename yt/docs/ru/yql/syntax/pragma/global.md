...
Импорт всех UDF из указанной библиотеки. Чтобы прагма сработала, библиотеку необходимо приложить к запросу через иконку ![иконка](../../../../_images/qt-ui-attach.png =30x30). Обратите внимание: библиотека должна быть разделяемой (.so) и она должна быть скомпилирована под Linux x64.
...
Включает стоимостный оптимизатор порядка `JOIN`. Оптимизатор переупорядочивает входы, минимизируя стоимость плана по статистике таблиц. Работает только при числе входов больше двух.

* `disable` — оптимизация выключена.
* `native` — нативный оптимизатор, рекомендуемый для YT.

...
- снимает с нод бандла `node_tag_filter`, что сгоняет таблет-целлы с нод, и переводит бандл на отдельный drills-фильтр вида `<zone>/<bundle_name>_drills_mode_on`;
- отключает проверки живости таблет-целлов (`mute_tablet_cells_check` и `mute_tablet_cell_snapshots_check`), чтобы не поднимать алерты во время учений.

При выключении (`enable_drills_mode = %false`) эти действия откатываются в обратном порядке: возвращается обычный `node_tag_filter`, затем, по истечении grace-периода, вновь включаются проверки живости.
...
Использование режима учений может быть опасно при работе с асинхронными репликами. Если во время учений реплика внезапно исчезает, транзакции асинхронной репликации могут «залипать» и мешать таблетам отмонтироваться — в таком состоянии таблетки надолго уходят в `freezing` (см. [YTADMINREQ-46877](https://st.yandex-team.ru/YTADMINREQ-46877)).

Риск сохраняется, даже если вы не отмонтируете таблицы руками: фоновая балансировка таблетов может отмонтировать их сама и триггерить ту же проблему.

В качестве workaround во время учений рекомендуется отключать балансировку таблетов. Отсутствие балансировки в течение нескольких часов, как правило, не влияет на работоспособность бандла. Подробности о проблеме с зависающими транзакциями репликации — в [YT-22930](https://st.yandex-team.ru/YT-22930).

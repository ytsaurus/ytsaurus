...
Для упрощения настройки типичного сценария начиная с версии 0.13.54 в состав пакета `ytsaurus-client` входит команда `yt admin bundle-controller` ([исходный код](https://github.com/ytsaurus/ytsaurus/blob/main/yt/python/yt/admin/bundle_controller.py)). В более ранних версиях та же функциональность доступна через standalone-утилиту [bundle_controller_tools](https://github.com/ytsaurus/ytsaurus/tree/main/yt/yt/scripts/dynamic_tables/bundle_controller_tools). Для работы необходимо сохранить токен в `~/.yt/token` или воспользоваться альтернативными способами конфигурации токена, описанными в [документации](../api/python/userdoc#configuration_token) по Python API. Адрес кластера указывается в переменной `YT_PROXY` или через аргумент `--proxy`.

Все подкоманды поддерживают флаг `--dry-run` (только вывести список действий, не изменяя кластер) и `--yes` (не запрашивать интерактивных подтверждений).

Рекомендуется запускать команду `yt admin bundle-controller init --init-all`. При запуске необходимо указать флаги `--cpu` и `--memory`, соответствующие параметрам инстансов таблетных нод: количество ядер CPU и байт RAM. Если в кластере есть инстансы разных размеров, рекомендуется выставлять параметры по минимальному инстансу.

В дальнейших разделах описаны шаги по настройке, выполняемые командой.

### Системные директории

**При запуске `yt admin bundle-controller init` системные директории создаются автоматически. Для пропуска этого шага можно использовать флаг `--no-init-system-directories`.**
...
**Для автоматической настройки зоны необходимо запустить `yt admin bundle-controller init` с флагом `--init-default-zone`.**
...
**Для автоматической настройки инстансов необходимо запустить `yt admin bundle-controller init` с флагом `--init-nodes`.**
...
**Для автоматической настройки бандлов необходимо запустить `yt admin bundle-controller init` с флагом `--init-bundles` или `--init-all`.**
...
Команда `yt admin bundle-controller init` вычисляет необходимое число нод для каждого существующего бандла по текущему количеству таблет-целлов. В случае, если необходимо выдать больше или меньше нод, используйте аргумент `--bundle-node-count`, принимающий на вход yson-словарь в формате `{<bundle_name>=<node_count>}`.

#### Аккаунты под журналы и снапшоты

**Для автоматической настройки бандлов необходимо запустить `yt admin bundle-controller init` с флагом `--init-bundle-system-quotas` или `--init-all`.**
...
**Для создания бандла можно использовать команду `yt admin bundle-controller create-bundle <bundle_name>`.**
...
После добавления инстансов в кластер необходимо разметить их атрибутом `bundle_controller_annotations`. Для этого можно использовать команду `yt admin bundle-controller init` с флагом `--init-nodes`.
...
Для упрощения настройки ограничений `cpu` и `memory` есть команда `yt admin bundle-controller set-bundle-resource-limits`, которая принимает ограничение на число нод и устанавливает лимиты в соответствии с конфигом зоны.
...
Флаг устанавливается на бандле на том кластере, который вы хотите прикрыть. Например, чтобы провести учения для бандла на `seneca-sas`, выставьте `enable_drills_mode = %true` в этом бандле на `seneca-sas`.
...
- запустить команду `yt admin bundle-controller drop-allocations <bundle_name>`. Эта команда удалит некорректные запросы на выдачу нод;
- убедиться, что конфигурация корректна (например, путём запуска `yt admin bundle-controller init --init-all --cpu <cpu_guarantee> --memory <memory_guarantee>`).

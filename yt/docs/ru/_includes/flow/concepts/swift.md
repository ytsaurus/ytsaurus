...
Требование распространяется и на привязку выходных сообщений к родительским (`set_parent_ids`): при повторном вычислении каждая группа вывода должна получить тех же родителей в том же порядке. Типичная ошибка — итерация по неупорядоченной структуре (например, hash-таблице или множеству) при группировке батча по ключам: порядок групп меняется от запуска к запуску, и повторное вычисление даёт другой вывод. При этом по умолчанию у каждого выходного сообщения Swift-компьютейшена должен быть ровно один родитель; несколько родителей допустимы только при [`allow_batching_with_relaxed_guarantees`](../../../flow/concepts/guarantees.md#swift-allow-batching-with-relaxed-guarantees) — подробнее см. [Когда задавать lineage явно](../../../flow/concepts/lineage.md#explicit-lineage).

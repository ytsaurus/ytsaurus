...
За счёт поля `group_by_schema` в [спеке](../../../flow/concepts/spec.md) [компьютейшена](#computation) входной поток группируется и данные по одному [ключу](#key) всегда будут попадать в конкретную партицию. Для этого за каждой партицией определён диапазон ключей `[LowerKey; UpperKey)`.

...
- **Клиент стейта (state client)** — типизированный по типу стейта объект (`Client<TState>`), который process function заводит по одному на каждый именованный стейт и инициализирует в `Init`. Через клиент происходит доступ к стейту по ключу. Бывает **read-write** (работает и с [внутренним](#internal-state), и с [внешним](#external-state) стейтом) и **read-only** ([джойнер](#external-state-joiner) к внешнему стейту).
...
Аксессор действителен только в пределах текущей [эпохи](../../../flow/concepts/glossary.md#epoch). Его нельзя сохранять в полях process function или переиспользовать между эпохами — на каждой эпохе стейт нужно получать через клиент заново.

...
|| `upload_temp_path` | Директория, в которой загружаемый файл создаётся как временный перед перемещением в кеш. По умолчанию родительская директория `cache_path`. Задайте её вместе с `cache_path`, если у пользователя нет доступа к `//tmp` ||
|#

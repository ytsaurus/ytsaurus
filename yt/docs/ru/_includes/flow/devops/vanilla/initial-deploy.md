...
Обязательные параметры — `pool` и `worker.count`. Остальные поля имеют разумные значения по умолчанию: контроллер — одна джоба, каждая джоба (и контроллера, и воркера) получает `cpu_limit = 6` и `memory_limit = 18 GiB`. Порты внутри джобы фиксированные — `rpc_port = 10080`, `monitoring_port = 10081` и `companion.port = 10082`: сетевая изоляция джоб исключает конфликты, а при необходимости порты переопределяются через `node_config`-патч. Отдельный случай — хост с общей сетью, где такой изоляции нет: см. [Дополнительные параметры](#advanced-config).
...
Значение `network_project = #` отключает внутренний дефолт. Без сетевого проекта фиксированные порты могут конфликтовать на хосте с общей сетью, поэтому в таком режиме задайте [`port_count`](#advanced-config) для каждой задачи.
...
|| `port_count` | Сколько портов задача запрашивает у {{product-name}} вместо фиксированных. Нужно на хосте с общей сетью ||
|#

На хосте с общей сетью фиксированные порты соседних джоб столкнулись бы — там порты нужно запросить у {{product-name}} полем `port_count` задачи. Выданные порты приходят в переменных окружения `YT_PORT_<i>` и имеют приоритет над конфигом: `YT_PORT_0` — `rpc_port` (и `bus_server.port`), `YT_PORT_1` — `monitoring_port`, `YT_PORT_2` — `companion.port`. Поэтому контроллеру и воркеру без компаньона нужно `port_count = 2`, а воркеру с [компаньоном](../../../../flow/concepts/companion.md) — `port_count = 3`; при меньшем значении часть портов останется фиксированной и снова может конфликтовать.

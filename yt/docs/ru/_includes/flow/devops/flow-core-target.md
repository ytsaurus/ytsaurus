...
{% if audience == "internal" %}- **На один запуск** — флагом `--skip-set-flow-core-target` в командной строке раннера. Удобно для разовых хотфиксов или экспериментов.{% else %}- **На один запуск для C++ и Java** — флагом `--skip-set-flow-core-target`. Раннеры Python и Go не передают этот флаг `flow_server`; для них используйте `set_flow_core_target = %false` в конфиге.{% endif %}
...
{% if audience == "internal" %}После сброса пайплайн теряет защиту от зомби-процессов. Если вы перезапустите раннер без флага `--skip-set-flow-core-target`, он немедленно запишет новый `FlowCoreTarget` поверх сброшенного.{% else %}После сброса пайплайн теряет защиту от зомби-процессов. При следующем запуске раннер снова запишет `FlowCoreTarget`, если автоматическая установка не отключена в конфиге или, для C++ и Java, флагом `--skip-set-flow-core-target`. Python и Go игнорируют этот флаг.{% endif %}

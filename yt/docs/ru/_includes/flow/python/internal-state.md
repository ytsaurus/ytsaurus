...
`set()` по-прежнему заменяет значение целиком, `clear()` удаляет стейт. Дефолт `None` значением не является и не привязывается. Значение, которое кодируется в пустые байты — например, protobuf-сообщение со всеми полями по умолчанию, — тоже значением не является: его запись удаляет стейт. Дефолт `get_or_default()` у `RawStateAccessor` привязывается так же, как у остальных аксессоров, и его вернёт последующий `get()`. Но объект `bytes` неизменяем, изменить привязанный дефолт на месте нельзя, поэтому непустое значение сырого стейта попадает в {{product-name}} только через `set()`.
...
| `clear()` | — | Удалить стейт для текущего ключа |
| `get_or_default(default: bytes)` | `bytes` | Вернуть текущее значение или привязать и вернуть `default` (в {{product-name}} попадает только через `set()`) |

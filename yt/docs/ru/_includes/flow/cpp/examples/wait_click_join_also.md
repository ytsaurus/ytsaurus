...
Все эти объекты необходимо создать в {{product-name}}, перед тем как [запускать работу пайплайна](../../../../flow/devops/index.md#launch-flow).

{% if audience == "internal" %}

Для создания объектов можно воспользоваться библиотекой [YtSync]({{yt-sync-docs}}/). Она позволяет лаконично описать объекты и их отличия между разными [окружениями](../../../../flow/concepts/glossary.md#environment) и выполнять операции создания, обновления, [миграции](../../../../flow/concepts/glossary.md#migration) (в ряде случаев).

В данном примере продемонстрировано использование режима easy mode, подробную документацию по которому можно найти [здесь]({{yt-sync-docs}}/stages_specification).

...
Во внутреннем контуре новый проект заводится IDP-шаблоном `yt_flow`: он генерирует скелет, спеку, yt_sync, CI и первый стенд, а создать приложение и пройти чек-лист помогает [AI-агент](../../flow/ai-agent.md) со скиллом `yt-flow-idp`. Шаги ниже объясняют, из чего состоит пайплайн, и остаются актуальными для его доработки.

{% endif %}

Независимо от выбранного языка, создание пайплайна включает следующие шаги:

1. **Попробуйте [Быстрый старт](../../flow/quickstart.md)** — запустите пример C++ Word Count, чтобы познакомиться с инфраструктурой Flow.
...
10. **Запустите пайплайн** и проверьте его состояние {% if audience == "internal" %}в UI {{product-name}} (ссылку печатает раннер) или {% endif %}командой `yt flow get-pipeline-state <pipeline_path>`. Перед выкаткой прочитайте [Правила выкатки и изменения таблиц](../../flow/devops/vanilla/releases.md#release-and-configure-basic-rules).

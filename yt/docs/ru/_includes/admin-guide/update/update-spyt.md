...
При обновлении SPYT кластер YTsaurus продолжает работать в обычном режиме, все операции с данными доступны.

Во время обновления SPYT происходят следующие изменения:

- Кластер YTsaurus работает в штатном режиме;
- Серверные компоненты YTsaurus не перезапускаются;
- Доступ к данным через YTsaurus API сохранён;
...
Описанный выше процесс добавляет новую версию SPYT как отдельный ресурс, не затрагивая существующий. Так на одном кластере YTsaurus сосуществуют несколько версий SPYT — это позволяет разным командам использовать разные версии одновременно.
...
2. Удалите артефакты старой версии из Кипариса через веб-интерфейс YTsaurus. Для каждой версии SPYT артефакты хранятся в двух узлах — с дистрибутивом и с конфигурацией. Откройте каталоги `//home/spark/spyt/releases` и `//home/spark/conf/releases`, найдите узлы с номером ненужной версии и удалите их из контекстного меню.

...
<small>Таблица 2 — Описание полей лога</small>
...
| `correlation_id`                             | Специальный идентификатор запроса, который генерируется клиентом и не изменяется в случае ретраев                             |
...
| `error`                                      | Стуктурированное описание ошибки, в случае если запрос завершился неуспешно                                                   |
...
| `cpu_time`                                   | Время, потраченное проксей на исполнение запроса (в него не входит время исполнения запроса в других компонентах кластера)    |
| `duration`                                   | Общая длительность запроса                                                                                                    |
...
### Поставка структурированных логов мастера в Кипарис { #structured_log_delivery }

{% include [Поставка структурированных логов мастера](structured-log-delivery.md) %}
...
<small>Таблица 3 — Настройки ротации логов </small>

...
В этой статье вы узнаете, как работает механизм шифрования в YT и как настроить его для защиты данных в вашем кластере.

{{product-name}} поддерживает шифрование внутреннего трафика между компонентами кластера с использованием TLS 1.2. В документе описана архитектура шифрования и инструкции по настройке для различных сценариев развертывания.
...
- **Одностороннее шифрование** — клиент проверяет подлинность сервера
- **Взаимная аутентификация (mTLS)** — обе стороны проверяют сертификаты друг друга

Механизм ротации сертификатов позволяет обновлять их без остановки сервисов. При развертывании в Kubernetes можно использовать cert-manager для автоматического управления сертификатами.
...
Чтобы правильно настроить шифрование, важно понимать, как устроено взаимодействие компонентов в кластере {{product-name}}. Рассмотрим архитектуру транспортного уровня и процесс установки защищенного соединения.

### Что такое уровень bus {#what-is-bus-layer}

`bus` — это транспортный уровень {{product-name}}, который обеспечивает передачу сообщений между компонентами. `bus` работает с сообщениями, что позволяет чётко определять их размер, в отличие от обычного TCP, который работает с потоком байт.
...
Каждый компонент выступает одновременно в двух ролях:
...
**THandshake** — это protobuf-сообщение, которым обмениваются компоненты сразу после установки TCP-соединения. Оно содержит:
...
- Принятие решения — каждая сторона анализирует THandshake противоположной стороны и свою конфигурацию для решения о включении шифровании.
...
## Настройка шифрования {#configuration}
...
При установке соединения результат зависит от комбинации режимов на клиенте и сервере:
...
- Kubernetes с оператором {selected}
  ...
  Эта конфигурация автоматически создает самоподписанный CA, выпускает сертификаты и настраивает компоненты для работы с шифрованием.
...
- Ручное развертывание
  ...
  2. Перезапустите компоненты кластера
  3. Проверьте установку шифрованных соединений в логах
...
- Внешние кластеры
  ...
  2. **Добавьте конфигурацию для подключения к кластеру B**:
     ```yaml
     cluster-b:
       discovery_servers:
  ...
  3. **Загрузите обновленную конфигурацию**:
     ```bash

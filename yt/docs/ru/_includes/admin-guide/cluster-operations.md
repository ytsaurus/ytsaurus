...
Бывает полезно узнать состояние ноды с точки зрения планировщика. Данаая информация содержится в [Орхидее](../../user-guide/storage/orchid.md) планирощика. Например, в Орхидее можно узнать состояние ноды `scheduler_state`.

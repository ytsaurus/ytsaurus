...
5. Запросы к динамическим таблицам (insert-rows, select-rows, lookup-rows и другие операции с данными) **не отображаются** в истории доступа к таблицам. Лог фиксирует только обращения к мастеру Кипариса (операции с метаданными и статическими таблицами); запросы к таблетным нодам проходят мимо мастера и в лог не попадают.
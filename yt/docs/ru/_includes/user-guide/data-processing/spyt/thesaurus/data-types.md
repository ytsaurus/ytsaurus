...
2. **Точность Decimal**: {{product-name}} ограничивает `precision` типа `decimal` значением 35 (не `scale`). [Подробнее про Decimal](../../../../../user-guide/storage/data-types.md#schema_decimal)
...
6. **«Arrow не поддерживается»**: для типов с такой пометкой при включённой опции [`spark.yt.read.arrow.enabled`](../../../../../user-guide/data-processing/spyt/cluster/configuration.md) (по умолчанию `true`) SPYT автоматически переключается на wire-протокол.
...
## Сопоставление типов при чтении из {{product-name}} { #read-types }
...
| any/yson             | BinaryType           | YsonType                | `YsonType` — кастомный тип SPYT (см. [раздел ниже](#yson-type)) |
...
| date32               | Date32Type           | Date32Type              | Кастомный тип SPYT расширенного диапазона (см. [раздел ниже](#wide-datetime-types)). Arrow не поддерживается |
| datetime64           | Datetime64Type       | Datetime64Type          | Кастомный тип SPYT расширенного диапазона (см. [раздел ниже](#wide-datetime-types)). Arrow не поддерживается |
| timestamp64          | Timestamp64Type      | Timestamp64Type         | Кастомный тип SPYT расширенного диапазона (см. [раздел ниже](#wide-datetime-types)). Arrow не поддерживается |
| interval64           | Interval64Type       | Interval64Type          | Кастомный тип SPYT расширенного диапазона (см. [раздел ниже](#wide-datetime-types)). Arrow не поддерживается |
...
## Сопоставление типов при записи в {{product-name}} { #write-types }

| Тип Spark            | Тип {{product-name}}                                                          | Примечания             |
|----------------------|-------------------------------------------------------------------------------|------------------------|
| NullType             | null                                                                          |                        |
| ByteType             | int8                                                                          |                        |
| ShortType            | int16 (или uint8 при использовании schema_hint)                                              | См. [пример ниже](#schema-hint) |
...
| UInt64Type           | uint64                                                                        |                        |
| YsonType             | any                                                                           | `YsonType` — кастомный тип SPYT (см. [раздел ниже](#yson-type)) |

### Переопределение типа через schema_hint { #schema-hint }

Если стандартное сопоставление не подходит, можно явно указать целевой {{product-name}}-тип через [schema_hint](../../../../../user-guide/data-processing/spyt/thesaurus/read-options.md#schema_hint).
...
## Кастомные типы SPYT { #custom-types }

Перечисленные ниже типы не входят в стандартный Apache Spark — они предоставляются библиотекой SPYT.

### Временны́е типы расширенного диапазона { #wide-datetime-types }

В {{product-name}} существуют два набора временны́х типов:
...
Подробнее про устройство временны́х типов в {{product-name}} см. в разделе [Временны́е типы](../../../../../user-guide/storage/data-types.md#temporal_types).

### YsonType { #yson-type }

`YsonType` — кастомный UDT (User Defined Type), хранящий данные в виде YSON-байтов.

- При чтении из {{product-name}}: тип `any`/`yson` → `YsonType` (расширенный режим) или `BinaryType` (базовый режим).
- При записи в {{product-name}}: `YsonType` → тип `any`.

Подробнее про работу с `YsonType` см. в разделах [Python API](../../../../../user-guide/data-processing/spyt/API/spyt-python.md) и [Scala API](../../../../../user-guide/data-processing/spyt/API/spyt-scala.md).

...
* `chunk_count_per_merge_job` — определяет максимальное количество чанков, которое планировщик будет пытаться объединить одним Merge-джобом за один проход (не выходя при этом за ограничение в `desired_chunk_size` в `job_io` у Merge джобов, по умолчанию 2 ГиБ).
* `chunk_size_threshold` — размер чанка в байтах, начиная с которого чанк считается достаточно большим, не участвует в слиянии и попадает в выходную таблицу как есть. То есть слиянию будут подлежать только те чанки, размер которых меньше `chunk_size_threshold` (по умолчанию 128 МиБ).

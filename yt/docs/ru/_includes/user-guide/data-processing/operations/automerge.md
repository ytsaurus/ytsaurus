...
* `chunk_count_per_merge_job` — определяет размер порции чанков, которую планировщик будет пытаться объединить за один проход (не выходя при этом за ограничение в `desired_chunk_size` в `job_io` у Merge джобов, по умолчанию 2 ГиБ).

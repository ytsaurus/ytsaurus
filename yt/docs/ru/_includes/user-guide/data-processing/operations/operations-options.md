...
- `data_weight_per_job` — рекомендованный объем данных на входе одного джоба в байтах. Значение по умолчанию зависит от типа операции: 128 Mb для Map и Reduce, 20 Gb для Merge. Опция не имеет смысла для составных операций, например MapReduce. В таких операциях нужно использовать следующие опции:
...
- `issue_temporary_token` (false) — выпустить временный токен для аутентифицированного пользователя, запустившего операцию, и передать его в пользовательские джобы через `secure_vault`. Токен дает доступ к исходному кластеру и удаляется после завершения операции. По умолчанию токен записывается в ключ `YT_TOKEN` secure vault и доступен в джобах как `YT_SECURE_VAULT_YT_TOKEN`.
- `temporary_token_environment_variable_name` (`YT_TOKEN`) — ключ secure vault, в который записывается временный токен при включенной опции `issue_temporary_token`. Соответствующая переменная окружения в джобе имеет вид `YT_SECURE_VAULT_<temporary_token_environment_variable_name>`. Если в `secure_vault` уже есть такой ключ, указанное пользователем значение `secure_vault` сохраняется, а временный токен под этим ключом не передается.
...
- `max_speculative_job_count_per_task` (10) — лимит числа спекулятивных джобов (пока поддерживаются для операций Map, Reduce, MapReduce, Sort, Merge).
- `job_speculation_timeout` — опциональный таймаут в миллисекундах, по истечении которого планировщик принудительно запускает спекулятивную копию исполняемого джоба (параллельную джобу на тех же входных данных). Отсчёт начинается с момента старта джоба и включает время подготовки (`prepare_duration`) и время выполнения (`exec_duration`). По умолчанию не задан. Это гарантированный триггер спекуляции, не зависящий от эвристик Job Splitter'а, который может запустить спекулятивную джобу и раньше — на основе статистик о скорости чтения/записи, времени подготовки и других эвристик. Параметр можно задать на двух уровнях: в `spec` операции (низкий приоритет) и в `user_job_spec` конкретного user job'а (высокий приоритет, перекрывает значение из `spec`).
...
- `job_speculation_timeout` — опциональный таймаут в миллисекундах, по истечении которого планировщик принудительно запускает спекулятивную копию исполняемого джоба (параллельную джобу на тех же входных данных). Отсчёт начинается с момента старта джоба и включает время подготовки (`prepare_duration`) и время выполнения (`exec_duration`). По умолчанию не задан. Это гарантированный триггер спекуляции, не зависящий от эвристик Job Splitter'а, который может запустить спекулятивную джобу и раньше — на основе статистик о скорости чтения/записи, времени подготовки и других эвристик. Значение, заданное в `user_job_spec`, имеет более высокий приоритет, чем значение, заданное в `spec` операции.
- `waiting_job_timeout` — опциональный таймаут, в течение которого джоб может находиться в очереди на запуск на ноде. По умолчанию не задан. Допустимый диапазон значений: от 10 секунд до 10 минут.

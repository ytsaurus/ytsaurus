...
{% if audience == "internal" %}В текущей реализации на кластерах {{mr-clusters}} диски под джобы находятся в медиуме `ssd_slots_physical`, соответственно, для их использования необходимо заказать дисковую квоту в аккаунт в данном медиуме. Подробнее о том, как запросить квоту, можно узнать в разделе [Запрос и получение ресурсов](../../../../user-guide/storage/quota-request.md#zapros-na-rasshirenie-kvoty-pod-sushestvuyushij-akkaunt).{% endif %}

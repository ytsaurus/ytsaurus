...
- `alias` — алиас данной операции. Делает операцию доступной в поиске и с использованием команды `get-operation` по указанной строке. Алиас должен начинаться с символа `*`. Одновременно может быть запущена только одна операция с данным алиасом. После завершения операции другая операция с таким же алиасом может быть перезапущена. `get-operation` возвращает последнюю опеарцию.

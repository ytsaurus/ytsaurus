...
Скорость передачи данных между кластерами квотируется. Подробнее см. в разделе [Квотирование межкластерной сетевой полосы](../../../../user-guide/data-processing/operations/cluster-throttlers.md).
...
Для одноразовых операций можно использовать коммунальные пулы вида `transfer_<target_cluster>`, например для переливки таблицы с кластера "foo" на кластер "bar" необходимо запустить операцию RemoteCopy на кластере "bar" и указать в параметрах `cluster_name = "foo"` и `pool = "transfer_foo"`.
{% endif %}
...
Операция RemoteCopy (`remote_copy`) поддержана только для [сортированных таблиц](../../../../user-guide/dynamic-tables/sorted-dynamic-tables). Для копирования упорядоченной таблицы следует сконвертировать её в статическую. Скопировать на другой кластер упорядоченную таблицу с сохранением таблетов невозможно.

{% if audience == "internal" %}

{% note info "Примечание" %}

Для `remote_copy` динамических таблиц рекомендуется использовать скрипт {{dynamic-tables.remote-copy.links.dyntable-remote-copy-script}}.
...
В процессе работы в структуре динамической таблицы могут появляться системные объекты, называемые chunk view. Операция `remote_copy` их не поддерживает, поэтому перед запуском операции необходимо их удалить, выставив в атрибут `forced_chunk_view_compaction_revision` значение `1`, после чего выполнить `remount-table`. Более подробно см. в разделе [Форсированная компактификация](../../user-guide/dynamic-tables/compaction#forced_compaction).

По умолчанию входная динамическая таблица должна быть заморожена (`frozen`) или отмонтирована (`unmounted`) — иначе операция завершится ошибкой. Параметр `allow_unfrozen_input_tables` (по умолчанию `false`), задаваемый в корне спецификации операции, снимает эту проверку и разрешает копирование в режиме best effort. В этом режиме проверка состояния таблетов пропускается, а данные, не сброшенные на диск на момент копирования, могут не попасть в результат, поэтому консистентность копии не гарантируется. Используйте параметр только тогда, когда допустимо получить потенциально неполную копию.

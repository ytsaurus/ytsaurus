...
Идентификатор основной транзакции операции также доступен внутри каждого джоба в переменной окружения `YT_OPERATION_TRANSACTION_ID`. Для операций с выходными таблицами это будет `output_transaction_id`; в противном случае используется `async_scheduler_transaction_id`. Эту переменную можно использовать, например, в качестве prerequisite при взятии локов в Cypress из джоба, чтобы обеспечить взаимное исключение между конкурентными операциями.
...
Чтение данных с другого кластера (удалённое чтение) подчиняется ограничениям на межкластерную сетевую полосу. Подробнее см. в разделе [Квотирование межкластерной сетевой полосы](../../../../user-guide/data-processing/operations/cluster-throttlers.md).

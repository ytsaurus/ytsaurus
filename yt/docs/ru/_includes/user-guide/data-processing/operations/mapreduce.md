...
* `partition_count`, `partition_data_weight` (`partition_data_size`) — опции, которые указывают, сколько партиций должно быть сделано в сортировке. `partition_count` задаёт количество партиций напрямую; `partition_data_weight` задаёт целевой объём несжатых данных на одну партицию и используется для автоматического вычисления количества партиций.
* `max_partition_factor` — максимальное количество дочерних партиций одной партиции при многоуровневом партицировании.
* `partition_data_weight_for_merging` — при включённом `enable_final_partitions_merging` задаёт максимальный объём данных партиции после слияния физических партиций. Не должно превышать `data_weight_per_sort_job`.
* `enable_final_partitions_merging` (false) — включает слияние соседних финальных партиций так, чтобы их суммарный размер не превышал `data_weight_per_sort_job`. Нельзя использовать вместе с `partition_count`.
...
* `compressed_data_size_per_map_job` — ограничение на сжатый объём входных данных на один map-джоб (рекомендательный характер).
...
* `data_weight_per_intermediate_partition_job` (2 ГБ) — целевой объём данных на один промежуточный partition-джоб.
* `data_weight_per_reduce_job` — целевой объём несжатых данных на один reduce-джоб (рекомендательный характер).
...
* `map_locality_timeout` (5 с) — время, в течение которого планировщик будет ждать появления свободных ресурсов на конкретных узлах кластера, чтобы запускать partition map-джобы с учётом локальности данных.
* `sort_locality_timeout` (5 с) — время, в течение которого планировщик будет ждать появления свободных ресурсов на конкретных узлах кластера для запуска sort/shuffle-джобов.
* `reduce_locality_timeout` (1 мин) — время, в течение которого планировщик будет ждать появления свободных ресурсов на конкретных узлах кластера, чтобы запускать все sort- и merge-джобы одной партиции на одном узле. Данное действие необходимо для обеспечения большей локальности чтения при последующем слиянии сортированных данных.
...
* `shuffle_start_threshold` (0.75) — доля map-джобов, которая должна завершиться, прежде чем будут разрешены shuffle/sort-джобы.
* `merge_start_threshold` (0.9) — доля map-джобов, которая должна завершиться, прежде чем будут разрешены reduce/merge-джобы.
* `enable_partitioned_data_balancing` (false) — если включено, планировщик стремится равномерно распределить partition-джобы по кластеру (по объёму несжатых входных данных) для балансировки нагрузки на I/O на этапе shuffle.
* `enable_intermediate_output_recalculation` (true) — если включено, недоступные промежуточные чанки пересчитываются перезапущенными джобами; иначе операция ждёт их появления (или завершается с ошибкой согласно тактике unavailable chunk).
* `max_shuffle_data_slice_count` (10 000 000) — жёсткое ограничение на суммарное количество слайсов данных в shuffle-пуле. При превышении операция завершится с ошибкой.
* `max_shuffle_job_count` (200 000) — жёсткое ограничение на суммарное количество shuffle-джобов. При превышении операция завершится с ошибкой.
* `max_merge_data_slice_count` (10 000 000) — жёсткое ограничение на суммарное количество слайсов данных по всем merge-пулам. При превышении операция завершится с ошибкой.
* `max_chunk_slice_per_shuffle_job` (8000) — ограничение на количество слайсов чанков в одном shuffle-джобе.
* `max_chunk_slice_per_intermediate_partition_job` (8000) — ограничение на количество слайсов чанков в одном промежуточном partition-джобе.
* `shuffle_network_limit` (0) — ограничение на количество shuffle-джобов, одновременно передающих данные по сети (0 — без ограничений).
* `force_job_size_adjuster` (false) — принудительно включает регулятор размера джобов для partition map и sorted reduce/merge джобов.

...
### ConvertYsonToJson
Если поддержанного функционала работы с YSON недостаточно, в CHYT есть возможность конвертировать YSON в JSON.
...
Функция `ytTables(arg1, [arg2, ...])` &mdash; позволяет прочитать объединение множества таблиц, указанного в аргументах.
...
Функция `ytTables` возвращает объединение множества всех таблиц, перечисленных в аргументах. По принципу работы она похожа на функцию `merge` в {{clickhouse}} (см. [документацию](https://clickhouse.com/docs/en/sql-reference/table-functions/merge)), но предлагает более гибкие способы указания таблиц и оптимизирована для работы с {{product-name}}.

В результате запроса помимо основных колонок таблиц всегда присутствуют виртуальные колонки `$table_index`, `$table_name` и `$table_path`. Они помогают определить, из какой таблицы взята каждая строка. Подробнее — в [разделе про виртуальные колонки](../../../../../user-guide/data-processing/chyt/yt-tables.md#virtual_columns).
...
- **Совместимые типы** — для каждой колонки выбирается "наиболее общий тип", к которому можно привести все типы.
  ...
  Ввиду особенностей хранения данных в {{product-name}}, беззнаковые числовые типы являются несовместимыми со знаковыми (т.е. типы `int32` и `uint32` несовместимы).  

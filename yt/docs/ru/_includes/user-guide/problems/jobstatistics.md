...
|`user_job/cpu_reserve` | Количество процессорного времени, гарантированное пользовательскому джобу в момент запуска. В сотых долях ядра (нужно поделить на 100, чтобы получить ядра). |

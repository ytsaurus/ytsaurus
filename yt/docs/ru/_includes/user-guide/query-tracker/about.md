...
+ `settings` — дополнительные параметры для запроса в формате YSON.
  ...
  + В [YQL](../../../yql/index.md) можно запускать запрос в разных режимах с помощью параметра `execution_mode`. Допустимые значения — `run`, `validate`, `optimize`. Значение по умолчанию — `run`.

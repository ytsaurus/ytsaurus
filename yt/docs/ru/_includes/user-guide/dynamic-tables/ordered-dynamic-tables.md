...
Аналогично, если в схеме таблицы есть колонка [`$cumulative_data_weight`](#cumulative_data_weight_column), то при создании таблицы можно указать её начальное значение в каждом таблете с помощью атрибута `cumulative_data_weights`. Требования к нему такие же, как и к `trimmed_row_counts`: если он передаётся, необходимо также указывать атрибут `tablet_count` равной длины.
...
Точно так же можно задать начальное значение колонки [`$cumulative_data_weight`](#cumulative_data_weight_column) в каждом создаваемом таблете — с помощью списка `cumulative_data_weights` той же длины.
...
### Ограничение на размер данных { #max_data_weight }

Для ограничения размера данных в упорядоченной таблице можно выставить максимальный допустимый размер таблета. Для этого достаточно указать его в байтах в `@mount_config/max_ordered_tablet_data_weight` на таблице.
...
По умолчанию отсчёт в каждом таблете начинается с нуля. Начальное значение можно задать явно с помощью атрибута `cumulative_data_weights` при [создании](#methods) таблицы или аргумента `cumulative_data_weights` команды [`reshard_table`](#reshard). Это нужно, например, при добавлении новой очереди в качестве [реплики] уже работающей реплицированной таблицы.

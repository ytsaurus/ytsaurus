...
Параметр `min_data_ttl` определяет интервал, в течение которого данные гарантированно существуют. Важно, чтобы все чтения по таймстампу - как те, в которых `timestamp` указывается явно, так чтения в рамках транзакции, когда в качестве таймстампа используется время начала транзакции - попадали в интервал, когда все данные живы, т.е. `[now() - min_data_ttl, now()]`. Если таймстамп будует старше, чем `now() - min_data_ttl` - успешность чтения не гарантируется. В частности, если на таблице выставлен `min_data_ttl = 0`, то читать из неё стоит только с `sync_last_committed` или `async_last_committed`.
...
Для оценки целесообразности включения фильтра полезно оценить отношение суммарного числа чанков, в которых был произведен поиск ключа, к числу прочитанных ключей (рекомендуем рассмотреть включение фильтра при соотношении больше 3):

...
### Ограничение временного диапазона чтения { #retention-timestamp }

При чтении сортированной динамической таблицы можно указать опцию `retention_timestamp` — включительную нижнюю границу временного диапазона. Если также указана опция `timestamp`, результат строится по версиям значений из отрезка `[retention_timestamp; timestamp]`. Значение `retention_timestamp` не может быть больше `timestamp`. Опцию `retention_timestamp` нельзя сочетать с версионированным lookup.

Граница применяется к каждой колонке отдельно. Если одну колонку записали до `retention_timestamp`, а другую — после, первая будет содержать `null`, а вторая — записанное значение. Если после применения границы в строке не осталось значений, строка считается отсутствующей. По умолчанию `lookup_rows` не возвращает такую строку; с опцией `keep_missing_rows` на её месте возвращается `null`.

`retention_timestamp` влияет только на результат чтения и не удаляет старые версии. Физическое удаление данных регулируют [TTL-настройки](#remove_old_data).

{% note warning "Внимание" %}

Для агрегирующих колонок семантика `retention_timestamp` **не определена**. Значения до границы не участвуют в агрегировании, а компактификация может заранее объединить версии по разные стороны границы. Тем самым результат чтения агрегирующей колонки до и после компактификации может отличаться. **Не используйте эту опцию для чтения хвоста изменений агрегирующей колонки.**
...
Параметр `min_data_ttl` определяет интервал гарантированного существования данных. 

Важно, чтобы все чтения по таймстампу попадали в интервал, когда все данные живы: `[now() - min_data_ttl, now()]`. И чтения с явным указанием `timestamp`; и чтения в рамках транзакции, где в роли таймстампа выступает время начала транзакции. 

Если таймстамп **старше** значения `now() - min_data_ttl`, успешность чтения **не гарантируется**. В частности, если у таблицы выставлено`min_data_ttl = 0`, то читать из неё стоит только с `sync_last_committed` или `async_last_committed`.
...
При записи в таблицу необходимо указывать значение в миллисекундах, в течение которого строка не должна удаляться. Требования к значениям, записываемым в колонку `$ttl`:
...
- Если нужна фильтрация в select-запросах по префиксу ключа, то есть в таких запросах, которые формулируются с использованием ключевых слов `IN` или `BETWEEN`, нужно аналогично включить запись через атрибут `@chunk_writer/key_prefix_filter` и указать в нем параметр prefix_lengths.
...
Параметры записи фильтра, указываются в `@chunk_writer/key_filter` для lookup-запросов и в `@chunk_writer/key_prefix_filter` для select-запросов:
...
- prefix_lengths - список длин префиксов ключей, для которых необходимо создать фильтр. Параметр валиден только для `@chunk_writer/key_prefix_filter`. Не обязательно строить фильтры для всех длин префиксов - select-запросы по префиксу длины l будут обращаться к фильтру, построенному по префиксу наибольшей длины, не превосходящей l.
...
Также для оценки эффективности работы фильтра рекомендуем рассмотреть следующее соотношение:
- yt.tablet_node.lookup.unmerged_missing_row_count.rate / yt.tablet_node.lookup.row_count.rate

В числителе значение соответствует количеству (unmerged) строк, которые были запрошены при чтении отдельно из каждого чанка, но которых в чанке не оказалось. А в знаменателе - итоговое прочитанное число (уже merged) строк. Работа xor filter, как правило, будет эффективна, если данное соотношение больше трех.

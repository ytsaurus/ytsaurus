...
- Для статических таблиц существует функционал [автоматического укрупнения чанков](../../../user-guide/data-processing/operations/chunk-merger.md). Чтобы это не повлияло на конвертацию таблицы, убедитесь, что для самой статической таблицы и для директории, в которой она находится, параметр @chunk_merger_mode установлен в значение none, иначе система автоматически увеличит размер чанков и даже при корректных настройках исходной таблицы в динамической таблице чанки окажутся слишком большими. Операция move не сохраняет явно установленный атрибут @chunk_merger_mode. После перемещения таблица берёт этот атрибут из корневой директории, игнорируя значение, которое было задано для неё ранее. Поэтому рекомендуется явно установить @chunk_merger_mode = none для директории, в которой вы работаете с этими таблицами.

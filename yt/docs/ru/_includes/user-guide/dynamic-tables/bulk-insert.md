...
Для запуска операций на production-кластерах необходимо указать параметр `{enable_bulk_insert=%true}` в спецификации операции.
...
Если для вашего пользователя ранее доступ не был выдан явно, необходимо указать в спецификации операции опцию `{enable_bulk_insert=%true}`.

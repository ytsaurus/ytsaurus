...
3. Создать новую реплику командой `create chaos_table_replica`. Это позволит сохранить данные в очередях репликации до того момента, когда реплика начнет работать. Получившийся идентификатор нужно запомнить для последующей подстановки в `upstream_replica_id`. (При создании реплики можно ей явно передать нужный прогресс, но вообще говоря это не обязательно - главное позже указать его на таблице-реплике перед монтированием.)
...
6. После окончания копирования установить на новой таблице правильный `replication_progress` и `upstream_replica_id` (на этом шаге стоит всегда устанавливать прогресс на таблице-реплике явно - так можно быть уверенными, что он будет именно тот, какой ожидается).

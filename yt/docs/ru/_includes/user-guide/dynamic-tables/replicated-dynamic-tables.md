...
Если новая таблица-реплика является [упорядоченной](../../../user-guide/dynamic-tables/ordered-dynamic-tables.md) и в её схеме есть колонка [`$cumulative_data_weight`](../../../user-guide/dynamic-tables/ordered-dynamic-tables.md#cumulative_data_weight_column), то при её создании стоит также указать атрибут `cumulative_data_weights` — начальное значение этой колонки в каждом таблете. Иначе отсчёт в новой реплике начнётся с нуля, и её значения `$cumulative_data_weight` разойдутся со значениями у остальных реплик. Подходящие значения можно взять из колонки `$cumulative_data_weight` последних уже отреплицированных строк любой из существующих реплик или воспользоваться готовым инструментом по копированию реплики.
...
### Автоматическое переключение синхронной реплики {#sync-replica-switch}
...
Порог `sync_replica_lag_threshold` определяет, когда синхронная реплика с отставанием считается недоступной: если лаг реплики превышает порог, автоматика перестаёт учитывать её как синхронную и при необходимости переключается на другую реплику. По умолчанию порог равен 10 минутам; рекомендуется ставить 1 минуту.
...
Текущий режим для таблицы-реплики можно посмотреть с помощью атрибута `replicated_table_tracker_enabled` объекта реплики мета-кластера:
...
| max_sync_queue_replica_count | integer | Аналогично `max_sync_replica_count`, но для реплик типа `queue` (используется в хаосной репликации). |
| min_sync_queue_replica_count | integer | Аналогично `min_sync_replica_count`, но для реплик типа `queue` (используется в хаосной репликации). | |
| preferred_sync_replica_clusters   | list | Список предпочтительных кластеров для синхронных реплик |
| sync_replica_lag_threshold | Duration | Максимальное значение лага, до которого синхронная реплика считается доступной. По умолчанию равен 10 минутам; рекомендуется ставить 1 минуту. |
| enable_preload_state_check | bool | Проверять ли preload state таблиц, примонтированных в память для определения их доступности. По умолчанию значение `%false`. |
| incomplete_preload_grace_period | Duration | При проверке доступности реплики в смысле preload state она считается доступной, если последний раз preload был успешно завершен не позднее, чем значение это параметра. По умолчанию равен 5 минутам. Данный параметр нужен, чтобы кратковременный preload при перебалансировке таблета не влиял на работу алгоритма. |

...
Далее в тексте примера используется утилита [curl](https://en.wikipedia.org/wiki/CURL) для работы через HTTP. Для удобства записи были выставлены переменные окружения, представленные ниже. Подробнее о получении токена можно прочитать в разделе [Аутентификация](../../../user-guide/storage/auth).

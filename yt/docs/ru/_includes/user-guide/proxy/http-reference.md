...
### Имперсонация пользователя { #user_impersonation }

HTTP-клиент может попросить прокси выполнить запрос от имени другого пользователя {{product-name}}, передав заголовок `X-YT-User-Name`. При этом в запросе по-прежнему должны быть корректные аутентификационные данные, например заголовок `Authorization: OAuth <token>` или `Authorization: Bearer <token>`.

Имперсонация разрешается только если аутентифицированный пользователь имеет право выполнять запросы от имени других пользователей. Обычно это право есть у суперпользователей и специально добавленных в allowlist сервисных пользователей. Если аутентифицированный пользователь не имеет права на имперсонацию или забанен, прокси отклонит запрос с ошибкой аутентификации.

При успешной имперсонации проверки прав доступа и выполнение команды происходят от имени пользователя из `X-YT-User-Name`. Поэтому команда `get_current_user` возвращает пользователя, указанного для имперсонации.

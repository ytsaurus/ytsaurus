...
Значение помещается в `Number`. Значения `date` и `datetime` экспортируются как `Number` со специальными стилями отображения и в Excel выглядят следующим образом:
* `date` — `2020-12-05`

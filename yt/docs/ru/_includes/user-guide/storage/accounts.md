...
Обратите внимание, для внутренних инсталляций {{product-name}} создавать корневые аккаунты необходимо через интерфейс ABC. Подробнее о создании аккаунта и других объектов через ABC рассказано в разделе [Запрос и получение ресурсов](../../../user-guide/storage/quota-request.md#sozdanie-akkaunta-i-diskovoj-kvoty-pod-novyj-proekt).

{% endnote %}

{% endif %}

{% if audience != "internal" %} Создать аккаунт верхнего уровня можно в веб-интерфейсе {{product-name}}, нажав **Create account** на странице **Accounts**. {% endif %}Вложенные аккаунты создаются из командой строки или в веб-интерфейсе {{product-name}}.
...
В случае ошибки `argument command: invalid choice` следует обновить утилиту `yt`. Подробнее можно прочитать в разделе {% if audience == "public" %}[Как попробовать](../../../overview/try-yt.md){% else %}[Как попробовать](../../../quickstart.md){% endif %}.
...
Чтобы добиться стабильной работы процессов поверх {{product-name}}, важно следить за размером свободной квоты в аккаунте. Рекомендуется настроить мониторинг. Подробнее о настройке рассказано в разделе [Мониторинг квот](../../../user-guide/best-practice/quota-monitoring.md).

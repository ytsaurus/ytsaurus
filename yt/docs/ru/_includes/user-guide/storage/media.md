...
Атрибуты `primary_medium` и `media` являются [наследуемыми](../../../user-guide/storage/attributes.md#system_attr). Это означает, что их можно устанавливать на директории, и тогда все создаваемые в поддереве владельцы чанков будут получать соответствующие значения атрибутов автоматически. Использование атрибута `primary_medium` является предпочтительным в большинстве случаев; использовать атрибут `media` таким образом не рекомендуется.

...
1. Число записывается в big-endian представлении.
2. Если число имеет знаковый тип, то старший (знаковый) бит инвертируется. Иначе этот шаг пропускается.
...
*Пример:* мы хотим сохранить момент времени 2025-01-01T00:00:00 в часовом поясе Москвы в типе tz_datetime64. Для этого мы делаем следующее.
    ...
    5. Переводим название часового пояса "Europe/Moscow" в его id, получается `1`.
    6. Записываем id в big-endian представлении, получается `"\x00\x01"`.
    7. Дописываем информацию о временной зоне, получается `"\x80\x00\x00\x00\x67\x74\x5b\x50\x00\x01"`.

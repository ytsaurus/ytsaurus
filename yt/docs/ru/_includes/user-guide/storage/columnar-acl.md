...
На такой случай существует специальный флаг `omit_inaccessible_columns` (значение по умолчанию `%false`), флаг можно указывать при операциях чтения. В CLI используйте `--omit-inaccessible-columns` с командами `read` или `read-table`. Одноименная настройка также имеется в спецификации операций.

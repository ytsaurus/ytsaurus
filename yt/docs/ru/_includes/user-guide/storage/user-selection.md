...
| Java API `ytsaurus-client` | Поддерживается, если в builder не передан `setAuth` | Клиент использует `YT_USER`, а если переменная не задана — значение системного свойства Java `user.name` (`System.getProperty("user.name")`). Если локальное имя не совпадает с логином в {{product-name}}, задайте `YT_USER` явно. |

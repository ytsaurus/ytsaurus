...
Для того, чтобы пользователь явно понимал, что может получить не все данные, необходимо указывать специальный флаг `omit_inaccessible_rows` (значение по умолчанию `%false`) при операциях чтения. В CLI используйте `--omit-inaccessible-rows` с командами `read` или `read-table`. Одноименная настройка также имеется в спецификации операций.

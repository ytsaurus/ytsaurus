...
* Дерево является [транзакционным](../../../user-guide/storage/transactions.md).
* В качестве узла Кипариса могут быть не только файлы и директории, но и [другие объекты](../../../user-guide/storage/objects.md). 

По аналогии с файловой системой в Кипарисе поддерживается [система контроля доступов](../../../user-guide/storage/access-control.md).
...
* Глубина Кипариса в теории не ограничена, но стоит помнить о том, что в [YPath](../../../user-guide/storage/ypath.md) существует лимит на глубину разрешения пути.
...
У узлов Кипариса есть общие для всех объектов [атрибуты](../../../user-guide/storage/attributes.md). Также есть дополнительные атрибуты, представленные в таблице:
...
Обращение к атрибутам при этом не учитывается (обращением к атрибуту считается указание полного пути к атрибуту внутри [YPath](../../../user-guide/storage/ypath.md)). Также не учитываются запросы с включенной опцией `suppress_access_tracking`. 
...
В частности, {{web-interface}} использует `suppress_access_tracking`, так что просмотры содержимого через web UI не приводят к обновлению `access_time`.
...
* В отличие от `access_time`, `touch_time` обновляется при обращении к атрибутам узла, а также при обращениях к узлу через {{web-interface}}. 
  
* Атрибут `touch_time` не учитывает изменение сыновей узла, т.е. атрибут `touch_time` у `map_node` не меняется, если где-то в глубине дерева происходят обращения. Также `touch_time` не обновляется в запросах с включенной опцией `supress_expiration_timeout_renewal`.

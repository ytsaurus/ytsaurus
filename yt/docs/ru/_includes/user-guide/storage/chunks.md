...
Данные, хранящиеся в [таблицах](../../../user-guide/storage/static-tables.md), [файлах](../../../user-guide/storage/files.md) и журналах, разбиты на части, называемые чанками. Чанк хранит непрерывный диапазон данных. В частности, в каждом чанке таблицы содержится некоторый диапазон ее строк.
...
Формат чанка задается атрибутом `optimize_for`. Для построчного формата `optimize_for` имеет значение `lookup`, для поколоночного — `scan`.
...
Установка атрибута влияет только на будущие чанки и не конвертирует таблицы (аналогично установке атрибутов `@erasure_codec` и `@compression_codec`).
...
Для **динамической таблицы** аналогичную задачу решает forced compaction. Перед его использованием обязательно изучите статью [Фоновая компактификация](../dynamic-tables/compaction.md#forced_compaction).

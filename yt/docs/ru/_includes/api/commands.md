...
| `job_cookie`                 | `int`             | Уникальный индекс джоба в операции, деталь реализации контроллера операции. В ванильных операциях может быть использован для группирования "воркеров": если джоб прерывается, то новый джоб, запущенный ему на замену, будет иметь такой же `job_cookie`. |

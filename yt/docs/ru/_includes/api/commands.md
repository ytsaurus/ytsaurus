...
| `ignore_type_mismatch` | Нет       | `false`                     | Не возвращать ошибку, если существующий узел имеет тип, отличный от указываемого. Узел не пересоздается, переданные атрибуты игнорируются. Используется только вместе с `ignore_existing`. |
...
Это низкоуровневая команда для эффективного распределённого и/или параллельного чтения. В отличие от `read_table` эта команда в типичном случае не делает дополнительных запросов к мастеру.

Для того, чтобы чтение корректно работало, на таблицу должна быть взята блокировка в режиме `snapshot` до вызова `partition_tables`, блокировка должна быть активна в момент запросов `read_table_partition`. В противном случае при какой либо модификации таблицы можно получить ошибку о недоступности данных.
...
| `search_by_token_prefix` | Нет |                           | Включить поиск по префиксу токена. |
| `use_full_text_search` | Нет |                           | Включить полнотекстовый поиск. |
| `tutorial_filter` | Нет |                           | Фильтровать обучающие запросы. |
| `sort_order` | Нет |                           | Порядок сортировки результатов. |

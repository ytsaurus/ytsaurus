...
CLI `yt` не читает переменную окружения `YT_USER`. При стандартной работе через HTTP-прокси пользователя определяет токен; для RPC- и native-драйверов действуют отдельные правила. Проверить имя аутентифицированного пользователя можно командой `yt whoami`. Подробнее см. в разделе [Выбор имени пользователя](../../../user-guide/storage/auth.md#user-selection).

...
usage: yt reshard-table [-h] [--params PARAMS] [--path PATH] [--first-tablet-index FIRST_TABLET_INDEX] [--last-tablet-index LAST_TABLET_INDEX] [--tablet-count TABLET_COUNT] [--sync]
                        [--uniform] [--enable-slicing] [--slicing-accuracy SLICING_ACCURACY] [--trimmed-row-counts TRIMMED_ROW_COUNTS [TRIMMED_ROW_COUNTS ...]]
...
```bash
usage: yt abort-op [-h] [--params PARAMS] [--message MESSAGE] [--operation OPERATION] [operation]
```
...
```bash
usage: yt check-operation-permission [-h] [--params PARAMS] [--operation OPERATION_ID] --user USER --permission PERMISSION [--format FORMAT] [operation_id]
```
...
```bash
usage: yt get-operation [-h] [--params PARAMS] [--attribute ATTRIBUTES] [--include-runtime] [--operation OPERATION_ID] [--format FORMAT] [operation_id]
```
...
```bash
usage: yt list-operation-events [-h] [--params PARAMS] [--operation OPERATION_ID] [--event-type EVENT_TYPE] [--format FORMAT] [operation_id]
```
...
```bash
usage: yt patch-op-spec [-h] [--params PARAMS] [--operation OPERATION_ID] [operation_id] patches
```
...
```bash
usage: yt suspend-op [-h] [--params PARAMS] [--operation OPERATION] [--abort-running-jobs] [--reason REASON] [operation]
```
...
```bash
usage: yt get-job-stderr [-h] [--params PARAMS] [--job-id JOB_ID] [--operation-id OPERATION_ID] [--stderr-type {user_job_stderr,gpu_check_stderr}] [job_id] [operation_id]
```
...
usage: yt list-jobs [-h] [--params PARAMS] [--operation OPERATION_ID] [--job-type JOB_TYPE] [--job-state JOB_STATE] [--address ADDRESS] [--job-competition-id JOB_COMPETITION_ID]
                    [--sort-field {type,state,start_time,finish_time,address,duration,progress,id}] [--sort-order SORT_ORDER] [--limit LIMIT] [--offset OFFSET]
                    [--operation-incarnation OPERATION_INCARNATION] [--monitoring-descriptor MONITORING_DESCRIPTOR] [--with-spec] [--with-stderr] [--with-fail-context]
...
```bash
usage: yt get-job-trace [-h] [--params PARAMS] [--operation OPERATION_ID] [--job-id JOB_ID] [--trace-id TRACE_ID] [--from-time FROM_TIME] [--to-time TO_TIME] [operation_id] [job_id]
```
...
> `--from-time`    lower limit for event’s timestamp. Time is accepted as unix timestamp or time string in {{product-name}} format

> `--to-time`    upper limit for event’s timestamp. Time is accepted as unix timestamp or time string in {{product-name}} format
...
```bash
usage: yt list-job-traces [-h] [--params PARAMS] [--operation OPERATION_ID] [--job-id JOB_ID] [--per-process] [--limit LIMIT] [--format FORMAT] [operation_id] [job_id]
```
...
> `--producer-path`    Path to producer in Cypress; cluster may be specified. See also: [YPATH](../../../user-guide/storage/ypath.md)
...
> `producer_path`    address in Cypress. See also: [YPATH](../../../user-guide/storage/ypath.md)
...
> `--producer-path`    address in Cypress. See also: [YPATH](../../../user-guide/storage/ypath.md)
...
> `--input-format`    input format. (yson string), one of «yson», «json», «yamr», «dsv», «yamred_dsv», «schemaful_dsv» with modifications. See also: [Formats](../../../user-guide/storage/formats.md)

> `--output-format`    output format. response or input format: yson or json, for example: «<format=binary>yson». See also: [Formats](../../../user-guide/storage/formats.md)
...
> `flow_command`    Possible choices: start-pipeline, stop-pipeline, pause-pipeline, get-pipeline-spec, set-pipeline-spec, remove-pipeline-spec, get-pipeline-dynamic-spec, set-pipeline-dynamic-spec, remove-pipeline-dynamic-spec, get-pipeline-state, get-flow-view, show-logs, execute, describe-pipeline
...
execute {{product-name}} Flow specific command. Execute {{product-name}} Flow specific command
...
> `--input-format`    response or input format: yson or json, for example: «<format=binary>yson». See also: [Formats](../../../user-guide/storage/formats.md)
...
execute {{product-name}} Flow describe-pipeline command. Execute {{product-name}} Flow describe command

```bash
yt flow describe-pipeline [-h] [--pipeline-path PIPELINE_PATH] [--output-format OUTPUT_FORMAT] [pipeline_path]
```
...
```bash
yt idm copy [-h] [--dst-pool-tree DST_POOL_TREE] [--immediate] [--erase] [--dst-cluster DST_CLUSTER] [--dry-run] destination
```
...
> `--share-operation-options`    share operation options in json format

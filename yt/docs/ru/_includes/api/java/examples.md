...
Если в builder клиента не передан `setAuth`, `ytsaurus-client` загружает имя пользователя из `YT_USER`, а если переменная не задана — использует системное имя пользователя Java. Токен загружается независимо: из `YT_TOKEN` или из файла `~/.yt/token`. Если локальное имя пользователя не совпадает с логином в {{product-name}}, задайте `YT_USER` явно; имя должно соответствовать пользователю токена. Подробнее см. в разделе [Выбор имени пользователя](../../../user-guide/storage/auth.md#user-selection).

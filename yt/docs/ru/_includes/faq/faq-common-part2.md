#### **Q: Что делать, если возникает ошибка «Account "..." is over disk space limit (node count limit, etc)»?** {#account-disk-space-limit}
...
------
#### **Q: Как найти, кто занимает место в аккаунте или все ноды с определённым аккаунтом?** {#find-account-usage}
...
------
#### **Q: Как узнать, сколько ресурсов занимает директория, включая все содержащиеся в ней таблицы, файлы и т. д.?** {#directory-resource-usage}

**A:** `yt get //path/to/dir/@recursive_resource_usage` или выбрать **Show all resources** в разделе **Navigation** в веб-интерфейсе.

------
#### **Q: При работе с системой возникла ошибка «Transaction has expired or was aborted». Что она означает и как с ней бороться?** {#transaction-expired-aborted}
...
-----
#### **Q: На странице операции появилось сообщение «"owners" field in spec ignored as it was specified simultaneously with "acl"». Что это значит?** {#owners-in-spec-ignored}
...
------
#### **Q: Как можно автоматически ротировать узлы, удалять старше определённого времени?** {#rotate-nodes-expiration}
...
------
#### **Q: Как можно автоматически удалять узлы, которыми не пользовались дольше определённого времени?** {#delete-unused-nodes}
...
------
#### **Q: В ходе работы операции появилось предупреждение «Detected excessive disk IO in <job_type> jobs. IO throttling was activated». Что это значит?** {#excessive-disk-usage}
...
------
#### **Q: Какие пути веб-интерфейс исправляет автоматически при включённой настройке Enable path autocorrection?** {#enable-path-autocorrection}
...
------
#### **Q: Как понять, успешно ли скачалась таблица из веб-интерфейса, а если нет, как посмотреть ошибку?** {#success-table-download}
...
------
#### **Q: Вы пытаетесь локально запустить программу, которая обращается через RPC в {{product-name}}, и получаете ошибку «Domain name not found»** {#domain-name-not-found}
...
------
#### **Q: Как скопировать не всю таблицу, а только некоторый диапазон строк?** {#copy-table-range}
...
------
#### **Q: Как узнать, кто работает с моими таблицами?** {#who-access-tables}
...
**A:** Если удаление было через UI и там не была выбрана опция `Delete permanently`, то можно поискать таблицы в [мусорке](https://yt.yandex-team.ru/kolmogorov/navigation?path=//trash/by-account/) в папке соответствующего аккаунта.
Если удаление было через `yt remove` или аналогичные вызовы API, то восстановление **невозможно**.

------
#### **Q: Ошибка «Operations of type "remote-copy" must have small enough specified resource limits in some of ancestor pools»** {#remote-copy-resource-limits}
...
------
#### **Q: Не создался пользователь на кластере для робота. Токен при этом получить смогли.** {#robot-user-not-created}
...
------
#### **Q: Можно ли выдать доступ через {{product-name}} ACL на ABC-сервис?** {#acl-access-abc-service}
...
------
#### **Q: При чтении или копировании таблицы получаю ошибку "Access denied for user "some_user": "full_read" permission for node //home/some_table is not allowed by any matching ACE"** {#access-denied-full-read}

...
`DoProcessMessage` is implemented for you: it reads from the raw `source` message the string column named by the `parameters/data_column` option (`"data"` by default) and parses it into `TProto`. You override one of the hooks:
...
* `DoProcessUnparsed(const TInputMessageConstPtr& inputMessage, TError error, IOutputCollectorPtr output)`: the column named by the `data_column` option is missing (`null`) or `Protobuf` parsing threw an exception; by default the hook rethrows `error`, and you can override this behavior, for example to silently drop invalid messages. An empty but present string isn’t the same as a missing value: it parses successfully into a message with default values if `TProto` has no required fields, and in that case it goes to `DoProcessProto`, not to `DoProcessUnparsed`.

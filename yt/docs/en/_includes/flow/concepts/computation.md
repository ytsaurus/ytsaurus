...
In C++, these classes define execution modes and are used by built-in adapters. Implement new user logic only as a [process function](../../../flow/cpp/process-functions.md), and select the appropriate `TProcessFunction*Computation` in the spec. Don’t create user subclasses of the Computation base classes.

### TTransformComputation {#ttransformcomputation}

This mode handles arbitrary transformations of input data. The processing result is stored in {{product-name}}, so the transformation needn’t be deterministic. It supports [timers](../../../flow/concepts/glossary.md#timer), [states](../../../flow/concepts/glossary.md#state), and [Sink](../../../flow/concepts/glossary.md#sink). Run user process functions in this mode with `TProcessFunctionComputation`; for a passthrough variant without business logic, use [TPassthroughComputation](#passthrough).

### TSwiftMapComputation {#tswiftmapcomputation}

This mode implements a deterministic Map without materializing results in {{product-name}}. It doesn’t support timers, [Source](../../../flow/concepts/glossary.md#source), or Sink. The process function must be strictly deterministic — the system recomputes the result if needed. Use `TProcessFunctionSwiftMapComputation` for user logic and [TSwiftPassthroughComputation](#passthrough) for passthrough.

### TSwiftOrderedSourceComputation {#tswiftorderedsourcecomputation}

This mode reads data from external sources. It requires that the data stream from each instance is ordered. It supports `WatermarkStrategy` to estimate [watermarks](../../../flow/concepts/glossary.md#timestamps-and-watermarks). Use `TProcessFunctionSourceComputation` for user logic and [TSwiftPassthroughOrderedSourceComputation](#passthrough) for passthrough.

### TTransformOrderedSourceComputation {#ttransformorderedsourcecomputation}

This mode processes `Source` data with arbitrary custom logic: parsing, filtering, or expanding one message into several. Run the user process function with `TProcessFunctionTransformOrderedSourceComputation` instead of chaining `TSwiftPassthroughOrderedSourceComputation` → `TProcessFunctionComputation`.
...
A process function keeps its state in a `TMutableStateKeyClient<T>` field, initializes it through `initContext->InitClient(...)` in `Init(const IRuntimeInitContextPtr&)`, and reads it through `GetState(message->Key)` during processing. For an example, see [Process functions (C++)](../../../flow/cpp/process-functions.md).

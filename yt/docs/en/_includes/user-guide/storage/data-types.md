...
| an integer in the range `[-53375809 * 86400 * 10^6, 53375808 * 86400 * 10^6 - 1]`, <br> representing the number of microseconds elapsed since the Unix epoch; <br> the representable time range extends approximately 145,000 years into the past and future <br> see the section on [temporal types](#temporal_types)| `timestamp64` | `timestamp64` |
...
| an arbitrary YSON structure, <br> physically represented as a byte sequence, <br> cannot have the `required=%true` attribute | `any` | `yson` (different from `type`) |
...
| a singular type whose only possible value is `null`; this type is distinct from `null` <br> (creating a separate column of this type is not particularly useful, <br> and we do not expect this type to appear in user tables, <br> but it is useful for YQL integration)| `void` | `void` |
...
All dates and times assume the Gregorian calendar. When working with points in time in the distant past, keep in mind that {{product-name}} does not account for the [adoption of the Gregorian calendar](https://en.wikipedia.org/wiki/Adoption_of_the_Gregorian_calendar), which occurred at different times in different countries. {{product-name}} assumes that the Gregorian calendar has always been used.
...
- a timestamp represented by an integer of the corresponding "timezone-free" type; the number represents a point in time in the [UTC](https://en.wikipedia.org/wiki/Coordinated_Universal_Time)
...
1. If the number has a signed type, the most significant (sign) bit is inverted. Otherwise, this step is skipped.
...
1. Convert the UTC time to a Unix timestamp. The result is `1735678800`.
1. Write this timestamp in big-endian representation. The result is `"\x00\x00\x00\x00\x67\x74\x5b\x50"`.
1. Because the `tz_datetime64` type is based on the signed `Int64` type, invert the most significant bit. The result is `"\x80\x00\x00\x00\x67\x74\x5b\x50"`.
1. Convert the `"Europe/Moscow"` time zone name to its id. The result is `1`.
1. Write the id in big-endian representation. The result is `"\x00\x01"`.
1. Append the time zone id to the timestamp. The result is `"\x80\x00\x00\x00\x67\x74\x5b\x50\x00\x01"`.
...
You need to perform the following steps to obtain a binary representation of a `decimal` number. These steps will be illustrated with the values `3.1415`, `-2.7182` of type `decimal(5, 4)`.
  ...
  1. Write the number as a big-endian sequence. In this example, the strings are `\x00\x00\x7A\xB7`, `\xFF\xFF\x95\xD2`.
  1. Invert the most significant bit. In this example, the strings are `\x80\x00\7A\xB7`, `\x7F\xFF\x95\xD2`.
...
#### Media in the web UI {#schema_tagged_media}
...
[View the full example](#schema_tagged_media_example)
...
{% note warning "Current limitations" %}
...
#### Limits {#schema_tagged_media_limits}

Two limits apply when viewing a table in the web UI:

#|
|| **Limit** | **When it applies** | **Value** ||
|| Cell data loading limit | When opening a table, as the UI loads the values of all cells on the page | 1 KiB by default, up to 64 KiB. Can be [changed](*cell-size-limit-settings) ||
|| Media preview limit | When clicking the preview button ![eye icon](../../../../images/table-cell-preview-icon.svg "preview" =16x), as the UI loads a single value in full | 16 MiB. Cannot be changed ||
|#
...
If the size of a Base64 string exceeds the current cell data loading limit, the UI receives only part of the value, marks it as incomplete, and displays a preview button ![eye icon](../../../../images/table-cell-preview-icon.svg "preview" =16x) in the cell. To view the image, click this button:

{% if audience == "internal" %}

![Incomplete image/png value and preview button](../../../images/table-media.png)

{% else %}

![Incomplete image and audio values and preview buttons](../../../../images/tagged-media-preview-button.png){.center}
...
Thus, media data within the cell limit (1 KiB by default, up to 64 KiB) is displayed immediately; data above the cell limit and up to 16 MiB is displayed after you click the preview button ![eye icon](../../../../images/table-cell-preview-icon.svg "preview" =16x); and data larger than 16 MiB is not displayed.

#### Example {#schema_tagged_media_example}

In this example, you will create a table with a column for a PNG image. You will then write the {{product-name}} logo to it in Base64 and verify the result in the web interface.

1. Save the [{{product-name}} logo](https://raw.githubusercontent.com/ytsaurus/ytsaurus/main/yt/docs/images/logo.png) to the current directory as `logo.png`.

1. Create the `//tmp/tagged-media` table with a column for a PNG image:
...
1. Encode the image contents in Base64 without the Data URL prefix and write the string to the table:
...
1. Open the `//tmp/tagged-media` table in the web interface. The `image` column should display the {{product-name}} logo.
...
The `dict` type with string keys has two representations. By default, the `positional` representation is used as a list (because this is how data is stored in {{product-name}}). For readability, you can enable `named` mode using the `string_keyed_dict_mode` flag.
...
- `text_yt` — uses a string representation in the {{product-name}} format consisting of 4 groups, for example, `61626364-65666768-696a6b6c-6d6e6f70`;
...
- `binary` (default) — encoded as a binary string containing the [binary representation](#schema_decimal_binary) of a `decimal` number.
...
[*cell-size-limit-settings]: To change the default limit, open **Settings** → **Table** → **Cell size limit**. To change the limit for the current table, click the gear icon above the table.

...
| ------------------- | ----------------------------- | ------------------------------------------------------------ |
| `append` | `bool` | Recognized by the data write commands (`write_table`, `write_file`, `upload_parquet` and `upload_orc`) and means that new data is added to the end of the existing data, but does not overwrite it. It is also recognized on the output paths of tables in the specifications of all scheduler operations and has similar semantics. |

...
For token authentication, the token authenticates a user. A username sent separately by a client does not replace the token: an RPC proxy checks that the username matches the user associated with the token and returns the `Manually specified and authenticated users mismatch` error if they differ.
...
| Java API `ytsaurus-client` | Supported when `setAuth` is not passed to the builder | The client uses `YT_USER`, or the value of the Java system property `user.name` (`System.getProperty("user.name")`) if the variable is not set. If the local username differs from the {{product-name}} username, set `YT_USER` explicitly. |

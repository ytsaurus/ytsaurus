...
`yt clickhouse ctl create <alias>`: Create a clique with the specified alias (name). The clique's alias must meet the ID requirements, that is, include only Latin characters, digits, and underscores (`_`) and never begin with a digit. You can also use `*` at the beginning (however, it will be ignored) and the `-` character.
...
`yt clickhouse ctl remove <alias>`: Fully delete the clique and its persistent state. Once the operation is completed, you can't recover the clique anymore. To execute this operation, you need the **Manage** role for this clique.


### Exists { #exists }

`yt clickhouse ctl exists <alias>`: Check whether the clique exists on the cluster. The result of the command is a `bool` value.
...
`yt clickhouse ctl list`: Look up all aliases of the cliques running in the cluster.
...
`yt clickhouse ctl set-option <key> <value> [--alias <alias>]`: Set the speclet's `key` option to `value`. To execute this command, you need the **manage** role for this clique.
...
```
- An arbitrary string. If the string includes characters other than alphanumeric ones, underscores (`_`), and dashes (`-`), to ensure it's parsed correctly as a string, enclose it in double quotes (`"like this"`). Most popular command‑line implementations, such as bash and zsh, parse `"` as a control character and might deliver values without the enclosing double quotes to the CLI. To overcome this issue, enclose the argument additionally in single quotes:
```bash
...
`yt clickhouse ctl set-speclet <speclet> [--alias <alias>]`: Set the clique's speclet in YSON string format. To execute this command, you need the **manage** role for this clique.
...
`yt clickhouse ctl stop <alias>`: Stop the clique. The clique operation will be stopped, and all pool resources allocated to the clique will be released. The clique configuration won't be deleted; the user can restart the clique later using the `start` command. To execute this command, you need the **manage** role for this clique.

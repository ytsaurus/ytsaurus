...
- `data_weight_per_job` — recommended size of input data per job (in bytes). The default value depends on the operation type: 128 MB for Map and Reduce, 20 GB for Merge. This option is useless for composite operations such as MapReduce, which instead call for the following options:
...
- `issue_temporary_token` (false) — issue a temporary token for the authenticated user that started the operation and make it available to user jobs via `secure_vault`. The token grants access to the host cluster and is removed when the operation finishes. By default, the token is stored under the `YT_TOKEN` secure vault key and is available in jobs as `YT_SECURE_VAULT_YT_TOKEN`.
- `temporary_token_environment_variable_name` (`YT_TOKEN`) — secure vault key used for the temporary token issued when `issue_temporary_token` is enabled. The corresponding job environment variable is `YT_SECURE_VAULT_<temporary_token_environment_variable_name>`. If `secure_vault` already contains the same key, the user-specified `secure_vault` value is preserved and no temporary token is exposed under this key.
...
- `max_speculative_job_count_per_task` (10) — limit on the number of speculative jobs (currently supported for Map, Reduce, MapReduce, Sort, and Merge operations).
- `job_speculation_timeout` — optional timeout in milliseconds, after which the scheduler forcibly launches a speculative copy of the currently executing job (a parallel job on the same input data). The countdown starts from the moment the job begins and includes the preparation time (`prepare_duration`) and execution time (`exec_duration`). Not set by default. This is a guaranteed speculation trigger that is independent of the Job Splitter heuristics, which may launch a speculative job earlier — based on read/write speed statistics, preparation time, and other heuristics. The parameter can be set at two levels: in the operation `spec` (low priority) and in the `user_job_spec` of a specific user job (high priority, overrides the value from `spec`).
...
- `job_speculation_timeout` — optional timeout in milliseconds, after which the scheduler forcibly launches a speculative copy of the currently executing job (a parallel job on the same input data). The countdown starts from the moment the job begins and includes the preparation time (`prepare_duration`) and execution time (`exec_duration`). Not set by default. This is a guaranteed speculation trigger that is independent of the Job Splitter heuristics, which may launch a speculative job earlier — based on read/write speed statistics, preparation time, and other heuristics. The value set in `user_job_spec` takes higher priority than the value set in the operation `spec`.
- `waiting_job_timeout` — optional timeout during which the job can remain queued for launch on the node. Not set by default. The allowed value range is from 10 seconds to 10 minutes.

...
Flow processes live in the jobs of a vanilla operation, so their logs are written to the job sandbox: the detailed process history goes to the `logs/flow.log` file, while the job’s `stderr` retains a bounded excerpt from the beginning and end of the stream, up to `max_stderr_size`. By default, the runner configures logging as follows:
...
* the same `info` stream, plus `error` messages from excluded categories, is written to job `stderr`. {{product-name}} retains up to `max_stderr_size` (5 MiB by default): the beginning and end of a long stream, including startup and the latest events.
...
The `zstd` utility may not be present in the job: the set of utilities is determined by the task image. If it isn’t there, add a layer containing it through the task’s `layers` or `system_layer_path` — see [Additional parameters](../initial-deploy.md#advanced-config).

The logging level and composition are overridden through the `node_config` patch in the `vanilla` block, the `logging` section — see [TLogManagerConfig](../../../generated_docs/all_yson_structs.md#NYT_NLogging_TLogManagerConfig).

## Finished jobs {#finished-jobs}

The sandbox disappears with a finished job. Its `stderr` remains available for failed and normally completed jobs, but may be missing when the operation was aborted. The runner sets `max_stderr_count = 150` by default; the `vanilla.max_stderr_count` field can override it. Take `<operation_id>` from the Vanilla operation page and `<job_id>` from its controller or worker task, then fetch the retained log:

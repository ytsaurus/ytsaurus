...
* `PRESORT` (optional) — specifies the sequence inside each group. Syntax is similar to that of [ORDER BY](select/order_by.md).
* `ON` (required) — specifies key columns.
* `USING` or `USING ALL` (required) — UDF call. See the detailed rules below.

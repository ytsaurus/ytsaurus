    ...
    for f in files:
        ...
        with open(f, "r") as fd:
            ...
            for line in fd:
                ...
                start_enum_match = re.match(r".*DEFINE_ENUM\(EErrorCode.*", line.strip())
                error_code_match = re.match(r".*\(\(([^ ]*)\).*\(([0-9]*)\)\).*", line.strip())
                end_enum_match=re.match(r"[^(]*\)[^(]*", line.strip())
                if start_namespace_match and start_namespace_match.groups():
                    ...
                    if current_namespace in ("NYT::NHiveClient", "NYT::NClickHouseServer"):
                        # Clash :(
                        if current_namespace == "NYT::NClickHouseServer":

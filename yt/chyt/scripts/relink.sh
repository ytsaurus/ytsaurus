...
if [[ "$kind" != "ytserver-log-tailer" && "$kind" != "ytserver-clickhouse" && "$kind" != "clickhouse-trampoline" && "$kind" != "ytserver-proxy" ]] ; then
    echo "Wrong kind ${kind}, expected one of: ytserver-log-tailer, ytserver-clickhouse, clickhouse-trampoline, ytserver-proxy"
    exit 1
fi

...
def remerge_tables(input_table_paths):
    ...
    def create_destination_table(input, client=None):
        ...
        logger.debug("Creating table %s with schema %s and setting log tailer attributes on it as for order kind %s, log tailer version = %s",
                     output_table_path, schema, table_kind, log_tailer_version)
    ...
    if not args.dry_run:
        for input_table_path, table_attrs in zip(input_table_paths, attrs):
            ...
            table_kind = get_table_kind(input_table_path)
            log_tailer_version = table_attrs.get("log_tailer_version")
            chyt.log_tailer.set_log_tailer_table_attributes(table_kind, output_table_path, 7 * 24 * 60 * 60 * 1000,
    ...
    min_timestamp = (now - datetime.timedelta(days=7)).strftime("%Y-%m-%d %H:%M:%S")
    ...
    with yt.OperationsTrackerPool(args.pool_size, poll_period=args.pool_poll_period) as tracker:
        for input_table_path in input_table_paths:
            ...
            spec_builder = yt.spec_builders.MergeSpecBuilder() \
                ...
                .input_query('* WHERE timestamp > "{}"'.format(min_timestamp)) \
                .begin_job_io() \
...
def batch_unmount(input_table_paths):
    logger.info("Unmounting %d tables", len(input_table_paths))
...
def batch_mount(input_table_paths):
    logger.info("Mounting %d tables", len(input_table_paths))
...
def batch_reshard_tables(input_table_paths):
    ...
    if not args.dry_run:
        ...
        for input_table_path in input_table_paths:
            table_kind = get_table_kind(input_table_path)
            chyt.log_tailer.reshard_log_tailer_table(table_kind, input_table_path, sync=False, client=batch_client)
        batch_client.commit_batch()
...
def reset_attributes(input_table_paths):
    ...
    for attr, input_table_path in zip(attrs, input_table_paths):
        ...
        if not args.dry_run:
            table_kind = get_table_kind(input_table_path)
            chyt.log_tailer.set_log_tailer_table_attributes(table_kind, input_table_path, 7 * 24 * 60 * 60 * 1000,
                                                            log_tailer_version=log_tailer_version, client=batch_client)
            chyt.log_tailer.set_log_tailer_table_dynamic_attributes(table_kind, input_table_path, client=batch_client)
...
def collect_tables(disable_logging_attr=None):
    kolkhoz_root = "//sys/clickhouse/kolkhoz"
    kolkhoz = yt.get(kolkhoz_root, attributes=["disable_logging"])
    for clique, directory in kolkhoz.items():
        if disable_logging_attr is not None and directory.attributes.get("disable_logging", False) != disable_logging_attr:
            continue
        for node in directory:
            name = str(node)
            if name.startswith("clickhouse") and name.endswith("log"):
                yield kolkhoz_root + "/" + clique + "/" + name
...
def main():
    ...
    parser.add_argument("--unmount-tables", action="store_true", help="Unmount tables with 'disable_logging' attribute")
    ...
    if args.from_stdin:
        stdin_table_paths = list(map(str.strip, sys.stdin.readlines()))

    if args.mount_tables:
        input_table_paths = stdin_table_paths or list(collect_tables(disable_logging_attr=False))
    ...
    if args.unmount_tables:
        input_table_paths = stdin_table_paths or list(collect_tables(disable_logging_attr=True))
        if yt.get("//sys/clickhouse/kolkhoz/manage_log_tables_lock/@lock_count") > 0:
            logger.info("There is a lock on //sys/clickhouse/kolkhoz/manage_log_tables_lock, doing nothing")
        else:
            batch_unmount(input_table_paths)

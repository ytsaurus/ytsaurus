...
class Clique(object):
    ...
    def __init__(self, instance_count, max_failed_job_count=0, config_patch=None, cpu_limit=None, alias=None, export_query_log=False, **kwargs):
        ...
        for key, value in Clique.tls_secrets.items():
            spec_builder = spec_builder.secure_vault_variable(key, value)
    ...
    def __exit__(self, exc_type, exc_value, traceback):
        print_debug("Exiting clique", self.op.id)
        time.sleep(2)
        try:
            self.op.complete()
...
class ClickHouseTestBase(YTEnvSetup):
    ...
    def setup_class(cls, test_name=None, run_id=None):
        ...
        if cls.Env.yt_config.enable_tls:
            ...
            Clique.tls_secrets = {
                "YT_BUS_CA_BUNDLE": pathlib.Path(cls.Env.yt_config.internal_ca_cert).read_text(),
                "YT_BUS_SERVER_CERTIFICATE": pathlib.Path(cls.Env.yt_config.rpc_cert).read_text(),
                "YT_BUS_SERVER_PRIVATE_KEY": pathlib.Path(cls.Env.yt_config.rpc_cert_key).read_text(),
                "YT_BUS_CLIENT_CERTIFICATE": pathlib.Path(cls.Env.yt_config.rpc_client_cert).read_text(),
                "YT_BUS_CLIENT_PRIVATE_KEY": pathlib.Path(cls.Env.yt_config.rpc_client_cert_key).read_text(),
            }

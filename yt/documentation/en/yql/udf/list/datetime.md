...
SELECT
    ...
    DateTime::MakeTimestamp(DateTime::Split(TzDatetime("2019-01-01T00:00:00,Europe/Moscow"))),
      -- 2018-12-31T21:00:00Z (convert to UTC)
...
SELECT
    ...
    DateTime::MakeTimestamp(DateTime::Update($tm, "Europe/Moscow" as Timezone)), -- 2018-12-31T22:02:03.456789Z (convert to UTC)
...
```DateTime::ToSeconds``` doesn't support intervals longer than 68 years. To work around the issue, use ```DateTime::ToMilliseconds(x) / 1000```
...
Interval can also be created from a [ISO 8601](https://en.wikipedia.org/wiki/ISO_8601%23Durations) string literal.
...
SELECT
    ...
    DateTime::MakeDatetime(DateTime::StartOfQuarter(Datetime("2019-06-06T01:02:03Z"))),
      -- 2019-04-01T00:00:00Z (time components reset to zero)
    ...
    DateTime::MakeDate(DateTime::StartOfWeek(Date("1970-01-01"))),
      -- NULL (the epoch starts on Thursday, the week starts on 1969-12-29, which is out of range)
...
If the resulting number of the day in the month exceeds the maximum allowed, then the `Day` field will accept the last day of the month
 without changing the time (see examples).
...
* `%%`: % character.
* `%Y` — year, 4 digits (1970)
* `%m` — month, 2 digits (1)
* `%d` — day, 2 digits (1)
* `%H` — hour, 2 digits (0)
* `%M` — minutes, 2 digits (0)
* `%S` — seconds (0). It can take the value of microseconds in the format xx. to xx.xxxxxx
...
SELECT
    ...
    DateTime::MakeTimestamp($parse4("Canada/Central")); -- 1970-01-01T06:00:00Z (convert to UTC)
```
...
SELECT
    ...
    -- INCORRECT (Date imports time as GMT, but AddTimezone doesn't affect ToSeconds, which always returns a GMT time)

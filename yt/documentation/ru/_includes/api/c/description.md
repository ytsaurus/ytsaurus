...
Обратите внимание, C++ клиент предоставляется в текущем виде как есть, поэтому:
- код клиента может меняться;
...
{% if audience == "internal" %}
Также существуют перегрузки вызовов операций, которые позволяют писать чуть меньше кода: [clubs.at.yandex-team.ru/yt/3462](https://clubs.at.yandex-team.ru/yt/3462).
{% endif %}
...
В этом случае методы `GetRow()` и `AddRow()` становятся шаблонными и должны использовать конкретные пользовательские типы. При чтении выбор вызываемой функции можно делать, применяя `GetTableIndex()` текущей записи перед вызовом `GetRow()`. При записи вызывается `AddRow(row, tableIndex)`. Есть контроль соответствия типов индексам таблиц. {% if audience == "internal" %}
Cм. [пример](https://a.yandex-team.ru/arc/trunk/arcadia/yt/cpp/mapreduce/examples/main.cpp#L320).{% else %}{% endif %}
...
Развёрнутый пример можно найти в [примерах использования](../../../api/c/examples.md#prepare_operation). Doxygen-документация находится [по ссылке](https://yt.yandex-team.ru/docs/c++/classNYT_1_1IJob.html#a125e26039a472403f60149029e4c3793).{% else %}{% endif %}

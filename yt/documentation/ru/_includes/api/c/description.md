...
Если программа не будет запускать операции, то можно вызвать `JoblessInitialize()`. Эта функция осуществит базовую инициализацию библиотеки (логирование, и т. д.), поскольку она не требует передавать `(argc, argv)`.

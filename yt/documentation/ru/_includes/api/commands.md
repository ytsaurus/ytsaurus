...
| `with_competitors`   | `bool`                                                         | Нет          | `Null`                      | При `Null`  возвращаются все джобы; при `True` возвращаются только джобы, для которых были запущены спекулятивные копии; при `False` возвращаются только джобы, у которых нет спекулятивных копий. |
| `job_competition_id` | `GUID`                                                        | Нет          | `Null`                      | При указании параметра в ответе будут только джобы, у которых есть копия, полученная в результате спекулятивного исполнения, и идентификатор копирующего джоба равен `job_competition_id`. |

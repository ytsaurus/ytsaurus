...
| `stderr_size`      | `integer`         | Размер сохранённого stderr джоба (сам stderr можно получить с помощью команды `get_job_stder`              |
...
| `address`           | `string`                                                       | Нет          | `Null`                      | При указании параметра в ответе будут только джобы с указанным `address`. |

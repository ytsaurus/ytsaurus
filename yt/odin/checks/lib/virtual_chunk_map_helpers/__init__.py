...
def get_chunk_sample(yt_client, options, check_name, logger):
    ...
    if yt_client.exists(sample_path):
        ...
        chunk_ids = yt_client.list(full_list_path, max_size=options["max_size"])
...
def check_virtual_map_size(yt_client, logger, options, states, check_name, count_threshold=0):
    ...
    if len(chunk_ids) > count_threshold:
        logger.info(f"Sample of {check_name} chunks: %s", " ".join(chunk_ids[:options["max_size"]]))
...
def check_virtual_map_age(yt_client, logger, options, check_name):
    ...
    if chunk_count > 0:
        logger.info(f"Sample of {check_name} chunks: {" ".join(chunk_ids[:options["max_size"]])}")

    flag_path = f"//sys/admin/odin/{check_name}_chunks_present_since_flag"
    count_threshold = options["chunk_count_threshold"]

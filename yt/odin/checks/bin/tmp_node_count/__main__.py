...
def check_node_count(yt_client, path, count_threshold, cluster_name, logger, states):
    if yt_client.exists(path):
        ...
        if count >= count_threshold:
            ...
            check_link = get_odin_metric_link(cluster_name, "tmp_node_count")
...
def run_check(yt_client, logger, options, states):
    ...
    root_state = check_node_count(yt_client, "//tmp", options["root_count_threshold"], cluster_name, logger, states)
    return min(file_state, table_state, root_state)

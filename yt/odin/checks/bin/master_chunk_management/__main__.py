...
from yt_odin_checks.lib.master_chunk_management import get_disabled_jobs
...
def run_check(yt_client, logger, options, states):
    ...
    for job_type in get_disabled_jobs(yt_client):
        final_state = states.UNAVAILABLE_STATE
        messages.append(f"{job_type.capitalize()} jobs are disabled via max_misscheduled_{job_type}_jobs_per_heartbeat config parameter")

...
def test_lost_vital_chunks(yt_env):
    proxy_url = yt_env.yt_client.config["proxy"]["url"]
    checks_path = make_check_dir("lost_vital_chunks", CHECK_OPTIONS)
    storage = configure_and_run_checks(proxy_url, checks_path)
    assert abs(storage.get_service_states("lost_vital_chunks")[-1] - FULLY_AVAILABLE_STATE) <= 0.001

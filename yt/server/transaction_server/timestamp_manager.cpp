...
private:
    ...
    void Calibrate()
    {
        ...
        LOG_DEBUG("Timestamp advanced (Timestamp: %x)",
            CurrentTimestamp_);
    ...
    void OnTimestampCommitted(TTimestamp timestamp, const TErrorOr<TMutationResponse>& result)
    {
        ...
        LOG_DEBUG("Timestamp committed (Timestamp: %x)",
            CommittedTimestamp_);
    }
    ...
    virtual void OnLeaderActive() override
    {
        ...
        auto callback = BIND([=, this_ = MakeStrong(this)] () {
            ...
            LOG_INFO("Timestamp generator is now active (Timestamp: %x)",
                persistentTimestamp);
        }).Via(invoker);
    ...
    void HydraCommitTimestamp(TReqCommitTimestamp* request)
    {
        ...
        LOG_DEBUG_UNLESS(IsRecovery(), "Persistent timestamp updated (Timestamp: %x)",
            PersistentTimestamp_);
    }

...
EPeerMessage TPeerConnection::ReceiveMessage()
{
    ...
    if (type == "HEAD_WANT") {
        ...
        YT_LOG_WARNING("Ignoring unknown message (Type: %v)", type);
...
std::pair<TString, INodePtr> TPeerConnection::ReceiveRawMessage()
{
    ...
    YT_LOG_DEBUG("Received message from peer (Type: %v)", type);
    return {
        type,
...
{
    YT_LOG_DEBUG("Sending message to peer (Type: %v)", type);

...
private:
    ...
    NInterop::TTablePartList PartitionChunks();
};
...
NInterop::TTablePartList TMultiTablesPartitioner::PartitionChunks()
{
    auto dataSlices = SplitUnversionedChunks(
        std::move(Chunks),
        NumParts);

    NInterop::TTablePartList tableParts;
...
NInterop::TTablePartList TMultiTablesPartitioner::PartitionTables() {
    InitializeTables();

...
public:
    ...
    void OnUpdate(
        const TString& path,
        TNodeRevision newRevision) override;
...
public:
    ...
    void OnUpdate(TNodeRevision newRevision);
    void OnRemove();
    void OnError(const TString& errorMessage);

private:
    TClusterDirectoryEventHandlerPtr CreateEventHandler();
    TClusterNodeNames ProcessNodeList(TDirectoryListing listing);
...
TClusterNodeTicket TClusterNodeTracker::EnterCluster(const std::string& id, const std::string& host, ui16 tcpPort, ui16 httpPort)
{
    ...
    OnUpdate(static_cast<TNodeRevision>(-2));
    return result;
}
...
void TClusterNodeTracker::OnUpdate(TNodeRevision newRevision)
{
...
TClusterNodeNames TClusterNodeTracker::ProcessNodeList(TDirectoryListing listing)
{
    LOG_INFO(
        Logger,

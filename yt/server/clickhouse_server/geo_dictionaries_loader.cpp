...
private:
    ...
    bool LooksLikeHierarchyFile(const TObjectListItem object) const;
...
void TRegionsHierarchiesDataProvider::CheckDefaultHierarchy() const
{
    ...
    if (attributes.Type != EObjectType::File) {
...
bool TRegionsHierarchiesDataProvider::LooksLikeHierarchyFile(
    const TObjectListItem object) const {
    return object.Attributes.Type == EObjectType::File &&
           object.Name.size() == 2;
}

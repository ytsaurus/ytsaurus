...
select  substring(cast(reason.r_reason_desc as string),1,20) bla
       ...
       ,avg(wr_fee) bla4
 from web_sales cross join web_returns cross join web_page cross join customer_demographics cd1 cross join
      customer_demographics cd2 cross join customer_address cross join date_dim cross join reason

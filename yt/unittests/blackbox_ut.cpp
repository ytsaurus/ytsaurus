...
{
public:
    MOCK_METHOD2(Call, TFuture<INodePtr>(const Stroka&, const yhash_map<Stroka, Stroka>&));
};
...
TEST_F(TTokenAuthenticatorTest, FailOnInvalidScope)
{
    ...
    MockCall(R"yy({status={id=0};oauth={scope="i-am-hacker";client_id="i-am-hacker";client_name="n"};login=hacker})yy");
    auto result = Invoke("mytoken", "myip").Get();
    ASSERT_TRUE(!result.IsOK());
...
{
    Config_->Scope = "yt:api";
    MockCall(R"yy({status={id=0};oauth={scope="x:1 yt:api x:2";client_id="secret_client_id";client_name="n"};login=sandello})yy");
    auto result = Invoke("mytoken", "myip").Get();
    ASSERT_TRUE(result.IsOK());

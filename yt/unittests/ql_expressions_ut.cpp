...
TEST_P(TTernaryLogicTest, Evaluate)
{
    ...
    Profile(expr1, TTableSchema(), nullptr, &variables)()(variables.GetOpaqueData(), &result, row, &executionContext);
    EXPECT_TRUE(CompareRowValues(result, expected) == 0);

    Profile(expr2, TTableSchema(), nullptr, &variables)()(variables.GetOpaqueData(), &result, row, &executionContext);
    EXPECT_TRUE(CompareRowValues(result, expected) == 0);
}

    ...
    for (int allocationSize = 1 << 5; allocationSize <= (1 << 20); allocationSize <<= 1) {
        {
            ...
            EXPECT_EQ(GetMemoryCounterForTag(42), allocationSize);
        }
        EXPECT_EQ(GetMemoryCounterForTag(42), 0);
    }
}
...
void TestStackingGuards()
{
    ...
    EXPECT_EQ(GetMemoryCounterForTag(1), 0);
    auto allocation1 = MakeAllocation(1 << 5);
    EXPECT_EQ(GetMemoryCounterForTag(1), 1 << 5);
    {
        TMemoryTagGuard guard2(2);
        auto allocation2 = MakeAllocation(1 << 6);
        EXPECT_EQ(GetMemoryCounterForTag(1), 1 << 5);
        EXPECT_EQ(GetMemoryCounterForTag(2), 1 << 6);
    }
    EXPECT_EQ(GetMemoryCounterForTag(1), 1 << 5);
    EXPECT_EQ(GetMemoryCounterForTag(2), 0);
    {
        TMemoryTagGuard guard2(std::move(guard1));
        auto allocation2 = MakeAllocation(1 << 7);
        EXPECT_EQ(GetMemoryCounterForTag(1), (1 << 5) + (1 << 7));
        EXPECT_EQ(GetMemoryCounterForTag(2), 0);
    }
    EXPECT_EQ(GetMemoryCounterForTag(1), (1 << 5));
    EXPECT_EQ(GetMemoryCounterForTag(2), 0);
...
void Action1()
{
    ...
    EXPECT_EQ(GetMemoryCounterForTag(1), (1 << 5) + (1 << 7) + (1 << 9));
}
...
void Action2()
{
    ...
    EXPECT_EQ(GetMemoryCounterForTag(2), (1 << 6) + (1 << 8));
    Yield();
    auto allocation3 = MakeAllocation(1 << 10);
    EXPECT_EQ(GetMemoryCounterForTag(2), (1 << 6) + (1 << 8) + (1 << 10));
}
...
void TestMemoryTaggingInvoker()
{
    ...
    EXPECT_EQ(controller->GetMemoryCounter(), 0);

    WaitFor(BIND(&Action3, controller)
        .AsyncVia(controller->GetControlInvoker())
        .Run())
        .ThrowOnError();
    EXPECT_NEAR(controller->GetMemoryCounter(), 128_MB, 1_KB);

    controller = nullptr;
    EXPECT_NEAR(GetMemoryCounterForTag(1), 0, 1_KB);
}
...
void TestControllersInThreadPool()
{
    ...
    WaitFor(Combine(futures))
        .ThrowOnError();
    for (int index = 0; index < controllerCount; ++index) {
        EXPECT_NEAR(memoryUsages[index], controllers[index]->GetMemoryCounter(), 10_KB);
    }
    controllers.clear();
    for (int index = 0; index < controllerCount; ++index) {
        EXPECT_NEAR(GetMemoryCounterForTag(index + 1), 0, 10_KB);
        EXPECT_GE(GetMemoryCounterForTag(index + 1), 0);
    }
}

...
class SchedulerReviveBase(YTEnvSetup):
    ...
    def test_missing_transactions(self):
        ...
        op = self._start_op("cat; sleep 10", dont_track=True)
    ...
    def test_completing_with_sleep(self, stage):
        ...
        op = self._start_op("cat; if [ \"$YT_JOB_INDEX\" != \"0\" ]; then sleep 100; fi;",
                 dont_track=True,
                 spec={
                     "testing": {
    ...
    def test_abort_during_complete(self):
        ...
        op = self._start_op("cat; if [ \"$YT_JOB_INDEX\" != \"0\" ]; then sleep 100; fi;", dont_track=True,
                 spec={
                     "testing": {
                         "delay_inside_operation_commit": 4000,

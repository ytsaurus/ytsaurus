    ...
    , public TCommonMapParameters
{
    ...
    REGISTER_YSON_STRUCT(TTransformParameters);
...
{
public:
    YT_FLOW_EXTEND_PARAMETERS(TTransformParameters);
        ...
        for (const auto& inputStreamId : inputStreamIds) {
            YQL_ENSURE(
                *streamSpecStorage->GetSchema(inputStreamId) == *sourceSchema,
                "All input streams of TTransformMap must have the same schema");
        }
...
public:
        ...
        NYT::NFlow::IOutputCollectorPtr output) override
    {
        ...
        MapComputationGraphWithCodecs->SetInput(messageHolders);

        InputMessagesCounter.Increment(messageHolders.size());
...
YT_FLOW_DEFINE_COMPUTATION(TTransformMap);

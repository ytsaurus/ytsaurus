...
TMaybeNode<TExprBase> TYtPhysicalOptProposalTransformer::Alter(TExprBase node, TExprContext& ctx) const {
    ...
    const TYtOutTableInfo outTable1(State_->TablesData->GetTable(alter.DataSink().Cluster().StringValue(), tableInfo.Name, tableInfo.Epoch).RowSpec);
            ...
            .Operation<TYtAlter>()
                .World<TCoLeft>()
                    .Input<TYtCopy>()
                        ...
                        .DataSink(alter.DataSink())
                        .Output()
                            .Add(outTable1.ToExprNode(ctx, alter.Table().Pos()))
                            .Build()
                        .Input()
                            .Add()
                                .Paths()

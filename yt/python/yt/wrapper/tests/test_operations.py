...
"""
    ...
    def test_hide_error_params(self):
        ...
        if client.config["backend"] == "http":
            ...
            # test for YtProxyUnavailable
            response = requests.Response()
            response.status_code = 500
            response._content = b'{}'
            response.raw = urllib3.HTTPResponse()

            with inject_http_error(client, filter_url="", interrupt_from=0, interrupt_till=10, interrupt_every=1, response=response):
                with pytest.raises(yt.YtProxyUnavailable) as ex:

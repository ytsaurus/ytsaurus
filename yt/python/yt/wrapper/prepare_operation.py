...
def _request_schemas(paths, client):
    ...
    for batch_response in batch_responses:
        if batch_response is None:
            schemas.append(None)
            continue
        error = batch_response.get_error()
        if error is not None and not error.is_resolve_error():
            raise error
        schemas.append(TableSchema.from_yson_type(batch_response.get_result()["schema"]))
    return schemas

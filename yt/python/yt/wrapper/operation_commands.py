...
def suspend_operation(operation, abort_running_jobs=False, reason="", client=None):
    """Suspends operation.

    :param str operation: operation id.
    """
    command_name = "suspend_operation" if get_api_version(client) == "v4" else "suspend_op"
    params = {"operation_id": operation, "abort_running_jobs": abort_running_jobs, "reason": reason}

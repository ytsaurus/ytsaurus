import dataclasses
import datetime
# TODO(ilyaibraev): remove import sys
import sys
import dateutil
import itertools
import logging
...
class AccessLogDeduplicator(yt.TypedJob):
    def __init__(self, ignored_users):
        self.last = None
        self.ignored_users = set(ignored_users)
    ...
    def __call__(self, row):
        ...
        if row.method is None or row.method == "Revise":
            return
        if row.user in self.ignored_users:
            return
...
class AccessLogReducer(yt.TypedJob):
    ...
    def start(self):
        ...
        self.client = yt.YtClient(proxy=self.cluster, token=os.environ.get("YT_SECURE_VAULT_yt_token"), config={"backend": "rpc", "dynamic_table_retries": {"count": 3}})
...
def remove_tables(cluster, token, result_root, excess):
    batch_client = yt.YtClient(cluster, token=token).create_batch_client()
    for table in excess:
        batch_client.remove(f"{result_root}/{'/'.join(table)}")
    batch_client.commit_batch()


def import_table(cluster, network_project, pool, token, log_root, tmp_root, out_root, log_duration, log_name, node_id_dict_path, ignored_users):
    if token is None:
        raise Exception("YT_TOKEN is empty")
    client = yt.YtClient(cluster, token=token)
    log_dir = log_root
    tmp_dir = tmp_root
    out_dir = out_root
    ...
    network_project_spec = dict()
    if network_project is not None:
        if client.exists(f"//sys/network_projects/{network_project}"):
            network_project_spec = dict(network_project=network_project)
        else:
            logger.warning("The specified network_project does not exist. Skipping adding it to the spec. (network_project: %v)", network_project)
    ...
    def get_deduplicate_op():
        return aops.AsyncOp(
            ...
            .begin_mapper()
                .spec(network_project_spec)
                .command(AccessLogDeduplicator(ignored_users=ignored_users))
    ...
    def get_reduce_op():
        return aops.AsyncOp(
            ...
            .begin_reducer()
                ...
                .cpu_limit(REDUCE_CPU_LIMIT)
                .memory_limit(REDUCE_MEMORY_LIMIT)
            .end_reducer()
            .max_failed_job_count(20)
            .output_table_paths([
...
def find_diff(cluster, token, log_root, res_root, period, store_from):
    ...
    excess = []
    missing = set()
    for cluster, tables in res_tables.items():
        excess.extend((cluster, *table) for table in tables.difference(log_tables))
...
def bulk(cluster, log_root, tmp_root, result_root, n_threads, period, pool, max_tables, store_for, network_project, node_id_dict_path, ignored_users):
    store_from = None
    ...
    aops.PipelineWatcher(
        aops.Parallel(
            *(
                import_table(cluster, network_project, pool, token, log_root, tmp_root, result_root, *path, node_id_dict_path, ignored_users)
                for path in new
            ),
...
def import_single(cluster, pool, log_root, tmp_root, result_root, n_threads, path, network_project, node_id_dict_path, ignored_users):
    logger.addHandler(logging.StreamHandler())
    logger.level = logging.DEBUG
    token = os.environ.get("YT_TOKEN")
    aops.PipelineWatcher(
        import_table(
            cluster, network_project, pool, token, log_root, tmp_root, result_root, *(path.rsplit("/", 2)[-2:]), node_id_dict_path, ignored_users
        ),
        logger,
        max_concurrent_operations=n_threads,
    ).wait(abort_concurrent_on_fail=False)
...
def clear(cluster, log_root, result_root, store_from):
    token = os.environ.get("YT_TOKEN")

...
def import_table(cluster, network_project, pool, token, log_root, tmp_root, out_root, log_duration, log_name, node_id_dict_path, users_to_ignore, rewrite):
    if token is None:
    ...
    if rewrite:
        ...
        all_up_to_date = True
        for out_cluster in all_clusters:
            if not exists_reqs[out_cluster].get_result():
                all_up_to_date = False
                continue
            try:
                stored_mtime = attr_reqs[out_cluster].get_result()
        ...
        if all_up_to_date:
            return aops.Function(
                lambda: logger.info("Skipping import of table %s, it is up to date", log_path),
                title=f"Skipping import of table {log_path}, it is up to date",
            )
    else:
        if all(exists_reqs[out_cluster].get_result() for out_cluster in all_clusters):
            return aops.Function(
...
def find_diff(cluster, token, log_root, res_root, period, store_from, check_stale):
    ...
    def split_log_table(path, root):
        # path == f"{root}/{period}/{iso}", period may contain '/' (e.g. "stream/5min").
        rel = path[len(root) + 1:]
        period_part, _, iso = rel.rpartition("/")
        return period_part, iso

    def split_res_table(path, root):
        # path == f"{root}/{cluster}/{period}/{iso}", period may contain '/'.
        rel = path[len(root) + 1:]
        cluster, _, rest = rel.partition("/")
        period_part, _, iso = rest.rpartition("/")
        return cluster, period_part, iso

    def log_path_filter(path, root, period, store_from=None):
        if period is None:
            return True
        if store_from is not None:
            ...
            if "T" not in table_date:
                if table_date < store_from:
                    return False
        return split_log_table(path, root)[0] == period

    def res_path_filter(path, root, period):
        if period is None:
            return True
        return split_res_table(path, root)[1] == period

    def list_log_tables(root):
        return (
            set(
                split_log_table(path, root)
                for path in yt_client.search(
                    root, node_type=["table"], path_filter=lambda path: log_path_filter(path, root, period, store_from)
                )
            )
    ...
    def list_res_tables(root, clusters):
        ...
        logs = [
            split_res_table(path, root)
            for path in yt_client.search(
                root, node_type=["table"], path_filter=lambda path: res_path_filter(path, root, period)
            )
    ...
    if check_stale:
        ...
        if existing:
            ...
            for key in existing:
                try:
                    source_mtime = source_mtime_reqs[key].get_result()
                except yt.YtResponseError:
                    continue
                is_stale = False
                for out_cluster in clusters:
                    try:
                        out_mtime = output_attr_reqs[(out_cluster, key)].get_result()
                    except yt.YtResponseError:
                        continue
                    if out_mtime != source_mtime:
                        is_stale = True
                        break
                if is_stale:
                    stale.append(key)
...
def bulk(cluster, log_root, tmp_root, result_root, n_threads, period, pool, max_tables, store_for, network_project, node_id_dict_path, users_to_ignore, token_env_variable, rewrite):
    store_from = None
    ...
    new, excess, stale = find_diff(cluster, token, log_root, result_root, period, store_from, rewrite)
    ...
    aops.PipelineWatcher(
        aops.Parallel(
            *(
                import_table(cluster, network_project, pool, token, log_root, tmp_root, result_root, *path, node_id_dict_path, users_to_ignore, rewrite)
...
def import_single(cluster, pool, log_root, tmp_root, result_root, n_threads, path, network_project, node_id_dict_path, users_to_ignore, token_env_variable, rewrite):
    logger.addHandler(logging.StreamHandler())
    logger.level = logging.DEBUG
    token = os.environ.get(token_env_variable)
    aops.PipelineWatcher(
        import_table(
            cluster, network_project, pool, token, log_root, tmp_root, result_root, *(path.rsplit("/", 2)[-2:]), node_id_dict_path, users_to_ignore, rewrite
        ),
        logger,
        max_concurrent_operations=n_threads,
    ).wait(abort_concurrent_on_fail=False)

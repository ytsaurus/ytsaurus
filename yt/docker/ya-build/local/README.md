...
Example: `ya package package.json --docker-target local --docker-registry my-registry.com`

The `--docker-registry` parameter only impacts the resulting image name, which will be `my-registry.com/ytsaurus-local:local-<commit-SHA>` in this case.

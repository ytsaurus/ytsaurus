    ...
    if (!cleanStart) {
        ...
        if (!snapshotOrError.IsOK()) {
            ...
            if (!Snapshot.IsLegacy) {
                YT_LOG_INFO("Snapshot is not legacy, will use clean start");
                cleanStart = true;
            }
        }
    }
...
void TOperationControllerBase::AttachOutputChunks(const std::vector<TOutputTablePtr>& tableList)
{
    for (const auto& table : tableList) {
        ...
        if (table->TableUploadOptions.TableSchema->IsSorted() && ShouldVerifySortedOutput()) {
            ...
            for (auto& [key, chunkTreeId] : table->OutputChunkTreeIds) {
...
void TOperationControllerBase::BuildProgress(TFluentMap fluent) const
{
    ...
    fluent
        ...
        .Item("legacy_controller").Value(true);
}
...
void TOperationControllerBase::ReleaseIntermediateStripeList(const NLegacyChunkPools::TChunkStripeListPtr& stripeList)
{

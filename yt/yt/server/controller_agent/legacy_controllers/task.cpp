...
#include <yt/server/lib/legacy_chunk_pools/helpers.h>
    ...
    TJobletPtr joblet)
{
    ...
    for (int index = 0; index < edgeDescriptors.size(); ++index) {
        ...
        const auto& outputTableSchema = edgeDescriptor.TableUploadOptions.TableSchema;
        ...
        for (const auto& streamSchema : edgeDescriptor.StreamSchemas) {
...
TSharedRef TTask::BuildJobSpecProto(TJobletPtr joblet)
{
    ...
    if (schedulerJobSpecExt->input_data_weight() > TaskHost_->GetSpec()->MaxDataWeightPerJob) {
        TaskHost_->OnOperationFailed(TError(
            NLegacyChunkPools::EErrorCode::MaxDataWeightPerJobExceeded,
...
void TTask::RegisterOutput(
    ...
    TJobletPtr joblet,
    const NLegacyChunkPools::TChunkStripeKey& key)
{

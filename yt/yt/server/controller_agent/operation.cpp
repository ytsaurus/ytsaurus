...
TOperation::TOperation(const NProto::TOperationDescriptor& descriptor)
    ...
    , ProvidedSpec_(ConvertToNode(TYsonString(descriptor.provided_spec(), EYsonType::Node))->AsMap())
    ...
    , AcoName_(descriptor.has_aco_name() ? std::optional<std::string>(descriptor.aco_name()) : std::nullopt)

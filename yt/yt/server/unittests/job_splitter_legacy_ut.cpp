...
#include <yt/server/controller_agent/legacy_controllers/job_splitter.h>

#include <yt/server/lib/legacy_chunk_pools/public.h>
#include <yt/server/lib/legacy_chunk_pools/chunk_stripe.h>
...
NLegacyChunkPools::TChunkStripeListPtr CreateTwoRowStripeList(bool isSplittable = true)
{
    NLegacyChunkPools::TChunkStripeListPtr stripeList = New<NLegacyChunkPools::TChunkStripeList>();
...
TEST(TJobSplitterLegacyTest, SplitLongAmongRunningInterruptableJob)
{
...
TEST(TJobSplitterLegacyTest, SpeculateLongAmongRunningNonInterruptableJob)
{
    auto jobSplitter = CreateJobSplitter(CreateSplitterConfig(), TOperationId());
...
TEST(TJobSplitterLegacyTest, SpeculateLongAmongRunningInterruptableJobWithTooSmallTotalDataWeight)
{
    auto jobSplitter = CreateJobSplitter(CreateSplitterConfig(), TOperationId());
...
TEST(TJobSplitterLegacyTest, SplitResidualInterruptableJob)
{
    auto jobSplitter = CreateJobSplitter(CreateSplitterConfig(), TOperationId());
...
TEST(TJobSplitterLegacyTest, SpeculateResidualNonInterruptableJob)
{
    auto jobSplitter = CreateJobSplitter(CreateSplitterConfig(), TOperationId());
...
TEST(TJobSplitterLegacyTest, DoNothingWithNotLongAndNotResidualJob)
{
    auto jobSplitter = CreateJobSplitter(CreateSplitterConfig(), TOperationId());
...
TEST(TJobSplitterLegacyTest, SpeculateLongJobWithNoProgressWhenHasCompletedJobs)
{
    auto jobSplitter = CreateJobSplitter(CreateSplitterConfig(), TOperationId());
...
TEST(TJobSplitterLegacyTest, SpeculateWhenInterruptTimeoutExpired)
{

...
TOperationController::TOperationController(
...
TControllerEpoch TOperationController::GetEpoch() const
{
    return Controller_->GetEpoch();
}

void TOperationController::OnScheduleAllocationStarted(const ISchedulingHeartbeatContextPtr& schedulingHeartbeatContext)
{
...
TAllocationGroupResourcesMap TOperationController::GetGroupedNeededResources() const
{
    return Controller_->GetGroupedNeededResources();
}

TAllocationGroupResourcesMap TOperationController::GetInitialGroupedNeededResources() const
{
    return Controller_->GetInitialGroupedNeededResources();
}

TJobResources TOperationController::GetAggregatedMinNeededAllocationResources() const
{
...
TJobResources TOperationController::GetAggregatedInitialMinNeededAllocationResources() const
{
...
void TOperationController::UpdateGroupedNeededResources()
{
    Controller_->UpdateGroupedNeededResources();
}

void TOperationController::UpdateConcurrentScheduleAllocationThrottlingLimits(
    const TStrategyOperationControllerConfigPtr& config)
{
...
bool TOperationController::CheckMaxScheduleAllocationCallsOverdraft(int maxScheduleAllocationCalls) const
{
    YT_ASSERT_THREAD_AFFINITY(ControlThread);
...
bool TOperationController::IsMaxConcurrentScheduleAllocationCallsPerNodeShardViolated(const ISchedulingHeartbeatContextPtr& schedulingHeartbeatContext) const
{
    auto nodeShardId = schedulingHeartbeatContext->GetNodeShardId();
    auto& shard = StateShards_[nodeShardId];
...
bool TOperationController::IsMaxConcurrentScheduleAllocationExecDurationPerNodeShardViolated(const ISchedulingHeartbeatContextPtr& schedulingHeartbeatContext) const
{
...
void TOperationController::AbortAllocation(TAllocationId allocationId, EAbortReason abortReason, TControllerEpoch allocationEpoch)
{
    Controller_->OnNonscheduledAllocationAborted(allocationId, abortReason, allocationEpoch);
}

TControllerScheduleAllocationResultPtr TOperationController::ScheduleAllocation(
...
void TOperationController::UpdateConfig(const TStrategyOperationControllerConfigPtr& config)
{
    YT_ASSERT_THREAD_AFFINITY(ControlThread);
...
TStrategyOperationControllerConfigPtr TOperationController::GetConfig()
{
    return Config_.Acquire();
}

void TOperationController::SetDetailedLogsEnabled(bool enabled)
{
    YT_ASSERT_THREAD_AFFINITY(ControlThread);

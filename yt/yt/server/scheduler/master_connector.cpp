...
static YT_DEFINE_LEAKY_GLOBAL(const NLogging::TLogger, Logger, "MasterConnector");
...
private:
    ...
    private:
        ...
        void RegisterInstance()
        {
            ...
            auto path = TYPath(Format("//sys/scheduler/instances/%v", ToYPathLiteral(GetDefaultAddress(ServiceAddresses_))));
            {
...

        {
            ...
            for (const auto& operation : operationsToFetchCommittedFlag) {
                ...
                for (auto transactionId : possibleTransactions)
                {
                    ...
                    ToProto(req->mutable_attributes()->mutable_keys(), std::vector<std::string>{CommittedAttribute});

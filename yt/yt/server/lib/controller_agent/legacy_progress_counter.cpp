#include "legacy_progress_counter.h"
...
TLegacyProgressCounter::TLegacyProgressCounter()
    : TotalEnabled_(false)
    , Total_(0)
    , Running_(0)
    , Pending_(0)
...
TLegacyProgressCounter::TLegacyProgressCounter(i64 total)
    : TLegacyProgressCounter()
{
    Set(total);
}

void TLegacyProgressCounter::Set(i64 total)
{
...
bool TLegacyProgressCounter::IsTotalEnabled() const
{
    return TotalEnabled_;
}

void TLegacyProgressCounter::Increment(i64 value)
{
    YT_VERIFY(TotalEnabled_);
    Total_ += value;
    YT_VERIFY(Total_ >= 0);
...
void TLegacyProgressCounter::Decrement(i64 value)
{
    YT_VERIFY(TotalEnabled_);
    Total_ -= value;
    YT_VERIFY(Total_ >= 0);
...
i64 TLegacyProgressCounter::GetTotal() const
{
    YT_VERIFY(TotalEnabled_);
    return Total_;
}

i64 TLegacyProgressCounter::GetRunning() const
{
    return Running_;
}

i64 TLegacyProgressCounter::GetCompletedTotal() const
{
    return std::accumulate(Completed_.begin(), Completed_.end(), 0LL);
}

i64 TLegacyProgressCounter::GetCompleted(EInterruptReason reason) const
{
    return Completed_[reason];
}

i64 TLegacyProgressCounter::GetInterruptedTotal() const
{
    return GetCompletedTotal() - Completed_[EInterruptReason::None];
}

i64 TLegacyProgressCounter::GetPending() const
{
    YT_VERIFY(TotalEnabled_);
    return Pending_;
}

i64 TLegacyProgressCounter::GetFailed() const
{
    return Failed_;
}

i64 TLegacyProgressCounter::GetAbortedTotal() const
{
    return std::accumulate(Aborted_.begin(), Aborted_.end(), 0LL);
}

i64 TLegacyProgressCounter::GetAbortedScheduled() const
{
    return GetAbortedTotal() - GetAbortedNonScheduled();
}

i64 TLegacyProgressCounter::GetAbortedNonScheduled() const
{
    i64 sum = 0;
...
i64 TLegacyProgressCounter::GetAborted(EAbortReason reason) const
{
    return Aborted_[reason];
}

i64 TLegacyProgressCounter::GetLost() const
{
    return Lost_;
}

void TLegacyProgressCounter::Start(i64 count)
{
    if (TotalEnabled_) {
...
void TLegacyProgressCounter::Completed(i64 count, EInterruptReason reason)
{
...
void TLegacyProgressCounter::Failed(i64 count)
{
    YT_VERIFY(Running_ >= count);
    Running_ -= count;
    Failed_ += count;
...
void TLegacyProgressCounter::Aborted(i64 count, EAbortReason reason)
{
...
void TLegacyProgressCounter::SetParent(const TLegacyProgressCounterPtr& parent)
{
    YT_VERIFY(!Parent_);
    Parent_ = parent;
    if (TotalEnabled_) {
...
const TLegacyProgressCounterPtr& TLegacyProgressCounter::Parent() const
{
    return Parent_;
}

void TLegacyProgressCounter::Persist(const TPersistenceContext& context)
{
    using NYT::Persist;
...
const TLegacyProgressCounterPtr NullLegacyProgressCounter = New<TLegacyProgressCounter>();
...
void Serialize(const TLegacyProgressCounterPtr& counter, IYsonConsumer* consumer)
{
...
void SerializeBriefVersion(const TLegacyProgressCounterPtr& counter, IYsonConsumer* consumer)
{
    BuildYsonFluently(consumer)
        .BeginMap()
            .DoIf(counter->IsTotalEnabled(), [&] (TFluentMap fluent) {

...
public:
    ...
    void AddDataSlice(const NChunkClient::TDataSlicePtr& dataSlice, IChunkPoolInput::TCookie cookie, bool isPrimary);
    // TODO(max42): this seems to be unused, clean up.
    void AddPreliminaryForeignDataSlice(const NChunkClient::TDataSlicePtr& dataSlice);
...
void Serialize(const TJobStub& jobStub, NYson::IYsonConsumer* consumer);
...
public:
    ...
    explicit TJobManager(const NLogging::TLogger& logger);

    //! NB: Without manual dtor, other classes cannot persist TJobManager
    //! because they do not know how to destruct std::unique_ptr<TJobOrder>.
    ~TJobManager();

    std::vector<IChunkPoolOutput::TCookie> AddJobs(std::vector<std::unique_ptr<TJobStub>> jobStubs);
...
private:
    ...
    public:
        ...
        TJob(TJobManager* owner, std::unique_ptr<TJobStub> jobStub, IChunkPoolOutput::TCookie cookie);
    ...
    PHOENIX_DECLARE_TYPE(TJobManager, 0x7e42ef5b);
};

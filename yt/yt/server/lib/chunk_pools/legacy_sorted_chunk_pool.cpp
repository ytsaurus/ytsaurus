...
public:
    ...
    TChunkStripeListPtr GetStripeList(IChunkPoolOutput::TCookie cookie) override
    {
        ...
        if (legacyStripeList->GetPartitionTag()) {
            newStripeList->SetPartitionTag(*legacyStripeList->GetPartitionTag());
        }
...
private:
        ...
        int splitJobCount)
    {
        ...
        auto jobSizeConstraints = CreateExplicitJobSizeConstraints(
            ...
            /*primaryCompressedDataSizePerJob*/ std::numeric_limits<i64>::max() / 4,
            maxDataSlicesPerJob,

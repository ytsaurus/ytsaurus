...
public:
        ...
        , ChunkPoolStatistics_(options.ChunkPoolStatistics)
    {
        ...
        YT_TLOG_INFO("Sorted chunk pool created")
    ...
    void Completed(IChunkPoolOutput::TCookie cookie, const TCompletedJobSummary& jobSummary) override
    {
        ...
        if (jobSummary.InterruptionReason != EInterruptionReason::None) {
            ...
            std::vector<TDataSlicePtr> foreignSlices;
...
private:
    ...
    void FetchNonTeleportDataSlices(const ISortedJobBuilderPtr& builder)
    {
        ...
        auto processDataSlice = [&] (const TDataSlicePtr& dataSlice, int inputCookie) {
        ...
        THashMap<TInputChunkPtr, std::pair<TDataSlicePtr, IChunkPoolInput::TCookie>> inputChunkToOwningDataSlice;
    ...
    void FindTeleportChunks()
    {
        ...
        std::vector<TDataSlicePtr> teleportDataSlices;
    ...
    void DoFinish()
    {
        ...
        std::vector<std::unique_ptr<TJobStub>> jobStubPtrs;
        jobStubPtrs.reserve(jobStubs.size());
        for (auto& jobStub : jobStubs) {
            jobStubPtrs.emplace_back(std::make_unique<TJobStub>(std::move(jobStub)));
        }
    ...
    std::vector<IChunkPoolOutput::TCookie> SplitJob(
        ...
        const std::vector<TDataSlicePtr>& foreignInputDataSlices,
        int splitJobCount,
        TOutputCookie cookie)
    {
        auto validateDataSlices = [&] (const std::vector<TDataSlicePtr>& dataSlices, int prefixLength) {
        ...
        std::vector<TDataSlicePtr> inputStreamIndexToLastDataSlice;
        ...
        for (auto& job : jobs) {
            ...
            jobStubs.push_back(std::make_unique<TJobStub>(std::move(job)));
        }
    ...
    PHOENIX_DECLARE_POLYMORPHIC_TYPE(TSortedChunkPool, 0x8ff1db04);
};

void TSortedChunkPool::RegisterMetadata(auto&& registrar)
{
...
PHOENIX_DEFINE_TYPE(TSortedChunkPool);

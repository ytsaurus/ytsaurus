...
public:
    ...
    void AddDataSlice(const TDataSlicePtr& originalDataSlice) override
    {
    ...
    std::vector<TJobStub> Build() override
    {
    ...
    void ValidateJob(const TJobStub* job)
    {
        ...
        if (Options_.ValidateOrder) {
            // The sorted pool guarantees that each stripe contains data slices in the
            ...
            auto validatePair = [&] (
                ...
                const TDataSlicePtr& lhs,
                const TDataSlicePtr& rhs)
            {
                YT_VERIFY(lhs->Tag);
                YT_VERIFY(rhs->Tag);
...
private:
    ...
    std::vector<TDataSlicePtr> ForeignSlices_;
    ...
    std::vector<TDataSlicePtr> InputStreamIndexToLastDataSlice_;

    //! Used for structured logging.
    std::vector<TDataSlicePtr> InputDataSlices_;
    ...
    void SortForeignSlices()
    {
        ...
        std::stable_sort(
            ...
            ForeignSlices_.end(),
            [&] (const TDataSlicePtr& lhs, const TDataSlicePtr& rhs) {
    ...
    void SortPrimaryEndpoints(std::vector<TPrimaryEndpoint>& endpoints)
    {
        ...
        // See TSortedChunkPoolTest::InterruptRowSlicedAfterAdjustment.
        std::stable_sort(
            endpoints.begin(),
            endpoints.end(),
    ...
    void Stage(TDataSlicePtr dataSlice, ESliceType sliceType, const TPeriodicYielderGuard& periodicYielder)
    {
    ...
    void PartitionSingletonAndLongDataSlices(TKeyBound lowerBound, std::deque<TDataSlicePtr>& dataSlices)
    {
        std::stable_sort(dataSlices.begin(), dataSlices.end(), [&] (const TDataSlicePtr& lhs, const TDataSlicePtr& rhs) {
    ...
    void StageRangeWithRowSlicing(TRange<TPrimaryEndpoint> endpoints, const TPeriodicYielderGuard& periodicYielder)
    {
        ...
        std::deque<TDataSlicePtr> dataSlices;

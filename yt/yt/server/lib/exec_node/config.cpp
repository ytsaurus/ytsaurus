    ...
    static const auto DefaultStatisticSensors = ConvertTo<THashMap<TString, TUserJobStatisticSensorPtr>>(BuildYsonStringFluently()
        .BeginMap()
            ...
            .Item("job_cpu/user").BeginMap()
                .Item("path").Value("/job/cpu/user")
                .Item("type").Value("counter")
                .Item("profiling_name").Value("/job/cpu/user")
            .EndMap()
            .Item("job_cpu/system").BeginMap()
                .Item("path").Value("/job/cpu/system")
                .Item("type").Value("counter")
                .Item("profiling_name").Value("/job/cpu/system")
            .EndMap()
            .Item("job_memory/rss").BeginMap()
                .Item("path").Value("/job/memory/rss")
                .Item("type").Value("gauge")
                .Item("profiling_name").Value("/job/memory/rss")
            .EndMap()
            .Item("job_memory/mapped_file").BeginMap()
                .Item("path").Value("/job/memory/mapped_file")
                .Item("type").Value("gauge")
                .Item("profiling_name").Value("/job/memory/mapped_file")
            .EndMap()

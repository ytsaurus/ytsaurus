    ...
    std::vector<std::pair<TOutputOrder::TEntry, TChunkTreeId>> chunkTrees)
{
    ...
    std::vector<TChunkTreeId> arrangedChunkTreeIds;
    arrangedChunkTreeIds.reserve(Pool_.size());
    for (int position = 0; position != -1; position = NextPosition_[position]) {
        if (chunkTreeIdByPosition[position]) {
            arrangedChunkTreeIds.emplace_back(chunkTreeIdByPosition[position]);
        }
    }

    return arrangedChunkTreeIds;
}

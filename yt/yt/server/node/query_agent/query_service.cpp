...
private:
    ...
    std::atomic<bool> UseQueryPoolForPullRows_;
    ...
    IInvokerPtr GetPullRowsInvoker(const NRpc::NProto::TRequestHeader& requestHeader)
    {
        ...
        if (UseQueryPoolForPullRows_.load()) {
            return Bootstrap_->GetQueryPoolInvoker(DefaultPullRowsPoolName, tag);
        } else {
    ...
    DECLARE_RPC_SERVICE_METHOD(NQueryClient::NProto, PullRows)
    {
        ...
        ExecuteRequestWithRetries<void>(
            ...
            [&] {
                ...
                TReplicationLogBatchDescriptor result;
                ...
                response->set_row_count(result.ResponseRowCount);
                response->set_data_weight(result.ResponseDataWeight);
    ...
    DECLARE_RPC_SERVICE_METHOD(NQueryClient::NProto, GetTabletInfo)
    {
        ...
        for (int index = 0; index < tabletCount; ++index) {
            ...
            for (const auto& [replicaId, replicaSnapshot] : tabletSnapshot->Replicas) {
                ...
                protoReplicaInfo->set_status(ToProto(replicaSnapshot->RuntimeData->Status.load()));
        ...
        const TClusterNodeDynamicConfigPtr& newConfig)
    {
        ...
        UseQueryPoolForPullRows_.store(
            newConfig->QueryAgent->UseQueryPoolForPullRows.value_or(Config_->UseQueryPoolForPullRows));
    }

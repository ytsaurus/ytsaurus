...
void TCompactionHintFetchPipeline::OnRequestFailed(const TError& error)
{
    ...
    YT_LOG_WARNING(error, "Failed to fetch compaction hint for store; retrying with backoff "
        "(StoreId: %v, ChunkId: %v, RetryIndex: %v, BackoffTime: %v)",
        Store_->GetId(),
        Store_->GetChunkId(),
        RetryBackoff_.GetInvocationIndex(),
        backoffTime);

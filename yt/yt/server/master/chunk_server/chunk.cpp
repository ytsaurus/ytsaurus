...
void TChunk::Load(NCellMaster::TLoadContext& context)
{
    ...
    if (context.GetTractoVersion() >= ETractoMasterReign::OffshoreReplicas) {
        if (Load<bool>(context)) {
    ...
    NChunkClient::EChunkMetaPersistence metaPersistence)
{
    ...
    data->StoredReplicas.push_back({replica, sourceUri, metaPersistence});
}
...
bool TChunk::RemoveOffshoreReplica(TMediumPtrWithReplicaInfo replica)
{
    ...
    for (int replicaIndex = 0; replicaIndex < std::ssize(data->StoredReplicas); ++replicaIndex) {
        ...
        if (existingReplica.GetMedium() == replica.GetPtr() && existingReplica.GetReplicaIndex() == replica.GetReplicaIndex()) {

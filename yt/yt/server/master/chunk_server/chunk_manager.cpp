...
private:
    ...
    virtual TErrorOr<TChunkLocationPtrWithReplicaInfoList> GetChunkReplicas(const TEphemeralObjectPtr<TChunk>& chunk) const override
    {
        YT_VERIFY(!HasMutationContext());
            ...
            .BeginList()
                .Item().DoListFor(replicasToAdd, [] (auto fluent, const auto& replica) {
                    fluent
                        .Item()
                        .BeginList()
                            .Item().Value(ToString(replica.GetChunkLocationUuid()))

...
{
    return std::find(BuiltinMaintenanceIds.begin(), BuiltinMaintenanceIds.end(), id) != BuiltinMaintenanceIds.end();
}
...
bool TNode::GetMaintenanceFlag(EMaintenanceType type) const
{
    return MaintenanceCounts_[type];
}

bool TNode::ClearMaintenanceFlag(EMaintenanceType type)
{
...
bool TNode::SetMaintenanceFlag(
    EMaintenanceType type,
    TString userName,
    TInstant timestamp)
{
    bool wasAlreadySet = ClearMaintenanceFlag(type);

    static_cast<void>(AddMaintenance(
        BuiltinMaintenanceIds[type],
        {std::move(userName), type, "Generated by directly setting corresponding flag", timestamp}));

    return !wasAlreadySet;
}

bool TNode::AddMaintenance(
    TMaintenanceId id,
...
std::optional<EMaintenanceType> TNode::RemoveMaintenance(TMaintenanceId id)
{
...
{
    return GetMaintenanceFlag(EMaintenanceType::Ban);
}

bool TNode::IsDecommissioned() const
{
    return GetMaintenanceFlag(EMaintenanceType::Decommission);
}

bool TNode::AreSchedulerJobsDisabled() const
{
    return GetMaintenanceFlag(EMaintenanceType::DisableSchedulerJobs);
}

bool TNode::AreTabletCellsDisabled() const
{
    return GetMaintenanceFlag(EMaintenanceType::DisableTabletCells);
}

bool TNode::AreWriteSessionsDisabled() const
{
    return GetMaintenanceFlag(EMaintenanceType::DisableWriteSessions);
}
...
void TNode::Save(TSaveContext& context) const
{
    ...
    Save(context, MaintenanceRequests_);
...
void TNode::Load(TLoadContext& context)
{
    ...
    if (context.GetVersion() < EMasterReign::MaintenanceRequests) {
        ...
        Load(context, MaintenanceRequests_);
        for (const auto& [requestId, request] : MaintenanceRequests_) {
...
TMaintenanceId TNode::GenerateMaintenanceId() const
{
    YT_VERIFY(NHydra::HasHydraContext());
    const auto& generator = NHydra::GetCurrentHydraContext()->RandomGenerator();
    TMaintenanceId id;
    do {
        id = TMaintenanceId(generator->Generate<ui64>(), generator->Generate<ui64>());
    } while (MaintenanceRequests_.contains(id) || IsBuiltinMaintenanceId(id));
    return id;
}

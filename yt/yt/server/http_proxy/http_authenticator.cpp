    ...
    if (auto authorizationHeader = request->GetHeaders()->Find(AuthorizationHeaderName)) {
        ...
        if (!credentials.Token.empty()) {
            ...
            if (auto userHeader = request->GetHeaders()->Find(UserNameHeader)) {
                ...
                auto isSuperuserOrError = WaitFor(NApi::NNative::IsSuperuser(Bootstrap_->GetNativeConnection(), TString(authenticationResult.Login)));
                ...
                auto isUserBannedOrError = WaitForFast(NApi::NNative::IsUserBanned(Bootstrap_->GetNativeConnection(), TString(authenticationResult.Login)));

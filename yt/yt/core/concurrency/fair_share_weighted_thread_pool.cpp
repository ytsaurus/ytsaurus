...
It adds weights to buckets (2nd level). It changes balancing logic from cpu time to expected size passed from user.
*/
...
YT_DEFINE_GLOBAL(const NLogging::TLogger, Logger, "FairShareWeightedThreadPool");
...
class TBucket
    : public IInvokerWithExpectedSize
...
public:
    ...
    void InvokeWithExpectedSize(TClosure callback, i64 expectedSize) override;

    void InvokeWithExpectedSize(TMutableRange<std::pair<TClosure, i64>> callbacks) override
    {
        for (auto& callback : callbacks) {
            InvokeWithExpectedSize(std::move(callback.first), std::move(callback.second));
        }
    }
...
public:
    ...
    virtual void Invoke(TClosure callback, i64 expectedSize, TBucket* bucket) = 0;

    // GetInvoker is protected by mapping lock (can be sharded).
    IInvokerWithExpectedSizePtr GetInvoker(const TString& poolName, const TString& bucketName, double bucketWeight)
    {
...
void TBucket::InvokeWithExpectedSize(TClosure callback, i64 expectedSize)
{
    Parent_->Invoke(std::move(callback), expectedSize, this);
}
...
public:
    ...
    */
    ...
    void Invoke(TClosure callback, i64 expectedSize, TBucket* bucket) override
    {
        ...
        action.ExpectedSize = expectedSize;
        // Callback keeps raw ptr to bucket to minimize bucket ref count.
    ...
    Y_NO_INLINE std::tuple<int, int> ServeCombinedRequests(TCpuInstant currentInstant, int currentThreadIndex)
    {
        ...
        for (int threadIndex = 0; threadIndex < threadCount; ++threadIndex) {
            ...
            if (auto* bucket = threadState.Action.BucketHolder.Get()) {
                ...
                auto bytesConsumed = threadState.Action.ExpectedSize;
                threadState.Action.ExpectedSize = 0;
...
public:
    ...
    IInvokerWithExpectedSizePtr GetInvokerWithExpectedSize(
        const TString& poolName,

...
FROM (
    ...
    FROM (
        ...
        SELECT
            Yson::ConvertTo(json_array_col, List<Struct<id: Int32, name: Text, value: Double>>) AS arr
        FROM (
            SELECT
                '[{"id": 1, "name": "Foo", "value":-2.5}, {"id": 2, "name": "Bar", "value": 3.14}]'j AS json_array_col -- Sample JSON literal string
        )
    )

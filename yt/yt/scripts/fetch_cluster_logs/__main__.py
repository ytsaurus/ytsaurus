...
class LogDownloader:
    ...
    def list_files(self, pod: str, directory: str) -> List[RemoteFile]:
        ...
        for line in lines:
            ...
            files.append(RemoteFile(pod, "ytserver", file_path, int(size_str), mtime, ctime, file_inode))
        return files
    ...
    def _get_target_pods(self, args: argparse.Namespace, group_info: YtComponentGroup) -> List[str]:
        all_pods = self.yt.get_pod_names(group_info)
        pods = [p for p in all_pods if p in args.pods] if args.pods else all_pods
        if not pods:
    ...
    def _get_log_patterns(self, args: argparse.Namespace, group_info: YtComponentGroup, config_dir: str, pod: str) -> Set[str]:
        patterns: Set[str] = set()
        if args.writer_force:
            patterns.update(args.writer_force)
        if args.writer:
            config_path = self.yt.get_config_path(group_info, config_dir, args.exec_slot_index)
            logger.debug(f"Reading config from {config_path}")
            patterns.update(self.get_filter_patterns(pod, config_path, args.writer))
        if patterns:
            logger.debug("Fetching logs that matches any of regexes %s", patterns)
        return patterns

    def _collect_logs_from_pods(self, pods: List[str], log_dir: str, patterns: Set[str], args: argparse.Namespace) -> Dict[str, List[RemoteFile]]:
            ...
            for filtered_log_file in log_files:
                ...
                if patterns:
                    if not any(re.search(p, filtered_log_file.name) for p in patterns):
                        continue
                if args.from_ts and filtered_log_file.mtime < args.from_ts.replace(tzinfo=timezone.utc):
                    continue
                if args.to_ts and filtered_log_file.ctime > args.to_ts.replace(tzinfo=timezone.utc):
                    continue
    ...
    def run(self, args: argparse.Namespace):
        group_info = self.yt.get_group_info(args.component_name, args.group_name)
        logger.debug(f"Fetching logs from {group_info.component_type} (group {group_info.group_name})")

        pods = self._get_target_pods(args, group_info)
        if not pods:
            return

        log_dir = self.yt.get_log_dir(group_info, args.exec_slot_index)
        logger.debug("Fetching logs from dir %s", log_dir)

        patterns = self._get_log_patterns(args, group_info, log_dir, pods[0])

        files_map = self._collect_logs_from_pods(pods, log_dir, patterns, args)
        ...
        if not args.yes:
            if input("Download? [y/N]: ").lower() not in ("y", "yes"):
                return
        self._download_logs(files_map, args.output, args.exec_slot_index, args.grep)
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description="Fetch YT cluster logs via Kubernetes API. NOTE: This tool is experimental and its interface may change.",
        formatter_class=argparse.ArgumentDefaultsHelpFormatter,
    )
    parser.add_argument("-n", "--namespace", type=str, default="default", help="Kubernetes namespace")
    parser.add_argument("cluster_name", type=str, help="YTsaurus cluster name")
    ...
    parser.add_argument("--exec-slot-index", type=int, help="Slot index for job proxy logs (usefull only for exec nodes)")
    ...
    if args.exec_slot_index is not None and args.component_name != "exec_nodes":
        parser.error("--exec-slot-index can only be used when component_name=exec_nodes")

    if args.from_ts:
        args.from_ts = date_parser.parse(args.from_ts)
    if args.to_ts:
        args.to_ts = date_parser.parse(args.to_ts)
...
def main():
    ...
    k8s = K8sExecutor(args.namespace)
    yt = YtClusterManager(k8s, args.cluster_name)
    app = LogDownloader(k8s, yt)

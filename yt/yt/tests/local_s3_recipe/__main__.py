...
def download_s3_proxy(path):
    ...
    if os.path.exists(S3_PROXY_BINARY):
        shutil.copy("s3proxy", path)
    else:
...
def start(_: List[str]) -> None:
    ...
    if os.path.exists(S3_PROXY_BINARY):
        env["PATH"] = os.pathsep.join(
            [os.path.dirname(yatest_common.java_bin())] +
            os.environ.get("PATH", "").split(os.pathsep)
        )

    s3 = subprocess.Popen(s3_args, stdout=s3_log_file, text=True, env=env)

...
class TestRequestThrottling(YTEnvSetup):
    ...
    def _prepare_automaton_request_throttling_parameters(_):
        cypress_limit_path = "//sys/@config/object_service/automaton_request_throttler/limit"
    ...
    def test_automaton_request_throttling_does_not_impact_read_requests(self):
        ...
        request_count = 10
        automaton_request_throttler_limit_path = "//sys/@config/object_service/automaton_request_throttler/limit"

        # This throttler should only impact write requests.
        set(automaton_request_throttler_limit_path, 1)

        start_time = time.time()

        with deferred(lambda: remove(automaton_request_throttler_limit_path)):

...
class TestNoQueueAgentStage(TestQueueAgentBase, ReplicatedObjectBase, MasterSnapshotsCompatibilityBase):
    QUEUE_SCHEMA = [
    ...
    def test(self):
        ...
        # Create chaos queue.
        path = "//tmp/my_queue"
        create("chaos_replicated_table", path, attributes={
            "chaos_cell_bundle": "c",
            "schema": self.QUEUE_SCHEMA,
        })

        assert "queue_agent_stage" not in get("//tmp/my_queue/@")

        self.restart_with_update(MASTERS_SERVICE)
        master_exit_read_only_sync()

        assert get("//tmp/my_queue/@queue_agent_stage") == "production"
        set("//tmp/my_queue/@queue_agent_stage", "testing")
        assert "queue_agent_stage" in get("//tmp/my_queue/@")
        assert get("//tmp/my_queue/@queue_agent_stage") == "testing"


class TestRemoveUserQueueAttributes(TestQueueAgentBase, ReplicatedObjectBase, MasterSnapshotsCompatibilityBase):
    QUEUE_SCHEMA = [
        {"name": "$timestamp", "type": "uint64"},
        {"name": "data", "type": "string"}
    ]
    ...
    def test(self):
        ...
        chaos_replicated_queue_replicas = [
            {"cluster_name": "primary", "content_type": "queue", "mode": "async", "enabled": True,
             "replica_path": f"{queue_queue_replica_path}_1"},
            {"cluster_name": "primary", "content_type": "queue", "mode": "sync", "enabled": True,
             "replica_path": f"{queue_queue_replica_path}_2"},
            {"cluster_name": "primary", "content_type": "queue", "mode": "sync", "enabled": True,
             "replica_path": f"{queue_queue_replica_path}_3"},
        ]
        ...
        set("//tmp/my_queue/@queue_agent_stage", "some_value")
        assert get("//tmp/my_queue/@queue_agent_stage") == "some_value"
        assert "queue_agent_stage" in get("//tmp/my_queue/@user_attributes")
        assert get("//tmp/my_queue/@user_attributes/queue_agent_stage") == "some_value"

        set("//tmp/my_queue/@queue_status", "some_status")
        assert get("//tmp/my_queue/@queue_status") == "some_status"
        assert "queue_status" in get("//tmp/my_queue/@user_attributes")
        assert get("//tmp/my_queue/@user_attributes/queue_status") == "some_status"

        self.restart_with_update(MASTERS_SERVICE)

        assert get("//tmp/my_queue/@queue_agent_stage") == "production"
        assert "queue_agent_stage" not in get("//tmp/my_queue/@user_attributes")
        assert "queue_status" not in get("//tmp/my_queue/@user_attributes")

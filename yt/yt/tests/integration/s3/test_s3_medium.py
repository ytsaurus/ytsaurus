...
from yt_commands import (
    ...
    copy, move, get_singular_chunk_id, wait, get,
    ...
    map, merge, select_rows, lookup_rows, print_debug)
...
class TestS3Medium(YTEnvSetup):
    ...
    @authors("achulkov2")
    def test_quick(self):
        create("table", "//tmp/t", attributes={"primary_medium": "default", "erasure_codec": "reed_solomon_3_3"})
        write_table("//tmp/t", [{"a": "b"}])
        ...
        get(f"#{chunk_id}/@requisition")

    @authors("achulkov2")
    def test_table_and_chunk_attributes(self):
        create("table", "//tmp/t", attributes={"primary_medium": self.get_s3_medium_name()})
        # TODO(achulkov2): Check that table has rf=1?
        ...
        for chunk_id in chunk_ids:
            ...
            # TODO(achulkov2): [PForReview] Test @offshore_replicas attribute.

    # TODO(achulkov2): Test chunk attributes: replicas, ???.
    ...
    # TODO(achulkov2): Artifacts. Implement or disable for now.
    ...
    # master jobs: replication, merge?, repair (sanity), others?

    # TODO(achulkov2): Double check test runtimes, they are taking a while to write data, there might be some issues, or it is just minio that is slow.

    # TODO(achulkov2): Test that remote copy is forbidden.

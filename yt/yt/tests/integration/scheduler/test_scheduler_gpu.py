...
class TestDryRunGpuSchedulingPolicyMultiModule(YTEnvSetup):
    ...
    def test_assignment_plan_update_after_controller_restart(self):
        ...
        wait_for_assignments_in_orchid(op, assignment_count=8)

        # with controller agent disabled
        with Restarter(self.Env, CONTROLLER_AGENTS_SERVICE):
            # wait for operation to get disabled in gpu policy
            wait(lambda: not get_operation_from_orchid(op)['enabled'])
        ...
        wait(lambda: get_operation_from_orchid(op)['enabled'])
    ...
    def test_disable_operation_clears_pending_assignments(self):
        ...
        wait_for_assignments_in_orchid(pending_op, assignment_count=1)
        wait(lambda: get_operation_from_orchid(pending_op)["assignments"][0].get("allocation_id", yson.YsonEntity()) == yson.YsonEntity())

        with Restarter(self.Env, CONTROLLER_AGENTS_SERVICE):
            wait(lambda: not get_operation_from_orchid(pending_op)["enabled"])
            wait(lambda: len(get_operation_from_orchid(pending_op)["assignments"]) == 0)

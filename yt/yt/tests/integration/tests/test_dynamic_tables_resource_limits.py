...
class TestDynamicTablesResourceLimits(DynamicTablesBase):
    ...
    def _verify_resource_usage(self, account, resource, expected):
        def resource_usage_matches(driver):
            return lambda: (
                ...
                == expected
                and get(
                    "//sys/accounts/{0}/@committed_resource_usage/{1}".format(
                        account, resource
                    ),
                    driver=driver,
                )
    ...
    def _multicell_set(self, path, value):
        ...
        for i in xrange(self.Env.secondary_master_cell_count):
            ...
            wait(
                lambda: exists(path, driver=driver)
                and get(path, driver=driver) == value
            )
    ...
    @pytest.mark.parametrize(
        "resource", ["chunk_count", "disk_space_per_medium/default"]
    )
    def test_resource_limits(self, resource):
        ...
        self._multicell_set(
            "//sys/accounts/test_account/@resource_limits/" + resource, 10000
        )
        insert_rows("//tmp/t", [{"key": 0, "value": "0"}])
    ...
    def test_tablet_count_limit_create(self):
        ...
        self._multicell_set(
            "//sys/accounts/test_account/@resource_limits/tablet_count", 0
        )
        with pytest.raises(YtError):
        ...
        self._multicell_set(
            "//sys/accounts/test_account/@resource_limits/tablet_count", 1
        )
        with pytest.raises(YtError):
            self._create_ordered_table(
                "//tmp/t", account="test_account", tablet_count=2
            )
        with pytest.raises(YtError):
            self._create_sorted_table(
                "//tmp/t", account="test_account", pivot_keys=[[], [1]]
            )

        assert get("//sys/accounts/test_account/@ref_counter") == 1

        self._multicell_set(
            "//sys/accounts/test_account/@resource_limits/tablet_count", 4
        )
        ...
        self._create_sorted_table(
            "//tmp/t2", account="test_account", pivot_keys=[[], [1]]
        )
    ...
    def test_mount_mounted_table(self):
        create_account("test_account")
        self._multicell_set(
            "//sys/accounts/test_account/@resource_limits/tablet_count", 2
        )
        sync_create_cells(1)

        def _create_table(table_name):
            self._create_sorted_table(
                table_name, account="test_account", external_cell_tag=1
            )
    ...
    def test_tablet_count_limit_copy(self):
        ...
        wait(
            lambda: get("//sys/accounts/test_account/@resource_usage/tablet_count") == 1
        )
    ...
    def test_tablet_count_copy_across_accounts(self):
        ...
        self._multicell_set(
            "//sys/accounts/test_account1/@resource_limits/tablet_count", 10
        )
        self._multicell_set(
            "//sys/accounts/test_account2/@resource_limits/tablet_count", 0
        )
        ...
        self._multicell_set(
            "//sys/accounts/test_account2/@resource_limits/tablet_count", 1
        )
        copy("//tmp/t", "//tmp/dir/t_copy", preserve_account=False)
    ...
    def test_in_memory_accounting(self, mode):
        ...
        def _verify():
            ...
            committed_resource_usage = get(
                "//sys/accounts/test_account/@committed_resource_usage"
            )
            return (
                ...
                and get("//tmp/@recursive_resource_usage/tablet_static_memory")
                == data_size
            )

        self._multicell_set(
            "//sys/accounts/test_account/@resource_limits/tablet_static_memory", 1000
        )
        sync_mount_table("//tmp/t")
        wait(_verify)
        ...
        self._multicell_set(
            "//sys/accounts/test_account/@resource_limits/tablet_static_memory", 0
        )
        with pytest.raises(YtError):
    ...
    def test_remount_in_memory_accounting(self):
        ...
        set("//tmp/t/@account", "test_account")
        self._multicell_set(
            "//sys/accounts/test_account/@resource_limits/tablet_static_memory", 2048
        )
    ...
    def test_insert_during_tablet_static_memory_limit_violation(self):
        ...
        sync_create_cells(1)
        self._create_sorted_table(
            "//tmp/t1", account="test_account", in_memory_mode="compressed"
        )
        sync_mount_table("//tmp/t1")
        insert_rows("//tmp/t1", [{"key": 0, "value": "0"}])
        sync_flush_table("//tmp/t1")
        assert (
            get("//sys/accounts/test_account/@resource_usage/tablet_static_memory") > 0
        )
        assert get(
            "//sys/accounts/test_account/@violated_resource_limits/tablet_static_memory"
        )
        with pytest.raises(YtError):
    ...
    def test_snapshot_account_resource_limits_violation(self):
        ...
        create_tablet_cell_bundle(
            "custom", attributes={"options": {"snapshot_account": "test_account"}}
        )
        ...
        self._multicell_wait(
            lambda driver: lambda: get(
                "//sys/accounts/test_account/@resource_limits/disk_space", driver=driver
            )
            == 0
        )
    ...
    def test_chunk_view_accounting(self):
        ...
        self._multicell_set(
            "//sys/accounts/test_account/@resource_limits/tablet_count", 10
        )
        self._multicell_set(
            "//sys/accounts/other_account/@resource_limits/tablet_count", 10
        )
        sync_create_cells(1)
        self._create_sorted_table("//tmp/t", account="test_account")
        ...
        def _verify(account, disk_space):
            def wait_func():
                ...
                committed_usage = get(
                    "//sys/accounts/{}/@committed_resource_usage".format(account)
                )
...
class TestDynamicTablesResourceLimitsShardedTxNoBoomerangs(
    TestDynamicTablesResourceLimitsShardedTx
):
    def setup_method(self, method):
        super(TestDynamicTablesResourceLimitsShardedTxNoBoomerangs, self).setup_method(
            method
        )

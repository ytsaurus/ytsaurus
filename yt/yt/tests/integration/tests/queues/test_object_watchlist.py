...
class QueueAgentHelpers:
    ...
    @staticmethod
    def assert_registered_objects_are(*paths):
        objects = QueueAgentHelpers.get_objects()
        assert objects.keys() == set(paths)
        for path in paths:
            assert objects[path] == get(path + "/@revision")
...
class TestQueueAgentObjectRevisions(YTEnvSetup):
    ...
    def test_create(self):
        ...
        create("table", "//tmp/qq", attributes={"dynamic": True, "schema": [{"name": "data", "type": "string"}]},
               recursive=True)
        QueueAgentHelpers.assert_registered_objects_are("//tmp/q", "//tmp/qq")

        remove("//tmp/q")
        remove("//tmp/qq")
        QueueAgentHelpers.assert_registered_objects_are()
    ...
    def test_transactional_create(self):
        ...
        QueueAgentHelpers.assert_registered_objects_are("//tmp/q")
    ...
    def test_transactional_fun(self):
        ...
        commit_transaction(tx)
        QueueAgentHelpers.assert_registered_objects_are("//tmp/q", "//tmp/qqq")
...
class TestQueueAgentObjectsRevisionsPortal(TestQueueAgentObjectRevisions):
    ...
    def test_objects_from_different_cells(self):
        ...
        QueueAgentHelpers.assert_registered_objects_are("//portals/p1/q", "//portals/p2/q", "//portals/q")

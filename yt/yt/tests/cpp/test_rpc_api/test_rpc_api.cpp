...
TEST_F(TArrowTest, ReadTablePathWithSpecifiedColumns)
{
    auto path = MakeRandomTmpPath();
    TCreateNodeOptions options;
    options.Attributes = NYTree::CreateEphemeralAttributes();
    options.Attributes->Set("schema", New<TTableSchema>(std::vector<TColumnSchema>{
        {"FloatColumn", ESimpleLogicalValueType::Float},
        {"DoubleColumn", ESimpleLogicalValueType::Double},
    }));
    options.Attributes->Set("optimize_for", "scan");
    options.Force = true;
...

    {
        ...
        auto doubleColumnId = writer->GetNameTable()->GetIdOrRegisterName("DoubleColumn");
        auto doubleValue = MakeUnversionedDoubleValue(3.14, doubleColumnId);

        TUnversionedRowBuilder rowBuilder;
        rowBuilder.AddValue(floatValue);
        rowBuilder.AddValue(doubleValue);
    ...
    auto apiServiceProxy =
        VerifyDynamicCast<NYT::NApi::NRpcProxy::TClientBase*>(Client_.Get())->CreateApiServiceProxy();
    ...
    auto pathWithColumns = TRichYPath(path);
    pathWithColumns.SetColumns({"FloatColumn"});
    ToProto(req->mutable_path(), pathWithColumns);
    auto stream = WaitFor(NRpc::CreateRpcClientInputStream(req))
        .ValueOrThrow();

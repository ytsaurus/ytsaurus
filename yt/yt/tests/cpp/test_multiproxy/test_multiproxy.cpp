...
TEST_F(TMultiproxyTest, TestReadAccess)
{
    ...
    EXPECT_EQ(readRedirectingClient->GetClusterName(), "target");
...
TEST_F(TMultiproxyTest, TestWriteAccess)
{
    ...
    EXPECT_EQ(writeRedirectingClient->GetClusterName(), "target");
...
{
    EXPECT_EQ(TargetClient_->GetClusterName(), "target");

    auto loopClient = CreateRedirectingClient("YT_PROXY_TARGET", "target");

    auto clusterName = loopClient->GetClusterName();

...
yt.run_map("./prepare.py | ./writer.py", source, stat_tables, files=["../mapreduce", "writer.py", "prepare.py"])

merge_tables([rec["table"] for rec in map(yt.line_to_record, yt.read_table(stat_tables))], destination)

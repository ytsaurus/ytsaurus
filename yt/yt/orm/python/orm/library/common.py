...
class MaxInstantiatedObjectLimitExceededError(ClientError):
    pass


class ValidationFailedError(YtResponseError):
    pass
...
def get_yt_response_error_code_to_class():
    return {
        ...
        error_pb2.EC_MAX_INSTANTIATED_OBJECTS_LIMIT_EXCEEDED: MaxInstantiatedObjectLimitExceededError,
        error_pb2.EC_VALIDATION_FAILED: ValidationFailedError,
    }
...
def wait_for_duration(statement, timeout, sleep_backoff=0.1):
...
def with_lock_conflict_retries(action, **retry_kwargs):
    def wrapper(*args, **kwargs):
        return run_with_retries(
            lambda: action(*args, **kwargs), exceptions=(YtTabletTransactionLockConflict,), **retry_kwargs
        )

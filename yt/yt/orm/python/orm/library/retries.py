...
def make_retrier(logger, action, description, local_master_checker, *args, **kwargs):
    def except_action(exception, attempt):
        ...
        if local_master_checker is not None:
            local_master_checker(existing_exception=exception)
...
def get_retrier_factory(
    logger, description=None, exceptions=tuple(), config=None, request_timeout=None, local_master_checker=None
):
    if config is None:

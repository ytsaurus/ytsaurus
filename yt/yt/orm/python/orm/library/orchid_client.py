...
def get_leader_fqdn(yt_client, service_cypress_path, service_name, throw_if_no_leader=True):
    ...
    if len(leader_locks) == 0:
        message = "None of {} instances is leading".format(service_name)
...
class OrmOrchidClient(object):
    ...
    def _get_from_monitoring(self, monitoring_address, path):
        self.get_from_monitoring_client(OrmMonitoringClient(monitoring_address), path)

    def get_from_monitoring_client(self, client, path):
    ...
    def get_at_instance(self, instance_fqdn, path, *args, **kwargs):
        ...
        if not monitoring_address:
            raise Exception(
                "Instance {} of {} has neither orchid nor monitoring configured".format(
    ...
    def get_default_instance(self, throw_if_no_present=True):
        leader_instance = self.get_leader_instance(throw_if_no_leader=False)
        if leader_instance:
        ...
        if not instances:
            message = "{} has no connected instances".format(self.human_readable_service_name)
            if throw_if_no_present:
                raise Exception(message)
            else:
                logger.info(message)
                return None

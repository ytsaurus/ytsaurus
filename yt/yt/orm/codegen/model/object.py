...
from . import compat
from .common import OrmColumn, PRIMARY_DATA_GROUP, ETC_NAME_SEPARATOR
...
class OrmObject:
    ...
    custom_null_key: Optional[str] = None
    ...
    def make(
        ...
        result.custom_null_key = getattr(object_type_option, "custom_null_key", None)
    ...
    def collect_reference_deprecated_attributes(self):
        reference_deprecated_paths = []
        reference_deprecated_paths.extend(self._collect_reference_deprecated_recursive(self.meta, "/meta"))
        reference_deprecated_paths.extend(self._collect_reference_deprecated_recursive(self.spec, "/spec"))
        reference_deprecated_paths.extend(self._collect_reference_deprecated_recursive(self.status, "/status"))
        return [self.resolve_attribute_by_path(path) for path in reference_deprecated_paths]

    def _collect_reference_deprecated_recursive(self, message, path, visited=None):
        if visited is None:
            visited = set()
        result = []
        if not message or message.full_name in visited:
            return result
        visited.add(message.full_name)
        for name, field in message.fields_by_name.items():
            field_path = path + "/" + name
            if field.foreign_object_type:
                result.append(field_path)
            if field.value_message and not field.foreign_view_key_field and not field.value_message.is_object_message:
                result.extend(self._collect_reference_deprecated_recursive(field.value_message, field_path, visited))
        return result

    def make_references_deprecated(self):
        self.references_deprecated = {}
        self._make_nested_references_deprecated(self.meta, "/meta")
        self._make_nested_references_deprecated(self.spec, "/spec")
        self._make_nested_references_deprecated(self.status, "/status")

    def _make_nested_references_deprecated(self, message, path, visited=None):
        if visited is None:
            visited = set()
        if not message or message.full_name in visited:
            return
        visited.add(message.full_name)
        for name, field in message.fields_by_name.items():
            if field.foreign_view_key_field or field.viewed_reference:
                continue
            field_path = path + "/" + name
            if field.foreign_object_type and not field.foreign_view_key_field:
                self.references_deprecated[field_path] = field
            if field.value_message and not field.value_message.is_object_message:
                self._make_nested_references_deprecated(field.value_message, field_path, visited)

    def make_references(self):
        self.references = []
        self._make_nested_references(self.meta)
        self._make_nested_references(self.spec)
        self._make_nested_references(self.status)

    def _make_nested_references(self, message):
        if not message:
            return
        for field in message.reference_fields:
            if field.reference:
                self.references.append(field.reference)
            if field.value_message:
                for message in field.value_message.nested_messages_by_name.values():
                    self._make_nested_references(message)
    ...
    def finalize(self, context: "OrmContext"):
        context.current_foreign_objects.clear()

        self.root.finalize(context)

        self.make_references_deprecated()
        self.make_references()

        for child in self.children:
            context.current_foreign_objects.add(child)
        if self.parent:
            context.current_foreign_objects.add(self.parent)

        context.current_foreign_objects.discard(self)
        self.foreign_objects = sorted(context.current_foreign_objects, key=lambda x: x.snake_case_name)
        context.current_foreign_objects.clear()
    ...
    def resolve_fields(self, path: str, allow_prefix: bool = False) -> list[OrmField]:
        ...
        assert len(parts) >= 2, "Attribute path {} must contain at least 2 parts".format(path)
    ...
    def _build_orm_tracker(self, tracker: object_pb2.TRevisionTracker):
        ...
        tracker_attribute = self.resolve_attribute_by_path(tracker.tracker_path)
        assert tracker_attribute.field.is_column, f" Tracked attribute {tracker_attribute.field.path} is not column"
        assert tracker_attribute.field.proto_type == "fixed64", "Revision attributes must have fixed64 proto type"
        ...
        return OrmRevisionTracker(
            ...
            name=tracker_attribute.field.snake_case_name,
            lock_group_restriction_enabled=bool(tracker.restrict_to_lock_group),
            tracker_field=tracker_attribute.field,

...
class OrmMessage(OrmStructure):
    ...
    def link(self, context: OrmContext):
        ...
        for field in self.reference_fields_deprecated:
            if field.reference_deprecated and field.reference_deprecated.foreign_object:
                context.current_foreign_objects.add(field.reference_deprecated.foreign_object)
        for field in self.reference_fields:
            if field.reference and field.reference.foreign_object:
                context.current_foreign_objects.add(field.reference.foreign_object)
    ...
    @property
    def foreign_view_fields_deprecated(self) -> list[OrmField]:
        views = []
        for field in self.fields:
            if field.foreign_view_key_field is not None:
                views.append(field)
        return views
    ...
    def computed_fields(self):
        return filter(
            lambda field: (
                field.computed and not (field.foreign_view_key_field or field.viewed_reference or field.system)
            ),
            self.fields,
        )
    ...
    @property
    def tags(self):
        return ", ".join(f"/*{tag.enum_value_name}*/ {tag.number}" for tag in self.message_tags)
    ...
    def add_view_field(self, name: str, number: int, reference: "OrmReference") -> OrmField:

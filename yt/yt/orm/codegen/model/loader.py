...
from .object import OrmObject, TOP_LEVEL_FIELDS
...
def _get_imports(
    ...
    for object in objects:
        for message in object.messages():
            imports.update(
                collect_imports(message, data_model_proto_path, source_package_paths, unexported_module_paths)
            )
...
def _get_server_proto_imports(
    ...
    for message in etc_messages:
        for f in message.direct_fields:
...
def _etc_messages(objects):
    for obj in objects:
        yield obj.meta
        obj.meta.etc_consumed = True
        yield from _transitive_etc_messages(obj.meta)
        yield from _transitive_etc_messages(obj.spec)
        yield from _transitive_etc_messages(obj.status)


def _get_filtered_sourced_messages(package_desc, objects):
    ...
    if not revision_enabled:
        return package_desc.get_sourced_messages(filter=lambda message: message.name != "TObjectTouchRevision")
    return package_desc.get_sourced_messages()


def _init_snapshots(context: OrmContext) -> dict[str, OrmSnapshot]:
    result = {
        name: OrmSnapshot(context, from_dict(OrmSnapshotConfig, parameters))
        for name, parameters in context.aux_parameters["snapshots_parameters"].items()
    }
    return result
...
def _finalize(context: OrmContext) -> OrmModel:
    ...
    sourced_messages = _get_filtered_sourced_messages(context.package, context.objects)
    ...
    etc_messages = list(_etc_messages(context.objects))
    server_proto_imports = _get_server_proto_imports(
        context.objects,
        etc_messages,
        data_model_proto_path,
        context.package.source_package_paths,

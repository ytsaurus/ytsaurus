...
class OrmField:
    ...
    def make_foreign_view_field_deprecated(
        ...
        if (
            ...
            and target.root.output_filename != parent_message.output_filename
            and not target.root.output_to_default
        ):
            result.source_module_paths.add(target.root.output_filename)
    ...
    def initialize(self, context: OrmContext):
        ...
        if not self.viewed_reference:  # Don't descend into other objects.
    ...
    def finalize(self, context: OrmContext):
        ...
        if self.aggregate is not None:
            assert self.is_column and not self.is_primary
            assert not self.is_timestamp_attribute
        ...
        if self.foreign_view_key_field is None and self.value_message:
    ...
    def _make_cpp_accessors(self, context: OrmContext):
        if not (self.is_column or self.reference or self.viewed_reference):
            return
    ...
    @property
    def path(self):
        return self.parent.path + FIELD_PATH_SEP + self.snake_case_name
    ...
    def _analyze_type(self, field_descriptor: descriptor.FieldDescriptor, package: "OrmPackage"):
        if field_descriptor.type == protobuf_descriptor_pb2.FieldDescriptorProto.TYPE_MESSAGE:
            ...
            if field_descriptor.message_type.has_options and field_descriptor.message_type.GetOptions().map_entry:
                ...
                ), "Map columns with scalar values are not supported"
            else:
    ...
    def _analyze_yt_type(self, field_descriptor: descriptor.FieldDescriptor, package: "OrmPackage"):
        ...
        if self.preferred_storage_type:
            ...
            ), "Bad default enum storage type {}, compatible storage types: {}".format(
                default_enum_storage_type, type_description.compatible_storage_types_by_priority
            )
    ...
    def _analyze_options(self, field_descriptor: descriptor.FieldDescriptor):
        ...
        if field_descriptor.type == protobuf_descriptor_pb2.FieldDescriptorProto.TYPE_ENUM:
            if self.preferred_enum_yson_storage_type is not None:
                ...
                ), "\"storage_type\" ({}) and \"enum_yson_storage_type\" ({}) options conflict (Field: {})".format(
                    self.preferred_storage_type,
                    self.preferred_enum_yson_storage_type,
                    self.snake_case_name,
                )
        else:
            ...
            ), "Non enum field {} can not have \"enum_yson_storage_type\" option".format(self.snake_case_name)
    ...
    @property
    def add_tags(self):
        return ", ".join("/*{}*/ {}".format(tag.enum_value_name, tag.number) for tag in self._add_tags)

    @property
    def remove_tags(self):
        return ", ".join("/*{}*/ {}".format(tag.enum_value_name, tag.number) for tag in self._remove_tags)
    ...
    def _analyze_update_policy(self, field_descriptor: descriptor.FieldDescriptor):
        ...
        ), "not_updatable option is deprecated, use update_policy, fd: {}".format(field_descriptor.full_name)
    ...
    def _initialize_enum_field(self, package: "OrmPackage"):
        ...
        if self.is_map:
            ...
            assert (
                self.preferred_storage_type is None and self.preferred_enum_yson_storage_type is None
            ), "Map field {} enum value storage type can only be set via OrmDatabaseOptions's default_enum_storage_type db_option".format(
                self.snake_case_name
            )
            return
        ...
        if self.is_column:
            ...
            if self.value_enum.string_serializable is not None:
                ...
                ), "Columnar enum {} cannot simultaneously have different storage types".format(self.proto_value_type)
            else:
                self.value_enum.string_serializable = string_serializable

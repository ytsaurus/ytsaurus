...
#include <yt/ytlib/table_chunk_format/floating_point_column_writer.h>
#include <yt/ytlib/table_chunk_format/floating_point_column_reader.h>
...
protected:
    ...
    virtual std::unique_ptr<IUnversionedColumnReader> DoCreateColumnReader() override
    {
        switch (GetParam()) {
            case ESimpleLogicalValueType::Float:
                return CreateUnversionedFloatingPointColumnReader<float>(ColumnMeta_, ColumnIndex, ColumnId);
            case ESimpleLogicalValueType::Double:
                return CreateUnversionedFloatingPointColumnReader<double>(ColumnMeta_, ColumnIndex, ColumnId);
            default:
                YT_ABORT();
        }
    }

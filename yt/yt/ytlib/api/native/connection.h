...
TFuture<bool> IsSuperuser(const NApi::NNative::IConnectionPtr& connection, const TString& user);

//! Uses user attribute cache to check if the user is banned.
TFuture<bool> IsUserBanned(const NApi::NNative::IConnectionPtr& connection, const TString& user);

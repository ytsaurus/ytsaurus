    ...
    const TServiceDiscoveryEndpointsConfigPtr& serviceDiscoveryEndpointsConfig)
{
    ...
    protoServiceDiscoveryEndpointsConfig->set_update_period(ToProto(serviceDiscoveryEndpointsConfig->UpdatePeriod));
}
    ...
    const TMasterConnectionConfigPtr& masterConnectionConfig)
{
    ...
    protoCellDirectoryItem->set_rpc_timeout(ToProto(masterConnectionConfig->RpcTimeout));
    ...
    if (masterConnectionConfig->HedgingDelay) {
        protoCellDirectoryItem->set_hedging_delay(ToProto(*masterConnectionConfig->HedgingDelay));
    }
    ...
    protoCellDirectoryItem->set_random_peer_eviction_period(ToProto(masterConnectionConfig->RandomPeerEvictionPeriod));
    protoCellDirectoryItem->set_enable_peer_polling(masterConnectionConfig->EnablePeerPolling);
    protoCellDirectoryItem->set_peer_polling_period(ToProto(masterConnectionConfig->PeerPollingPeriod));
    protoCellDirectoryItem->set_peer_polling_period_splay(ToProto(masterConnectionConfig->PeerPollingPeriodSplay));
    protoCellDirectoryItem->set_peer_polling_request_timeout(ToProto(masterConnectionConfig->PeerPollingRequestTimeout));
    protoCellDirectoryItem->set_discovery_session_timeout(ToProto(masterConnectionConfig->DiscoverySessionTimeout));
    ...
    protoCellDirectoryItem->set_discover_timeout(ToProto(masterConnectionConfig->DiscoverTimeout));
    protoCellDirectoryItem->set_acknowledgement_timeout(ToProto(masterConnectionConfig->AcknowledgementTimeout));
    protoCellDirectoryItem->set_rediscover_period(ToProto(masterConnectionConfig->RediscoverPeriod));
    protoCellDirectoryItem->set_rediscover_splay(ToProto(masterConnectionConfig->RediscoverSplay));
    protoCellDirectoryItem->set_hard_backoff_time(ToProto(masterConnectionConfig->HardBackoffTime));
    protoCellDirectoryItem->set_soft_backoff_time(ToProto(masterConnectionConfig->SoftBackoffTime));
    protoCellDirectoryItem->set_retry_backoff_time(ToProto(masterConnectionConfig->RetryBackoffTime));
    protoCellDirectoryItem->set_retry_attempts(masterConnectionConfig->RetryAttempts);
    if (masterConnectionConfig->RetryTimeout) {
        protoCellDirectoryItem->set_retry_timeout(ToProto(*masterConnectionConfig->RetryTimeout));
    }
}

...
private:
    ...
    void HandleSyncResult(TClusterDirectoryUpdateResult result)
    {
        ...
        // NB(apachee): Workaround for 25.4, since YT_LOG_ALERT_AND_THROW does not yet throw meaningful errors,
        // i.e. it does not contain the log message in the error thrown.
        if (!cumulativeError.IsOK()) {
            YT_LOG_ALERT(cumulativeError);
            THROW_ERROR_EXCEPTION(cumulativeError);
        }
    }

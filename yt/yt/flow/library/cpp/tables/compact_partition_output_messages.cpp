...
class TCompactPartitionOutputMessages
    : public ICompactPartitionOutputMessages
{
public:
    TCompactPartitionOutputMessages(TContextPtr context, TDynamicTableRequestSpecPtr dynamicSpec)
        : Context_(context->WithTableName(CompactPartitionOutputMessagesTableName))
        , DynamicSpec_(std::move(dynamicSpec))
        , TablePath_(NYPath::YPathJoin(Context_->PipelinePath.GetPath(), CompactPartitionOutputMessagesTableName))
        , Logger(Context_->Logger)
        , Tag_(Context_->Tag.value_or(std::string{CompactPartitionOutputMessagesTableName}))
    ...
    TFuture<std::vector<TChunk>> LoadAll(TFilter filter) override
    {
        return BIND([this, strongThis = MakeStrong(this), filter] {
            ...
            if (filter.PartitionId) {
                const auto partitionIdStr = ToString(*filter.PartitionId);
                conditions.push_back(Format("(hash, partition_id) = (farm_hash(%Qv), %Qv)",
                    partitionIdStr,
                    partitionIdStr));
            }
            auto query = Format("partition_id, stream_id, chunk_id, data, data_codec, processed_mask from [%v]%v",
                TablePath_,
            ...
            i64 totalBytes = 0;
            {
                ...
                for (const auto& row : rowset->GetRows()) {
                    ...
                    chunks.push_back(TChunk{
                        .Key = TTableKey{
                            .PartitionId = FromUnversionedValue<TPartitionId>(row[partitionIdField]),
        ...
        NCompression::ECodec codecId) override
    {
        ...
        for (const auto& chunk : chunks) {
            ...
            const auto partitionIdStr = ToString(chunk.Key.PartitionId);
            TUnversionedRowBuilder builder;
            builder.AddValue(MakeUnversionedStringValue(partitionIdStr, partitionIdField));
        ...
        const std::vector<TMaskUpdate>& updates) override
    {
        ...
        for (const auto& update : updates) {
            const auto partitionIdStr = ToString(update.Key.PartitionId);
            TUnversionedRowBuilder builder;
            builder.AddValue(MakeUnversionedStringValue(partitionIdStr, partitionIdField));
        ...
        const std::vector<TTableKey>& tableKeys) override
    {
        ...
        for (const auto& tableKey : tableKeys) {
            const auto partitionIdStr = ToString(tableKey.PartitionId);
            TUnversionedRowBuilder builder;
            builder.AddValue(MakeUnversionedStringValue(partitionIdStr, partitionIdField));
...
{
    return New<TCompactPartitionOutputMessages>(std::move(context), std::move(dynamicSpec));
}

...
class JoinProcessFunctionTest {
    ...
    fun testHitMessageStoresHitPayloadInState() {
        ...
        val state = response.allStates().get(JoinProcessFunction.JOINED_ACTION_STATE, key).get()!!
    ...
    fun testMultipleHitsAreProcessedIndependently() {
        ...
        val state1 = response.allStates().get(JoinProcessFunction.JOINED_ACTION_STATE, key1).get()!!
        assertEquals("payload-30", state1.get("hit_payload", String::class.java))

        val state2 = response.allStates().get(JoinProcessFunction.JOINED_ACTION_STATE, key2).get()!!

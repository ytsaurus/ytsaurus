"""E2E test for the Kotlin static-table-join companion."""
...
PIPELINE_CONFIG_PATH = yatest.common.source_path(
    "yt/yt/flow/examples/kotlin/static_table_join/static_table_join/src/main/resources/pipeline.yson"
)
...
class TestStaticTableJoin(FlowTestJavaBase):
    JAVA_RUNNER_BINARY_DIR = yatest.common.binary_path(
        "yt/yt/flow/examples/kotlin/static_table_join/static_table_join"
    )
    JAVA_RUNNER_MAIN_CLASS = "tech.ytsaurus.flow.examples.statictablejoin.RunnerMain"
    ...
    def prepare_environment(self):
        ...
        # The reference stream is uint64-keyed, so the snapshot table must declare
        # a matching schema (an unschematized table would infer int64 for the key).
        reference_schema = [

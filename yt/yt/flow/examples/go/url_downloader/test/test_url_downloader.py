"""E2E test for the Go url_downloader companion."""
...
def generate_data(hosts, max_urls):
    ...
    for i in range(hosts):
        ...
        for j in range(max_urls // (i + 1)):
            ...
            rows.append({"host": host, "url": url, "data": f"length: {len(url)}, digits: {digits}"})

    return rows


def generate_answer(data):
    return {row["url"]: (row["host"], row["data"]) for row in data}
...
class TestUrlDownloader(FlowTestGoBase):
    GO_COMPANION_BINARY = yatest.common.binary_path("yt/yt/flow/examples/go/url_downloader/url_downloader")
    ...
    def test_basic(self, workers_count, controllers_count):
        ...
        with self.start_flow_process_federation(
            ...
            processed = {row["url"]: (row["host"], row["data"]) for row in rows}
            assert processed == EXPECTED_ANSWER

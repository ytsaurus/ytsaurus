"""E2E test for the Go wait-click-join companion."""
...
assert TOTAL_ACTIONS // 2 < len(EXPECTED_OUTPUT) < TOTAL_ACTIONS
...
class TestWaitClickJoin(FlowTestGoBase):
    GO_COMPANION_BINARY = yatest.common.binary_path("yt/yt/flow/examples/go/wait_click_join/wait_click_join")
    ...
    def prepare_environment(self):
        ...
        batching_write_rows(
            generate_action_log(tablet_count), lambda batch: self.client.insert_rows(self.action_queue, batch), 100
        )

        batching_write_rows(
            generate_hit_log(tablet_count), lambda batch: self.client.insert_rows(self.hit_queue, batch), 100
        )
    ...
    def test_basic(self, workers_count, controllers_count):
        ...
        with self.start_flow_process_federation(
            ...
            for key in sorted(expected.keys())[:100]:
                logging.info("Expected item: %s", expected[key])
                logging.info("     Got item: %s", got.get(key))
                assert expected[key] == got.get(key)

            assert expected == got
            assert len(EXPECTED_OUTPUT) == len(rows)

            assert list(self.client.select_rows(f"* from [{self.join_state}]")) == []
            assert list(self.client.select_rows(f"* from [{self.pipeline_path}/partition_states]")) == []

"""E2E test for the Go retryable-async-request companion."""
...
class TestRetryableAsyncRequest(FlowTestGoBase):
    GO_COMPANION_BINARY = yatest.common.binary_path(
        "yt/yt/flow/examples/go/retryable_async_request/retryable_async_request"
    )
    ...
    def setup_method(self, method):
        ...
        batching_write_rows(
            generate_log(tablet_count), lambda batch: self.client.insert_rows(self.input_queue, batch), 1000
        )
    ...
    def test_basic(self, workers_count, controllers_count):
        ...
        with self.start_flow_process_federation(
            ...
            data = {}
            for row in self.client.select_rows(f"* from [{self.data_state}]"):
                data[row["key"]] = row["total_length"]
            logging.info("Got totals: %s", data)

"""E2E test for the Go shuffle companion."""
...
class Test(FlowTestGoBase):
    GO_COMPANION_BINARY = yatest.common.binary_path("yt/yt/flow/examples/go/shuffle/shuffle")
    ...
    def test_basic(self, workers_count, controllers_count):
        ...
        with self.start_flow_process_federation(
            ...
            rows = list(self.client.select_rows(f"* from [{data_state}]"))
            got = {row["value"]: row["count"] for row in rows}
            expected = {f"data_{i}": SHUFFLE_STAGES for i in range(TOTAL_EVENTS)}

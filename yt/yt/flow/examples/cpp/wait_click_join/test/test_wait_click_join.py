...
class TestWaitClickJoin(FlowTestBase):
    FLOW_BINARY_PATH = yatest.common.binary_path(f"{yatest.common.context.project_path}/../wait_click_join")
    ...
    def prepare_environment(self):
        ...
        batching_write_rows(
            generate_hit_log(tablet_count), lambda batch: self.client.insert_rows(self.hit_queue, batch), 10000
        )
    ...
    def test_basic(self, workers_count, controllers_count, problems, cache):
        ...
        with self.start_flow_process_federation(
            ...
            assert {key: expected.get(key, {}) for key in keys} == {key: got.get(key, {}) for key in keys}

...
PIPELINE_CONFIG_PATH = yatest.common.source_path(
    f"{yatest.common.context.project_path}/../wait_click_join/src/main/resources/pipeline.yson"
)
...
class TestWaitClickJoin(FlowTestJavaBase):
    JAVA_RUNNER_BINARY_DIR = yatest.common.binary_path(f"{yatest.common.context.project_path}/../wait_click_join/")
    JAVA_RUNNER_MAIN_CLASS = "tech.ytsaurus.flow.examples.waitclickjoin.RunnerMain"
    ...
    def prepare_pipeline_config(self, cache=True):
        ...
        if not cache:
            if "dynamic_spec" not in pipeline_config:
                pipeline_config["dynamic_spec"] = {}
            if "job_tracker" not in pipeline_config["dynamic_spec"]:
                pipeline_config["dynamic_spec"]["job_tracker"] = {}

            pipeline_config["dynamic_spec"]["job_tracker"].update(
                {
                    "state_cache": {
                        "compressed_cache_weight": 0,
                        "uncompressed_cache_weight": 0,
                    }
                }
            )
    ...
    @pytest.mark.parametrize(
        ("workers_count", "controllers_count", "problems", "cache"),
        [
            pytest.param(
                1,
                1,
            ...
            pytest.param(4, 2, True, True, id="2c_4w_unstable"),
            pytest.param(4, 2, True, False, id="2c_4w_nocache_unstable"),
        ],
    )
    def test_basic(self, workers_count, controllers_count, problems, cache):
        self.prepare_environment()
        pipeline_config_path = self.prepare_pipeline_config(cache)

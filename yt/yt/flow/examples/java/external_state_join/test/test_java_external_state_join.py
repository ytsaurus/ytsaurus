"""E2E test for the Java external-state-join companion.

The companion looks up a pre-built dynamic reference table reached through the
``current`` Cypress symlink. The test repoints that symlink to a v2 snapshot
mid-flight and asserts that the SAME key resolves to the new value, proving
that the read-only external state joiner re-reads YT on every lookup (its
default ``cache.ttl`` is 0, i.e. no caching across epochs).
"""
...
PIPELINE_CONFIG_PATH = yatest.common.source_path(
    "yt/yt/flow/examples/java/external_state_join/external_state_join/src/main/resources/pipeline.yson"
)

#################################################################


class TestExternalStateJoin(FlowTestJavaBase):
    JAVA_RUNNER_BINARY_DIR = yatest.common.binary_path(
        "yt/yt/flow/examples/java/external_state_join/external_state_join"
    )
    JAVA_RUNNER_MAIN_CLASS = "tech.ytsaurus.flow.examples.externalstatejoin.RunnerMain"
    ...
    def test_join(self):
        ...
        with self.start_flow_process_federation(
            ...
            # Build a v2 snapshot carrying the SAME key with a new value and
            # atomically repoint the "current" symlink. The joiner's default
            # cache.ttl=0 means every lookup re-reads YT — so the same key,
            # looked up again, now resolves through v2.

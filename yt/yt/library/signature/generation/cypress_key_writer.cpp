...
TCypressKeyWriter::TCypressKeyWriter(TCypressKeyWriterConfigPtr config, TOwnerId ownerId, IClientPtr client)
...
TFuture<void> TCypressKeyWriter::DoCleanUpOnLimitReached(const TCypressKeyWriterConfigPtr& config, const TErrorOr<TYsonString>& ownerNode)
{
    ...
    removeFutures.reserve(keysToDelete);
    for (const auto& [_, keyId] : sortedKeys | std::views::take(keysToDelete)) {
        removeFutures.push_back(Client_->RemoveNode(
            MakeCypressKeyPath(config->Path, OwnerId_, TKeyId(keyId)),
            removeOptions));
    }

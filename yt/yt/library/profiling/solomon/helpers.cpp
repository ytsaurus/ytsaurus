...
TOutputEncodingContext CreateOutputEncodingContextFromHeaders(const THeadersPtr& headers)
{
    ...
    if (auto isSolomonFormat = headers->Find(IsSolomonPullHeaderName)) {
        if (!TryFromString<bool>(*isSolomonFormat, context.IsSolomonFormat)) {
            THROW_ERROR_EXCEPTION("Invalid value of %Qv header", IsSolomonPullHeaderName)
                << TErrorAttribute("value", *isSolomonFormat);
        }
    } else {
        context.IsSolomonFormat = context.Format == ::NMonitoring::EFormat::JSON || context.Format == ::NMonitoring::EFormat::SPACK;
    }

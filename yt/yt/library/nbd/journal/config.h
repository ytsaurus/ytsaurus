...
//! Optional overrides for a device's run-time knobs; the geometry (see #TJournalBlockDeviceOptions)
//! is fixed when the device is created and is not overridable.
struct TJournalBlockDeviceDynamicConfig
    : public NYTree::TYsonStruct
{
    std::optional<int> ThreadPoolSize;

    TSlruCacheDynamicConfigPtr BlockCache;

    std::optional<int> ReplicationFactor;
    std::optional<int> ReadQuorum;
    std::optional<int> WriteQuorum;
    std::optional<int> WriteParallelism;
    std::optional<TDuration> ChunkMaintenancePeriod;
    std::optional<i64> MaxChunkDataSize;
    std::optional<TExponentialBackoffOptions> WriteBackoff;
    NApi::TJournalChunkWriterConfigPtr ChunkWriter;
    NChunkClient::TChunkFragmentReaderConfigPtr ChunkReader;

    std::optional<TDuration> FlushPeriod;
    std::optional<i64> DirtyBlockPoolCapacity;
    std::optional<double> DirtyFractionThreshold;

    REGISTER_YSON_STRUCT(TJournalBlockDeviceDynamicConfig);

    static void Register(TRegistrar registrar);
};

DEFINE_REFCOUNTED_TYPE(TJournalBlockDeviceDynamicConfig)
    ...
    : public TBlockDeviceConfigBase
{
    ...
    TJournalBlockDeviceConfigPtr ApplyDynamic(const TJournalBlockDeviceDynamicConfigPtr& dynamicConfig) const;
    void ApplyDynamicInplace(const TJournalBlockDeviceDynamicConfigPtr& dynamicConfig);

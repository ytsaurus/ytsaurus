...
void ToAny(TRowBuffer* rowBuffer, TPIValue* result, TPIValue* value)
{
    auto unversionedResult = BorrowFromPI(result);
    auto unversionedValue = BorrowFromPI(value);

    *unversionedResult.GetValue() = EncodeUnversionedAnyValue(*unversionedValue.GetValue(), rowBuffer->GetPool());
}

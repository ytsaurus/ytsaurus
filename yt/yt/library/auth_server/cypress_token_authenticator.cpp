...
public:
        ...
        const TTokenCredentials& credentials) override
    {
        if (!credentials.Token && !credentials.TokenSha256) {
            return MakeFuture<TAuthenticationResult>(TError("Token or token sha256 should be provided to authenticate"));
        }
        ...
        std::vector<TFuture<TYsonString>> getAttributeFutures = {
...
private:
    ...
    TFuture<TAuthenticationResult> OnCallTokenResult(std::vector<TErrorOr<TYsonString>>&& responsesOrErrors)
    {
        ...
        auto missingAttribute = [] (const auto& rspOrError) {
            return !rspOrError.IsOK() && rspOrError.FindMatching(NYTree::EErrorCode::ResolveError);
        };

        auto missingUserAttribute = missingAttribute(userAttributeRspOrError);
        auto missingUserIdAttribute = missingAttribute(userIdAttributeRspOrError);

        if (missingUserAttribute && missingUserIdAttribute) {
        ...
        if (!userAttributeRspOrError.IsOK() && !missingUserAttribute) {
            THROW_ERROR_EXCEPTION(userAttributeRspOrError);
        }

        if (!userIdAttributeRspOrError.IsOK() && !missingUserIdAttribute) {

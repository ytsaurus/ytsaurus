...
{
    {
        ...
        TStringBuf expectedSkeleton = "#42: foo bar <guid> <path> <address> <guid>";
        EXPECT_EQ(
            expectedSkeleton,
            GetErrorSkeleton(error));
    }

    {
        ...
        TStringBuf expectedSkeleton = "#42: Key <key> with key <key> and attribute <attribute> not found for Account <account> with timestamp <timestamp>";
        EXPECT_EQ(
            expectedSkeleton,
            GetErrorSkeleton(error));
    }

    {
        ...
        TStringBuf expectedSkeleton = "#42: Undefined reference <reference>";
        EXPECT_EQ(
            expectedSkeleton,
            GetErrorSkeleton(error));
    }

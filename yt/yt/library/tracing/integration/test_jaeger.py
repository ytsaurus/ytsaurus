...
def jaeger_server():
    with network.PortManager() as pm:
        ...
        try:
            yield {"collector_port": collector_port, "query_port": query_port}
        finally:
            p.kill()
...
def test_start(jaeger_server):
    cmd = [
        yatest.common.binary_path("yt/yt/library/tracing/example/example"),
        f"127.0.0.1:{jaeger_server['collector_port']}"
    ]
    ...
    rsp = requests.get(f"http://127.0.0.1:{jaeger_server['query_port']}/api/traces/{trace_id}")

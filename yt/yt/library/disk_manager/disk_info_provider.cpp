...
void Serialize(const TDiskInfo& diskInfo, IYsonConsumer* consumer)
{
    NYT::NYTree::BuildYsonFluently(consumer)
        .BeginMap()
            .Item("disk_id").Value(diskInfo.DiskId)
            .Item("layout_id").Value(diskInfo.DiskLayoutId)
            .Item("disk_path").Value(diskInfo.DevicePath)
            .Item("device_name").Value(diskInfo.DeviceName)
            .Item("disk_model").Value(diskInfo.DiskModel)
            .Item("capacity").Value(diskInfo.Capacity)
            .Item("state").Value(diskInfo.State)
            .Item("class").Value(diskInfo.StorageClass)
            .Item("configured").Value(diskInfo.Configured)
            .Item("ready").Value(diskInfo.Ready)
            .Item("absent").Value(diskInfo.Absent)
            .Item("error").Value(diskInfo.Error)
        .EndMap();
}

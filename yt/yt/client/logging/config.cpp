...
void TDynamicTableLogWriterConfig::Register(TRegistrar registrar)
{
    ...
    registrar.Preprocessor([] (TThis* config) {
        ...
        config->WriteBackoff->InvocationCount = std::numeric_limits<int>::max();
        config->WriteBackoff->MinBackoff = TDuration::MilliSeconds(100);
        config->WriteBackoff->MaxBackoff = TDuration::Seconds(5);
    });

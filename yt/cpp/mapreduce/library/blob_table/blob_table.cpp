...
TTableSchema TBlobTableSchema::CreateYtSchema()
{
    TTableSchema result;
    result.Columns(BlobIdColumns_);
    result.AddColumn(PartIndexColumnName_, VT_INT64);
    result.AddColumn(DataColumnName_, VT_STRING);
    return result;
}
...
TTableSchema CreateSortedBlobTableSchema(const TTableSchema& tableSchema, const TBlobTableSchema& blobTableSchema)
{
...
public:
    TBlobTableWriter(
        ...
        , PartIndex_(firstPartIndex)
        , AutoFinishOfWriter_(autoFinishOfWriter) // Parameter for use with MapReduce.
    { }

    ~TBlobTableWriter()
    {
        try {
            if (!Finished_ && AutoFinishOfWriter_) {
                Writer_->Finish();
            }
        } catch (...) {
        }
...
private:
    ...
    {
        Y_ABORT_UNLESS(!Finished_, "Writer is finished");
        while (size > 0) {
            Y_ABORT_UNLESS(Data_.size() < PartSize_);
    ...
    {
        Y_ABORT_UNLESS(!Finished_);
        Finished_ = true;
        SendData();
        if (AutoFinishOfWriter_) {
            Writer_->Finish();
        }
    }
...
private:
    ...
    i64 PartIndex_;
    bool AutoFinishOfWriter_;
    bool Finished_ = false;
};

////////////////////////////////////////////////////////////////////////////////

IFileWriterPtr CreateBlobTableWriter(
    TTableWriterPtr<TNode> writer,
    TNode node,
    const TBlobTableSchema& schema,
    int firstPartIndex,
    bool autoFinishOfWriter)
{
    return MakeIntrusive<TBlobTableWriter>(writer, node, schema, firstPartIndex, autoFinishOfWriter);
}
    ...
    , Schema_(blobTableSchema)
{
    ...
    if (!tableExists) {
        if (!openOptions.Create_) {
            ...
            attributes["schema"] = Schema_.CreateYtSchema().ToNode();
...
NYT::IOperationPtr TBlobTable::SortAsync()
{
    ...
    auto sortedSchema = CreateSortedBlobTableSchema(tableSchema, Schema_);
...
IFileWriterPtr TBlobTable::AppendBlob(const TKey& key)
{
    ...
    return CreateBlobTableWriter(writer, keyNode, Schema_);
}

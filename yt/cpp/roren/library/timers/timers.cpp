...
std::optional<TTimer> TTimersContainer::GetLastTimer(const TGuard& lock) const
{
    Y_UNUSED(lock);
    if (TimersIndex_.empty()) {
        return {};
    }
    return *TimersIndex_.crbegin();
}
...
void TTimersContainer::ResetInFly(const TGuard& lock, const TTimer& timer)
{
    ...
    if (TimersInFly_.erase(timer)) {
        auto it = TimersIndex_.find(timer);
        Y_ABORT_UNLESS(it != TimersIndex_.end());
...
void TTimers::ReInit()
{
    {
        const auto lock = GetLock();
        Y_ABORT_UNLESS(false == PopulateInProgress_);
...
void TTimers::Migrate(const TTimer& timer, const TTimer::TShardId shardId)
{
    ...
    YT_UNUSED_FUTURE(tx->Commit());
}

void TTimers::PopulateIndex()
{
    size_t offset = 0;
    std::optional<TTimer> lastTimer;
    {
        const auto lock = GetLock();
        ...
        lastTimer = TTimersContainer::GetLastTimer(lock);
    }

    try {
        auto selectedTimers = YtSelectIndex(offset, lastTimer);
...
TVector<TTimer> TTimers::YtSelectIndex(const size_t offset, const std::optional<TTimer>& lastKey)
{
    const size_t limit = MIN(IndexSelectBatch_, IndexLimit_ - offset);
    if (limit == 0) {
        return {};
    }
    return NPrivate::YtSelectIndex(YtClient_, YTimersIndexPath_, ShardId_, lastKey, limit);
}

...
TMoveIndicator::TMoveIndicator(TMoveIndicator&& other)
{
    other.IsMoved_ = true;
}

TMoveIndicator& TMoveIndicator::operator=(TMoveIndicator&& other)
{
    other.IsMoved_ = this != &other;
    return *this;
}

bool TMoveIndicator::IsMoved() const
{
    return IsMoved_;
}
...
std::optional<T> GetOption(const NProtoBuf::Message& msg, const TString& name) {
    ...
    Y_ABORT_UNLESS(field->cpp_type() == cppType, "option type mismatch");
...
void ConfigureLogs(const google::protobuf::Message& config) {
    ...
    Y_ABORT_UNLESS(logConfig.has_value(), "missing Logs in config");
    Y_ABORT_UNLESS(NYTEx::NLogging::TConfig::descriptor() == (*logConfig)->GetDescriptor(), "expected Logs has type NYTEx::NLogging::TConfig");

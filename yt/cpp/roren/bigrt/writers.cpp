...
public:
    ...
    {
        const auto& compositeWriter = TBigRtExecutionContextOps::GetWriter(GetExecutionContext()->As<IBigRtExecutionContext>());
...
public:
    ...
    void Start(TOutput<void>&) override
    {
        const auto& compositeWriter = TBigRtExecutionContextOps::GetWriter(GetExecutionContext()->template As<IBigRtExecutionContext>());
        Writer_ = &compositeWriter->GetWriter(WriterId_);
    }
...
class TWriteToDestinationParDo
    : public IDoFn<TSerializedRow, void>
{
public:
    using TTag = TTypeTag<IDestinationWriter>;

    TWriteToDestinationParDo() = default;

    explicit TWriteToDestinationParDo(TTag id)
        : WriterId_(std::move(id))
    { }
    ...
    void Do(const TInputRow& input, TOutput<void>&) override
    {
        TInputRow row = input;
        row.DataSource = GetExecutionContext()->As<IBigRtExecutionContext>()->GetInputTag();
        Writer_->Write(row);
    }
...
private:
    ...
    IDestinationWriter* Writer_ = nullptr;
...
TTransform<TLogbrokerData, void> WriteRawLogbroker(NUserSessions::NRT::TLogbrokerWriterConfig config, TMaybe<NYT::NProfiling::TTagSet> profilerTags)
{
    auto configPtr = std::make_shared<const NUserSessions::NRT::TLogbrokerWriterConfig>(std::move(config));
    return TTransform<TLogbrokerData, void>{
        "WriteRawLogbroker",
        [configPtr=configPtr, profilerTags=std::move(profilerTags)] (const TPCollection<TLogbrokerData>& pCollection) -> void {
            ...
            TWriterRegistrator writerRegistrator = [tag = tag, config = configPtr, profilerTags = std::move(profilerTags)] (
                TCompositeBigRtWriterFactory& factory,
                const NYT::NAuth::ITvmServicePtr& tvmService)
            {
                factory.Add(tag, NUserSessions::NRT::CreateLogbrokerWriterFactory(*config, tvmService, profilerTags.GetOrElse(NYT::NProfiling::TTagSet{})));
            };
...
{
    return TTransform<TKV<ui64, TString>, void> {
        "WriteRawYtQueue",
...
TTransform<TSerializedRow, void> WriteToDestination(TString destinationName)
{
    ...
    ] (const TPCollection<TSerializedRow>& pCollection) {
        using namespace NPrivate;

        auto tag = TTypeTag<NPrivate::IDestinationWriter>{destinationName};
        auto write = MakeParDo<NPrivate::TWriteToDestinationParDo>(tag);

        TWriterRegistrator registrator = [] (TCompositeBigRtWriterFactory&, const NYT::NAuth::ITvmServicePtr&) {};
        SetAttribute(write, WriterRegistratorTag, registrator);
...
NPrivate::TPackAndWriteToYtQueueTransform PackAndWriteToYtQueue(NBigRT::TYtQueueWriterConfig config, NFraming::EFormat format, size_t maxSize)
{
    return {config, format, maxSize};
}

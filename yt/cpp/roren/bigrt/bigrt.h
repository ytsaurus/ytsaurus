...
#include <bigrt/lib/processing/state_manager/profile_manager/manager.h>
...
///
/// @param inputTag InputTag referencing Consumer from BigRtProgramConfig.
TReadTransform<NBigRT::TMessageBatch> ReadMessageBatch(const TString& inputTag = "input");
TReadTransform<TString> ReadMessageBatchV3(const TString& inputTag = "input");

///
/// @brief Creates pipeline that stores bigrt graph but cannot be executed.
///
/// Returned pipeline must be runned manually using @ref NYT::TBigRtExecutorPool
...
TPState<typename TState::TKey, TState> MakeBigRtProfilePState(const TPipeline& bigRtPipeline, const NBigRT::TProfileStateManagerConfig& managerConfig, const TBigRtStateConfig& config)
{

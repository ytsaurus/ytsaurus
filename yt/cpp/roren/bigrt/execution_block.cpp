...
static void WriteParDoArg(IOutputStream* out, const IRawParDo& parDo, const TString& indent)
{
    ...
    if (const auto* parDoTree = dynamic_cast<const IParDoTree*>(&parDo)) {
        ...
        (*out) << NPrivate::GetAttributeOrDefault(parDo, TransformNameTag, {"<unknown>"});
    }
    (*out) << '\n';
}
...
static void CheckError(const NYT::TFuture<void>& future)
{
...
{
    TStringStream out;
    WriteDebugDescription(&out, /*indent*/ "");
    return out.Str();
}
...
private:
    ...
    void Flush()
    {
        YT_UNUSED_FUTURE(Output_->AsyncDo(std::move(Buffer_.Slice(Buffer_.Begin(), BufferWriteOffset_))));
        ResetBuffer();
    }
...
{
public:
    TSerializedExecutionBlock(TBlockPipeConfig config, IRawParDoPtr parDo, THashMap<TString, NPrivate::TStatefulTimerParDoWrapperPtr> timersCallbacks, std::vector<IExecutionBlockPtr> outputBlocks, TVCpuMetricsPtr vcpuMetrics)
        : TExecutionBlockBase(std::move(outputBlocks))
        , InputPipeConfig_(config)
        , ParDo_(std::move(parDo))
        , TimersCallbacks_(std::move(timersCallbacks))
        , InputRowVtable_(GetRowVtable(ParDo_))
        , Outputs_(MakeBlockPipes(OutputBlocks_))
        , VCpuMetrics_(vcpuMetrics)
    { }

    TSerializedExecutionBlock(TBlockPipeConfig config, IRawParDoPtr parDo, THashMap<TString, NPrivate::TStatefulTimerParDoWrapperPtr> timersCallbacks, std::vector<IRawOutputPtr> outputs, TVCpuMetricsPtr vcpuMetrics)
        : TExecutionBlockBase({})
        , InputPipeConfig_(config)
        , ParDo_(std::move(parDo))
        , TimersCallbacks_(std::move(timersCallbacks))
    ...
    {
        Y_ABORT("not supported after Timers implementation.");
        // We can clone only unbound execution blocks;
        Y_ABORT_UNLESS(Invoker_ == nullptr);
    ...
    TFuture<void> Start(IExecutionContextPtr executionContext) override
    {
        ...
        AsyncOpsChain_ = NYT::MakeFuture<void>({});
    ...
    void AttachCallback(NYT::TExtendedCallback<void ()> callback)
    {
        auto g = Guard(Lock_);
        CheckError(AsyncOpsChain_);
        AsyncOpsChain_ = AsyncOpsChain_.Apply(callback.AsyncVia(Invoker_));
    }

    TDoStartedFuture AsyncDo(NYT::TSharedRef data) override
    {
        auto count = data.Size() / InputRowVtable_.DataSize;
        auto taskStartedPromise = NYT::NewPromise<void>();

        AttachCallback(BIND([parDo = ParDo_, data = std::move(data), count = count, taskStartedPromise, vcpuMetrics = VCpuMetrics] {
            taskStartedPromise.Set();

            NBigRT::TFiberVCpuTimeGuard guard([vcpuMetrics](auto& value) {
                if (vcpuMetrics) {
                    vcpuMetrics->AddProcess(value);
                }
            });

            parDo->Do(data.Begin(), count);
        }));
        return taskStartedPromise.ToFuture();
    }

    void AsyncOnTimer(const TString& callbackId, std::vector<TTimer> readyTimers) override
    {
        AttachCallback(BIND([
            statefulTimerDoWrapper = TimersCallbacks_.at(callbackId),
            readyTimers = std::move(readyTimers)
        ] () mutable {
            statefulTimerDoWrapper->OnTimer(std::move(readyTimers));
        }));
    }
    ...
    {
        auto finished = BIND([
            asyncOpsChain = std::move(AsyncOpsChain_),
            parDo = ParDo_,
            childrenFinished = ChildrenFinished_,
            outputs = Outputs_,
            outputBlocks = OutputBlocks_,
            vcpuMetrics = VCpuMetrics_
        ] () {
            NYT::NConcurrency::WaitFor(asyncOpsChain).ThrowOnError();

            {
                NBigRT::TFiberVCpuTimeGuard guard([vcpuMetrics](auto& value) {
                    if (vcpuMetrics) {
                        vcpuMetrics->AddPrepareForAsyncWrite(value);
                    }
                });
                parDo->Finish();
            }
    ...
    const std::vector<IExecutionBlockPtr>& GetOutputBlocks() const override
    {
    ...
    const THashMap<TString, NPrivate::TStatefulTimerParDoWrapperPtr>& GetTimersCallbacks() const override
    {
        return TimersCallbacks_;
    }

private:
    const TBlockPipeConfig InputPipeConfig_;
    const IRawParDoPtr ParDo_;
    const THashMap<TString, NPrivate::TStatefulTimerParDoWrapperPtr> TimersCallbacks_;
    const TRowVtable InputRowVtable_;
    const std::vector<IRawOutputPtr> Outputs_;
    const TVCpuMetricsPtr VCpuMetrics_;

    NYT::TFuture<void> AsyncOpsChain_;
...
IExecutionBlockPtr CreateSerializedExecutionBlock(
    ...
    TVCpuMetricsPtr vcpuMetrics)
{
    return ::MakeIntrusive<TSerializedExecutionBlock>(config, parDo, std::move(timersCallbacks), outputs, vcpuMetrics);
}
...
{
public:
    TConcurrentExecutionBlock(TBlockPipeConfig config, IRawParDoPtr parDo, std::vector<IExecutionBlockPtr> outputBlocks, TVCpuMetricsPtr vcpuMetrics)
        : TExecutionBlockBase(std::move(outputBlocks))
        , InputPipeConfig_(config)
    ...
    void AsyncFinish() override
    {
        auto finished = BIND([
            ...
            for (auto&& closure : parDoClosureList) {
                auto finished = BIND([closure = std::move(closure), vcpuMetrics] {
                    NBigRT::TFiberVCpuTimeGuard guard([vcpuMetrics](auto& value) {
                        if (vcpuMetrics) {
                            vcpuMetrics->AddPrepareForAsyncWrite(value);
                        }
    ...
    const THashMap<TString, NPrivate::TStatefulTimerParDoWrapperPtr>& GetTimersCallbacks() const override
    {
        static const THashMap<TString, NPrivate::TStatefulTimerParDoWrapperPtr> empty;
        return empty;
    }

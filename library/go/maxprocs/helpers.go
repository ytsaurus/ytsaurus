...
func tryApplyIntStringLimit(val string) (int, bool) {
	maxProc, err := strconv.Atoi(val)
	if err != nil {
		return 0, false
	}

	return Adjust(maxProc), true
}
...
func tryApplyFloatStringLimit(val string) (int, bool) {
	maxProc, err := strconv.ParseFloat(val, 64)
	if err != nil {
		return 0, false
	}

	return applyFloatLimit(maxProc), true
}

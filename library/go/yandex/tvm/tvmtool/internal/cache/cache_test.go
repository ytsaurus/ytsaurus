...
func TestCache_Load(t *testing.T) {
	...
	c.Store(testDstID, testDst, cache.Entry{
		Value: testValue,
		Born:  time.Now(),
	})
	...
	// checking before
	{
		...
		assert.NotEmpty(t, r, "failed to get '%d' from cache before deadline", testDstID)
		...
		assert.NotEmpty(t, r, "failed to get %q from tickets before deadline", testDst)
		assert.Equal(t, testValue, r)
	}
	{
		r, hit := c.Load(999833321)
		assert.Equal(t, cache.Miss, hit, "got tickets for '999833321', but that key must be never existed")
		assert.Empty(t, r, "got tickets for '999833321', but that key must be never existed")

		r, hit = c.LoadByAlias("kek")
		assert.Equal(t, cache.Miss, hit, "got tickets for 'kek', but that key must be never existed")
		assert.Empty(t, r, "got tickets for 'kek', but that key must be never existed")
	}
...
func TestCache_Keys(t *testing.T) {
	...
	c.Store(testDstID, testDstAlias, cache.Entry{
		Value: testValue,
		Born:  time.Now(),
	})
...
func TestCache_ExpiredKeys(t *testing.T) {
	...
	c.Store(tvm.ClientID(2), "miss", cache.Entry{
		Value: "miss_value",
		Born:  time.Now().Add(-maxTTL).Add(-time.Second),
	})
	...
	c.Store(newDstID, newDst, cache.Entry{
		Value: testValue,
		Born:  time.Now(),
	})

...
func apiSettings(t testing.TB, client tvm.ClientID) tvmauth.TvmAPISettings {
	...
	if client == 1000501 {
		return tvmauth.TvmAPISettings{
			SelfID:            1000501,
			AdditionalSelfIds: []tvm.ClientID{1000502},
...
const userTicketStr4 = "3:user:CA0Q__________9_GkIKAgh7CgMIyAMKCQiVBhDbBxjnBxCVBhoIYmI6c2VzczEaCGJiOnNlc3MyINKF2MwEKAEyDXRlc3QtbG9naW4taWQ:CBsz6nbAWGqI2DoFpr6Wgvsthno3CRPtR4qELWDP76iYB6quXH6C8Q-MSwDyCUnKzlBE1SXGXR7mOA_YPujCqfaMGZHKChJlkI7TJaxhODoxYD3jr84AxPJbBrPVJABtQDpTIKjcZ20a2QDYjKNgiB-GR1iCGVzGQMARkm5PMzQ"
...
const userTicketWithLocation77 = "3:user:CAsQ__________9_Gi8KBAjMjwYQzI8GGgpzb21lOnNjb3BlINKF2MwEKAAyDXRlc3QtbG9naW4taWQ4TQ:GkV-rSvnGe1fEEhZuK56cc4DVeurY68Q7mNVk3dnSUvqdWPEnE9LzO4WWeF9M1PSSy5T5cD-k3qPa0gikR1I-vcPkACzjnhTF9Y8_hSDbE78p1L9-oaNJ-_cTZnsLQy7HhOcMgZkyY0OhatZPcuc_cfNyi6zttfwpLlzjbailOA"
const userTicketWithDeviceType777 = "3:user:CAsQ__________9_GiQKBAjMjwYQzI8GINKF2MwEKAAyDXRlc3QtbG9naW4taWRAiQY:BAnKpT_7WXWlDP-8YFIxTAUbOQK95hfFgpoDRdFXeMXv2RARZYCVEJF7CQ5-dNvWIWVqqlUlGoQrDOzyX6q9z4EKWtZOpBDrJ6WD6n4_w_DPfXgLkyTnR3ZQ0N2K9u5hjBSXktUCT2nrDfmL71QKzA6QZ6adULnXSc1mIZhmhRg"
...
func TestDebugInfo(t *testing.T) {
	...
	ticketU, err = c.CheckUserTicket(context.Background(), userTicketStr4)
	require.NoError(t, err)
	require.Equal(t, map[tvm.UID]tvm.UserExtFields{123: {UID: 123, CurrentPorgID: 0}, 456: {UID: 456, CurrentPorgID: 0}, 789: {UID: 789, CurrentPorgID: 987, CurrentMUid: 999}}, ticketU.UidsExtFieldsMap)
	require.Equal(t, 789, int(ticketU.DefaultUIDExtFields.UID))
	require.Equal(t, 987, int(ticketU.DefaultUIDExtFields.CurrentPorgID))
	require.Equal(t, 999, int(ticketU.DefaultUIDExtFields.CurrentMUid))
...
func TestLocationCanBeParsedFromUserTicket(t *testing.T) {
	c1000502, err := tvmauth.NewAPIClient(apiSettings(t, 1000502), &nop.Logger{})
	require.NoError(t, err)

	ticketU, err := c1000502.CheckUserTicket(context.Background(), userTicketWithLocation77)
	require.NoError(t, err)
	require.Equal(t, []tvm.UID{100300}, ticketU.UIDs)
	require.Equal(t, tvm.UID(100300), ticketU.DefaultUID)
	require.Equal(t, tvm.Location(77), ticketU.Location)
	require.Equal(t, "ticket_type=user;expiration_time=9223372036854775807;scope=some:scope;default_uid=100300;uid=100300;env=Prod;login_id=test-login-id;location=77;", ticketU.DbgInfo)
	require.Equal(t, "3:user:CAsQ__________9_Gi8KBAjMjwYQzI8GGgpzb21lOnNjb3BlINKF2MwEKAAyDXRlc3QtbG9naW4taWQ4TQ:", ticketU.LogInfo)
}

func TestDeviceTypeCanBeParsedFromUserTicket(t *testing.T) {
	c1000502, err := tvmauth.NewAPIClient(apiSettings(t, 1000502), &nop.Logger{})
	require.NoError(t, err)

	{
		ticketU, err := c1000502.CheckUserTicket(context.Background(), userTicketStr)
		require.NoError(t, err)
		require.Equal(t, tvm.DeviceType(0), ticketU.DeviceType)
	}

	{
		ticketU, err := c1000502.CheckUserTicket(context.Background(), userTicketWithDeviceType777)
		require.NoError(t, err)
		require.Equal(t, tvm.DeviceType(777), ticketU.DeviceType)
		require.Equal(t, "ticket_type=user;expiration_time=9223372036854775807;default_uid=100300;uid=100300;env=Prod;login_id=test-login-id;device_type=777;", ticketU.DbgInfo)
	}
}

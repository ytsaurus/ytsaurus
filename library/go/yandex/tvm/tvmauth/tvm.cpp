...
void TVM_DestroyMemPool(TVM_MemPool* pool) {
    ...
    freeStr(pool->PublicKey);
    freeStr(pool->PrivateKey);
}
...
static void UnpackSettings(
    ...
    if (in->AdditionalSelfIds != nullptr) {
        THashSet<TTvmId> additionalSelfTvmIds;
        for (int i = 0; i < in->AdditionalSelfIdsSize; ++i) {
            additionalSelfTvmIds.insert(TTvmId(in->AdditionalSelfIds[i]));
        }
        out->AdditionalSelfTvmIds = std::move(additionalSelfTvmIds);
    }
}
...
static void PackUidsExtFields(
    ...
    for (const auto& [uid, userExtFields] : uidsExtFields) {
        ...
        ticket->UidsExtFields[i].CurrentMUid = userExtFields.CurrentMUid;
        i++;
    }
...
static void PackDefaultUidExtFields(
    ...
    ticket->DefaultUidExtFields->CurrentMUid = defaultUidExtFields->CurrentMUid;
}
...
static void PackUserTicket(
    ...
    out->Location = copy->GetLocation();

    out->DeviceType = copy->GetDeviceType();
}
...
static void PackKeyPair(
    NRw::TKeyPair in,
    TVM_KeyPair* out,
    TVM_MemPool* pool) noexcept {
    PackStr(in.Public, &out->Public, pool->PublicKey);
    PackStr(in.Private, &out->Private, pool->PrivateKey);
}
...
extern "C" void TVM_GenerateKeyPair(
    int keySize,
    TVM_KeyPair* keyPair,
    TVM_MemPool* pool) {
    NRw::TKeyPair pair = NRw::GenKeyPair(keySize);
    PackKeyPair(pair, keyPair, pool);
}

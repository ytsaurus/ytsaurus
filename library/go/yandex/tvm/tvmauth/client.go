...
import (
	...
	"go.ytsaurus.tech/library/go/cgosem"
	"go.ytsaurus.tech/library/go/core/log"
	"go.ytsaurus.tech/library/go/yandex/tvm"
)

// GenerateKeyPair creates asymmetric key pair for client authentication
func GenerateKeyPair(size int) *tvm.KeyPair {
	var pool C.TVM_MemPool
	defer C.TVM_DestroyMemPool(&pool)

	var keyPair C.TVM_KeyPair
	C.TVM_GenerateKeyPair(
		C.int(size),
		&keyPair,
		&pool)

	return &tvm.KeyPair{
		Public:  unpackString(&keyPair.Public),
		Private: unpackString(&keyPair.Private),
	}
}
...
func (o *TvmAPISettings) pack(out *C.TVM_ApiSettings) {
	...
	if o.AdditionalSelfIds != nil {
		out.AdditionalSelfIds = (*C.uint32_t)(&o.AdditionalSelfIds[0])
		out.AdditionalSelfIdsSize = C.int(len(o.AdditionalSelfIds))
	}
}
...
func unpackScopes(scopes *C.TVM_String, scopeSize C.int) (s []string) {
	...
	scopesSlice := unsafe.Slice(scopes, size)

	for i := 0; i < size; i++ {
		s[i] = C.GoStringN(scopesSlice[i].Data, scopesSlice[i].Size)
	}
...
func unpackUidsExtFieldsMap(uidsExtFields *C.TVM_UserExtFields, uidsExtFieldsSize C.int) map[tvm.UID]tvm.UserExtFields {
	size := int(uidsExtFieldsSize)
	res := make(map[tvm.UID]tvm.UserExtFields, size)

	if size == 0 {
		return res
	}

	extFieldsSlice := unsafe.Slice(uidsExtFields, size)

	for i := 0; i < size; i++ {
		uid := tvm.UID(uint64(extFieldsSlice[i].Uid))

		res[uid] = tvm.UserExtFields{
			UID:           uid,
			CurrentPorgID: tvm.PorgID(uint64(extFieldsSlice[i].CurrentPorgId)),
			CurrentMUid:   tvm.MUid(uint64(extFieldsSlice[i].CurrentMUid)),
		}
	}
...
func unpackDefaultUIDExtFields(defaultUIDExtFields *C.TVM_UserExtFields) *tvm.UserExtFields {
	...
	currentMUid := C.uint64_t(fields.CurrentMUid)

	res := &tvm.UserExtFields{
		UID:           tvm.UID(uid),
		CurrentPorgID: tvm.PorgID(currentPorgId),
		CurrentMUid:   tvm.MUid(currentMUid),
	}
...
func unpackUserTicket(t *C.TVM_UserTicket) (*tvm.CheckedUserTicket, error) {
	...
	if t.UidsSize != 0 && t.Uids != nil {
		size := int(t.UidsSize)
		ticket.UIDs = make([]tvm.UID, size)

		uidsSlice := unsafe.Slice(t.Uids, size)

		for i := 0; i < size; i++ {
			ticket.UIDs[i] = tvm.UID(uint64(uidsSlice[i]))
		}
	}
	...
	ticket.Location = tvm.Location(t.Location)
	ticket.DeviceType = tvm.DeviceType(t.DeviceType)
	return ticket, unpackStatus(t.Status)
}
...
// See detials: https://godoc.yandex-team.ru/pkg/go.ytsaurus.tech/library/go/yandex/tvm/#Client

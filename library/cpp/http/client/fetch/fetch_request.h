...
namespace NHttpFetcher {
    inline constexpr TDuration DEFAULT_REQUEST_TIMEOUT = TDuration::Minutes(1);
    inline constexpr TDuration DEFAULT_REQUEST_FRESHNESS = TDuration::Seconds(10000);
    ...
    public:
        TRequest(TString url = {}, TCallBack onFetch = TCallBack());
        TRequest(TString url, bool ignoreRobotsTxt, TDuration timeout,
                 TDuration freshness, TCallBack onFetch = TCallBack());
        TRequest(TString url, TDuration timeout, TDuration freshness, bool ignoreRobots,
                 size_t priority, TMaybe<TString> login = TMaybe<TString>(),
                 TMaybe<TString> password = TMaybe<TString>(),

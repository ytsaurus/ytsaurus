...
namespace NTskvFormat {
    namespace NDetail {
        ...
        void AssignOrEmplace(TContainer& container, TKey&& key, TValue&& value) {
            if constexpr (requires { container[std::forward<TKey>(key)] = std::forward<TValue>(value); }) {
                container[std::forward<TKey>(key)] = std::forward<TValue>(value);
            } else {
                container.emplace(std::forward<TKey>(key), std::forward<TValue>(value));
            }
        }
    }
    ...
    void DeserializeMap(const TStringBuf& data, T& result, TString& buffer, bool unescape = true) {
        ...
        StringSplitter(data.begin(), data.end()).Split('\t').Consume([&](const TStringBuf kv){
            ...
            NDetail::AssignOrEmplace(result, key, value);
        });
    ...
    void DeserializeMap(const TStringBuf& data, T& result, bool unescape = true) {
        ...
        StringSplitter(data.begin(), data.end()).Split('\t').Consume([&](const TStringBuf kv){
            ...
            NDetail::AssignOrEmplace(result, std::move(key), std::move(value));
        });
    }
}

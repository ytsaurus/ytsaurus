...
public:
    ...
    TFuture<std::vector<TTablePtr>> ListTables(
    ...
    TFuture<std::optional<TRevision>> GetObjectRevision(
        const IAuthorizationToken& token,
        const TString& name,
        bool throughCache)
    {
...
private:
    ...
    std::vector<TTablePtr> DoListTables(
        const NApi::NNative::IClientPtr& client,
        const TString& path,
        bool recursive);
    ...
    std::optional<TRevision> DoGetObjectRevision(
        const NApi::NNative::IClientPtr& client,
        const TString& name,
        bool throughCache);
...
std::optional<TRevision> TStorage::DoGetObjectRevision(
    const NApi::NNative::IClientPtr& client,
    const TString& name,
...
public:
    ...
    std::optional<TRevision> GetObjectRevision(
        const IAuthorizationToken& token,
        const TString& name,

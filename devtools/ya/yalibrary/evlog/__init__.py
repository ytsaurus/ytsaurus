...
class Evlog(object):
    ...
    def write(self, namespace, event, **kwargs):
        value = {'timestamp': time.time(), 'thread_name': threading.current_thread().name, 'namespace': namespace, 'event': event, 'value': kwargs}
        try:
            s = yjson.dumps(value) + '\n'
...
def with_evlog(params, evlog_dir, days_to_save, now_time, run_uid, hide_token):
    ...
    filename = getattr(params, 'evlog_file', None) or now_time.strftime(_LOG_FILE_NAME_FMT) + '.' + run_uid + _EVLOG_SUFFIX

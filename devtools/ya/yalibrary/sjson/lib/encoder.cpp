...
namespace NSJson {
    ...
    namespace {
        ...
        struct TPythonString {
            NPyBind::TPyObjectPtr ObjRef;
            TStringBuf Str;
        };

        bool GetUtf8String(PyObject* obj, TPythonString& result) {
#if PY_MAJOR_VERSION == 2
            if (PyString_Check(obj)) {
                char* data{};
                Py_ssize_t len{};
                Y_ENSURE(PyString_AsStringAndSize(obj, &data, &len) != -1);
                result.ObjRef.Reset(obj);
                result.Str = {data, static_cast<std::size_t>(len)};
                return true;
            }
            if (PyUnicode_Check(obj)) {
                TPyObjectPtr utf8Str{PyUnicode_AsUTF8String(obj), BORROW};
                if (!utf8Str.Get()) {
                    ythrow yexception() << "Cannot convert unicode to utf-8";
                }
                char* data{};
                Py_ssize_t len{};
                Y_ENSURE(PyString_AsStringAndSize(utf8Str.Get(), &data, &len) != -1);
                result.ObjRef.Reset(utf8Str);
                result.Str = {data, static_cast<std::size_t>(len)};
                return true;
            }
#else
            if (PyUnicode_Check(obj)) {
                ...
                char* data{};
                Py_ssize_t len{};
                Y_ENSURE(PyBytes_AsStringAndSize(utf8Str.Get(), &data, &len) != -1);
                result.ObjRef.Reset(utf8Str);
                result.Str = {data, static_cast<std::size_t>(len)};
                return true;
            }
        ...
        TString GetObjectStr(PyObject* obj) {
            ...
            if (objStrRef.Get()) {
                if (TPythonString str; GetUtf8String(objStrRef.Get(), str)) {
        ...
        public:
            ...
            {
                if (!WriteMethod_.Get()) {
                    ythrow TValueError() << "Stream doesn't have 'write' attribute";
                }
                if (!PyCallable_Check(WriteMethod_.Get())) {
                    ythrow TValueError() << "Stream 'write' attribute is not callable";
                }
            }
            ...
            void AddIndexToError(TValueError& error, TVector<PyObject*>::const_iterator item) {
                ...
                if (PyDict_Check(obj)) {
                    ...
                    while (PyDict_Next(obj, &pos, &key, &value)) {
                        if (value == nextObj) {
                            TPythonString keyStr;
                            Y_ENSURE(GetUtf8String(key, keyStr));
                            error << "[\"" << keyStr.Str << "\"]";
                            return;
                        }
                    }
        ...
        private:
            void DoEncode(PyObject* obj) {
                if (TPythonString str; GetUtf8String(obj, str)) {
                ...
                if (PyLong_Check(obj)) {
                    ...
                    PyErr_Clear();
                    ythrow TValueError() << "Cannot convert the following value to [unsigned] long long: " << GetObjectStr(obj);
                }
        #if PY_MAJOR_VERSION == 2
                if (PyInt_Check(obj)) {
                ...
                if (PyFloat_Check(obj)) {
                    ...
                    if (!PyErr_Occurred()) {
                        if (std::isnan(value) || std::isinf(value)) {
                            ythrow TValueError() << "Nan and Inf values are not permitted: " << GetObjectStr(obj);
                        }
                    ...
                    PyErr_Clear();
                    ythrow TValueError() << "Cannot convert the following value to double: " << GetObjectStr(obj);
                }
                if (PyDict_Check(obj)) {
                    ...
                    while (PyDict_Next(obj, &pos, &key, &value)) {
                        ...
                        TPythonString keyStr;
                        if (!GetUtf8String(key, keyStr)) {
                            ythrow TValueError() << "Dict key is not a string: " << GetObjectStr(key);
                        }
                        WriteEscapedString(keyStr.Str);
                        Out_.Write(':');
                        DoEncode(value);
                    }
                    Out_.Write('}');
                ...
                if (PyCallable_Check(obj)) {
                    ...
                    if (!result.Get()) {
                        ythrow yexception() << "External serialization method failed: " << GetObjectStr(obj);
                    }
                    return;
                }
                ythrow TValueError() << "Unsupported object: " << GetObjectStr(obj);
            }

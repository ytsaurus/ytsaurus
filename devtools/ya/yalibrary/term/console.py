...
def strip_ansi_codes_wisely(s):
    is_bytes = not isinstance(s, six.string_types)
    if len(s) < 100000:
        for rx in ansi_regexs(True, is_bytes):
            s = rx.sub(empty_str(is_bytes), s)
...
def strip_ansi_codes(s):
    is_bytes = not isinstance(s, six.string_types)
    for rx in ansi_regexs(False, is_bytes):
        s = rx.sub(empty_str(is_bytes), s)
    return s
...
def connect_real_tty(device='/dev/tty'):
    ...
    afile = open(device, 'wb+', buffering=0)

...
class YPathTokenizer(object):
    ...
    def advance(self):
        ...
        while current != len(self._path):
            ...
            if token == TOKEN_LEFT_BRACE or token == TOKEN_LEFT_BRACKET or self._path[current] in "/@&*":
                break

            elif self._path[current] == "\\":
        ...
        if current == self._offset:
            ...
            elif self._path[current] in "/@&*":
                self._token = self._path[current]

...
$ cat ya.make:
    ...
    TEST_SRCS(
        mapreduce/yt/tests/run_yt_unittests.py # path to current script
    )
    PEERDIR(
        mapreduce/yt/python           # path to yt_stuff that provides local yt
    )
    DEPENDS(
        yt/packages/19_4              # path to yt binaries (required by yt_stuff)
        mapreduce/yt/tests/native_new # path to unittest directory
    )
    END()
$ cat yt_unittest_conf.py
    BINARY_PATH = "mapreduce/yt/tests/native_new/test-native-interface" # path to unittest binary
"""

...
#include <mapreduce/yt/util/wait_for_tablets_state.h> // for WaitForTabletsState
...
int main(int argc, const char** argv) {
    ...
    //
    // Программа принимает первым аргументом путь к таблице, с которой она будет работать (таблица не должна существовать).
    // Мы не можем «захардкодить» табличку в //tmp, т.к. в //tmp нельзя монтировать таблицы.
    // Вам придётся получить права на работу с динтаблицами на каком-нибудь кластере и создавать динтаблицу в своём аккаунте.
    if (argc != 3) {
        Cerr << "Usage:\n";
    ...
    //  - указать атрибут dynamic в true
    //  - указать схему
    // Это нужно сделать обязательно в Create. Т.е. не получится сначала создать таблицу,

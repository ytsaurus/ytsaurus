...
class FilterRobotsReducer(yt.wrapper.TypedJob):
    ...
    # Отдельный набор для входных таблиц.
    STAFF_TABLE_INDEX = 0
    ...
    def prepare_operation(self, context, preparer):
        preparer.input(self.STAFF_TABLE_INDEX, type=StaffRow).input(self.IS_ROBOT_TABLE_INDEX, type=IsRobotRow).outputs(
            [self.ROBOT_TABLE_INDEX, self.HUMAN_TABLE_INDEX], type=StaffRow
        )

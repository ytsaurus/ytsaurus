...
def main():
    main_param(os.getcwd())

def main_param(work_path):
    files = [os.path.join(root, file) for root, dir, files in os.walk(work_path) for file in files]
    ...
    for file1, *others in consistent_files:
        f1 = os.path.join(work_path, file1)
        for file2 in others:
            f2 = os.path.join(work_path, file2)

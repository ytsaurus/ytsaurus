...
def libpath(major, minor, work_path=None):
    ...
    for v in versions:
        for top in ["stdlib", "third_party"]:
            if work_path:
                p = os.path.join(work_path, top, v)
            else:
...
def main():
    ...
    main_param(None, os.path.dirname(__file__), args)


def main_param(work_path, file_path, args):
    ...
    for major, minor in versions:
        roots = libpath(major, minor, work_path=work_path)
        files = []

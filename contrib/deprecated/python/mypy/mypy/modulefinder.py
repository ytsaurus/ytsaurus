...
class ModuleNotFoundReason(Enum):
    ...
    def error_message_templates(self) -> Tuple[str, str]:
        if self is ModuleNotFoundReason.NOT_FOUND:
            ...
            note = "See https://mypy.readthedocs.io/en/latest/running_mypy.html#missing-imports"
        elif self is ModuleNotFoundReason.IGNORED_PROTO_MODULE:
            msg = "Skipping analyzing '{}': mypy does not support _pb2 imports"
            note = "See DTCC-2162"
        else:
            assert False
        return msg, note
...
class FindModuleCache:
    ...
    def find_module(self, id: str) -> ModuleSearchResult:
        ...
        if id not in self.results:
            if id.endswith('_pb2'):
                self.results[id] = ModuleNotFoundReason.IGNORED_PROTO_MODULE
            else:
...
def default_lib_path(data_dir: str,
    ...
    if custom_typeshed_dir:
        ...
        if not os.path.isdir(typeshed_dir) and pkg_resources.resource_isdir(__package__, "typeshed"):
    ...
    for v in versions + [str(pyversion[0]), '2and3']:
        for lib_type in ['stdlib', 'third_party']:
            ...
            if os.path.isdir(stubdir) or pkg_resources.resource_isdir(__package__, stubdir):
                path.append(stubdir)

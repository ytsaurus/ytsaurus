...
def parse_config_file(options: Options, set_strict_flags: Callable[[], None],
                      ...
                      config_package: Optional[str] = None) -> None:
    ...
    for config_file in config_files:
        if not os.path.exists(config_file) and config_package is None:
            continue
        try:
            if config_package:
                import pkgutil
                _data = pkgutil.get_data(config_package, config_file)
                assert _data is not None
                _data = _data.decode('utf-8')
                parser.read_string(_data, source=config_file)
            else:

...
const char *MAPSTRUCT_SPI_PREFIX = "META-INF/services/org.mapstruct.ap.spi.";
const size_t MAPSTRUCT_SPI_PREFIX_LENGTH = strlen(MAPSTRUCT_SPI_PREFIX);
...
class JarStripperProcessor : public JarExtractorProcessor {
 public:
  using ClassMetadata =
      std::unordered_map<std::string, std::vector<u1>>;

  explicit JarStripperProcessor(const ClassMetadata *metadata)
      : metadata_(metadata) {}
...
static void RejectMapStructSpi(const char *filename) {
  fprintf(stderr,
          "ijar: %s is a compile-time MapStruct SPI provider and cannot be "
          "stripped; use JAVA_LIBRARY_NON_SPLIT for this module.\n",
          filename);
  exit(EXIT_FAILURE);
}
...
bool JarStripperProcessor::Accept(const char *filename, const u4 /*attr*/) {
  ...
  if (StartsWith(filename, filename_len, MAPSTRUCT_SPI_PREFIX,
                 MAPSTRUCT_SPI_PREFIX_LENGTH)) {
    RejectMapStructSpi(filename);
  }
...
void JarStripperProcessor::Process(const char *filename, const u4 /*attr*/,
  ...
  if (IsModuleInfo(filename) || IsKotlinModule(filename, strlen(filename)) ||
    ...
    const u1 *metadata = data;
    size_t metadata_size = size;
    if (metadata_ != NULL) {
      const auto found = metadata_->find(filename);
      if (found != metadata_->end()) {
        metadata = found->second.data();
        metadata_size = found->second.size();
      }
    }
    u1 *buf = reinterpret_cast<u1 *>(malloc(size + metadata_size));
    u1 *classdata_out = buf;
    if (!StripClass(buf, data, size, metadata, metadata_size)) {
      free(classdata_out);
      return;
    }
    u1 *q = builder_->NewFile(filename, 0);
...
class ClassMetadataCollector : public ZipExtractorProcessor {
 public:
  using ClassMetadata = JarStripperProcessor::ClassMetadata;
  ...
  bool Accept(const char *filename, const u4 /*attr*/) override {
    ...
    return EndsWith(filename, filename_len, CLASS_EXTENSION,
                    CLASS_EXTENSION_LENGTH);
  }

  void Process(const char *filename, const u4 /*attr*/, const u1 *data,
               size_t size) override {
    classes_.emplace(filename, std::vector<u1>(data, data + size));
  }

  ClassMetadata classes_;
};
...
static void OpenFilesAndProcessJar(const char *file_out, const char *file_in,
  ...
  ClassMetadataCollector metadata_collector;
  std::unique_ptr<ZipExtractor> metadata_in;
  if (metadata_source != NULL) {
    metadata_in.reset(
        ZipExtractor::Create(metadata_source, &metadata_collector));
    if (metadata_in == NULL) {
      fprintf(stderr, "Unable to open metadata Zip file %s: %s\n",
              metadata_source, strerror(errno));
      abort();
    }
    if (metadata_in->ProcessAll() < 0) {
      fprintf(stderr, "%s\n", metadata_in->GetError());
      abort();
    }
  }
  std::unique_ptr<JarExtractorProcessor> processor;
  if (strip_jar) {
    processor = std::unique_ptr<JarExtractorProcessor>(
        new JarStripperProcessor(metadata_source == NULL
                                     ? NULL
                                     : &metadata_collector.classes_));
  } else {
    processor =
  ...
  if (metadata_in != NULL) {
    output_length += metadata_in->CalculateOutputLength();
  }
...
int main(int argc, char **argv) {
  ...
  const char *metadata_source = NULL;
  ...
  for (int ii = 1; ii < argc; ++ii) {
    ...
    } else if (strcmp(argv[ii], "--metadata_source") == 0) {
      if (++ii >= argc) {
        usage();
      }
      metadata_source = argv[ii];

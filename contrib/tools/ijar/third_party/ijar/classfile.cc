...
struct Constant_Utf8 : Constant
{
  Constant_Utf8(u4 length, const u1 *utf8) :
      ...
      utf8_(reinterpret_cast<const char *>(utf8), length) {}

  void Write(u1 *&p) {
    put_u1(p, tag_);
    put_u2be(p, utf8_.size());
    put_n(p, reinterpret_cast<const u1 *>(utf8_.data()), utf8_.size());
  }
...
// Code is not part of an interface jar, but javac stores source parameter
// names in its nested LocalVariableTable unless -parameters is enabled. Keep
// just that metadata while reading and continue dropping the complete Code
// attribute while writing.
struct CodeAttribute : Attribute {
  ...
  static CodeAttribute *Read(const u1 *&p, Constant *attribute_name,
                             u4 /*attribute_length*/) {
    auto attr = new CodeAttribute;
    attr->attribute_name_ = attribute_name;
    get_u2be(p);  // max_stack
    get_u2be(p);  // max_locals
    u4 code_length = get_u4be(p);
    p += code_length;
    u2 exception_table_length = get_u2be(p);
    p += exception_table_length * 8;
    u2 attributes_count = get_u2be(p);
    for (int ii = 0; ii < attributes_count; ++ii) {
      Constant *nested_name = constant(get_u2be(p));
      u4 nested_length = get_u4be(p);
      if (nested_name->Display() != "LocalVariableTable") {
        p += nested_length;
        continue;
      }

      u2 local_variable_table_length = get_u2be(p);
      for (int jj = 0; jj < local_variable_table_length; ++jj) {
        LocalVariable local;
        local.start_pc = get_u2be(p);
        local.length = get_u2be(p);
        local.name = constant(get_u2be(p));
        get_u2be(p);  // descriptor_index
        local.index = get_u2be(p);
        attr->local_variables_.push_back(local);
      }
    }
    return attr;
  }
  ...
  for (int ii = 0; ii < attributes_count; ii++) {
    ...
    if (attr_name == "SourceFile" ||
      ...
      attributes.push_back(
          CodeAttribute::Read(p, attribute_name, attribute_length));
...
using MethodParameterMap =
    std::unordered_map<std::string, std::vector<MethodParameterInfo>>;

static std::string MethodKey(const Member *method) {
  return method->name->Display() + "\n" + method->descriptor->Display();
}

static MethodParametersAttribute *FindMethodParameters(Member *method) {
  for (Attribute *attribute : method->attributes) {
    auto *parameters = dynamic_cast<MethodParametersAttribute *>(attribute);
    if (parameters != NULL) {
      return parameters;
    }
  }
...
static const CodeAttribute *FindCode(const Member *method) {
  for (const Attribute *attribute : method->attributes) {
    const auto *code = dynamic_cast<const CodeAttribute *>(attribute);
    if (code != NULL) {
      return code;
    }
  }
...
static std::vector<u2> ParameterSlots(const Member *method) {
  std::vector<u2> slots;
  const std::string descriptor = method->descriptor->Display();
  size_t pos = 0;
  if (descriptor.empty() || descriptor[pos++] != '(') {
    return slots;
  }
  u2 slot = (method->access_flags & ACC_STATIC) == 0 ? 1 : 0;
  while (pos < descriptor.size() && descriptor[pos] != ')') {
    ...
    if (descriptor[pos] == 'L') {
      ...
      const char type = descriptor[pos++];
      slot += !array && (type == 'J' || type == 'D') ? 2 : 1;
    }
  }
  return slots;
}

static bool Complete(const std::vector<MethodParameterInfo>& parameters) {
  for (const MethodParameterInfo& parameter : parameters) {
...
static std::vector<MethodParameterInfo> ReadParameterInfo(Member *method) {
  MethodParametersAttribute *parameters = FindMethodParameters(method);
  if (parameters != NULL) {
    std::vector<MethodParameterInfo> result;
    for (const auto *parameter : parameters->parameters_) {
      result.push_back(
          {parameter->name_ == NULL ? "" : parameter->name_->Display(),
           parameter->access_flags_});
    }
    if (Complete(result)) {
      return result;
    }
  ...
  if ((method->access_flags & ACC_SYNTHETIC) != 0) {
    return std::vector<MethodParameterInfo>();
  }
  const CodeAttribute *code = FindCode(method);
  if (code == NULL) {
    return std::vector<MethodParameterInfo>();
  }

  const std::vector<u2> slots = ParameterSlots(method);
  std::vector<MethodParameterInfo> result;
  for (u2 slot : slots) {
    const CodeAttribute::LocalVariable *best = NULL;
    for (const CodeAttribute::LocalVariable& local : code->local_variables_) {
      if (local.index != slot ||
          (best != NULL &&
           (local.length < best->length ||
            (local.length == best->length && local.start_pc >= best->start_pc)))) {
        continue;
      }
      best = &local;
    }
    ...
    result.push_back({best->name->Display(), 0});
  }
  return result;
}

static MethodParameterMap CollectMethodParameters(ClassFile *clazz) {
  MethodParameterMap result;
  for (Member *method : clazz->methods) {
    std::vector<MethodParameterInfo> parameters = ReadParameterInfo(method);
    if (!parameters.empty() && Complete(parameters)) {
      result.emplace(MethodKey(method), std::move(parameters));
    }
  }
  return result;
}

static Constant *FindOrAddUtf8(const std::string& value) {
  for (Constant *entry : const_pool_in) {
    if (entry != NULL && entry->tag_ == CONSTANT_Utf8 &&
        entry->Display() == value) {
      return entry;
    }
  }
  Constant *entry = new Constant_Utf8(
      value.size(), reinterpret_cast<const u1 *>(value.data()));
  const_pool_in.push_back(entry);
  return entry;
}

static void ApplyMethodParameters(ClassFile *clazz,
                                  const MethodParameterMap& metadata) {
  for (Member *method : clazz->methods) {
    const auto found = metadata.find(MethodKey(method));
    if (found == metadata.end() ||
        found->second.size() != ParameterSlots(method).size()) {
      continue;
    }

    MethodParametersAttribute *parameters = FindMethodParameters(method);
    if (parameters == NULL) {
      parameters = new MethodParametersAttribute;
      parameters->attribute_name_ = FindOrAddUtf8("MethodParameters");
      method->attributes.push_back(parameters);
      for (const MethodParameterInfo& parameter : found->second) {
        auto *output = new MethodParametersAttribute::MethodParameter;
        output->name_ = FindOrAddUtf8(parameter.name);
        output->access_flags_ = parameter.access_flags;
        parameters->parameters_.push_back(output);
      }
      continue;
    }

    if (parameters->parameters_.size() != found->second.size()) {
      continue;
    }
    for (size_t ii = 0; ii < found->second.size(); ++ii) {
      if (parameters->parameters_[ii]->name_ == NULL) {
        parameters->parameters_[ii]->name_ =
            FindOrAddUtf8(found->second[ii].name);
      }
    }
  }
}
...
struct ClassEdit {
  const u1 *begin;
  const u1 *end;
  std::vector<u1> replacement;
};

static const u1 *ConstantPoolEnd(const u1 *classdata, u2 *constant_pool_count) {
  const u1 *p = classdata + 8;
  *constant_pool_count = get_u2be(p);
...
static void SkipAttributes(const u1 *&p) {
  u2 attributes_count = get_u2be(p);
  for (int ii = 0; ii < attributes_count; ++ii) {
    p += 2;
    u4 attribute_length = get_u4be(p);
...
static void SkipMember(const u1 *&p) {
  p += 6;
  SkipAttributes(p);
}

static u2 FindOrAppendUtf8(const std::string& value, u2 constant_pool_count,
                           std::vector<std::string> *appended_utf8) {
  for (u2 ii = 1; ii < const_pool_in.size(); ++ii) {
    Constant *entry = const_pool_in[ii];
    if (entry != NULL && entry->tag_ == CONSTANT_Utf8 &&
        entry->Display() == value) {
      return ii;
    }
  }
  for (size_t ii = 0; ii < appended_utf8->size(); ++ii) {
    if ((*appended_utf8)[ii] == value) {
      return constant_pool_count + ii;
    }
  }
  if (value.size() > UINT16_MAX ||
      constant_pool_count + appended_utf8->size() >= UINT16_MAX) {
    fprintf(stderr, "ijar: too many constants while preserving method "
                    "parameter names.\n");
    abort();
  }
  appended_utf8->push_back(value);
  return constant_pool_count + appended_utf8->size() - 1;
}

static std::vector<u1> EncodeMethodParameters(
    const std::vector<MethodParameterInfo>& parameters,
    u2 constant_pool_count, std::vector<std::string> *appended_utf8) {
  if (parameters.size() > UINT8_MAX) {
    fprintf(stderr, "ijar: too many method parameters.\n");
    abort();
  }
  std::vector<u1> result(7 + parameters.size() * 4);
  u1 *p = result.data();
  put_u2be(p, FindOrAppendUtf8("MethodParameters", constant_pool_count,
                              appended_utf8));
  put_u4be(p, 1 + parameters.size() * 4);
  put_u1(p, parameters.size());
  for (const MethodParameterInfo& parameter : parameters) {
    put_u2be(p, FindOrAppendUtf8(parameter.name, constant_pool_count,
                                 appended_utf8));
    put_u2be(p, parameter.access_flags);
  }
  return result;
}

// Kotlin metadata is opaque to ijar, so Kotlin ABI classes must otherwise be
// copied byte-for-byte. Add only MethodParameters to the already stripped
// classfile instead of replacing it with the corresponding full class.
static void InjectMethodParameters(u1 *&classdata_out,
                                   const u1 *classdata_in, size_t in_length,
                                   const MethodParameterMap& metadata) {
  u2 constant_pool_count;
  const u1 *constant_pool_end =
      ConstantPoolEnd(classdata_in, &constant_pool_count);
  const u1 *p = constant_pool_end;
  p += 6;  // access_flags, this_class, super_class
  u2 interfaces_count = get_u2be(p);
  p += interfaces_count * 2;
  u2 fields_count = get_u2be(p);
  for (int ii = 0; ii < fields_count; ++ii) {
    SkipMember(p);
  }
  ...
  for (int ii = 0; ii < methods_count; ++ii) {
    ...
    u2 name_index = get_u2be(p);
    u2 descriptor_index = get_u2be(p);
    const std::string key = constant(name_index)->Display() + "\n" +
                            constant(descriptor_index)->Display();
    const u1 *attributes_count_pos = p;
    u2 attributes_count = get_u2be(p);
    const auto found = metadata.find(key);
    const u1 *attributes_end = p;
    bool has_method_parameters = false;
    for (int jj = 0; jj < attributes_count; ++jj) {
      u2 attribute_name_index = get_u2be(attributes_end);
      u4 attribute_length = get_u4be(attributes_end);
      const u1 *attribute_payload = attributes_end;
      attributes_end += attribute_length;
      if (constant(attribute_name_index)->Display() != "MethodParameters") {
        continue;
      }
      has_method_parameters = true;
      if (found == metadata.end() || attribute_length < 1) {
        continue;
      }
      const u1 *parameter = attribute_payload;
      u1 parameters_count = get_u1(parameter);
      if (parameters_count != found->second.size() ||
          attribute_length != 1 + parameters_count * 4) {
        fprintf(stderr, "ijar: inconsistent MethodParameters attribute.\n");
        abort();
      }
      for (int kk = 0; kk < parameters_count; ++kk) {
        const u1 *name_index_pos = parameter;
        u2 parameter_name_index = get_u2be(parameter);
        if (parameter_name_index == 0) {
          std::vector<u1> name_index(2);
          u1 *name_index_out = name_index.data();
          put_u2be(name_index_out,
                   FindOrAppendUtf8(found->second[kk].name,
                                    constant_pool_count, &appended_utf8));
          edits.push_back(
              {name_index_pos, name_index_pos + 2, std::move(name_index)});
        }
        parameter += 2;  // access_flags
      }
    }
    p = attributes_end;
    ...
    if (attributes_count == UINT16_MAX) {
      fprintf(stderr, "ijar: too many method attributes while preserving "
                      "method parameter names.\n");
      abort();
    }
    std::vector<u1> encoded = EncodeMethodParameters(
        found->second, constant_pool_count, &appended_utf8);
    std::vector<u1> new_attributes_count(2);
    u1 *count_out = new_attributes_count.data();
    put_u2be(count_out, attributes_count + 1);
    edits.push_back({attributes_count_pos, attributes_count_pos + 2,
                     std::move(new_attributes_count)});
    edits.push_back({attributes_end, attributes_end, std::move(encoded)});
  }
  ...
  put_n(classdata_out, classdata_in, 8);
  put_u2be(classdata_out, constant_pool_count + appended_utf8.size());
  put_n(classdata_out, classdata_in + 10,
        constant_pool_end - (classdata_in + 10));
  for (const std::string& value : appended_utf8) {
    put_u1(classdata_out, CONSTANT_Utf8);
    put_u2be(classdata_out, value.size());
    put_n(classdata_out, reinterpret_cast<const u1 *>(value.data()),
          value.size());
  }

  const u1 *input = constant_pool_end;
  for (const ClassEdit& edit : edits) {
    put_n(classdata_out, input, edit.begin - input);
    if (!edit.replacement.empty()) {
      put_n(classdata_out, edit.replacement.data(), edit.replacement.size());
    }
    input = edit.end;
  }
  put_n(classdata_out, input, classdata_in + in_length - input);
}

static void CleanupClassState() {
  for (Constant *entry : const_pool_in) {
    delete entry;
  }
  const_pool_in.clear();
...
static MethodParameterMap ReadMethodParameterMetadata(
    const u1 *classdata, size_t length) {
  ClassFile *clazz = ReadClass(classdata, length);
  MethodParameterMap result;
  if (clazz != NULL) {
    result = CollectMethodParameters(clazz);
    delete clazz;
  }
  CleanupClassState();
  return result;
...
bool StripClass(u1 *&classdata_out, const u1 *classdata_in, size_t in_length,
                const u1 *metadata_classdata, size_t metadata_length) {
  MethodParameterMap metadata;
  if (metadata_classdata != NULL && metadata_classdata != classdata_in) {
    metadata = ReadMethodParameterMetadata(metadata_classdata, metadata_length);
  }
  ClassFile *clazz = ReadClass(classdata_in, in_length);
  bool keep = true;
  if (clazz == NULL || clazz->KeepForCompile()) {
    if (clazz != NULL && !metadata.empty()) {
      InjectMethodParameters(classdata_out, classdata_in, in_length, metadata);
    } else {
      put_n(classdata_out, classdata_in, in_length);
    }
    delete clazz;
    ...
    MethodParameterMap input_metadata = CollectMethodParameters(clazz);
    for (auto& entry : input_metadata) {
      metadata.emplace(entry.first, std::move(entry.second));
    }
    ApplyMethodParameters(clazz, metadata);
    clazz->length += metadata_length;

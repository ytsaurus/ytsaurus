...
const size_t VCDiffCodeTableWriter::kSdch3DmBlockSize;
...
//
VCDiffCodeTableWriter::VCDiffCodeTableWriter(bool interleaved, bool sdch3dm)
    : max_mode_(VCDiffAddressCache::DefaultLastMode()),
      dictionary_size_(0),
      sdch3dm_enabled_(sdch3dm),
      target_length_(0),
...
bool VCDiffCodeTableWriter::Init(
    const char* dictionary_start,
    size_t dictionary_size) {
  dictionary_start_ = dictionary_start;
  ...
  if (sdch3dm_enabled_) {
    sdch3_sent_blocks_.clear();
    sdch3_sent_blocks_.resize(
      (dictionary_size + kSdch3DmBlockSize - 1) / kSdch3DmBlockSize, false);
    if (!dictionary_start_) {
        VCD_DFATAL << "Init() is used with dictionary_start = NULL while "
                   << "sdch3dm is enabled" << VCD_ENDL;
    }
  }
  return ResetTarget();
}

bool VCDiffCodeTableWriter::ResetTarget() {
...
void VCDiffCodeTableWriter::WriteHeader(
  ...
  DeltaFileHeader hdr = kHeaderStandardFormat;
  if (format_extensions != VCD_STANDARD_FORMAT) {
    hdr.header4 = 'S'; // VCDIFF/SDCH, extensions used
    if (format_extensions & VCD_FORMAT_SDCH3DM)
      hdr.hdr_indicator |= VCD_SDCH3DM;
  }
  out->append(reinterpret_cast<const char*>(&hdr),
              sizeof(hdr));
  if (format_extensions & VCD_FORMAT_SDCH3DM) {
    AppendSizeToOutputString(kSdch3DmBlockSize, out);
...
void VCDiffCodeTableWriter::Copy(int32_t offset, size_t size) {
  ...
  // When sdch3dm is enabled, we add every kSdch3DmBlockSize-sized fragment
  // of the dictionary that is touched by [offset, size) range into
  // data_for_add_and_run_, unless it has already been added.
  // NOTE: if non-zero per-window dict offsets get introduced in encoder,
  // this code should also be adjusted.
  if (sdch3dm_enabled_ && size && offset < (ptrdiff_t)dictionary_size_) {
    size_t from_block = offset / kSdch3DmBlockSize;
    size_t clip_size = std::min(offset + size, dictionary_size_);
    size_t to_block = (clip_size + kSdch3DmBlockSize - 1) / kSdch3DmBlockSize;
    if (to_block > sdch3_sent_blocks_.size()) {
      VCD_DFATAL << "Unexpected sent blocks in VCDiffCodeTableWriter::Copy()"
                 << VCD_ENDL;
      return;
    }
    for (size_t n = from_block; n < to_block; n++) {
      if (!sdch3_sent_blocks_[n]) {
        sdch3_sent_blocks_[n] = true;
        size_t offset = kSdch3DmBlockSize * n;
        data_for_add_and_run_->append(dictionary_start_ + offset,
          std::min(dictionary_size_ - offset, kSdch3DmBlockSize));
      }
    }
  }
  target_length_ += size;
...
void VCDiffCodeTableWriter::Output(OutputStringInterface* out) {
  ...
  // dictionary is used.
  //
  // Notably, ResetTarget() calls address_cache_.Init().  This resets the
  // address cache between delta windows, as required by RFC section 5.1.
  if (!ResetTarget()) {
...
bool VCDiffCodeTableWriter::Sdch3FinishDictionaryEmbedding(
    OutputStringInterface* out) {
  size_t num_blocks = 0;
  for (bool b : sdch3_sent_blocks_)
    num_blocks += !b;
  // We can omit this window completely if necessary
  if (!num_blocks)
    return true;
  // Make sure that "out" is not resized too many times
  out->ReserveAdditionalBytes(num_blocks * kSdch3DmBlockSize + 1);
  out->push_back(VCD_FINAL);
  for (size_t n = 0; n < sdch3_sent_blocks_.size(); n++) {
    if (!sdch3_sent_blocks_[n]) {
      size_t offset = kSdch3DmBlockSize * n;
      out->append(dictionary_start_ + offset,
        std::min(dictionary_size_ - offset, kSdch3DmBlockSize));
    }
  }
  return true;

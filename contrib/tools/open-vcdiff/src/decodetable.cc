...
//
// VCDiffCodeReader is a class to interpret a stream of opcodes
...
VCDiffInstructionType VCDiffCodeReader::GetNextInstruction(
    int32_t* size,
    unsigned char* mode) {
  ...
  do {
    ...
    if (code_table_data_->inst2[opcode] != VCD_NOOP) {
      ...
      pending_second_instruction_ = *position_;
    }
    ++position_;
  ...
  if (instruction_size == 0) {
    ...
    switch (*size = VarintBE<int32_t>::Parse(end_, &position_)) {
      case RESULT_ERROR:

...
// Allow some tuning of the compression process.
// Generally, the greater the value, the worse the compression ratio, but
// at the same time, when vcdiff delta is further compressed by gzip or brotli,
// etc., overall compression is typically (but not always) better.
enum CompressionOptions : size_t {
  COMP_MIN_8_BLOCK_8 = 0,
  COMP_MIN_16_BLOCK_8 = 1,
  COMP_MIN_16_BLOCK_16 = 2,
  COMP_MIN_32_BLOCK_16 = 3,
  COMP_MIN_32_BLOCK_32 = 4,
  COMP_MIN_64_BLOCK_32 = 5,
  COMP_MIN_64_BLOCK_64 = 6,
  COMP_MIN_128_BLOCK_64 = 7,
  COMP_DEFAULT = COMP_MIN_32_BLOCK_16,
  COMP_VALUE_MAX = 7
};
...
class HashedDictionary {
 public:
  HashedDictionary(const char* dictionary_contents,
                   ...
                   CompressionOptions comp_opt = COMP_DEFAULT);
  ~HashedDictionary();
  ...
  // Access internal copy of dictionary_contents
...
class VCDiffEncoder {
 public:
  VCDiffEncoder(const char* dictionary_contents, size_t dictionary_size,
                CompressionOptions comp_opt = COMP_DEFAULT)
      : dictionary_(dictionary_contents, dictionary_size, comp_opt),
        encoder_(NULL),

...
class VCDiffStreamingDecoder {
 public:
  ...
  // Sets up the data structures for decoding stream with sdch3dm - embedded
  // dictionary. "dict" must remain valid until FinishDecoding is called,
  // upon that moment it will contain the dictionary, if the decoding
  // successfully processed the entire stream.
  // If the dictionary size exceeds "max_size", the decoding will fail.
  // Setting "max_size" to zero means no limit.
  void StartSdch3DmDecoding(std::string* dict, size_t max_size = 0);
...
class VCDiffDecoder {
 public:
  ...
  //
  template<class OutputType, class EncodingType>

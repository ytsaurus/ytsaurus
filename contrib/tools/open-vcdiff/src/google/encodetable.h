...
class VCDiffCodeTableWriter : public CodeTableWriterInterface {
 public:
  ...
  // If sdch3dm is true, dictionary will be embedded into output.
  // This is also not consistent with the VCDIFF draft standard.
  VCDiffCodeTableWriter(bool interleaved, bool sdch3dm);
  ...
  //
  virtual bool Init(const char* dictionary_start, size_t dictionary_size);
  ...
  virtual bool FinishEncoding(OutputStringInterface* out) {
    if (sdch3dm_enabled_)
      return Sdch3FinishDictionaryEmbedding(out);
    return true;
  }
 ...
 private:
  ...
  // Prepare to encode a new target window.
  bool ResetTarget();
  ...
  bool Sdch3FinishDictionaryEmbedding(OutputStringInterface* out);
  ...
  const char* dictionary_start_ = nullptr;
  size_t dictionary_size_;

  // When sdch3dm_enabled_ is true, sdch3dm extension is enabled,
  // so that the dictionary can be reconstructed by the client.
  // When kSdch3DmBlockSize-byte sized block of the dictionary is referred
  // in a Copy instruction for the first time, that block will be copied
  // to data_for_add_and_run_.
  // sdch3_sent_blocks_ controls which blocks have already been copied.
  static const size_t kSdch3DmBlockSize = 64;
  bool sdch3dm_enabled_;
  std::vector<bool> sdch3_sent_blocks_;

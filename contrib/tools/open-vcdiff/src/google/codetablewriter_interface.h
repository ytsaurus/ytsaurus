...
class CodeTableWriterInterface {
 public:
  ...
  // NULL may sometimes be passed as "dictionary_start" in unit tests,
  // however, do aviod that in practical use.
  virtual bool Init(const char* dictionary_start, size_t dictionary_size) = 0;
  ...
  // Finishes encoding.
  virtual bool FinishEncoding(OutputStringInterface* out) = 0;

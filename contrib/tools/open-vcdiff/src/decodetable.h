...
//
// NOT threadsafe.
class VCDiffCodeReader : public ParseableChunk {
 public:
  struct SavedState {
    const char* position;
    OpcodeOrNone pending_second_instruction;
  };
  ...
  // should be called after the VCDiffCodeReader has been constructed.
  ...
  // the object. When the client extends or relocates underlying data,
  // without changing the unread part, it should call SetDataBuffer() so that
  // the instruction decoding can resume from where the process was left.
  //
  void Init(const char* data_start, size_t data_size) {
    ParseableChunk::SetDataBuffer(data_start, data_size);
    pending_second_instruction_ = kNoOpcode;
  }

  // Save internal state that can be restored by Restore().
  SavedState Save() const {
    return { UnparsedData(), pending_second_instruction_ };
  }

  // Restore internal state. Must be called with previously saved state if
  // GetNextInstruction() has returned VCD_INSTRUCTION_END_OF_DATA or there
  // was not enough data to process the instruction.
  void Restore(SavedState state) {
    SetPosition(state.position);
    pending_second_instruction_ = state.pending_second_instruction;
  }
  ...
  // In the first of these cases, increments UnparsedData()
 ...
 private:
  ...
  // when the VCDiffCodeReader is destroyed.  This will keep the
  ...
  VCDiffCodeReader(const VCDiffCodeReader&);
  void operator=(const VCDiffCodeReader&);
};

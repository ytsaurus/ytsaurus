...
CodeTableWriterInterface* create_writer(
  ...
  if (format_extensions & VCD_FORMAT_JSON) {
    ...
    return new VCDiffCodeTableWriter(
        ...
        (format_extensions & VCD_FORMAT_SDCH3DM) != 0);
  }
}
...
const char* HashedDictionary::data() const { return engine_->dictionary(); }

size_t HashedDictionary::size() const { return engine_->dictionary_size(); }
...
inline bool VCDiffStreamingEncoderImpl::StartEncoding(
  ...
  if (!coder_->Init(engine_->dictionary(), engine_->dictionary_size())) {
...
inline bool VCDiffStreamingEncoderImpl::FinishEncoding(
  ...
  encode_chunk_allowed_ = false;
  return coder_->FinishEncoding(out);
}

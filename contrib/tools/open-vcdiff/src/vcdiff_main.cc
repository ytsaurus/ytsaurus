...
DEFINE_bool(sdch3dm, false, "Sdch3Dm mode: embed dictionary into patch; "
                            "Restore dictionary from patch");
...
DEFINE_uint64(repeat, 1, "Repeat N times (for performance testing)");
DEFINE_uint64(compress_opt, open_vcdiff::COMP_DEFAULT,
              "Encoder compression option (0..7)");
...
class VCDiffFileBasedCoder {
 ...
 private:
  ...
  std::string dictionary_sdch3dm_;
  ...
  // In performance-test "repeat" mode, blocks writing output (except for the
  // last iteration).
  bool block_output_ = false;
...
bool VCDiffFileBasedCoder::WriteSdch3DmDictionary() {
  if (block_output_) {
    return true;
  }
  assert(!FLAGS_dictionary.empty());
  FILE* dictionary_file = fopen(FLAGS_dictionary.c_str(), "wbx");
  if (!dictionary_file) {
    std::cerr << "Error opening dictionary file '" << FLAGS_dictionary
              << "': " << strerror(errno) << std::endl;
    return false;
  }
  if (fwrite(&dictionary_sdch3dm_[0], 1, dictionary_sdch3dm_.size(),
        dictionary_file) != dictionary_sdch3dm_.size()) {
    std::cerr << "Unable to write dictionary file '" << FLAGS_dictionary
              << "': " << strerror(errno) << std::endl;
    fclose(dictionary_file);
    return false;
  }
...
bool VCDiffFileBasedCoder::OpenFileForReading(const string& file_name,
                                              ...
                                              std::vector<char>* buffer) {
  assert(buffer->empty());
  size_t buffer_size = static_cast<size_t>(FLAGS_buffersize);
  if (!*file && file_name.empty()) {
    ...
    *file = stdin;
    if (FLAGS_repeat > 1) {
      std::cerr << "Error: using stdin for performance testing is not supported"
                << std::endl;
      return false;
    }
  } else {
    ...
    if (file_size < buffer_size || FLAGS_repeat > 1) {
...
bool VCDiffFileBasedCoder::ReadInput(size_t* bytes_read) {
  if (block_input_ || !input_buffer_.size()) {
    *bytes_read = input_buffer_.size();
    return true;
  }
...
bool VCDiffFileBasedCoder::Encode() {
  ...
  auto compress_opt = open_vcdiff::CompressionOptions(FLAGS_compress_opt);
  ...
  if (FLAGS_json) {
    ...
    writer.reset(new VCDiffCodeTableWriter(FLAGS_interleaved, FLAGS_sdch3dm));
  }
  if (FLAGS_sdch3dm) {
      format_flags |= open_vcdiff::VCD_FORMAT_SDCH3DM;
  }
  ...
  for (size_t rep = 0; rep < FLAGS_repeat; ++rep) {
    input_size = output_size = 0;
    block_output_ = rep + 1 != FLAGS_repeat;
    block_input_ = rep != 0;
    ...
    do {
      ...
      output.clear();
      if (!bytes_read) {  // this happens only when the file is empty
        break;
      }
      input_size += bytes_read;
      size_t in_offset = 0;
      while (bytes_read) {  // we've read entire file in repeat mode
        size_t to_read = std::min<size_t>(bytes_read, FLAGS_buffersize);
        if (!encoder.EncodeChunk(&input_buffer_[in_offset], to_read, &output)) {
...
bool VCDiffFileBasedCoder::Decode() {
  ...
  // With sdch3dm, we will write dictionary instead
  if (!FLAGS_sdch3dm && !OpenDictionary()) {
    return false;
  }
  if (!OpenInputFile() || !OpenOutputFile()) {
    return false;
  }
  ...
  for (size_t rep = 0; rep < FLAGS_repeat; ++rep) {
    ...
    block_input_ = rep != 0;
    if (FLAGS_sdch3dm) {
      decoder.StartSdch3DmDecoding(&dictionary_sdch3dm_, 0);
    } else {
  ...
  if (FLAGS_sdch3dm && !WriteSdch3DmDictionary()) {
    return false;
  }
  return true;
}
...
bool VCDiffFileBasedCoder::DecodeAndCompare() {
  ...
  if (FLAGS_sdch3dm) {
    if (dictionary_ != dictionary_sdch3dm_) {
      std::cerr << "Decoded sdch3dm dictionary does not match the original"
                << std::endl;
      return false;
    }
  }

...
template<class B> class BlockHashTest;
...
// BlockHashBase is meant to collect as much kBlockSize-independent code as
// possible, in order to reduce the code size of multiple instantiations of
// BlockHash<> with different kBlockSize.
//
class BlockHashBase {
 public:
  // We have 32-bit arithmetic in the encoder, and possibly in decoder.
  static const size_t kMaxDictSize = 0x7FFFFFFF;
  ...
  // source data. For performance considerations, kBlockSize is specialized
  // at compile time for the BlockHash<> derived class.
  //
  ...
  //
  bool Init(const char* source_data, size_t source_size,
            bool populate_hash_table);

  bool IsInitialized() const { return !hash_table_.empty(); }
 ...
 protected:
  ...
  //
  BlockHashBase(int starting_offset, size_t block_size);

  virtual ~BlockHashBase();
  ...
  size_t GetNumberOfBlocks() const {
    return source_size_ / block_size_;
  }

  // Called from Init(). See description in BlockHash<>
  virtual void AddAllBlocks() = 0;
  ...
  // An array containing one element for each source block.  Each element's
  // blk, diff_str_blk and last_block data members are either -1 (== not found)
  // or the index of the next block whose hash value would produce a matching
  // result from GetHashTableIndex(). For each active entry in hash_table_,
  // this effectively organizes several stripes of block numbers, blocks in
  // each stripe (b1, b2... and a1, a2... on the scheme) have identical
  // underlying text.
  // |stripe_size| is used to prevent overly large stripes from forming.
  // |last_block| is used to track the end of the L1 list of the stripe to
  // provide efficient addition to the end. Smaller block numbers get added
  // earlier, and we want them still to be found first, because the encoder
  // likes smaller offsets.
  //           -1
  //           ^^
  //           || ([b1].diff_str_blk = -1)
  //           ||
  //  next_block_table_[b1] (.blk = b2) -> next_block_table_[b2] (.blk = -1) -> -1
  //           ^^
  //           || ([a1].diff_str_blk = b1)
  //           ||
  //  next_block_table_[a1] (.blk = a2) -> next_block_table_[a2] ...
  //           ^^
  //           ||
  //  hash_table_[hash_value] = a1
  struct NextBlock {
    int blk = -1;
    union {
      int diff_str_blk = -1;  // only first block in the stripe has it
      uint32_t stripe_size;   // only second block in the stripe has it
      int last_block;         // only third block in the stripe has it
    };
  };
  std::vector<NextBlock> next_block_table_;
  ...
  size_t next_block_to_add_;

  // Block size of the derived class
  const size_t block_size_;

 private:
  // Making these private avoids implicit copy constructor & assignment operator
  BlockHashBase(const BlockHashBase&);  // NOLINT
  void operator=(const BlockHashBase&);
};

template<size_t block_size>
class BlockHash : public BlockHashBase {
 public:
  ...
  //
  // However if vcdiff output is later compressed, increasing kBlockSize
  // may improve overall compression ratio.
  //
  ...
  static const size_t kBlockSize = block_size;

  BlockHash(int starting_offset) : BlockHashBase(starting_offset, kBlockSize) {}
  ...
  // [next_block_to_add_ * kBlockSize, end_index), exclusive of the endpoint.
  // If end_index < the next index to add (next_block_to_add_ * kBlockSize),
  ...
  //
  // Example: assume kBlockSize = 4, next_block_to_add_ = 2, and there are
  ...
  //
  Match FindBestMatch(uint32_t hash_value,
                     const char* target_candidate_start,
                     const char* target_start,
                     size_t target_size) const {
    ptrdiff_t block_number = hash_table_[GetHashTableIndex(hash_value)];
    if (block_number >= 0) {
      block_number = SkipNonMatchingBlocks(block_number,
                                           target_candidate_start);
      if (block_number >= 0) {
        return FindBestMatchImpl(block_number, target_candidate_start,
 ...
 protected:
  ...
  size_t NextIndexToAdd() const {
    return next_block_to_add_ * kBlockSize;
  }
  ...
  // where block_number is always next_block_to_add_.  That is,
  ...
  static bool BlockContentsMatch(const char* block1, const char* block2) {
    return memcmp(block1, block2, kBlockSize) == 0;
  }
  ...
  // the next block number that matches the same hash value and data.
  // Returns -1 if no match was found.
  int NextMatchingBlock(int block_number) const;
  ...
  ptrdiff_t SkipNonMatchingBlocks(ptrdiff_t block_number, const char* block_ptr) const {
    while ((block_number >= 0) &&
           !BlockContentsMatch(block_ptr,
                               &source_data_[block_number * kBlockSize])) {
      block_number = next_block_table_[block_number].diff_str_blk;
    }
    return block_number;
  }
  ...
  // Actual implementation for FindBestMatch()
  [[gnu::hot, gnu::pure]]
  Match FindBestMatchImpl(ptrdiff_t block_number,
                          const char* target_candidate_start,
                          const char* target_start,
                          size_t target_size) const;
  ...
  friend class BlockHashTest<BlockHash<block_size>>;
};

...
BRO_ARGS = [yatest_common.binary_path('contrib/libs/brotli/python/bro/bro')]
...
TESTDATA_DIR = os.path.join(os.getcwd(), 'testdata')
...
def get_temp_compressed_name(filename):
    return os.path.join(yatest_common.output_path(), os.path.basename(filename + '.bro'))


def get_temp_uncompressed_name(filename):
    return os.path.join(yatest_common.output_path(), os.path.basename(filename + '.unbro'))

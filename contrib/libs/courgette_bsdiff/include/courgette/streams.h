...
class SourceStream {
 public:
  ...
  void Init(TStringBuf strbuf) { Init(strbuf.data(), strbuf.size()); }
  ...
  // the last use of the stream.
  void Init(const TString& string) { Init(string.c_str(), string.size()); }
...
class SinkStream {
 public:
  ...
  bool Write(const void* data, size_t byte_count) Y_WARN_UNUSED_RESULT;

  // Appends the 'varint32' encoding of |value| to the stream.
  bool WriteVarint32(uint32_t value) Y_WARN_UNUSED_RESULT;

  // Appends the 'varint32' encoding of |value| to the stream.
  bool WriteVarint32Signed(int32_t value) Y_WARN_UNUSED_RESULT;

  // Appends the 'varint32' encoding of |value| to the stream.
  // On platforms where sizeof(size_t) != sizeof(int32_t), do a safety check.
  bool WriteSizeVarint32(size_t value) Y_WARN_UNUSED_RESULT;

  // Contents of |other| are appended to |this| stream.  The |other| stream
  // becomes retired.
  bool Append(SinkStream* other) Y_WARN_UNUSED_RESULT;
  ...
  bool Reserve(size_t length) Y_WARN_UNUSED_RESULT {
    buffer_.Reserve(length + buffer_.size());
    return true;
  }
...
class SinkStreamSet {
 public:
  ...
  bool CopyTo(SinkStream* combined_stream) Y_WARN_UNUSED_RESULT;
  ...
  bool WriteSet(SinkStreamSet* set) Y_WARN_UNUSED_RESULT;

 private:
  bool CopyHeaderTo(SinkStream* stream) Y_WARN_UNUSED_RESULT;

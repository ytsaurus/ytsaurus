...
bool UncheckedMalloc(SizeT size, void** ptr) {
    *ptr = malloc(size);
    return *ptr != nullptr;
}
...
T* UncheckedNew(ArgTypes... args) {
  void* ram = nullptr;
  return UncheckedMalloc(sizeof(T), &ram) ? new (ram) T(args...)
                                                : nullptr;
}
 ...
 public:
  ...
  pointer allocate(size_type count) {
    ...
    if (count < kMaxHeapAllocationSize &&
        UncheckedMalloc(bytes, reinterpret_cast<void**>(&mem))) {
...
class MemoryAllocator {
 public:
  ...
  pointer allocate(size_type count) {
    ...
    return UncheckedMalloc(count * sizeof(T),
...
class NoThrowBuffer {
 public:
  ...
  CheckBool reserve(size_t size) Y_WARN_UNUSED_RESULT {
    if (failed())
      return false;
  ...
  CheckBool append(const T* data, size_t size) Y_WARN_UNUSED_RESULT {
    if (failed())
      return false;
  ...
  CheckBool resize(size_t size, const T& init_value) Y_WARN_UNUSED_RESULT {
    if (size > size_) {
      if (!reserve(size))
        return false;
  ...
  CheckBool push_back(const T& item) Y_WARN_UNUSED_RESULT {

...
class RandenPoolEntry {
 public:
  ...
  void Init(y_absl::Span<const uint32_t> data) {
  ...
  // Copy bytes into out.
  void Fill(uint8_t* out, size_t bytes) Y_ABSL_LOCKS_EXCLUDED(mu_);

  // Returns random bits from the buffer in units of T.
  template <typename T>
  inline T Generate() Y_ABSL_LOCKS_EXCLUDED(mu_);

  inline void MaybeRefill() Y_ABSL_EXCLUSIVE_LOCKS_REQUIRED(mu_) {
    if (next_ >= kState) {
      next_ = kCapacity;
 ...
 private:
  // Randen URBG state.
  uint32_t state_[kState] Y_ABSL_GUARDED_BY(mu_);  // First to satisfy alignment.
  SpinLock mu_;
  const Randen impl_;
  size_t next_ Y_ABSL_GUARDED_BY(mu_);
};
...
Y_ABSL_CACHELINE_ALIGNED static RandenPoolEntry* shared_pools[kPoolSize];
...
Y_ABSL_ATTRIBUTE_NOINLINE size_t GetPoolID() {
  ...
  static pthread_key_t tid_key = [] {
    ...
    if (err) {
      Y_ABSL_RAW_LOG(FATAL, "pthread_key_create failed with %d", err);
    }
    return tmp_key;
  }();
  ...
  if (Y_ABSL_PREDICT_FALSE(my_pool_id == 0)) {
    ...
    if (err) {
      Y_ABSL_RAW_LOG(FATAL, "pthread_setspecific failed with %d", err);
    }
  }
  return my_pool_id - 1;
#endif
...
RandenPoolEntry* PoolAlignedAlloc() {
  constexpr size_t kAlignment =
      Y_ABSL_CACHELINE_SIZE > 32 ? Y_ABSL_CACHELINE_SIZE : 32;
...
RandenPoolEntry* GetPoolForCurrentThread() {
  y_absl::call_once(pool_once, InitPoolURBG);
...
template <typename T>
void RandenPool<T>::Fill(y_absl::Span<result_type> data) {
  auto* pool = GetPoolForCurrentThread();

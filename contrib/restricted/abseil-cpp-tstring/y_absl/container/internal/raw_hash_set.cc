...
size_t SingleGroupTableH1(size_t hash, ctrl_t* control) {
  return static_cast<size_t>(y_absl::popcount(
...
void HashSetResizeHelper::GrowIntoSingleGroupShuffleControlBytes(
  ...
  Y_ABSL_ASSUME(old_capacity_ < kHalfWidth);
  Y_ABSL_ASSUME(old_capacity_ > 0);
  ...
  uint64_t copied_bytes =
      y_absl::little_endian::Load64(old_ctrl() + old_capacity_);
  ...
  if (Group::kWidth == 8) {
    ...
    y_absl::little_endian::Store64(new_ctrl + new_capacity, copied_bytes);
...
void HashTableSizeOverflow() {
  Y_ABSL_RAW_LOG(FATAL, "Hash table size overflow");
}

...
#include <contrib/ydb/core/tx/columnshard/overload_manager/overload_subscribers.h>
...
class TOverloadManager: public NActors::TActor<TOverloadManager> {
    ...
    STRICT_STFUNC(StateMain, hFunc(NOverload::TEvOverloadSubscribe, Handle) hFunc(NOverload::TEvOverloadUnsubscribe, Handle)
                                 hFunc(NOverload::TEvOverloadPipeServerDisconnected, Handle)
                                     hFunc(NOverload::TEvOverloadResourcesReleased, Handle) hFunc(NOverload::TEvOverloadColumnShardDied, Handle))

...
#include <contrib/ydb/core/tx/columnshard/bg_tasks/abstract/adapter.h>
...
protected:
    virtual bool DoLoadSessionsFromLocalDatabase(
        NTabletFlatExecutor::TTransactionContext& txc, std::deque<NKikimr::NOlap::NBackground::TSessionRecord>& records) override {
    ...
    virtual void DoSaveProgressToLocalDatabase(
        NTabletFlatExecutor::TTransactionContext& txc, const NKikimr::NOlap::NBackground::TSessionRecord& container) override {
        NIceDb::TNiceDb db(txc.DB);
        using BackgroundSessions = typename Schema::BackgroundSessions;
    ...
    virtual void DoSaveSessionToLocalDatabase(
        ...
        db.Table<BackgroundSessions>()
            .Key(container.GetClassName(), container.GetIdentifier())
            .Update(NIceDb::TUpdate<typename BackgroundSessions::LogicDescription>(container.GetLogicDescription()),
    ...
    virtual void DoRemoveSessionFromLocalDatabase(
        NTabletFlatExecutor::TTransactionContext& txc, const TString& className, const TString& identifier) override {
        NIceDb::TNiceDb db(txc.DB);
        using BackgroundSessions = typename Schema::BackgroundSessions;

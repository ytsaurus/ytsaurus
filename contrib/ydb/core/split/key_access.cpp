...
TSerializedCellVec FindSplitKeyPrefix(const TKeyAccessHistogram& keysHist, const TKeyTypes& keyColumnTypes, const size_t prefixSize) {
    ...
    // Find the position of total/2, total/10, total*9/10.
    // None of these iterators can be end(): the last element has value == total,
    // which is strictly greater than total*0.9, total*0.5, and total*0.1 (for total > 0).
    // total > 0 is guaranteed because every sampled key has weight >= 1.

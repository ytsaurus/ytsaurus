...
ASTDIFF_PATH = yql_utils.yql_binary_path(os.getenv('YQL_ASTDIFF_PATH') or 'contrib/ydb/library/yql/tools/astdiff/astdiff')
...
def test(case):
    ...
    yqlrun_res = YQLRun(udfs_dir=udfs_dir, prov='yt', use_sql2yql=False, cfg_dir=os.getenv('YQL_CONFIG_DIR') or 'contrib/ydb/library/yql/cfg/udf_test').yql_exec(
        program=program,
        run_sql=True,
        tables=in_tables,
        files=files,

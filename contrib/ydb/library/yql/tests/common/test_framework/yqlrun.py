...
class YQLRun(object):
    ...
    def __init__(self, udfs_dir=None, prov='yt', use_sql2yql=False, keep_temp=True, binary=None, gateway_config=None, fs_config=None, extra_args=[], cfg_dir=None, support_udfs=True):
        ...
        try:
            self.sql2yql_binary = yql_utils.yql_binary_path(os.getenv('YQL_SQL2YQL_PATH') or 'contrib/ydb/library/yql/tools/sql2yql/sql2yql')
        except BaseException:
            self.sql2yql_binary = None

        try:
            self.udf_resolver_binary = yql_utils.yql_binary_path(os.getenv('YQL_UDFRESOLVER_PATH') or 'contrib/ydb/library/yql/tools/udf_resolver/udf_resolver')
        except Exception:
        ...
        if cfg_dir is None:
            cfg_dir = 'contrib/ydb/library/yql/cfg/tests'

...
def test_reschedule(job):
    ...
    job._scheduler.reschedule_job.assert_called_once_with(
        job.id, None, "trigger", bah=1, foo="x"
    )
...
def test_private_modify_bad_func(job):
    ...
    with pytest.raises(
        TypeError, match="func must be a callable or a textual reference to one"
    ):
        job._modify(func=1)
...
def test_private_modify_func_ref(job):
    ...
    job._modify(func="__tests__.test_job:dummyfunc")
    assert job.func is dummyfunc
    assert job.func_ref == "__tests__.test_job:dummyfunc"
...
def test_private_modify_bad_misfire_grace_time(job, value):
    ...
    assert (
        str(exc.value) == "misfire_grace_time must be either None or a positive integer"
    )


@pytest.mark.parametrize(
    "value", [None, "foo", 0, -1], ids=["None", "string", "zero", "negative"]
)
...
def test_private_modify_bad_argument(job):
    ...
    assert (
        str(exc.value)
        == "The following are not modifiable attributes of Job: scheduler"
    )
...
def test_getstate(job):
    ...
    assert state == dict(
        ...
        trigger=job.trigger,
        executor="default",
        func="__tests__.test_job:dummyfunc",

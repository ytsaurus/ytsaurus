...
namespace numpy {
    ...
    static void set_error_check_fpe(const char *func_name) {
...
// See https://github.com/numpy/numpy/blob/main/numpy/_core/include/numpy/npy_common.h
    ...
    static PyObject *ufunc(ufunc_overloads func, int nout, const char *name, const char *doc) {
    ...
    static PyObject *ufunc(ufunc_overloads overloads, const char *name, const char *doc) {
        int nout = overloads.has_return();

        return ufunc(std::move(overloads), nout, name, doc);
    }

    static PyObject *gufunc(

...
public:
    ...
    explicit LDAPAccessStorage(const String & storage_name_, AccessControl & access_control_, const DBPoco::Util::AbstractConfiguration & config, const String & prefix);
...
private: // IAccessStorage implementations.
    ...
    std::optional<AuthResult> authenticateImpl(const Credentials & credentials, const DBPoco::Net::IPAddress & address, const ExternalAuthenticators & external_authenticators, bool throw_if_user_not_exists, bool allow_no_password, bool allow_plaintext_password) const override;

    void setConfiguration(const DBPoco::Util::AbstractConfiguration & config, const String & prefix);

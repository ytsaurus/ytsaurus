...
public:
    ...
    private:
        ...
        Entry(HostResolver & pool_, DBPoco::Net::IPAddress address_)
...
protected:
    explicit HostResolver(
        String host_,
        DBPoco::Timespan history_ = DBPoco::Timespan(DEFAULT_RESOLVE_TIME_HISTORY_SECONDS, 0));

    using ResolveFunction = std::function<std::vector<DBPoco::Net::IPAddress> (const String & host)>;
    HostResolver(ResolveFunction && resolve_function_,
                    String host_,
                    DBPoco::Timespan history_);

    friend class Entry;
    WeakPtr getWeakFromThis();

    void setSuccess(const DBPoco::Net::IPAddress & address);
    void setFail(const DBPoco::Net::IPAddress & address);

    struct Record
    {
        Record(DBPoco::Net::IPAddress address_, DBPoco::Timestamp resolve_time_)
            : address(address_)
        ...
        DBPoco::Net::IPAddress address;
        DBPoco::Timestamp resolve_time;
        size_t usage = 0;
        bool failed = false;
        DBPoco::Timestamp fail_time = 0;
        ...
        bool setFail(const DBPoco::Timestamp & now)
        {
            bool was_ok = !failed;
    ...
    DBPoco::Net::IPAddress selectBest() TSA_REQUIRES(mutex);
    Records::iterator find(const DBPoco::Net::IPAddress & address) TSA_REQUIRES(mutex);
    bool isUpdateNeeded();

    void updateImpl(DBPoco::Timestamp now, std::vector<DBPoco::Net::IPAddress> & next_gen) TSA_REQUIRES(mutex);
    ...
    DBPoco::Timespan getRecordHistoryTime(const Record&) const;

    const String host;
    const DBPoco::Timespan history;
    ...
    DBPoco::Timestamp last_resolve_time TSA_GUARDED_BY(mutex) = DBPoco::Timestamp::TIMEVAL_MIN;
    Records records TSA_GUARDED_BY(mutex);

    DBPoco::Logger * log = &DBPoco::Logger::get("ConnectionPool");
};

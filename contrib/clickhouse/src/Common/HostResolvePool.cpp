...
HostResolver::HostResolver(String host_, DBPoco::Timespan history_)
    : host(std::move(host_))
    , history(history_)
...
HostResolver::HostResolver(
    ResolveFunction && resolve_function_, String host_, DBPoco::Timespan history_)
...
void HostResolver::setSuccess(const DBPoco::Net::IPAddress & address)
{
    std::lock_guard lock(mutex);
...
void HostResolver::setFail(const DBPoco::Net::IPAddress & address)
{
    DBPoco::Timestamp now;
...
DBPoco::Net::IPAddress HostResolver::selectBest()
{
...
HostResolver::Records::iterator HostResolver::find(const DBPoco::Net::IPAddress & addr) TSA_REQUIRES(mutex)
{
    auto it = std::lower_bound(
        records.begin(), records.end(), addr, [](const Record & rec, const DBPoco::Net::IPAddress & value) { return rec.address < value; });
...
void HostResolver::updateImpl(DBPoco::Timestamp now, std::vector<DBPoco::Net::IPAddress> & next_gen)
    TSA_REQUIRES(mutex)
{
    ...
    for (auto & record : merged)
    {
        ...
        auto banned_until = now - DBPoco::Timespan(history.totalMicroseconds() * (1ull << (record.consecutive_fail_count - 1)));

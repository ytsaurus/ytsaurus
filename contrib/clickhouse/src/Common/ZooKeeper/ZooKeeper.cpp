...
ZooKeeper::ZooKeeper(const DBPoco::Util::AbstractConfiguration & config, const std::string & config_name, std::shared_ptr<DB::ZooKeeperLog> zk_log_)
    : zk_log(std::move(zk_log_))
{
...
bool ZooKeeper::configChanged(const DBPoco::Util::AbstractConfiguration & config, const std::string & config_name) const
{
...
{
    zkutil::EventPtr eph_node_disappeared = std::make_shared<DBPoco::Event>();
    String content;
    Coordination::Stat stat;
...
ZooKeeperPtr ZooKeeper::create(const DBPoco::Util::AbstractConfiguration & config, const std::string & config_name, std::shared_ptr<DB::ZooKeeperLog> zk_log_)
{

...
protected:
    ...
    String getAliasToOrName(const String & name) const
    {
        ...
        else if (String name_lowercase = DBPoco::toLower(name); case_insensitive_aliases.contains(name_lowercase))
...
public:
    ...
    const String & aliasTo(const String & name) const
    {
        if (auto it = aliases.find(name); it != aliases.end())
            return it->second;
        else if (auto jt = case_insensitive_aliases.find(DBPoco::toLower(name)); jt != case_insensitive_aliases.end())
            return jt->second;

    ...
    public:
        ...
        void flushRequest() override
        {
            if (bool(request_stream))
            {
                ...
                if (auto * fixed_steam = dynamic_cast<DBPoco::Net::HTTPFixedLengthOutputStream *>(request_stream))
                {
                    request_stream_completed = fixed_steam->isComplete();
                }
                else if (auto * chunked_steam = dynamic_cast<DBPoco::Net::HTTPChunkedOutputStream *>(request_stream))
                {
                    chunked_steam->rdbuf()->close();
                    request_stream_completed = chunked_steam->isComplete();
                }
                else if (auto * http_stream = dynamic_cast<DBPoco::Net::HTTPOutputStream *>(request_stream))
                {
        ...
        std::ostream & sendRequest(DBPoco::Net::HTTPRequest & request) override
        {
            auto idle = idleTime();
        ...
        std::istream & receiveResponse(DBPoco::Net::HTTPResponse & response) override
        {
        ...
        ~PooledConnection() override
        {
            ...
            {
                if (auto * fixed_steam = dynamic_cast<DBPoco::Net::HTTPFixedLengthInputStream *>(response_stream))
                {
                    response_stream_completed = fixed_steam->isComplete();
                }
                else if (auto * chunked_steam = dynamic_cast<DBPoco::Net::HTTPChunkedInputStream *>(response_stream))
                {
                    response_stream_completed = chunked_steam->isComplete();
                }
                else if (auto * http_stream = dynamic_cast<DBPoco::Net::HTTPInputStream *>(response_stream))
                {
...
createConnectionPool(ConnectionGroup::Ptr group, std::string host, UInt16 port, bool secure, ProxyConfiguration proxy_configuration)
{
    ...
    {
#if USE_SSL
        return EndpointConnectionPool<DBPoco::Net::HTTPSClientSession>::create(
    ...
    {
        return EndpointConnectionPool<DBPoco::Net::HTTPClientSession>::create(
            group, std::move(host), port, secure, std::move(proxy_configuration));
    }
}
...
private:
    ...
    const DBPoco::Timespan wipe_timeout = DBPoco::Timespan(DEFAULT_WIPE_TIMEOUT_SECONDS, 0);
    ...
    DBPoco::Timestamp last_wipe_time TSA_GUARDED_BY(mutex);

public:
    IHTTPConnectionPoolForEndpoint::Ptr getPool(HTTPConnectionGroupType type, const DBPoco::URI & uri, const ProxyConfiguration & proxy_configuration)
    {
        DBPoco::Timestamp now;
...
protected:
    ...
    IExtendedPool::Ptr getPoolImpl(HTTPConnectionGroupType type, const DBPoco::URI & uri, const ProxyConfiguration & proxy_configuration)
        TSA_REQUIRES(mutex)
    {
    ...
    static bool useSecureConnection(const DBPoco::URI & uri, const ProxyConfiguration & proxy_configuration)
    {
    ...
    static std::tuple<std::string, UInt16, bool> getHostPortSecure(const DBPoco::URI & uri, const ProxyConfiguration & proxy_configuration)
    {
...
HTTPConnectionPools::getPool(HTTPConnectionGroupType type, const DBPoco::URI & uri, const ProxyConfiguration & proxy_configuration)
{

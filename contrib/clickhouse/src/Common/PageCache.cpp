    ...
    , rng(randomSeed())
{
    ...
    if (use_huge_pages)
    {
        ...
        try
        {
            ...
            {
                LOG_WARNING(&DBPoco::Logger::get("PageCache"), "The OS huge page size is too large for our purposes: {} KiB. Using regular pages. Userspace page cache will be relatively slow.", huge_page_size);
            }
        }
        catch (Exception & e)
        {
...
#endif
        if (print_warning)
            LOG_WARNING(&DBPoco::Logger::get("PageCache"), "The OS doesn't support transparent huge pages. Userspace page cache will be relatively slow.");
    }
...
static void logUnexpectedSyscallError(std::string name)
{
    ...
    LOG_WARNING(&DBPoco::Logger::get("PageCache"), "{}", message);
...
PageCache::Mmap::Mmap(size_t bytes_per_page_, size_t pages_per_chunk_, size_t pages_per_big_page_, size_t num_chunks_, void * address_hint, bool use_huge_pages_)
{
    ...
    if (use_huge_pages_)
    {
        ...
        {
            LOG_DEBUG(&DBPoco::Logger::get("PageCache"), "mmap() returned address not aligned on huge page boundary.");
        ...
        if (madvise(ptr, size, MADV_HUGEPAGE) != 0)
            LOG_WARNING(&DBPoco::Logger::get("PageCache"),

...
inline void checkNoOldLeaders(LoggerPtr log, ZooKeeper & zookeeper, const String path)
{
    ...
    while (num_tries--)
    {
        ...
        else
        {
            ...
            if (!identifier.ends_with(suffix))
                throw DBPoco::Exception(fmt::format("Found leader replica ({}) with too old version (< 20.6). Stop it before upgrading", identifier));
    ...
    throw DBPoco::Exception("Cannot check that no old leaders exist");
}

...
DataTypePtr getFieldType(const DBPoco::JSON::Object::Ptr & field, const String & type_key, bool required);
...
std::pair<NamesAndTypesList, Int32> parseTableSchema(const DBPoco::JSON::Object::Ptr & metadata_object, int format_version, bool ignore_schema_evolution)
{
    DBPoco::JSON::Object::Ptr schema;
    ...
    if (format_version == 2)
    {
        ...
        auto schemas = metadata_object->get("schemas").extract<DBPoco::JSON::Array::Ptr>();
        if (schemas->size() == 0)
    ...
    else
    {
        ...
        if (!ignore_schema_evolution && metadata_object->has("schemas") && metadata_object->get("schemas").extract<DBPoco::JSON::Array::Ptr>()->size() > 1)
            throw Exception(ErrorCodes::UNSUPPORTED_METHOD,
    ...
    auto fields = schema->get("fields").extract<DBPoco::JSON::Array::Ptr>();
    for (size_t i = 0; i != fields->size(); ++i)
    {
    ...
    ContextPtr local_context)
{
    ...
    DBPoco::JSON::Parser parser; /// For some reason base/base/JSON.h can not parse this json file
    DBPoco::Dynamic::Var json = parser.parse(json_str);
    DBPoco::JSON::Object::Ptr object = json.extract<DBPoco::JSON::Object::Ptr>();
    ...
    auto snapshots = object->get("snapshots").extract<DBPoco::JSON::Array::Ptr>();
...
Strings IcebergMetadata::getDataFiles() const
{
    ...
    for (const auto & manifest_file : manifest_files)
    {
        ...
        // DBPoco::Dynamic::Var json = parser.parse(schema_json_string);
        // DBPoco::JSON::Object::Ptr schema_object = json.extract<DBPoco::JSON::Object::Ptr>();

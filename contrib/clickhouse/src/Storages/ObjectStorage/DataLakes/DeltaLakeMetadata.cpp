...
struct DeltaLakeMetadataImpl
{
        ...
        std::set<String> & result)
    {
        ...
        while (!buf->eof())
        {
            ...
            {
                auto path = object->get("remove").extract<DBPoco::JSON::Object::Ptr>()->getValue<String>("path");
                result.erase(fs::path(configuration->getPath()) / path);
            }
        }
    }

    NamesAndTypesList parseMetadata(const DBPoco::JSON::Object::Ptr & metadata_json)
    {
        NamesAndTypesList schema;
    ...
    DataTypePtr getFieldType(const DBPoco::JSON::Object::Ptr & field, const String & type_key, bool is_nullable)
    {

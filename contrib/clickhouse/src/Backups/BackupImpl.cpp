...
void BackupImpl::readBackupMetadata()
{
    ...
    DBPoco::AutoPtr<DBPoco::XML::Document> config = dom_parser.parseMemory(str.data(), str.size());
    const DBPoco::XML::Node * config_root = getRootNode(config);
    ...
    for (const DBPoco::XML::Node * child = contents->firstChild(); child; child = child->nextSibling())
    {
        if (child->nodeName() == "file")
        {
            const DBPoco::XML::Node * file_config = child;
            BackupFileInfo info;

...
namespace
{
    ...
    struct IPRecord
    {
        DBPoco::Net::IPAddress addr;
        UInt8 prefix;
        size_t row;
        bool isv6;

        IPRecord(const DBPoco::Net::IPAddress & addr_, UInt8 prefix_, size_t row_)
            : addr(addr_)
            , prefix(prefix_)
            , row(row_)
            , isv6(addr.family() == DBPoco::Net::IPAddress::IPv6)
        {
        }
...
static std::pair<DBPoco::Net::IPAddress, UInt8> parseIPFromString(const std::string_view addr_str)
{
    try
    {
        ...
        {
            DBPoco::Net::IPAddress addr{std::string(addr_str.substr(0, pos))};
            ...
            addr = addr & DBPoco::Net::IPAddress(prefix, addr.family());
            return {addr, prefix};
        }

        DBPoco::Net::IPAddress addr{std::string(addr_str)};
        return {addr, addr.length() * 8};
    }
    catch (DBPoco::Exception & ex)
    {
...
{
    return DBPoco::ByteOrder::fromNetwork(*reinterpret_cast<const UInt32 *>(addr));
}
...
inline static void mapIPv4ToIPv6(UInt32 addr, uint8_t * buf)
{
    ...
    buf[10] = '\xFF';
    buf[11] = '\xFF';
    addr = DBPoco::ByteOrder::toNetwork(addr);
...
void IPAddressDictionary::loadData()
{
    ...
    while (executor.pull(block))
    {
        ...
        for (const auto row : collections::range(0, rows))
        {
            ...
            has_ipv6 = has_ipv6 || (addr.family() == DBPoco::Net::IPAddress::IPv6);

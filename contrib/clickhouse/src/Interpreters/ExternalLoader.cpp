...
private:
    struct FileInfo
    {
        std::optional<DBPoco::Timestamp> last_update_time;
        bool in_use = true; // Whether the `FileInfo` should be destroyed because the correspondent file is deleted.
        DBPoco::AutoPtr<DBPoco::Util::AbstractConfiguration> file_contents; // Parsed contents of the file.
                                 ...
                                 bool check_no_errors) const
{
    ...
    if (result.exception)
    {
        ...
        {
            /// This will create a copy for DBPoco::Exception and DB::Exception
            e.rethrow();
        }
        catch (...)
        {

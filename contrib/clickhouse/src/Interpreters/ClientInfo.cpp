...
void ClientInfo::read(ReadBuffer & in, UInt64 client_protocol_revision)
{
    ...
    initial_address = DBPoco::Net::SocketAddress(initial_address_string);
...
void ClientInfo::setFromHTTPRequest(const DBPoco::Net::HTTPRequest & request)
{
    http_method = ClientInfo::HTTPMethod::UNKNOWN;
    if (request.getMethod() == DBPoco::Net::HTTPRequest::HTTP_GET)
        http_method = ClientInfo::HTTPMethod::GET;
    else if (request.getMethod() == DBPoco::Net::HTTPRequest::HTTP_POST)
    ...
    for (const auto & header : request)
    {
        ...
        String key_lowercase = DBPoco::toLower(header.first);

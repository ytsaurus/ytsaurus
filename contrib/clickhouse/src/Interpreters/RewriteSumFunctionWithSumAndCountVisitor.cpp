...
void RewriteSumFunctionWithSumAndCountMatcher::visit(const ASTFunction & function, ASTPtr & ast, const Data & data)
{
    ...
    if (!function.arguments || DBPoco::toLower(function.name) != "sum" || function.arguments->children.size() != 1)
        return;

    const auto * func_plus_minus = function.arguments->children[0]->as<ASTFunction>();

    if (!func_plus_minus || !function_supported.contains(DBPoco::toLower(func_plus_minus->name)) || func_plus_minus->arguments->children.size() != 2)
        return;

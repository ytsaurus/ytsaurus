...
void PocoHTTPClientConfiguration::updateSchemeAndRegion()
{
    if (!endpointOverride.empty())
    {
        ...
        DBPoco::URI uri(endpointOverride);
...
ConnectionTimeouts getTimeoutsFromConfiguration(const PocoHTTPClientConfiguration & client_configuration)
{
    return ConnectionTimeouts()
        .withConnectionTimeout(DBPoco::Timespan(client_configuration.connectTimeoutMs * 1000))
        .withSendTimeout(DBPoco::Timespan(client_configuration.requestTimeoutMs * 1000))
        .withReceiveTimeout(DBPoco::Timespan(client_configuration.requestTimeoutMs * 1000))
        .withTCPKeepAliveTimeout(DBPoco::Timespan(
            client_configuration.enableTcpKeepAlive ? client_configuration.tcpKeepAliveIntervalMs * 1000 : 0))
        .withHTTPKeepAliveTimeout(DBPoco::Timespan(client_configuration.http_keep_alive_timeout, 0))
...
String getMethod(const Aws::Http::HttpRequest & request)
{
    ...
    {
        case Aws::Http::HttpMethod::HTTP_GET:
            return DBPoco::Net::HTTPRequest::HTTP_GET;
        case Aws::Http::HttpMethod::HTTP_POST:
            return DBPoco::Net::HTTPRequest::HTTP_POST;
        case Aws::Http::HttpMethod::HTTP_DELETE:
            return DBPoco::Net::HTTPRequest::HTTP_DELETE;
        case Aws::Http::HttpMethod::HTTP_PUT:
            return DBPoco::Net::HTTPRequest::HTTP_PUT;
        case Aws::Http::HttpMethod::HTTP_HEAD:
            return DBPoco::Net::HTTPRequest::HTTP_HEAD;
        case Aws::Http::HttpMethod::HTTP_PATCH:
            return DBPoco::Net::HTTPRequest::HTTP_PATCH;
    }
}
    ...
    Aws::Utils::RateLimits::RateLimiterInterface *) const
{
    ...
    try
    {
        ...
        for (unsigned int attempt = 0; attempt <= s3_max_redirects; ++attempt)
        {
            ...
            DBPoco::Net::HTTPRequest poco_request(DBPoco::Net::HTTPRequest::HTTP_1_1);
            ...
            /** According to RFC-2616, Request-URI is allowed to be encoded.
              ...
              * Effectively, `DBPoco::URI` chooses smaller subset of characters to encode,
            ...
            const std::string reserved = "?#:;+@&=%"; /// DBPoco::URI::RESERVED_QUERY_PARAM without '/' plus percent sign.
            DBPoco::URI::encode(target_uri.getPath(), reserved, path_and_query);
            ...
            DBPoco::Net::HTTPResponse poco_response;
            ...
            if (request.GetContentBody())
            {
                ...
                auto size = DBPoco::StreamCopier::copyStream(*request.GetContentBody(), request_body_stream);
                if (enable_s3_requests_logging)
            ...
            if (poco_response.getStatus() == DBPoco::Net::HTTPResponse::HTTP_TEMPORARY_REDIRECT)
            {
                auto location = poco_response.get("location");
                remote_host_filter.checkURL(DBPoco::URI(location));
                uri = location;
                if (enable_s3_requests_logging)

...
{
    DBPoco::URI uri(getAWSMetadataEndpoint() + EC2_AVAILABILITY_ZONE_RESOURCE);
    DBPoco::Net::HTTPClientSession session(uri.getHost(), uri.getPort());
    session.setTimeout(DBPoco::Timespan(AVAILABILITY_ZONE_REQUEST_TIMEOUT_SECONDS, 0));

    DBPoco::Net::HTTPResponse response;
    DBPoco::Net::HTTPRequest request(DBPoco::Net::HTTPRequest::HTTP_GET, uri.getPath());
    ...
    DBPoco::StreamCopier::copyToString(rs, response_data);
    return response_data;
}

String getGCPAvailabilityZoneOrException()
{
    DBPoco::URI uri(String(GCP_METADATA_SERVICE_ENDPOINT) + "/computeMetadata/v1/instance/zone");
    DBPoco::Net::HTTPClientSession session(uri.getHost(), uri.getPort());
    session.setTimeout(DBPoco::Timespan(AVAILABILITY_ZONE_REQUEST_TIMEOUT_SECONDS, 0));

...
struct URIConverter
{
    static void modifyURI(DBPoco::URI & uri, std::unordered_map<std::string, std::string> mapper)
    {
        Macros macros({{"bucket", uri.getHost()}});
        uri = macros.expand(mapper[uri.getScheme()]).empty() ? uri : DBPoco::URI(macros.expand(mapper[uri.getScheme()]) + uri.getPathAndQuery());
    }
};
...
URI::URI(const std::string & uri_, bool allow_archive_path_syntax)
{
    ...
    /// DBPoco::URI will ignore '?' when parsing the path, but if there is a versionId in the http parameter,
    ...
    if (!has_version_id && uri_.find('?') != String::npos)
    {
        ...
        DBPoco::URI::encode(uri_, "?", uri_with_question_mark_encode);
        uri = DBPoco::URI(uri_with_question_mark_encode);
    }
...
void URI::validateBucket(const String & bucket, const DBPoco::URI & uri)
{

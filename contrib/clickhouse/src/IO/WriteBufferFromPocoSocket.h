...
using AsyncCallback = std::function<void(int, DBPoco::Timespan, AsyncEventTimeoutType, const std::string &, uint32_t)>;
...
{
public:
    explicit WriteBufferFromPocoSocket(DBPoco::Net::Socket & socket_, size_t buf_size = DBMS_DEFAULT_BUFFER_SIZE);
    explicit WriteBufferFromPocoSocket(DBPoco::Net::Socket & socket_, const ProfileEvents::Event & write_event_, size_t buf_size = DBMS_DEFAULT_BUFFER_SIZE);

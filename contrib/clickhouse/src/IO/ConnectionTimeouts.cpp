...
DBPoco::Timespan ConnectionTimeouts::saturate(DBPoco::Timespan timespan, DBPoco::Timespan limit)
{
...
ConnectionTimeouts ConnectionTimeouts::getHTTPTimeouts(const Settings & settings, DBPoco::Timespan http_keep_alive_timeout)
{
    return ConnectionTimeouts()
...
private:
    ...
    static constexpr DBPoco::Timestamp::TimeDiff first_byte_ms[known_methods_count][2] =
    {
        /* GET */ {200, 200},
        /* POST */ {200, 200},
        /* DELETE */ {200, 200},
    ...
    static constexpr DBPoco::Timestamp::TimeDiff rest_bytes_ms[known_methods_count][2] =
    {
        /* GET */ {500, 500},
        /* POST */ {1000, 30000},
        /* DELETE */ {1000, 10000},
    ...
    static_assert(sizeof(first_byte_ms) == known_methods_count * sizeof(DBPoco::Timestamp::TimeDiff) * 2);
...
public:
    static std::pair<DBPoco::Timespan, DBPoco::Timespan> getSendReceiveTimeout(const String & method, bool first_byte)
    {
        auto idx = getMethodIndex(method);

        if (first_byte)
            return std::make_pair(
                DBPoco::Timespan(first_byte_ms[idx][0] * 1000),
                DBPoco::Timespan(first_byte_ms[idx][1] * 1000)
            );

        return std::make_pair(
            DBPoco::Timespan(rest_bytes_ms[idx][0] * 1000),
            DBPoco::Timespan(rest_bytes_ms[idx][1] * 1000)
        );
    }
};

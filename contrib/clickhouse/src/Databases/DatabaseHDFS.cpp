    ...
    , log(getLogger("DatabaseHDFS(" + name_ + ")"))
{
    if (!source.empty())
    {
        ...
        context_->getGlobalContext()->getRemoteHostFilter().checkURL(DBPoco::URI(source));
    }
}
...
bool DatabaseHDFS::checkUrl(const std::string & url, ContextPtr context_, bool throw_on_error) const
{
    ...
    {
        checkHDFSURL(url);
        context_->getGlobalContext()->getRemoteHostFilter().checkURL(DBPoco::URI(url));
    }
    catch (...)
    {
        if (throw_on_error)
...
StoragePtr DatabaseHDFS::tryGetTable(const String & name, ContextPtr context_) const
{
    ...
    catch (const DBPoco::URISyntaxException &)
    {
        return nullptr;
    }
}

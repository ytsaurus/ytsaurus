...
// Helper to map _BitInt(8) to int8_t (or int, or any preferred standard integer type)
template <typename T>
constexpr auto FormatArgsIntegerCast(T v) {
                    ...
                    if (!is_valid_input)
                    {
                        ...
                        else
                        {
                            throw Exception(ErrorCodes::CANNOT_CONVERT_TYPE, "Value {} cannot be safely converted into type {}",
                                FormatArgsIntegerCast(vec_from[i]), TypeName<ValueType>);
                        }
                    }
                }
            }

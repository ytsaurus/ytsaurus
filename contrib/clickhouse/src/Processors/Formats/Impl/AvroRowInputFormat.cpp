...
private:
    avro::ValidSchema fetchSchema(uint32_t id)
    {
        try
        {
            try
            {
                DBPoco::URI url(base_url, base_url.getPath() + "/schemas/ids/" + std::to_string(id));
                ...
                DBPoco::Net::HTTPRequest request(DBPoco::Net::HTTPRequest::HTTP_GET, url.getPathAndQuery(), DBPoco::Net::HTTPRequest::HTTP_1_1);
                request.setHost(url.getHost());

                if (!url.getUserInfo().empty())
                {
                    DBPoco::Net::HTTPCredentials http_credentials;
                    DBPoco::Net::HTTPBasicCredentials http_basic_credentials;

                    http_credentials.fromUserInfo(url.getUserInfo());

                    std::string decoded_username;
                    DBPoco::URI::decode(http_credentials.getUsername(), decoded_username);
                    http_basic_credentials.setUsername(decoded_username);

                    if (!http_credentials.getPassword().empty())
                    {
                        std::string decoded_password;
                        DBPoco::URI::decode(http_credentials.getPassword(), decoded_password);
                ...
                auto json_body = parser.parse(*response_body).extract<DBPoco::JSON::Object::Ptr>();

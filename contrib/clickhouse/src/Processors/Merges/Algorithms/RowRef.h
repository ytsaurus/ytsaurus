...
///  does not use DBPoco::SharedPtr, since you need to allocate a block and `refcount` in one piece;
///  does not use DBPoco::AutoPtr, since it does not have a `move` constructor and there are extra checks for nullptr;

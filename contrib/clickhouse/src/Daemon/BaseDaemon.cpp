...
static bool tryCreateDirectories(DBPoco::Logger * logger, const std::string & path)
{
    try
    {
        fs::create_directories(path);
...
{
    if (::raise(SIGTERM) != 0)
        throw DBPoco::SystemException("cannot terminate process");
}
...
void BaseDaemon::initialize(Application & self)
{
    ...
    if (is_daemon)
    {
        ...
        if (0 != chdir(path.c_str()))
            throw DBPoco::Exception("Cannot change directory to " + path);
    }
    ...
    if (config().has("timezone"))
    {
        ...
        if (0 != setenv("TZ", config_timezone.data(), 1)) // NOLINT(concurrency-mt-unsafe) // ok if not called concurrently with other setenv/getenv
            throw DBPoco::Exception("Cannot setenv TZ variable");
    ...
    if ((!log_path.empty() && is_daemon) || config().has("logger.stderr"))
    {
        ...
        if (access(stderr_path.c_str(), W_OK))
        {
            ...
            if ((fd = creat(stderr_path.c_str(), 0600)) == -1 && errno != EEXIST)
                throw DBPoco::OpenFileException("File " + stderr_path + " (logger.stderr) is not writable");
            if (fd != -1)
            {
                int err = ::close(fd);
        ...
        if (!freopen(stderr_path.c_str(), "a+", stderr))
            throw DBPoco::OpenFileException("Cannot attach stderr to " + stderr_path);
    ...
    if ((!log_path.empty() && is_daemon) || config().has("logger.stdout"))
    {
        ...
        if (!freopen(stdout_path.c_str(), "a+", stdout))
            throw DBPoco::OpenFileException("Cannot attach stdout to " + stdout_path);
    }
    ...
    else
    {
        if (is_daemon
            && chdir("/tmp") != 0)
            throw DBPoco::Exception("Cannot change directory to /tmp");
    }
    ...
    if (is_daemon)
    {
        ...
        if (0 != chdir(core_path.c_str()))
            throw DBPoco::Exception("Cannot change directory to " + core_path);
    }
...
void BaseDaemon::initializeTerminationAndSignalProcessing()
{
    ...
    DBPoco::ErrorHandler::set(&killing_error_handler);
...
void BaseDaemon::defineOptions(DBPoco::Util::OptionSet & new_options)
{
    new_options.addOption(
        DBPoco::Util::Option("config-file", "C", "load configuration from a given file")
            .required(false)
            .repeatable(false)
            .argument("<file>")
            .binding("config-file"));
    ...
    new_options.addOption(
        DBPoco::Util::Option("log-file", "L", "use given log file")
            .required(false)
            .repeatable(false)
            .argument("<file>")
            .binding("logger.log"));
    ...
    new_options.addOption(
        DBPoco::Util::Option("errorlog-file", "E", "use given log file for errors only")
            .required(false)
            .repeatable(false)
            .argument("<file>")
    ...
    new_options.addOption(
        DBPoco::Util::Option("pid-file", "P", "use given pidfile")
            .required(false)
            .repeatable(false)
            .argument("<file>")
            .binding("pid"));

    DBPoco::Util::ServerApplication::defineOptions(new_options);
}
...
void BaseDaemon::setupWatchdog()
{
    ...
    while (true)
    {
        ...
        if (0 == pid)
        {
            ...
            {
                notify_sync.close(DBPoco::Pipe::CLOSE_WRITE);
                ...
                if (0 != notify_sync.readBytes(buf, sizeof(buf)))
                    throw DBPoco::Exception("Unexpected result while waiting for watchdog synchronization pipe to close.");
            }
        ...
        {
            DBPoco::AutoPtr<OwnPatternFormatter> pf;
            if (config().getString("logger.formatting.type", "") == "json")
                pf = new OwnJSONPatternFormatter(config());
            else
                pf = new OwnPatternFormatter;
            DBPoco::AutoPtr<OwnFormattingChannel> log = new OwnFormattingChannel(pf, new DBPoco::ConsoleChannel(std::cerr));
        ...
        {
            channel->setChannelProperty("log", DBPoco::FileChannel::PROP_ROTATION, "never");
            channel->setChannelProperty("log", DBPoco::FileChannel::PROP_ROTATEONOPEN, "false");
        }
        ...
        do
        {
            ...
            if (-1 != waitpid(pid, &status, WUNTRACED | WCONTINUED) || errno == ECHILD)
            {
                ...
                throw DBPoco::Exception("Cannot waitpid, errno: " + errnoToString());
        } while (true);

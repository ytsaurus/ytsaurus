...
std::unique_ptr<SeekableReadBuffer> ReadBufferFromWebServer::initialize()
{
    ...
    connection_timeouts.withConnectionTimeout(std::max<DBPoco::Timespan>(settings.http_connection_timeout, DBPoco::Timespan(20, 0)));
    connection_timeouts.withReceiveTimeout(std::max<DBPoco::Timespan>(settings.http_receive_timeout, DBPoco::Timespan(20, 0)));

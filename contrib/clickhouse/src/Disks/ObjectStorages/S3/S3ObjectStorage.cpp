...
private:
    bool getBatchAndCheckNext(RelativePathsWithMetadata & batch) override
    {
        ...
        if (outcome.IsSuccess())
        {
            ...
            {
                ObjectMetadata metadata{static_cast<uint64_t>(object.GetSize()), DBPoco::Timestamp::fromEpochTime(object.GetLastModified().Seconds()), object.GetETag(), {}};
        ...
        for (const auto & object : objects)
            children.emplace_back(std::make_shared<RelativePathWithMetadata>(
                ...
                ObjectMetadata{
                    ...
                    DBPoco::Timestamp::fromEpochTime(object.GetLastModified().Seconds()),
                    object.GetETag(),
                    {}}));
...
std::optional<ObjectMetadata> S3ObjectStorage::tryGetObjectMetadata(const std::string & path) const
{
    ...
    result.last_modified = DBPoco::Timestamp::fromEpochTime(object_info.last_modification_time);

...
#    error #include <azure/storage/common/storage_exception.hpp>
#endif
...
std::optional<DBPoco::Timestamp> MetadataStorageFromPlainRewritableObjectStorage::getLastModifiedIfExists(const String & path) const
{
    /// Path corresponds to a directory.
    if (auto remote = path_map->getRemotePathInfoIfExists(path))
        return DBPoco::Timestamp::fromEpochTime(remote->last_modified);

    /// A file.
    if (auto res = getObjectMetadataEntryWithCache(path))
        return DBPoco::Timestamp::fromEpochTime(res->last_modified);
    return std::nullopt;
}
...
MetadataStorageFromPlainRewritableObjectStorage::getDirectChildrenOnDisk(const std::filesystem::path & local_path) const
{
    ...
    for (auto it = path_map->map.lower_bound(local_path); it != end_it; ++it)
    {
        ...
        auto slash_num = std::count(subdirectory.begin() + local_path.string().size(), subdirectory.end(), '/');

...
public:
    /// Default constructor.
    IDisk(const String & name_, const DBPoco::Util::AbstractConfiguration & config, const String & config_prefix)
        : name(name_)
        , copying_thread_pool(
    ...
    virtual void setLastModified(const String & path, const DBPoco::Timestamp & timestamp) = 0;

    /// Get last modified time of file or directory at `path`.
    virtual DBPoco::Timestamp getLastModified(const String & path) const = 0;
    ...
    virtual void applyNewSettings(const DBPoco::Util::AbstractConfiguration & config, ContextPtr context, const String & config_prefix, const DisksMap & map);

...
void TableFunctionURL::parseArgumentsImpl(ASTs & args, const ContextPtr & context)
{
    if (auto named_collection = tryGetNamedCollectionWithOverrides(args, context))
    {
        ...
        if (format == "auto")
            format = FormatFactory::instance().tryGetFormatFromFileName(DBPoco::URI(filename).getPath()).value_or("auto");
    ...
    if (structure == "auto")
    {
        ...
        if (format == "auto")
            return StorageURL::getTableStructureAndFormatFromData(
                       filename,
                       chooseCompressionMethod(DBPoco::URI(filename).getPath(), compression_method),
                       configuration.headers,
                       std::nullopt,
                       context).first;
...
{
    return FormatFactory::instance().tryGetFormatFromFileName(DBPoco::URI(filename).getPath());
}

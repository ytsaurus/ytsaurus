...
public:
    ...
    void enterImpl(QueryTreeNodePtr & node)
    {
        ...
        if (!function_node || DBPoco::toLower(function_node->getFunctionName()) != "sum")
            return;
        ...
        if (!func_plus_minus_node || !func_supported.contains(DBPoco::toLower(func_plus_minus_node->getFunctionName())))
            return;
        ...
        auto new_node = std::make_shared<FunctionNode>(DBPoco::toLower(func_plus_minus_node->getFunctionName()));
        if (column_id == 0)

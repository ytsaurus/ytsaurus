...
namespace
{
    DBPoco::Timespan::TimeDiff float64AsSecondsToTimespan(Float64 d)
    {
...
#pragma clang diagnostic ignored "-Wimplicit-const-int-float-conversion"
        if (d * 1000000 > std::numeric_limits<DBPoco::Timespan::TimeDiff>::max() || d * 1000000 < std::numeric_limits<DBPoco::Timespan::TimeDiff>::min())
            throw Exception(
                ErrorCodes::BAD_ARGUMENTS, "Cannot convert seconds to microseconds: the setting's value in seconds is too big: {}", d);
#pragma clang diagnostic pop

        return static_cast<DBPoco::Timespan::TimeDiff>(d * 1000000);
    }
...
SettingFieldSeconds::SettingFieldTimespan(const Field & f)
    : SettingFieldTimespan(DBPoco::Timespan{float64AsSecondsToTimespan(fieldToNumber<Float64>(f))})
{
}
...
{
    *this = DBPoco::Timespan{float64AsSecondsToTimespan(fieldToNumber<Float64>(f))};
    return *this;
}
...
void SettingFieldSeconds::parseFromString(const String & str)
{
    ...
    *this = DBPoco::Timespan{static_cast<DBPoco::Timespan::TimeDiff>(n * microseconds_per_unit)};
}

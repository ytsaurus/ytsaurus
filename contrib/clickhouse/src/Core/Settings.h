...
#define COMMON_SETTINGS(M, ALIAS) \
    ...
    M(Bool, enable_named_columns_in_function_tuple, false, "Generate named tuples in function tuple() when all names are unique and can be treated as unquoted identifiers.", 0) \
    \
...
struct Settings : public BaseSettings<SettingsTraits>, public IHints<2>
{
     ...
     */
    void setProfile(const String & profile_name, const DBPoco::Util::AbstractConfiguration & config);

    /// Load settings from configuration file, at "path" prefix in configuration.
    void loadSettingsFromConfig(const String & path, const DBPoco::Util::AbstractConfiguration & config);
    ...
    static void checkNoSettingNamesAtTopLevel(const DBPoco::Util::AbstractConfiguration & config, const String & config_path);

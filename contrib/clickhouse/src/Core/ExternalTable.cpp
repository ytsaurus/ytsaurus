...
void ExternalTablesHandler::handlePart(const DBPoco::Net::MessageHeader & header, ReadBuffer & stream)
{
    ...
    DBPoco::Net::NameValueCollection content;
    std::string label;
    DBPoco::Net::MessageHeader::splitParameters(header.get("Content-Disposition"), label, content);

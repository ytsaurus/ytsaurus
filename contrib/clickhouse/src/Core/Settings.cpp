...
/** Set the settings from the profile (in the server configuration, many settings can be listed in one profile).
    * The profile can also be set using the `set` functions, like the `profile` setting.
    */
void SettingsImpl::setProfile(const String & profile_name, const DBPoco::Util::AbstractConfiguration & config)
{
...
void SettingsImpl::loadSettingsFromConfig(const String & path, const DBPoco::Util::AbstractConfiguration & config)
{
    if (!config.has(path))
...
void SettingsImpl::checkNoSettingNamesAtTopLevel(const DBPoco::Util::AbstractConfiguration & config, const String & config_path)
{
...
void Settings::addToClientOptions(DBPoco::Util::LayeredConfiguration &config, const boost::program_options::variables_map &options, bool repeated_settings) const
{
    for (const auto & setting : impl->all())
    {
        const auto & name = setting.getName();

...
int CertificateReloader::setCertificate(SSL * ssl, const CertificateReloader::MultiData * pdata)
{
    ...
    {
        LOG_ERROR(log, "Clear certificates {}", DBPoco::Net::Utility::getLastError());
        return -1;
    }
    if (auto err = SSL_use_certificate(ssl, const_cast<X509 *>(current->certs_chain[0].certificate())); err != 1)
    {
        LOG_ERROR(log, "Use certificate {}", DBPoco::Net::Utility::getLastError());
        return -1;
    }
    for (auto cert = current->certs_chain.begin() + 1; cert != current->certs_chain.end(); cert++)
    {
        ...
        {
            LOG_ERROR(log, "Add certificate to chain {}", DBPoco::Net::Utility::getLastError());
            return -1;
        }
    }
    if (auto err = SSL_use_PrivateKey(ssl, const_cast<EVP_PKEY *>(static_cast<const EVP_PKEY *>(current->key))); err != 1)
    {
        LOG_ERROR(log, "Use private key {}", DBPoco::Net::Utility::getLastError());
        return -1;
    }
    if (auto err = SSL_check_private_key(ssl); err != 1)
    {
        LOG_ERROR(log, "Unusable key-pair {}", DBPoco::Net::Utility::getLastError());
        return -1;
    }
    return 1;
}
...
void CertificateReloader::tryLoad(const DBPoco::Util::AbstractConfiguration & config)
{
    tryLoad(config, nullptr, DBPoco::Net::SSLManager::CFG_SERVER_PREFIX);
}


void CertificateReloader::tryLoad(const DBPoco::Util::AbstractConfiguration & config, SSL_CTX * ctx, const std::string & prefix)
{
...
std::list<CertificateReloader::MultiData>::iterator CertificateReloader::findOrInsert(SSL_CTX * ctx, const std::string & prefix)
{
    ...
    {
        if (!ctx)
            ctx = DBPoco::Net::SSLManager::instance().defaultServerContext()->sslContext();
...
void CertificateReloader::tryLoadImpl(const DBPoco::Util::AbstractConfiguration & config, SSL_CTX * ctx, const std::string & prefix)
{
...
void CertificateReloader::tryReloadAll(const DBPoco::Util::AbstractConfiguration & config)
{
    std::lock_guard lock{data_mutex};
...
CertificateReloader::Data::Data(std::string cert_path, std::string key_path, std::string pass_phrase)
    : certs_chain(DBPoco::Crypto::X509Certificate::readPEM(cert_path)), key(/* public key */ "", /* private key */ key_path, pass_phrase)
{
}

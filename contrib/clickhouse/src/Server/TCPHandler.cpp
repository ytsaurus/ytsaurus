    ...
    while (tcp_server.isOpen())
    {
        ...
        try
        {
                ...
                && client_logs_level != LogsLevel::none)
            {
                ...
                state.logs_queue->max_priority = DBPoco::Logger::parseLevel(client_logs_level.toString());
...
std::string formatHTTPErrorResponseWhenUserIsConnectedToWrongPort(const DBPoco::Util::AbstractConfiguration& config)
{
...
void TCPHandler::receiveHello()
{
    ...
    if (dynamic_cast<DBPoco::Net::SecureStreamSocketImpl*>(socket().impl()))
    {
        DBPoco::Net::SecureStreamSocket secure_socket(socket());
...
void TCPHandler::initBlockOutput(const Block & block)
{
    if (!state.block_out)
    {
        ...
        {
            std::string method = DBPoco::toUpper(query_settings.network_compression_method.toString());
...
void TCPHandler::run()
{
    ...
    catch (DBPoco::Exception & e)
    {
        ...
        {
            LOG_DEBUG(log, "DBPoco::Exception. Code: {}, e.code() = {}, e.displayText() = {}, e.what() = {}", ErrorCodes::POCO_EXCEPTION, e.code(), e.displayText(), e.what());
        }
        else
            throw;
    }
...
DBPoco::Net::SocketAddress TCPHandler::getClientAddress(const ClientInfo & client_info)
{
    ...
    if (!forwarded_address.empty() && server.config().getBool("auth_use_forwarded_address", false))
        return DBPoco::Net::SocketAddress(forwarded_address, socket().peerAddress().port());
    else

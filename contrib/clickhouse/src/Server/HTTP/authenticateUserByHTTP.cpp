    ...
    LoggerPtr log)
{
    ...
    else if (has_http_credentials)
    {
        ...
        if (DBPoco::icompare(scheme, "Basic") == 0)
        {
            DBPoco::Net::HTTPBasicCredentials credentials(auth_info);
            user = credentials.getUsername();
            password = credentials.getPassword();
            checkUserNameNotEmpty(user, "Authorization HTTP header");
        }
        else if (DBPoco::icompare(scheme, "Negotiate") == 0)
        {
...
#pragma clang diagnostic pop
    ...
    try
    {
        if (!forwarded_address.empty() && global_context->getConfigRef().getBool("auth_use_forwarded_address", false))
            session.authenticate(*current_credentials, DBPoco::Net::SocketAddress(forwarded_address, request.clientAddress().port()));
        else

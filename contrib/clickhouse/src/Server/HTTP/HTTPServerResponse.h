...
protected:
    void nextImpl() override
    {
        ...
        DBPoco::NumberFormatter::appendHex(chunk_header, offset());
        chunk_header.append("\r\n", 2);
...
{
public:
    explicit HTTPWriteBufferFixedLength(DBPoco::Net::Socket & socket_, size_t fixed_length_, size_t buf_size = DBMS_DEFAULT_BUFFER_SIZE)
        : WriteBufferFromPocoSocket(socket_, buf_size)
    {
        fixed_length = fixed_length_;
    }
    explicit HTTPWriteBufferFixedLength(DBPoco::Net::Socket & socket_, size_t fixed_length_, const ProfileEvents::Event & write_event_, size_t buf_size = DBMS_DEFAULT_BUFFER_SIZE)
...
{
public:
    explicit HTTPWriteBuffer(DBPoco::Net::Socket & socket_, size_t buf_size = DBMS_DEFAULT_BUFFER_SIZE)
        : WriteBufferFromPocoSocket(socket_, buf_size)
    {
    }
    explicit HTTPWriteBuffer(DBPoco::Net::Socket & socket_, const ProfileEvents::Event & write_event_, size_t buf_size = DBMS_DEFAULT_BUFFER_SIZE)
        : WriteBufferFromPocoSocket(socket_, write_event_, buf_size)
    {
    }
...
{
public:
    explicit HTTPServerResponse(DBPoco::Net::HTTPServerSession & session, const ProfileEvents::Event & write_event_ = ProfileEvents::end());
    ...
    DBPoco::Net::StreamSocket & getSocket() { return session.socket(); }

    void attachRequest(HTTPServerRequest * request_) { request = request_; }

    const DBPoco::Net::HTTPServerSession & getSession() const { return session; }

private:
    DBPoco::Net::HTTPServerSession & session;

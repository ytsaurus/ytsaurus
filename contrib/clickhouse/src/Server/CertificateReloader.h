...
public:
    ...
    struct Data
    {
        DBPoco::Crypto::X509Certificate::List certs_chain;
        DBPoco::Crypto::EVPPKey key;
    ...
    void tryLoad(const DBPoco::Util::AbstractConfiguration & config);

    /// Handle configuration reload
    void tryLoad(const DBPoco::Util::AbstractConfiguration & config, SSL_CTX * ctx, const std::string & prefix);

    /// Handle configuration reload for all contexts
    void tryReloadAll(const DBPoco::Util::AbstractConfiguration & config);
...
private:
    ...
    void tryLoadImpl(const DBPoco::Util::AbstractConfiguration & config, SSL_CTX * ctx, const std::string & prefix) TSA_REQUIRES(data_mutex);

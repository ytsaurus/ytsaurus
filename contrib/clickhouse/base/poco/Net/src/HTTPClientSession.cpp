...
HTTPClientSession::HTTPClientSession(const std::string& host, DBPoco::UInt16 port):
	_host(host),
	_port(port),
	_proxyConfig(_globalProxyConfig),
	_keepAliveTimeout(DEFAULT_KEEP_ALIVE_TIMEOUT, 0),
...
HTTPClientSession::HTTPClientSession(const std::string& host, DBPoco::UInt16 port, const ProxyConfig& proxyConfig):
	_host(host),
	_port(port),
...
void HTTPClientSession::setPort(DBPoco::UInt16 port)
{
	if (!connected())
		_port = port;
	else
...
void HTTPClientSession::setProxy(const std::string& host, DBPoco::UInt16 port, const std::string& protocol, bool tunnel)
{
...
void HTTPClientSession::setProxyPort(DBPoco::UInt16 port)
{
	if (!connected())
		_proxyConfig.port = port;
	else
...
void HTTPClientSession::setKeepAliveTimeout(const DBPoco::Timespan& timeout)
{
    if (connected())
    {
        throw DBPoco::IllegalStateException("cannot change keep alive timeout on initiated connection, "
...
void HTTPClientSession::setKeepAliveMaxRequests(int max_requests)
{
    if (connected())
    {
        throw DBPoco::IllegalStateException("cannot change keep alive max requests on initiated connection, "
                                          "That value is managed privately after connection is established.");
    }
...
void HTTPClientSession::setLastRequest(DBPoco::Timestamp time)
{
    if (connected())
    {
        throw DBPoco::IllegalStateException("cannot change last request on initiated connection, "
                                          "That value is managed privately after connection is established.");
    }
    _lastRequest = time;
}
...
std::istream& HTTPClientSession::receiveResponse(HTTPResponse& response)
{
    ...
    if (!_mustReconnect)
    {
        ...
        if (timeout > 0)
            _keepAliveTimeout = std::min(_keepAliveTimeout, DBPoco::Timespan(timeout, 0));
...
{
	DB_poco_assert (!_responseReceived);
...
void HTTPClientSession::assign(DBPoco::Net::HTTPClientSession & session)
{
    DB_poco_assert (this != &session);

    if (session.buffered())
        throw DBPoco::LogicException("assign a session with not empty buffered data");

    if (buffered())
        throw DBPoco::LogicException("assign to a session with not empty buffered data");

    DB_poco_assert(!connected());

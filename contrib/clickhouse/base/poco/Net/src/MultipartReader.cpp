...
using DBPoco::BufferedStreamBuf;
...
MultipartStreamBuf::MultipartStreamBuf(std::istream& istr, const std::string& boundary):
	...
	_lastPart(false)
{
	DB_poco_assert (!boundary.empty() && boundary.length() < STREAM_BUFFER_SIZE - 6);
}
...
{
	DB_poco_assert_dbg (length >= _boundary.length() + 6);

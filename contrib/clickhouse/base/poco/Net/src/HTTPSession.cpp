...
void HTTPSession::setTimeout(const DBPoco::Timespan& timeout)
{
	setTimeout(timeout, timeout, timeout);
}


void HTTPSession::setTimeout(const DBPoco::Timespan& connectionTimeout, const DBPoco::Timespan& sendTimeout, const DBPoco::Timespan& receiveTimeout)
{
     try
     {
...
void HTTPSession::setException(const DBPoco::Exception& exc)
{
	delete _pException;
...
void HTTPSession::attachSessionData(const DBPoco::Any& data)
{
	_data = data;
}


void HTTPSession::drainBuffer(DBPoco::Buffer<char>& buffer)
{

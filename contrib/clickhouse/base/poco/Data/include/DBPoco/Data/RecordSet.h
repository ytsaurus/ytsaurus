...
#ifndef DB_Data_RecordSet_INCLUDED
#define DB_Data_RecordSet_INCLUDED


#include <limits>
#include <ostream>
#include "DBPoco/AutoPtr.h"
#include "DBPoco/Data/BulkExtraction.h"
#include "DBPoco/Data/Data.h"
#include "DBPoco/Data/Extraction.h"
#include "DBPoco/Data/LOB.h"
#include "DBPoco/Data/RowFilter.h"
#include "DBPoco/Data/RowIterator.h"
...
namespace Data
{
    ...
    public:
        ...
        DBPoco::Dynamic::Var value(std::size_t col, std::size_t row, bool checkFiltering = true) const;
        /// Returns the data value at column, row location.

        DBPoco::Dynamic::Var value(const std::string & name, std::size_t row, bool checkFiltering = true) const;
        /// Returns the data value at named column, row location.

        template <typename T>
        DBPoco::Dynamic::Var nvl(const std::string & name, const T & deflt = T()) const
        /// Returns the value in the named column of the current row
        /// if the value is not NULL, or deflt otherwise.
        {
            if (isNull(name))
                return DBPoco::Dynamic::Var(deflt);
            else
                return value(name, _currentRow);
        }

        template <typename T>
        DBPoco::Dynamic::Var nvl(std::size_t index, const T & deflt = T()) const
        ...
        DBPoco::Dynamic::Var value(const std::string & name);
        /// Returns the value in the named column of the current row.

        DBPoco::Dynamic::Var value(std::size_t index);
        /// Returns the value in the given column of the current row.

        DBPoco::Dynamic::Var operator[](const std::string & name);
        /// Returns the value in the named column of the current row.

        DBPoco::Dynamic::Var operator[](std::size_t index);
        /// Returns the value in the named column of the current row.
    ...
    private:
        ...
        /// Returns the position of the column with specified name.
        {
            ...
            for (; it != end; ++it)
            {
                ...
                if (pExtraction)
                {
                    ...
                    if (0 == DBPoco::icompare(name, col.name()))
                        return col.position();
                }
            }

            if (typeFound)
                throw NotFoundException(DBPoco::format("Column name: %s", name));
            else
                throw NotFoundException(DBPoco::format("Column type: %s, name: %s", std::string(typeid(T).name()), name));
        }
        ...
        /// Returns the reference to column at specified position.
        {
            ...
            if (0 == s || pos >= s)
                throw RangeException(DBPoco::format("Invalid column index: %z", pos));
            ...
            {
                throw DBPoco::BadCastException(
                    DBPoco::format("Type cast failed!\nColumn: %z\nTarget type:\t%s", pos, std::string(typeid(T).name())));
            }
        }
        ...
        void filter(const DBPoco::AutoPtr<RowFilter> & pFilter);
        /// Sets the filter for the RecordSet.

        const DBPoco::AutoPtr<RowFilter> & getFilter() const;
        ...
        RowIterator * _pEnd;
        RowMap _rowMap;
        DBPoco::AutoPtr<RowFilter> _pFilter;
    ...
    inline DBPoco::Dynamic::Var RecordSet::value(const std::string & name)
    {
        return value(name, _currentRow);
    }


    inline DBPoco::Dynamic::Var RecordSet::value(std::size_t index)
    {
        return value(index, _currentRow);
    }


    inline DBPoco::Dynamic::Var RecordSet::operator[](const std::string & name)
    {
        return value(name, _currentRow);
    }


    inline DBPoco::Dynamic::Var RecordSet::operator[](std::size_t index)
    {
        return value(index, _currentRow);
    }
    ...
    inline const DBPoco::AutoPtr<RowFilter> & RecordSet::getFilter() const
    {
        return _pFilter;
    }
...
#endif // DB_Data_RecordSet_INCLUDED

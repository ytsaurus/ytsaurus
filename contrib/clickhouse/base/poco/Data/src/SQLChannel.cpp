...
{
	DB_poco_check_ptr (_pLogStatement);
...
void SQLChannel::initLogStatement()
{
	...
	std::string sql;
	DBPoco::format(sql, "INSERT INTO %s VALUES (?,?,?,?,?,?,?,?)", _table);
	*_pLogStatement << sql,
		use(_source),
		use(_name),
		use(_pid),
...
{
	DBPoco::LoggingFactory::defaultFactory().registerChannelClass("SQLChannel", 
		new DBPoco::Instantiator<SQLChannel, DBPoco::Channel>);
}

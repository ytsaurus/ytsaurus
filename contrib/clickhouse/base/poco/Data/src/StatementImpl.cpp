...
#include "DBPoco/Data/StatementImpl.h"
...
void StatementImpl::fixupExtraction()
{
	...
	DBPoco::Data::AbstractExtractionVec::iterator it    = extractions().begin();
	DBPoco::Data::AbstractExtractionVec::iterator itEnd = extractions().end();
	for (; it != itEnd; ++it)
	{
...
void StatementImpl::resetExtraction()
{
	...
	DB_poco_assert (_curDataSet < _columnsExtracted.size());
	...
	for (int i = 0; i < count; ++i)
	{
		...
		switch (mc.type())
		{
			...
			case MetaColumn::FDT_WSTRING:
				addInternalExtract<DBPoco::UTF16String>(mc); break;
			...
			default:
				throw DBPoco::InvalidArgumentException("Data type not supported.");
		}
	}
}
...
std::size_t StatementImpl::columnsExtracted(int dataSet) const
{
	...
	{
		DB_poco_assert (dataSet >= 0 && dataSet < _columnsExtracted.size());
...
std::size_t StatementImpl::rowsExtracted(int dataSet) const
{
	...
	{
		DB_poco_assert (dataSet >= 0 && dataSet < _extractors.size());
...
std::size_t StatementImpl::subTotalRowCount(int dataSet) const
{
	...
	{
		DB_poco_assert (dataSet >= 0 && dataSet < _subTotalRowCount.size());
...
{
	std::string sql;
	DBPoco::format(sql, _ostr.str(), arguments);
	_ostr.str("");
	_ostr << sql;
}

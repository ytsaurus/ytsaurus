...
#include "DBPoco/Data/ArchiveStrategy.h"
...
void ArchiveByAgeStrategy::initStatements()
{
	...
	_archiveCount = 0;
	std::string sql;
	DBPoco::format(sql, "SELECT COUNT(*) FROM %s WHERE DateTime < ?", src);
	getCountStatement() << sql, into(_archiveCount), use(_archiveDateTime);

	setCopyStatement();
	sql.clear();
	DBPoco::format(sql, "INSERT INTO %s SELECT * FROM %s WHERE DateTime < ?", dest, src);
	getCopyStatement() << sql, use(_archiveDateTime);

	setDeleteStatement();
	sql.clear();
	DBPoco::format(sql, "DELETE FROM %s WHERE DateTime < ?", src);

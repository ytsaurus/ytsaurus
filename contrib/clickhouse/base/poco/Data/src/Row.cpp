...
#include "DBPoco/Data/Row.h"
#include "DBPoco/Data/SimpleRowFormatter.h"
...
DBPoco::Dynamic::Var& Row::get(std::size_t col)
{
	try
	{
...
{
	DB_poco_assert (pos <= _values.size());
	...
	else if ((_values[pos].type() == typeid(DBPoco::Int8))   ||
		(_values[pos].type() == typeid(DBPoco::UInt8))  ||
		(_values[pos].type() == typeid(DBPoco::Int16))  ||
		(_values[pos].type() == typeid(DBPoco::UInt16)) ||
		(_values[pos].type() == typeid(DBPoco::Int32))  ||
		(_values[pos].type() == typeid(DBPoco::UInt32)) ||
		(_values[pos].type() == typeid(DBPoco::Int64))  ||
		(_values[pos].type() == typeid(DBPoco::UInt64)) ||
...
{
	DB_poco_assert (oldPos <= _values.size());
	DB_poco_assert (newPos <= _values.size());
	...
	else if ((_values[newPos].type() == typeid(DBPoco::Int8)) ||
		(_values[newPos].type() == typeid(DBPoco::UInt8))     ||
		(_values[newPos].type() == typeid(DBPoco::Int16))     ||
		(_values[newPos].type() == typeid(DBPoco::UInt16))    ||
		(_values[newPos].type() == typeid(DBPoco::Int32))     ||
		(_values[newPos].type() == typeid(DBPoco::UInt32))    ||
		(_values[newPos].type() == typeid(DBPoco::Int64))     ||
		(_values[newPos].type() == typeid(DBPoco::UInt64))    ||
...
{
	std::vector<DBPoco::Dynamic::Var>::const_iterator it = _values.begin();
	std::vector<DBPoco::Dynamic::Var>::const_iterator end = _values.end();
...
bool Row::operator < (const Row& other) const
{
	...
	for (; it != end; ++it)
	{
		...
		case COMPARE_AS_INTEGER:
			if (_values[it->get<0>()].convert<DBPoco::Int64>() < 
				other._values[it->get<0>()].convert<DBPoco::Int64>())
				return true;
			else if (_values[it->get<0>()].convert<DBPoco::Int64>() != 
				other._values[it->get<0>()].convert<DBPoco::Int64>())
				return false;
			break;

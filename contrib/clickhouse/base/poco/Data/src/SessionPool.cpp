...
SessionPool::SessionPool(const std::string& connector, const std::string& connectionString, int minSessions, int maxSessions, int idleTime):
	...
	_shutdown(false)
{
	DBPoco::TimerCallback<SessionPool> callback(*this, &SessionPool::onJanitorTimer);
...
{
	DBPoco::Mutex::ScopedLock lock(_mutex);
...
void SessionPool::setProperty(const std::string& name, const DBPoco::Any& value)
{
	DBPoco::Mutex::ScopedLock lock(_mutex);
	if (_shutdown) throw InvalidAccessException("Session pool has been shut down.");
...
DBPoco::Any SessionPool::getProperty(const std::string& name)
{
...
void SessionPool::putBack(PooledSessionHolderPtr pHolder)
{
	...
	{
		DB_poco_bugcheck_msg("Unknown session passed to SessionPool::putBack()");
	}
}


void SessionPool::onJanitorTimer(DBPoco::Timer&)
{
	DBPoco::Mutex::ScopedLock lock(_mutex);
	if (_shutdown) return;

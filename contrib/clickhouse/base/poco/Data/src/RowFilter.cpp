...
#include "DBPoco/Data/RecordSet.h"
...
RowFilter::RowFilter(Ptr pParent, LogicOperator op): _pRecordSet(0),
	...
	_not(false)
{
	DB_poco_check_ptr(_pParent.get());
	init();
	duplicate();
...
bool RowFilter::isAllowed(std::size_t row) const
{
	...
	for (; it != end; ++it)
	{
		for (std::size_t col = 0; col < columns; ++col)
		{
			...
			DBPoco::Dynamic::Var ret;
			CompT compOp = 0;
			DBPoco::Dynamic::Var val = rs.value(col, row, false);
...
void RowFilter::doCompare(DBPoco::Dynamic::Var& ret,
	DBPoco::Dynamic::Var& val,
	CompT comp,

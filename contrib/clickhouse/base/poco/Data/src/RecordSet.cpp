...
using namespace DBPoco::Data::Keywords;
using DBPoco::DateTime;
using DBPoco::UTF16String;
...
DBPoco::Dynamic::Var RecordSet::value(std::size_t col, std::size_t row, bool useFilter) const
{
	if (useFilter && isFiltered() && !isAllowed(row))
		throw InvalidAccessException("Row not allowed");

	if (isNull(col, row)) return DBPoco::Dynamic::Var();
...
DBPoco::Dynamic::Var RecordSet::value(const std::string& name, std::size_t row, bool useFilter) const
{
	if (useFilter && isFiltered() && !isAllowed(row))
		throw InvalidAccessException("Row not allowed");

	if (isNull(metaColumn(name).position(), row)) return DBPoco::Dynamic::Var();
...
{
	DB_poco_assert (extractions().size());
...
void RecordSet::filter(const DBPoco::AutoPtr<RowFilter>& pFilter)
{
	_pFilter = pFilter;
}

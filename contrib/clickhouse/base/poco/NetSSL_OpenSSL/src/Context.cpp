...
Context::~Context()
{
	try
	{
		...
		DBPoco::Crypto::OpenSSLInitializer::uninitialize();
	}
	catch (...)
	{
		DB_poco_unexpected();
...
{
	DBPoco::Crypto::OpenSSLInitializer::initialize();
	...
	try
	{
		...
		{
			DBPoco::File aFile(params.caLocation);
			if (aFile.isDirectory())
				errCode = SSL_CTX_load_verify_locations(_pSSLContext, 0, DBPoco::Path::transcode(params.caLocation).c_str());
			else
				errCode = SSL_CTX_load_verify_locations(_pSSLContext, DBPoco::Path::transcode(params.caLocation).c_str(), 0);
			if (errCode != 1)
			{
		...
		{
			errCode = SSL_CTX_use_PrivateKey_file(_pSSLContext, DBPoco::Path::transcode(params.privateKeyFile).c_str(), SSL_FILETYPE_PEM);
			if (errCode != 1)
			{
				std::string msg = Utility::getLastError();
		...
		{
			errCode = SSL_CTX_use_certificate_chain_file(_pSSLContext, DBPoco::Path::transcode(params.certificateFile).c_str());
			if (errCode != 1)
			{
...
void Context::useCertificate(const DBPoco::Crypto::X509Certificate& certificate)
{
...
void Context::addChainCertificate(const DBPoco::Crypto::X509Certificate& certificate)
{
...
void Context::usePrivateKey(const DBPoco::Crypto::RSAKey& key)
{
...
#endif
		default:
			throw DBPoco::InvalidArgumentException("Invalid or unsupported usage");
		}
	}
	if (!_pSSLContext)
	{

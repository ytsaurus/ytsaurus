...
class TestTransaction(object):
    ...
    def test_remove_create_pod(self, yp_env):
        ...
        def run(pod_id, pod_set_id):
            ...
            yp_client.create_object("pod", attributes=dict(meta=dict(id=pod_id, pod_set_id=pod_set_id)), transaction_id=transaction_id)
            yp_client.commit_transaction(transaction_id)
            scheduling, resource_requests = yp_client.get_object(
                "pod",
                pod_id,
                selectors=["/status/scheduling", "/spec/resource_requests"],
            )
    ...
    def test_remove_create_endpoint_set(self, yp_env):
        ...
        yp_client.create_object("endpoint_set", attributes=dict(meta=dict(id=endpoint_set_id)), transaction_id=transaction_id)
        yp_client.commit_transaction(transaction_id)

        timestamp = yp_client.get_object("endpoint_set", endpoint_set_id, selectors=["/status/last_endpoints_update_timestamp"])[0]
        assert timestamp > 0
    ...
    def test_remove_create_endpoint(self, yp_env):
        ...
        endpoint_set_id = yp_client.create_object("endpoint_set")
        endpoint_id = yp_client.create_object("endpoint", attributes=dict(meta=dict(endpoint_set_id=endpoint_set_id)))

        timestamp1 = yp_client.get_object("endpoint_set", endpoint_set_id, selectors=["/status/last_endpoints_update_timestamp"])[0]

        transaction_id = yp_client.start_transaction()
        yp_client.remove_object("endpoint", endpoint_id, transaction_id=transaction_id)
        yp_client.create_object("endpoint", attributes=dict(meta=dict(id=endpoint_id, endpoint_set_id=endpoint_set_id)), transaction_id=transaction_id)
        yp_client.commit_transaction(transaction_id)

        timestamp2 = yp_client.get_object("endpoint_set", endpoint_set_id, selectors=["/status/last_endpoints_update_timestamp"])[0]
    ...
    def test_remove_child_remove_parent_create_child(self, yp_env):
        ...
        def create_pod(transaction_id=None):
            yp_client.create_object(
                "pod",
                attributes=dict(
                    meta=dict(id=pod_id, pod_set_id=pod_set_id),
                ),
                transaction_id=transaction_id,
            )

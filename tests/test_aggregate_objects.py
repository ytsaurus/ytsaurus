...
class TestAggregateObjects(object):
    ...
    def test_column_order(self, yp_env):
        ...
        assert len(result) == 1
        assert result[0] == [pod_set_id, max(memory_limits), float(sum(vcpu_guarantees)) / len(vcpu_guarantees)]
    ...
    def test_secrets_not_allowed_in_group_by(self, yp_env):
        ...
        with pytest.raises(YtResponseError):
            yp_client.aggregate_objects(
                "pod",
                group_by=["[/spec/secrets/0/secret_id]"],
            )
    ...
    def test_annotations_in_filter(self, yp_env):
        yp_client = yp_env.yp_client
        pod_set_id = yp_client.create_object("pod_set")
        yp_client.update_object("pod_set", pod_set_id, set_updates=[{"path": "/annotations/a", "value": "b"}])
        with pytest.raises(YtResponseError):
            yp_client.aggregate_objects(
                "pod_set",
                group_by=["[/meta/id]"],
                filter="[/annotations/a] = \"b\"",
            )

    def test_annotations_in_aggregator(self, yp_env):
        yp_client = yp_env.yp_client
        pod_set_id = yp_client.create_object("pod_set")
        yp_client.update_object("pod_set", pod_set_id, set_updates=[{"path": "/annotations/a", "value": 5}])
        with pytest.raises(YtResponseError):
            yp_client.aggregate_objects(
                "pod_set",
                group_by=["[/meta/id]"],
                aggregators=["sum(int64([/annotations/a]))"],
            )

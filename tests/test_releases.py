...
class TestReleases(object):
    ...
    def test_release_id_validation_failure(self, yp_env):
        ...
        with pytest.raises(YtResponseError):
            yp_client.create_object(
                "release", attributes={"meta": {"id": "inv*"}, "spec": valid_spec}
            )

    def test_release_sandbox_validation(self, yp_env):
        yp_client = yp_env.yp_client
        valid_resource = {"type": "some_resource_type", "skynet_id": "some-id"}
        ...
        with pytest.raises(YtResponseError):
            yp_client.create_object(
                "release", attributes={"meta": {"id": "val"}, "spec": invalid_spec_no_task_id}
            )

        # Case 2: no resouces in sandbox release
        with pytest.raises(YtResponseError):
            yp_client.create_object(
                "release", attributes={"meta": {"id": "val"}, "spec": invalid_spec_no_resources}
            )

        # Case 3: no resouce type in sandbox release
        with pytest.raises(YtResponseError):
            yp_client.create_object(
                "release", attributes={"meta": {"id": "val"}, "spec": invalid_spec_invalid_resource}
            )

        # Case 4: all is ok
        release_id = yp_client.create_object(
            "release", attributes={"meta": {"id": "val"}, "spec": valid_spec}
        )

        # Case 5: task_id cannot be empty
        with pytest.raises(YtResponseError):
            yp_client.update_object(
                "release", release_id, set_updates=[{"path": "/spec/sandbox/task_id", "value": ""}]
            )
    ...
    def test_release_docker_validation(self, yp_env):
        ...
        with pytest.raises(YtResponseError):
            yp_client.create_object(
                "release", attributes={"meta": {"id": "val"}, "spec": invalid_spec_no_image_name}
            )

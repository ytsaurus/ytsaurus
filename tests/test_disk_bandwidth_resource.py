...
class TestDiskBandwidthResource(object):
    ...
    def test(self, yp_env):
        ...
        pod_spec = dict(
            ...
            disk_volume_requests=[
                dict(
                    ...
                    quota_policy=dict(
                        capacity=pod_capacity, bandwidth_guarantee=pod_bandwidth_guarantee,
                    ),
                ),
            ],
        )
        ...
        assert volume_allocation["read_bandwidth_guarantee"] == int(
            pod_bandwidth_guarantee * read_bandwidth_factor
        )
        assert "read_bandwidth_limit" not in volume_allocation
        assert volume_allocation["write_bandwidth_guarantee"] == int(
            pod_bandwidth_guarantee * write_bandwidth_factor
        )
        assert "write_bandwidth_limit" not in volume_allocation
        assert volume_allocation["read_operation_rate_guarantee"] == int(
            pod_bandwidth_guarantee / read_operation_rate_divisor
        )
        ...
        wait(
            lambda: is_assigned_pod_scheduling_status(get_pod_scheduling_status(yp_client, pod_id2))
        )

        volume_allocation2 = self._get_pod_disk_volume_allocation(yp_client, pod_id2)
        assert volume_allocation2["read_bandwidth_limit"] == int(
            pod_bandwidth_limit * read_bandwidth_factor
        )
        assert volume_allocation2["write_bandwidth_limit"] == int(
            pod_bandwidth_limit * write_bandwidth_factor
        )
        assert volume_allocation2["read_operation_rate_limit"] == int(
            pod_bandwidth_limit / read_operation_rate_divisor
        )
        ...
        account_disk_totals = yp_client.get_object(
            ...
            DEFAULT_ACCOUNT_ID,
            selectors=[
                "/status/resource_usage/per_segment/default/disk_per_storage_class/{}".format(
                    pod_storage_class
                )
            ],
        )[0]
            ...
            disk_volume_requests=[
                dict(
                    ...
                    exclusive_policy=dict(
                        min_capacity=pod_capacity, min_bandwidth=pod_bandwidth_guarantee,
                    ),
                ),
            ],
        )
        ...
        wait(
            lambda: is_error_pod_scheduling_status(
                get_pod_scheduling_status(yp_client, exclusive_pod_id)
            )
        )
        ...
        exclusive_volume_allocation = self._get_pod_disk_volume_allocation(
            yp_client, exclusive_pod_id
        )
        assert exclusive_volume_allocation["read_bandwidth_guarantee"] == int(
            pod_bandwidth_guarantee * read_bandwidth_factor
        )
        assert "read_bandwidth_limit" not in exclusive_volume_allocation
        assert exclusive_volume_allocation["write_bandwidth_guarantee"] == int(
            pod_bandwidth_guarantee * write_bandwidth_factor
        )
        assert "write_bandwidth_limit" not in exclusive_volume_allocation
        assert exclusive_volume_allocation["read_operation_rate_guarantee"] == int(
            pod_bandwidth_guarantee / read_operation_rate_divisor
        )

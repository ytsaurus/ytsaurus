...
class TestPodDisruptionBudgets(object):
    YP_MASTER_CONFIG = dict(
        scheduler=dict(disable_stage=dict(run_pod_disruption_budget_controller=True,),),
    )
    ...
    def test_attributes(self, yp_env_configurable):
        ...
        assert (
            attributes[1]["allowed_pod_disruptions"] == 0
            and attributes[1]["last_update_time"]["seconds"] > 0
            and len(attributes[1]["last_update_message"]) > 0
        )
        ...
        yp_client.update_object(
            "pod_disruption_budget",
            pod_disruption_budget_id,
            set_updates=[dict(path="/spec/max_pods_unavailable", value=1,)],
        )
        assert (
        ...
        with pytest.raises(YtResponseError):
            yp_client.update_object(
                "pod_disruption_budget",
                pod_disruption_budget_id,
                set_updates=[dict(path="/status/allowed_pod_disruptions", value=1,)],
            )

        for field_name in ("max_pods_unavailable", "max_pod_disruptions_between_syncs"):
            yp_client.create_object(
                "pod_disruption_budget", attributes=dict(spec={field_name: 1}),
            )
            with pytest.raises(YtResponseError):
                yp_client.create_object(
                    "pod_disruption_budget", attributes=dict(spec={field_name: -1}),
            ...
            with pytest.raises(YtResponseError):
                yp_client.update_object(
                    "pod_disruption_budget",
                    pod_disruption_budget_id,
                    set_updates=[dict(path="/spec/" + field_name, value=-1,)],
                )
    ...
    def test_status_update_time_monotonicity(self, yp_env_configurable):
        ...
        yp_client.update_object(
            ...
            pod_set_id,
            set_updates=[
                dict(path="/spec/pod_disruption_budget_id", value=pod_disruption_budget_id2,)
            ],
        )
        ...
        with yp_env_configurable.yp_instance.create_client(config=dict(user="u1")) as yp_client1:
            ...
            yp_client.update_object(
                ...
                set_updates=[
                    dict(
                        path="/meta/acl/end",
                        value=dict(permissions=["create"], subjects=["u1"], action="allow",),
                    )
                ],
            )
            yp_env_configurable.sync_access_control()
    ...
    def test_pod_sets_budgeting(self, yp_env_configurable):
        ...
        def _get_pod_set_disruption_budget(pod_set_id):
            return yp_client.get_object(
                "pod_set", pod_set_id, selectors=["/spec/pod_disruption_budget_id"],
            )[0]
        ...
        _update_pod_set_disruption_budget(
            pod_set_id, yp_client.create_object("pod_disruption_budget")
        )
...
def validate_controller_liveness(yp_client):
    pod_disruption_budget_id = yp_client.create_object(
        "pod_disruption_budget",
        attributes=dict(spec=dict(max_pods_unavailable=5, max_pod_disruptions_between_syncs=2,)),
    )
...
def validate_controller_lifelessness(yp_client):
    ...
    last_update_time = get_last_update_time(yp_client, pod_disruption_budget_id)
    assert_over_time(
        lambda: get_last_update_time(yp_client, pod_disruption_budget_id) == last_update_time
    )
...
def request_pod_eviction_forcefully(yp_client, pod_id, validate_disruption_budget=False):
    def impl():
        return yp_client.request_pod_eviction(
            pod_id, "Test", validate_disruption_budget=validate_disruption_budget,
        )
...
class TestPodDisruptionBudgetController(object):
    def test_scheduler_disable_stage_reconfiguration(self, yp_env):
        ...
        yp_env.set_cypress_config_patch(
            dict(scheduler=dict(disable_stage=dict(run_pod_disruption_budget_controller=True,)))
        )
    ...
    def test_options_reconfiguration(self, yp_env):
        ...
        def is_controller_configured():
            ...
            return (
                controller_config["updates_per_iteration"] == updates_per_iteration
                and controller_config["update_concurrency"] == update_concurrency
            )
    ...
    def test_not_assigned_pods(self, yp_env):
        ...
        pod_disruption_budget_id = yp_client.create_object(
            "pod_disruption_budget",
            attributes=dict(
                spec=dict(max_pods_unavailable=10, max_pod_disruptions_between_syncs=3,)
            ),
        )
    ...
    def test_pod_disruption_budget_controller_profiling(self, yp_env):
        ...
        def _prepare_pod_set_with_disruption_budget(pod_count):
            pod_disruption_budget_id = yp_client.create_object(
                "pod_disruption_budget",
                attributes=dict(
                    spec=dict(max_pods_unavailable=2, max_pod_disruptions_between_syncs=1,)
                ),
            )
            pod_set_id = create_pod_set(yp_client)
            attach_pod_set_to_disruption_budget(yp_client, pod_set_id, pod_disruption_budget_id)
        ...
        def _get_update_lags():
            ...
            samples = orchid.get(
                master_instance, "/profiling/scheduler/pod_disruption_budget_controller/update_lag"
            )
        ...
        for _ in xrange(POD_SET_COUNT):
            new_pod_set_id, new_pod_ids = _prepare_pod_set_with_disruption_budget(
                POD_COUNT_PER_POD_SET
            )
        ...
        assert all(
            map(lambda value: value >= 0 and value <= update_lag_silly_threshold, update_lags)
        )
...
class TestPodDisruptionBudgetEviction(object):
    def test_evicted_pods(self, yp_env):
        ...
        for case_index in xrange(len(parameters)):
            max_pods_unavailable, max_pod_disruptions_between_syncs, pod_count = parameters[
                case_index
            ]
...
class TestPodDisruptionBudgetIntegrationWithEvictionRequest(object):
    def test_request_eviction_by_hfsm_do_not_validate_budget(self, yp_env):
        ...
        pod_disruption_budget_id = yp_client.create_object(
            "pod_disruption_budget",
            attributes=dict(
                spec=dict(max_pods_unavailable=0, max_pod_disruptions_between_syncs=0,)
            ),
        )
        ...
        wait(lambda: are_pods_assigned(yp_client, pod_ids))
        assert_over_time(
            lambda: get_allowed_pod_disruptions(yp_client, pod_disruption_budget_id) == 0
        )
    ...
    def test_validate_disruption_budget_option(self, yp_env):
        ...
        for i in xrange(2 * iteration_count):
            ...
            request_pod_eviction_forcefully(
                yp_client, pod_ids[i], validate_disruption_budget=validate_disruption_budget,
            )
        ...
        with pytest.raises(YtResponseError):
            request_pod_eviction_forcefully(
                yp_client, extra_pod_id, validate_disruption_budget=True,
            )

        request_pod_eviction_forcefully(
            yp_client, extra_pod_id, validate_disruption_budget=False,
        )
    ...
    def test_budget_update(self, yp_env):
        ...
        pod_disruption_budget_id = yp_client.create_object(
            "pod_disruption_budget",
            attributes=dict(
                spec=dict(max_pods_unavailable=3, max_pod_disruptions_between_syncs=3,)
            ),
        )
        ...
        commit_timestamp = request_pod_eviction_forcefully(yp_client, pod_ids[0])[
            "commit_timestamp"
        ]
        assert (
            get_allowed_pod_disruptions(
                yp_client, pod_disruption_budget_id, timestamp=commit_timestamp,
            )
            == 2
        )

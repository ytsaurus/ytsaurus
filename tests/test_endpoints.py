...
class TestEndpoints(object):
    ...
    def test_status(self, yp_env):
        ...
        for ready in [True, False]:
            yp_client.update_object("endpoint", endpoint_id, set_updates=[{"path": "/status/ready", "value": ready}])
            assert yp_client.get_object("endpoint", endpoint_id, selectors=["/status/ready"])[0] == ready

            yp_client.update_object("endpoint", endpoint_id, set_updates=[{"path": "/status", "value": {"ready": not ready}}])
            assert yp_client.get_object("endpoint", endpoint_id, selectors=["/status/ready"])[0] == (not ready)

            endpoint_with_status_id = yp_client.create_object("endpoint", attributes={"meta": {"endpoint_set_id": endpoint_set_id}, "status": {"ready": ready}})
            assert yp_client.get_object("endpoint", endpoint_with_status_id, selectors=["/status/ready"])[0] == ready

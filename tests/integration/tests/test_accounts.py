...
class TestAccounts(YTEnvSetup):
    ...
    def test_init(self):
        self.assertItemsEqual(ls('//sys/accounts'), ['sys'])
        assert get('//@account') == 'sys'
        assert get('//home/@account') == 'sys'

    def test_account_create1(self):
        create_account('max')
        self.assertItemsEqual(ls('//sys/accounts'), ['sys', 'max'])
    ...
    def test_account_attr1(self):
        set('//tmp/a', {})
        assert get('//tmp/a/@account') == 'sys'

    def test_account_attr3(self):
        set('//tmp/a', {'x' : 1, 'y' : 2})
        assert get('//tmp/a/@account') == 'sys'
        assert get('//tmp/a/x/@account') == 'sys'
        assert get('//tmp/a/y/@account') == 'sys'
        copy('//tmp/a', '//tmp/b')
        assert get('//tmp/b/@account') == 'sys'
        assert get('//tmp/b/x/@account') == 'sys'
        assert get('//tmp/b/y/@account') == 'sys'
    ...
    def test_account_attr4(self):
        ...
        sys_node_count = self._get_account_node_count('sys')
        set('//tmp/a/@account', 'max')
        assert self._get_account_node_count('sys') == sys_node_count - 1
    ...
    def test_file1(self):
        assert self._get_account_disk_space('sys') == 0

        content = "some_data"
        upload('//tmp/f1', content)
        space = self._get_account_disk_space('sys')
        assert space > 0

        upload('//tmp/f2', content)
        assert self._get_account_disk_space('sys') == 2 * space

        remove('//tmp/f1')
        gc_collect()
        assert self._get_account_disk_space('sys') == space
    ...
    def test_table2(self):
        ...
        for i in xrange(0, 5):
            ...
            account_space = self._get_account_disk_space('sys')
            tx_space = self._get_tx_disk_space(tx, 'sys')
    ...
    def test_table3(self):
        create('table', '//tmp/t')
        write('//tmp/t', {'a' : 'b'})
        space1 = self._get_account_disk_space('sys')
        assert space1 > 0

        tx = start_transaction()
        write('<overwrite=true>//tmp/t', {'xxxx' : 'yyyy'}, tx=tx)
        space2 = self._get_tx_disk_space(tx, 'sys')
        assert space1 != space2
        assert self._get_account_disk_space('sys') == space1 + space2
        assert self._get_node_disk_space('//tmp/t') == space1
        assert self._get_node_disk_space('//tmp/t', tx=tx) == space2

        commit_transaction(tx)
        assert self._get_account_disk_space('sys') == space2
    ...
    def test_table5(self):
        ...
        assert self._get_account_node_count('sys') == sys_node_count
        assert self._get_account_node_count('max') == 1
        assert self._get_account_disk_space('sys') == 0
        assert self._get_account_disk_space('max') == space

        set('//tmp/t/@account', 'sys')
        assert self._get_account_node_count('sys') == sys_node_count + 1
        assert self._get_account_node_count('max') == 0
        assert self._get_account_disk_space('sys') == space
        assert self._get_account_disk_space('max') == 0

...
class TestCli(object):
    def test_common(self, yp_env):
        ...
        pod_set_id = check_output([cli_path, "create", "pod_set"]).strip()
        pod_id = check_output([cli_path, "create", "pod", "--attributes", yson.dumps({"meta": {"pod_set_id": pod_set_id}})]).strip()

        result = check_output([cli_path, "get", "pod", pod_id, "--selector", "/status/agent/state", "--selector", "/meta/id"])
        assert yson.loads(result) == ["unknown", pod_id]

        result = check_output([cli_path, "select", "pod", "--filter", '[/meta/id] = "{}"'.format(pod_id), "--no-tabular"])

...
class TestSchedulerHints(object):
    def _create_pod_with_scheduling_hint(self, yp_client, pod_set_id, node_filter, scheduling_hints, enable_scheduling=True):
        pod_id = yp_client.create_object("pod", dict(
        ...
        if enable_scheduling:
            yp_client.update_object("pod", pod_id, set_updates=[dict(path="/spec/enable_scheduling", value=True)])
        return pod_id
    ...
    def test_weak(self, yp_env):
        ...
        scheduling_hint_responses = yp_client.get_objects("pod", pod_ids, selectors=["/spec/scheduling/hints"])[0]
        for scheduling_hint_request, scheduling_hint_response in zip(scheduling_hint_requests, scheduling_hint_responses):
        ...
        assert list(map(lambda response: response[0], yp_client.get_objects("pod", pod_ids, selectors=["/status/scheduling/node_id"]))) == bigger_node_ids
    ...
    def test_weak_and_strong(self, yp_env):
        ...
        pod_set_id = create_pod_set(yp_client)
        pod_id = self._create_pod_with_scheduling_hint(yp_client, pod_set_id, "true", [(node_ids[0], False), (node_ids[1], False), (node_ids[2], True)])

        wait(lambda: is_pod_assigned(yp_client, pod_id))
        assert yp_client.get_object("pod", pod_id, selectors=["/status/scheduling/node_id"])[0] == node_ids[2]
    ...
    def test_strong_hints(self, yp_env):
        ...
        pod_set_id = create_pod_set(yp_client)
        contradictory_hints_pod_id = self._create_pod_with_scheduling_hint(yp_client, pod_set_id, "true", [(node_id, True) for node_id in node_ids])

        # Contradictory strong hints cannot be satisfied.
        wait(lambda: is_error_pod_scheduling_status(get_pod_scheduling_status(yp_client, contradictory_hints_pod_id)))

        scheduling_hints = yp_client.get_object("pod", contradictory_hints_pod_id, selectors=["/spec/scheduling/hints"])[0]
        scheduling_hint_uuids = [hint["uuid"] for hint in scheduling_hints]

        yp_client.add_scheduling_hint(contradictory_hints_pod_id, scheduling_hints[-1]["node_id"], scheduling_hints[-1]["strong"])
        ...
        assert yp_client.get_object("pod", contradictory_hints_pod_id, selectors=["/status/scheduling/node_id"])[0] == node_ids[-1]
    ...
    def test_strong_hint_with_node_filter(self, yp_env):
        ...
        pod_id_1 = self._create_pod_with_scheduling_hint(yp_client, pod_set_id, "[/labels/filter_value]=1", [(node_ids[0], True)])
        pod_id_2 = self._create_pod_with_scheduling_hint(yp_client, pod_set_id, "[/labels/filter_value]>2", [(node_ids[1], True)])

        wait(lambda: is_pod_assigned(yp_client, pod_id_1))
        assert yp_client.get_object("pod", pod_id_1, selectors=["/status/scheduling/node_id"])[0] == node_ids[0]
    ...
    def test_weak_hints_with_node_filter(self, yp_env):
        ...
        pod_id_1 = self._create_pod_with_scheduling_hint(yp_client, pod_set_id, "[/labels/filter_value]=1", [(small_node_ids[0], False)])
        pod_id_2 = self._create_pod_with_scheduling_hint(yp_client, pod_set_id, "[/labels/filter_value]=2", [(big_node_id, False)])

        wait(lambda: are_pods_assigned(yp_client, [pod_id_1, pod_id_2]))
        assert yp_client.get_object("pod", pod_id_1, selectors=["/status/scheduling/node_id"])[0] == small_node_ids[0]
        assert yp_client.get_object("pod", pod_id_2, selectors=["/status/scheduling/node_id"])[0] == small_node_ids[1]
    ...
    def test_remove_node_after_setting_hint(self, yp_env):
        ...
        pod_id_1 = self._create_pod_with_scheduling_hint(yp_client, pod_set_id, "true", [(node_ids[1], True)], False)
        pod_id_2 = self._create_pod_with_scheduling_hint(yp_client, pod_set_id, "true", [(node_ids[1], False)], False)
    ...
    def test_cannot_change_scheduling_hints_manually(self, yp_env):
        ...
        pod_set_id = create_pod_set(yp_client)
        pod_id = self._create_pod_with_scheduling_hint(yp_client, pod_set_id, "true", [(node_ids[0], False), (node_ids[1], True)], False)

        for relative_path, value in [("/scheduling", {}), ("/scheduling/hints", {}), ("/scheduling/hints/0", {})]:
        ...
        for relative_path, value in [("uuid", "other-uuid"), ("creation_time", {}), ("creation_time/seconds", 12), ("creation_time/nanos", 123456789), ("strong", True), ("node_id", node_ids[1])]:
        ...
        for indices in indices_buckets:
            with pytest.raises(YtResponseError):
                yp_client.update_object("pod", pod_id, set_updates=[dict(path="/spec/scheduling/hints", value=[scheduling_hints[index] for index in indices])])

        yp_client.update_object("pod", pod_id, set_updates=[dict(path="/spec/scheduling/hints", value=[scheduling_hints[0], scheduling_hints[1]])])

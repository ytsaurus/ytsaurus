...
class TestResources(object):
    def test_node_required_on_create(self, yp_env):
        ...
        with pytest.raises(YtResponseError):
            yp_client.create_object(
                "resource", attributes={"spec": {"cpu": {"total_capacity": 100}}}
            )
    ...
    def test_kind_required_on_create(self, yp_env):
        ...
        with pytest.raises(YtResponseError):
            yp_client.create_object(
                "resource", attributes={"meta": {"node_id": node_id}, "spec": {}}
            )
    ...
    def test_kind_set_on_create(self, yp_env):
        ...
        node_id = yp_client.create_object("node")
        cpu_resource_id = yp_client.create_object(
            "resource",
            attributes={"meta": {"node_id": node_id}, "spec": {"cpu": {"total_capacity": 1}}},
        )
        memory_resource_id = yp_client.create_object(
            "resource",
            attributes={"meta": {"node_id": node_id}, "spec": {"memory": {"total_capacity": 1}}},
        )
        disk_resource_id = yp_client.create_object(
            "resource",
            attributes={
                "meta": {"node_id": node_id},
                "spec": {
                    "disk": {"total_capacity": 1000, "storage_class": "hdd", "device": "/dev/null"}
                },
            },
        )
        slot_resource_id = yp_client.create_object(
            "resource",
            attributes={"meta": {"node_id": node_id}, "spec": {"slot": {"total_capacity": 1}}},
        )
        gpu_resource_id = yp_client.create_object(
            "resource",
            attributes={
                "meta": {"node_id": node_id},
        ...
        network_resource_id = yp_client.create_object(
            "resource",
            attributes={"meta": {"node_id": node_id}, "spec": {"network": {"total_bandwidth": 64}}},
        )

        assert (
            yp_client.get_object("resource", cpu_resource_id, selectors=["/meta/kind"])[0] == "cpu"
        )
        assert (
            yp_client.get_object("resource", memory_resource_id, selectors=["/meta/kind"])[0]
            == "memory"
        )
        assert (
            yp_client.get_object("resource", disk_resource_id, selectors=["/meta/kind"])[0]
            == "disk"
        )
        assert (
            yp_client.get_object("resource", slot_resource_id, selectors=["/meta/kind"])[0]
            == "slot"
        )
        assert (
            yp_client.get_object("resource", gpu_resource_id, selectors=["/meta/kind"])[0] == "gpu"
        )
        assert (
            yp_client.get_object("resource", network_resource_id, selectors=["/meta/kind"])[0]
            == "network"
        )
    ...
    def test_cannot_change_kind(self, yp_env):
        ...
        with pytest.raises(YtResponseError):
            yp_client.update_object(
                "resource",
                resource_id,
                set_updates=[{"path": "/spec", "value": {"memory": {"total_capacity": 1}}}],
            )
    ...
    def test_get(self, yp_env):
        ...
        result = yp_client.get_object(
            "resource",
            resource_id,
            selectors=["/meta/kind", "/spec/memory/total_capacity", "/meta/id", "/meta/node_id"],
        )
    ...
    def test_parent_node_must_exist(self, yp_env):
        ...
        with pytest.raises(YtResponseError):
            yp_client.create_object(
                object_type="resource", attributes={"meta": {"node_id": "nonexisting_node_id"}}
            )
    ...
    def test_resource_create_destroy(self, yp_env):
        ...
        resource_ids = [
            yp_client.create_object(object_type="resource", attributes=resource_attributes)
            for i in xrange(10)
        ]
                ...
                len(
                    list(
                        yt_client.select_rows(
                            "* from [//yp/db/nodes] where is_null([meta.removal_time])"
                        )
                    )
                ),
                len(
                    list(
                        yt_client.select_rows(
                            "* from [//yp/db/resources] where is_null([meta.removal_time])"
                        )
                    )
                ),
    ...
    def test_node_segment_totals(self, yp_env):
        ...
        segment_id = yp_client.create_object(
            "node_segment", attributes={"spec": {"node_filter": '[/labels/status] = "good"'}}
        )
        ...
        def _check(totals_field, n):
            totals = yp_client.get_object(
                "node_segment", segment_id, selectors=["/status/" + totals_field]
            )[0]
            return (
                totals != YsonEntity()
                and totals["cpu"]["capacity"] == n
                and totals["memory"]["capacity"] == n * 10
                and totals["disk_per_storage_class"]
                == {"hdd": {"capacity": n * 100, "bandwidth": n * 3}}
                and totals["gpu_per_model"] == {"v100": {"capacity": n}}
            )
    ...
    def test_gpu_allocation_optimality(self, yp_env):
        ...
        node_id = create_nodes(
            yp_client,
            1,
            gpu_specs=[dict(model="v100", total_memory=capacity) for capacity in memory_capacities],
        )[0]
        ...
        gpu_resources = yp_client.select_objects(
            "resource",
            filter='[/meta/node_id] = "{}" and [/meta/kind] = "gpu"'.format(node_id),
            selectors=["/meta/id", "/spec/gpu/uuid", "/spec/gpu/total_memory"],
        )
        get_pod_id = lambda resource_id: yp_client.get_object(
            "resource", resource_id, selectors=["/status/scheduled_allocations/0/pod_id"]
        )[0]
        wait(lambda: all(get_pod_id(resource[0]) == pod_id for resource in gpu_resources))

        gpu_allocations = yp_client.get_object(
            "pod", pod_id, selectors=["/status/gpu_allocations"]
        )[0]
    ...
    def test_gpu_allocation_no_shuffle(self, yp_env):
        ...
        node_id = create_nodes(
            ...
            gpu_specs=[dict(model="v100", total_memory=2 ** 10)] * 50,
        )[0]
        ...
        gpu_allocations = yp_client.select_objects(
            "resource",
        ...
        yp_client.update_object(
            ...
            set_updates=[
                dict(
                    path="/spec/gpu_requests/end",
                    value=dict(id="new_gpu", model="v100", min_memory=2 ** 10),
                )
            ],
        )
    ...
    def test_gpu_allocation_node_resource_change(self, yp_env):
        ...
        get_gpu_id = lambda: yp_client.get_object(
            "pod", pod_id, selectors=["/status/gpu_allocations/0/resource_id"]
        )[0]
        old_gpu_id = get_gpu_id()
        yp_client.update_object(
            "resource", old_gpu_id, set_updates=[dict(path="/spec/gpu/total_memory", value=0)]
        )
        assert get_gpu_id() == old_gpu_id

        yp_client.update_object(
            "resource", old_gpu_id, set_updates=[dict(path="/spec/gpu/model", value="k200")]
        )
        assert get_gpu_id() == old_gpu_id
    ...
    def test_resource_used_free_status(self, yp_env):
        ...
        memory_resource_id = yp_client.create_object(
            "resource",
            attributes={"meta": {"node_id": node_id}, "spec": {"memory": {"total_capacity": 200}}},
        )
        disk_resource_id = yp_client.create_object(
            "resource",
            attributes={
        ...
        gpu_resource_id = yp_client.create_object(
            ...
            attributes={
                "meta": {"node_id": node_id},
                "spec": {"gpu": {"model": "v100", "total_memory": 3000, "uuid": "123"}},
            },
        )
        slot_resource_id = yp_client.create_object(
            "resource",
        ...
        network_resource_id = yp_client.create_object(
            ...
            attributes={
                "meta": {"node_id": node_id},
                "spec": {"network": {"total_bandwidth": 2000}},
            },
        )
        ...
        assert yp_client.get_object(
            "resource",
            cpu_resource_id,
            selectors=["/status/free/cpu/capacity", "/status/used/cpu/capacity"],
        ) == [100, 0]
        assert yp_client.get_object(
            "resource",
            memory_resource_id,
            selectors=["/status/free/memory/capacity", "/status/used/memory/capacity"],
        ) == [200, 0]
        assert yp_client.get_object(
            "resource",
            disk_resource_id,
            selectors=["/status/free/disk/capacity", "/status/used/disk/capacity"],
        ) == [1000, 0]
        assert yp_client.get_object(
            "resource",
            slot_resource_id,
            selectors=["/status/free/slot/capacity", "/status/used/slot/capacity"],
        ) == [300, 0]
        assert yp_client.get_object(
            "resource",
            gpu_resource_id,
            selectors=["/status/free/gpu/capacity", "/status/used/gpu/capacity"],
        ) == [1, 0]
        assert yp_client.get_object(
            "resource",
            network_resource_id,
            selectors=["/status/free/network/bandwidth", "/status/used/network/bandwidth"],
        ) == [2000, 0]

...
class TestNodeSegmentPodConstraints(object):
    def _update_default_node_segment_vcpu_constraint(self, yp_client, multiplier, additive):
        yp_client.update_object("node_segment", "default", set_updates=[{
            "path": "/spec/pod_constraints",
            "value": {"vcpu_guarantee_to_limit_ratio": {"multiplier": multiplier, "additive": additive}},
        }])
    ...
    def test_vcpu_guarantee_to_limit_ratio(self, yp_env):
        ...
        pod_set_id = yp_client.create_object("pod_set", {"spec": {
        ...
        def _violate_node_segment_vcpu_constraint(violate):
            yp_client.update_object("pod_set", pod_set_id, [{
        ...
        def _update_vcpu_guarantee_and_limit(pod_id, guarantee, limit):
            yp_client.update_object("pod", pod_id, set_updates=[
                ...
                {
                    "path": "/spec/resource_requests/vcpu_limit",
                    "value": limit,
                },
            ])
        ...
        assert_over_time(lambda: are_pods_assigned(yp_client, [pod_id, pod_id_within_limits, pod_id_within_limits_2]))

        self._update_default_node_segment_vcpu_constraint(yp_client, 1.0, 0)

        yp_client.update_object("node_segment", "default", remove_updates=[{"path": "/spec/pod_constraints/vcpu_guarantee_to_limit_ratio/additive"}])
        with pytest.raises(YtResponseError):
            _update_vcpu_guarantee_and_limit(pod_id, 150, 151)
        _update_vcpu_guarantee_and_limit(pod_id, 150, 150)

        yp_client.update_object("node_segment", "default", remove_updates=[{"path": "/spec/pod_constraints/vcpu_guarantee_to_limit_ratio"}])
    ...
    def test_vcpu_guarantee_to_limit_ratio_validation(self, yp_env):
        ...
        for multiplier, additive in pod_vcpu_ratios:
            with pytest.raises(YtResponseError):
                yp_client.create_object("node_segment", {
                    "spec": {
                        "node_filter": "",
                        "pod_constraints": {"vcpu_guarantee_to_limit_ratio": {"multiplier": multiplier, "additive": additive}},
                    },
                })
            with pytest.raises(YtResponseError):
        ...
        with pytest.raises(YtResponseError):
            yp_client.update_object("node_segment", "default", remove_updates=[{"path": "/spec/pod_constraints/vcpu_guarantee_to_limit_ratio/multiplier"}])
    ...
    def test_vcpu_guarantee_to_limit_ratio_acls(self, yp_env):
        ...
        with yp_env.yp_instance.create_client(config={"user": username}) as yp_client_simple_user:
            def _create_pod_set(yp_client, violate):
                return yp_client.create_object("pod_set", attributes={
                    "spec": {
                        "node_segment_id": "default",
                        "violate_node_segment_constraints": {"vcpu_guarantee_to_limit_ratio": violate},
                    },
                })
            ...
            yp_client_root.update_object("node_segment", node_segment_id, set_updates=[{
                "path": "/meta/acl",
                "value": [{
                    "action": "allow", "permissions": ["use"], "subjects": [username],
                    "attributes": ["/access/pod_constraints/violate/vcpu_guarantee_to_limit_ratio"]
                }],
            }])
            yp_env.sync_access_control()

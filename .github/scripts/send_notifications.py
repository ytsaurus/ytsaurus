...
def send_notify(args, wf_state):
    message = '\n'.join((
        f'Workflow *{args.workflow}* {wf_state}: {args.git_server_url}/{args.repo}/actions/runs/{args.current_job_id}',
...
def main():
    ...
    r = requests.get(f'https://api.github.com/repos/{args.repo}/actions/workflows/{args.workflow}/runs?status=completed&{args.ref_type}={args.ref}',
                     headers=headers)
    r.raise_for_status()
